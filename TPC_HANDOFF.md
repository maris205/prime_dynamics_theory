...
original slots与 deterministic route，定义 \(m(o)\)、\(D(o)=\operatorname{rad}m(o)\)
以及 occurrence-native Euler tensor

~~~text
F_p(h)=p/(p-1) 1_(h!=-2 mod p),
G_p,z(h)=F_p(h)                                      (p<=z),
G_p,z(0)=p/(p-1), G_p,z(h!=0)=p(p-2)/(p-1)^2        (p>z),
Delta_m,z(h)=prod_(p|m)F_p(h)-prod_(p|m)G_p,z(h),
Mloc_x(h)=sum_(MASTER o) a_o Delta_(m(o),z)(h).
~~~
...
Mloc_x(0)=0,
J(Mloc_x)+E(Mloc_x)=0.                               (V28.1)
~~~

所以任何强制 `M(0)=r_x(0)=S_x` 的 generic smooth main仍以系数一返回原目标并
STOP；但这个 source-native local Euler carrier不是循环主项。它仍只是 exact
algebraic carrier，不是已经证明的 weighted AP asymptotic。

对 \(q\nmid D\) 的 inverse-residue row令
\(g=(n,D),D=gR,n=ga\)。CRT 精确消去 \(g\)，并给出
...
R=R0R1, p|R0 iff p<=z,
0<|a|<=qR/H, R>=H/q=x^(31/96+o(1)),
qR/H<=x^(17/96+o(1)).                               (V28.2)
~~~

actual selected-group mass与 radical Euler sum给
`||B_F||_2<=x^(1+o(1))R^(-3/2)`；hybrid \(G\) 对固定 \(R_1\) 给
`x^(1+o(1))R0^(-3/2)R1^(-3)`。Bettin--Chandee
`arXiv:1502.00769v1`, Theorem 1因此成为真实的 conditional local engine：

~~~text
V28_SHORT_INVERSE_RESIDUE_CORRIDOR_EXPONENT = 1891/1920
399/400-1891/1920 = 121/9600 > 0.                    (V28.3)
~~~

这仍要求 exact smooth emitter、\(R_0/R_1\) provenance、`q|D` rows在绝对值前
集体消去、both signs与 one-outer-absolute reassembly；因此整体
`ARITHMETIC_ADVANCE=NO`。MRT Proposition 3.1只登记为 abstract product-local
L2 reduction。literal two-sided product-local flatness与同一个 tagged residual
的 joint Jutila main/error theorem仍 OPEN；one-sided MRT即使理想化也损失
`H^(1/4)`，超过 endpoint `1549/9600`。

动力学侧证明 stationary mixing Logistic/Hénon system不能
measure-preservingly factor到 residue rotation/odometer：有限 quotient character
会拉回 nontrivial root-of-unity eigenfunction。非自治出口仍在，但必须产生
target-independent Riesz kernel \(K_j\) 且
`||K_j||_2 V_j=o(x_j/log^2 x_j)`。full primorial point kernel的 norm为
`\sqrt P=exp((1/2+o(1))sqrt x)`，因此 generic Haar variance route停止。
...
V28_L2 = NONE
V28_TPC_207_TRIGGER = false                           (V28.4)
~~~

proof 与 checker 为
`research/tpc-big-road/bridge_b_euler_zero_axis_and_kernel_carrier.md` 和
`research/tpc-big-road/tpc_bridge_b_euler_kernel_checker.py`。当前 checker冻结
49-field contract、36-row registry，registry SHA-256为
`2926e4dc94080ff3179970dc134c1a1edb76bcb5b7f64be783b4bc747d5c7a0b`；
149/75/12/15/149 个 contract/registry/source/dependency/result mutations必须全部
拒绝。V28是不编号 big-road release，不创建 TPC-207、paper、PDF或 build output。
...
对 fixed `h0=2`、`x=2X`、V19 ordered `+2,-1` raw row 与
同一 V21 hybrid residual，逐 occurrence 展开

```text
G_x(alpha)=sum_nu A_x(nu)e(alpha D_x(nu)),
S_x=int_0^1 G_x(alpha)dalpha=sum_(D_x(nu)=0)A_x(nu). (V24.1)
```

在这里 `d_phys,d_rough,e_sieve,q_J,d_J,c_F,b_F,u_F,t_F,ell_J,d_BL` 全部保持
异型。Blomer--Li `arXiv:2511.03294v1` Lemma 1 给 exact Jutila main/error split。
其 Lemma 2 printed formula却有两个 literal defects：interval上界写成 `max`而非
Farey geometry要求的 `min`，且 inverse-`b` phase没有随左右半弧变号。V24用
`C=c=1,z=1/4`与 `C=5,c=3,b=1,z=-1/22`给出 finite counterexamples，再从 exact
Farey neighbors独立证明 corrected interval与 `sigma(z)` phase。其 bare identity
branch逐 atom出现

```text
sum_(b_F mod c_F)^* e_cF(b_F D+sigma(z_F)u_F inverse(b_F))
  =S(D,sigma(z_F)u_F;c_F).                           (V24.2)
```

所以 literal determinant 到 complete Kloosterman atoms 已在 `L0` 闭合。这不是
cancellation theorem。打开 rational `chi(b_F/c_F+z_F)` 后仍有完整 signed
`(q_J,c_F,b_F,d_J,ell_J)` ensemble；不得在其中提前取绝对值，也不得把
`q_J=c_F`、`d_phys=d_J` 或 source 的 factorable `q=pt` 与 V23 prime-only shell拼接。

Blomer--Li 的最终 `41/42` theorem使用 GL(3) coefficients与 divisor/Voronoi chain，
并在 (3.14) 选择 `q=pt`、其中 `t` 必须 unrestricted smooth以便后续 Poisson；它不接受
literal Möbius/log HB2 row。Blomer--Pascadi仍只接受已经发射出的 fixed-modulus、
fixed-unit short arrays。因此 V24 精确裁决为

```text
V24_LITERAL_DETERMINANT_JUTILA_FAREY_ATOMIZATION = PROVED_EXACT_L0
V24_BLOMER_LI_LEMMA1_JUTILA_INTERFACE = PROVED_SOURCE_BACKED
...
V24_SIGNED_q_c_b_COLLECTIVE_PHYSICAL_EMITTER = OPEN_NEW_CONSTRUCTION
...
STRICT_1_OVER_400 = UNPAID
L2 = NONE
TPC_207_TRIGGER = false.                              (V24.3)
```

proof 与 checker 为
`research/tpc-big-road/bridge_b_literal_jutila_farey_atom_compiler.md` 和
`research/tpc-big-road/tpc_bridge_b_literal_jutila_farey_atom_checker.py`。checker 当前
冻结 58-field contract、55-row registry，registry SHA-256 为
`6ce7f2c3d2eb3a2e30900aaa7285d8e99bca139711db04915bdd9453a0a1808e`；
176/170 个 contract/registry mutations与7个 dependency-lock attacks必须全部拒绝。
V24 是不编号大路节点，不创建
TPC-207、paper、PDF 或 build output。
...
于是 V21 exact split与 paid mean合成

```text
Cbar_Q=<beta_x^raw,(I-Pbar_Q)w_x^(z)>=S_x-Hbar_Q,
Hbar_Q<<_(A,K)x/log^A x.                             (V22.1)
```

这不是新的 saving，而是一个精确 firewall。对每个
`r_q=(I-E_q)w_x^(z)`与每个 `h mod q`，

```text
sum_(t in I_x)r_q(t)e_q(ht)=0.                      (V22.2)
```

所以 direct residue Fourier或 additive congruence transform看不见 centered
directions；covariance仍可非零，因为 `beta_x^raw`在同一 fiber内变化。另一方面

```text
rank(Pbar_Q)<=sum_q q=O(x^(2/3)),
dim ker(Pbar_Q)>=|I_x|-sum_q q=x/2-o(x),             (V22.3)
```

故完整 ensemble没有把 centered operator压成 low rank：`I-Pbar_Q`在这个大空间上
exactly等于 identity。`x=1000`、`q=11,13,17,19` fixture给 mean-span rank `57`、
identity multiplicity `443`。另在 `x=166,q=7`取 literal `beta_166^raw`，但刻意设置
synthetic `w_syn=beta_166^raw`；全部 residue marginals为零而 centered energy为
`2359675/77616`。这是 operator information-loss witness，不是 actual prime residual
`w_166^(z)` covariance，也不给 arithmetic credit。
...
后一个 projector/off-diagonal branch由 V21 paid mean控制；前一个 diagonal仍是
original literal SHB-D2 determinant target。Blomer--Pascadi balanced local engine若
通过 exact compiler接入，可给 `q^(-1/32)=x^(-1/96)`，未计 compiler losses前相对
strict `1/400`的 margin为 `19/2400`。但 direct centered-residue attachment类型为零。
full-`q` Fourier completion再切 `sqrt(q)` blocks且只用 black-box Cauchy时，uniform
proof会引入可能 sharp 的 `q^(1/4)` factor，所以没有额外 block structure就不能认证
net gain；V22只停止这个 Cauchy-only版本。
因此 V22解析 current gate不是继续展开 paid projector，而是

```text
V22_LITERAL_SHBD2_DIAGONAL_POST_POISSON_COMPILER_GATE
  = OPEN_CONDITIONAL.                                (V22.5)
```

动力学侧，本轮在 profinite odometer `T(r)=r+1`、distinguished seed `0`上 exact编码

```text
Phi_x(T^t0):=beta_x^raw(t)R_x^(-1)sum_q(I-E_q)w_x^(z)(t),
Cbar_Q=sum_(t in I_x)Phi_x(T^t0).                    (V22.6)
```

它是 lossless `L0` orbit-sum/Bratteli return，不是 cancellation theorem。普通 ergodicity
只 center `w`，没有 center `beta*w`；growing signed observable、fixed seed与 uniform
all-large-stage量词仍未被现有 a.e.-seed ASIP/DBC或 a.e.-parameter typicality支付。
动力学保留的大胆 gate为

```text
V22_TRANSVERSAL_COMMON_RETURN_CENTERED_PHYSICAL_CARRIER_GATE
  = OPEN_NEW_CONSTRUCTION.                           (V22.7)
```

它要求 positive-measure parameter family上的 parameter-independent common arithmetic
return、coefficientwise exact physical intertwiner、独立 small carrier mean与同一 good
parameter上的 uniform triangular pointed bound。解析与动力学两 gate独立，不拼接 theorem
credit。V22 arithmetic advance=`NO`；fixed-atom credit=`0`、strict
`1/400=UNPAID`、`L2=NONE`、TPC-207=false。proof为
`research/tpc-big-road/bridge_b_centered_projector_firewall.md`；checker为
`research/tpc-big-road/tpc_bridge_b_centered_projector_checker.py`，20-field contract与
36-row registry
SHA-256为
...
residual更小、更低秩或自动 telescoping的对象。逐 parent fiber exact projection给

```text
eta_p(V)(q_j)=V(q_j),                         j deleted,
eta_p(V)(q_j)=V(q_j)-Vbar_r,                  j surviving.
```

在 terminal no-wrap clock `P_(s-1)>x`，physical shell每个 fiber至多占一个 child，故
...
terminal `p asymp sqrt(x)`，所以 ratio趋于 1；source innovation没有 automatic small
norm。对 V19 source-locked combined raw MASTER covector还有 exact comparison
...
因此任意 fixed log-power target上，terminal-innovation saving与原 combined raw
MASTER saving等价。growing-horizon Duhamel weights逐 coordinate非负且总和为一；
terminal survivor weight为 `(p-3)/(p-2)`，不存在由 stage bookkeeping自动生成的
alternating cancellation。

affine/path state

```text
V_a direct_sum E_(a+1) direct_sum ... direct sum E_b -> V_b
```

是 exact weighted isometry，dimension仍为 `P_b`。actual row witness
`L_(beta_168)((I-Pi_13)e_90)=20/11`排除 quotient掉整个 innovation space；
`beta_166^raw(84)=1`而 `x=168` strict shell不含 84，排除 changing-scale raw rows
自动形成一条 pullback/coboundary orbit。V19 fixture的 independent exact component
ranks为 raw `65`、base `56`、terminal eta `50`、all eta `54`、eta+base union `76`；
finite component family也没有自动低秩塌缩。

所以 V20新增且只新增一个 broad cell：

```text
DECLARED_TPC_BRIDGE_B_20260807_SHBD2_LONG_HORIZON_SOURCE_INNOVATION_
SMALL_NORM_AUTOMATIC_TELESCOPE_OR_LOW_RANK_BYPASS_V1
  = STOP_SCOPED_EXACT_TERMINAL_NEAR_IDENTITY.
```

它只停止把 canonical innovation本身当作免费 smoothing/telescope/low-rank bypass；不
停止真正的 signed arithmetic/dynamical theorem、合法 physical quotient、A1/A2、两个
O161 parents、pair-native、H1或 global architecture。current primary收紧为
...
它必须直接证明 combined raw MASTER target或等价 terminal-innovation target的 uniform
log-power saving，并保留 fixed physical `h0=2`、`x=2X`、literal ordered
coefficients、actual masks、normalization、separation/reassembly与完整 physical-loss
ledger。抽象 direct-sum coding、positive measure、a.e. genericity、complete-frequency
mean或仅有 orbit recurrence均不给 arithmetic credit。V20不是 arithmetic advance。
TPC-207=false、fixed-atom credit=`0`、strict `1/400=UNPAID`、`L2=NONE`。
proof为 `research/tpc-big-road/bridge_b_terminal_innovation_floor.md`；checker为
`research/tpc-big-road/tpc_bridge_b_terminal_innovation_checker.py`，37-row registry
final-LF SHA-256为
...
`P=F^(2-o(1))` top collar。写 `rho=fm`，其中 `f=cond(chi)`、
`chi=Ind_f^(fm) psi`且 `psi` primitive。odd squarefree induction精确给

```text
tau_(fm)(conjugate chi)^2
 = conjugate(psi)(m)^2 tau_f(conjugate psi)^2,
mu(rho)/phi(rho)=mu(f)mu(m)/[phi(f)phi(m)].
```

所以 Gauss square只消去内部 `mu(m)^2`；outer `mu(m)`不消失。每条 character
polynomial还必须显式保留 `(n,m)=1` mask，orientation仍是
`E_1(conjugate psi)E_2(conjugate psi)U(psi)V_v(psi)`，physical phase为
`psi(c_v)conjugate(psi)(m)^2`。这是 exact finite decomposition，不是 saving。

对第 54.18、57.3 节已经冻结的 common-coefficient source contract，固定 `m` 后
arithmetic arrays不依赖 varying primitive modulus `f`；残余 smooth modulus scalar由
bounded-variation partial summation支付。因此可合法使用 primitive multiplicative
large sieve。若 `N_E,N_L<=F^2`且两条 grouped energies分别为
`F^2X^o(1)`与 `F^2/(GAv)X^o(1)`，则
...
V_(m>=M)(P)
 << [F^4+F^2P^2/M^2]X^o(1).                         (TOP-CUTOFF)
```

在 `P=F^2=D`取 `M=P^kappa`，被删除的 low-secondary-conductor sector
`m>=P^kappa`在 dyadic exponent尺度对应 `f<<P^(1-kappa)`；特别地全部
`f<=P^(1-kappa)`均被删除，并获得
`D^(-min(1,2kappa)+o(1))`。若 downstream另损 `D^lambda_D`，仅就该 sector支付
local strict `1/200`须
`min(1,2kappa)-lambda_D>1/200`；在 `0<kappa<=1/2`时等价于
`kappa>1/400+lambda_D/2`，而 `kappa>=1/2`后 saving饱和于 `D^(-1+o(1))`。
等号不能吸收 `X^o(1)`。这个正面结论只量化
common-coefficient contract，不能升级到 V13额外允许的 arbitrary
`rho`-dependent joint-`L2` superclass；actual-atom membership仍 OPEN。

剩余墙不能再命名为 primitive-only。bounded cofactor `rho=3p,f=p,m=3`已有 exact
operator fixture：`p=17`时 normalized Gauss-square coefficient为 `17/32`；一般
`p=1 (mod 24)`时趋于 `1/2`，没有 fixed-power gain。prime modulus上全部
nonprincipal characters本来就是 maximal Gauss-square eigenspace；literal
Möbius/smooth equal-row subfamily的 primitive fourth-moment floor为 `pF^2`。因此
frozen superclass上的 phase-blind eigenspace avoidance、fourth moment/large sieve
均精确 `STOP_SCOPED`。这不是 actual signed packet lower bound；它反而说明下一步
必须保留 root-number-square phase。prime sector又有 `mu(p)=-1`恒定，outer
Möbius sign单独至多给 logarithmic而非 fixed-power saving。

current first subgate更新为
`HB4_EXACT_HALF_TOP_NEAR_PRIMITIVE_GAUSS_SQUARE_FOUR_POLYNOMIAL_ANGLE`：对每个 fixed
`kappa>0`，只需处理 `m<P^kappa`的 primitive/near-primitive collar，并在 actual
四多项式、physical `c_v=-2v conjugate(gab^2t^2)`、全部 masks/shells与 outer signs
仍在式内时创造 fixed power。V14最高 claim仍是
`CONTRACT_LEVEL_PARTIAL_ARITHMETIC_RANGE_ADVANCE_AND_CORE_SHRINK`。actual-atom、
all-`D`、exact cover、tails、A/B selection、original/global normalization与
provenance仍 OPEN；TPC-207=false、fixed-atom credit=`0`、global strict
`1/400=UNPAID`、`L2=NONE`。A2、Bridge B与 Hénon辅助岛保持独立。
...
两条 literal `e_1,e_2~F` rows执行 exact inverse-residue transfer。冻结
...
在 `P>=F^(4/3)`，实际支撑最终短于 `rho~P`，故两条 modular inverse maps均
单射；把 coefficients按 `m,n`置于 complete residue hull并补零，逐字保留两条
`L2` norm，且
...
[Blomer--Pascadi Lemma 5.1](https://arxiv.org/abs/2607.24311)对任意 modulus与
unnormalized `S(am,n;c)`给 complete bilinear `L2` bound；等价地，打开
Kloosterman sum后 complete additive Parseval给 literal constant-one estimate

```text
local e_1,e_2 sum <= rho||alpha||_2||beta||_2 << F P X^o(1).
```

支付 `u`、Ramanujan `k`-row、outer coefficient及 `GPTAB` cell count，且不给
`mu(g)mu(rho)mu(b)`符号 credit，得到

```text
|V(P)| << F^3P^(2+o(1)).
```

因此全部 squarefree projectors统一在

```text
F^(4/3)<=P<=F^(3/2-delta),  0<delta<1/6,
```

获得任意 fixed `eta_D<delta`。局部 strict endpoint须
`delta>1/200`加 downstream ledger margin；等号不能吸收 `X^o(1)`。旧
composite `4/3`与 prime `42/31` core均降为 ancestor；current first subgate是
`HB4_EXACT_HALF_THREE_HALVES_PROJECTOR_CORE_DISPERSION`，只剩
`P=F^(3/2-o(1))` lower collar与全部 `P>=F^(3/2)` squarefree projectors。

BP2607 的 nontrivial short-interval theorem不能把 scattered inverse support的
cardinality `F`改写成 interval length，故该 direct extension精确
`STOP_SCOPED_INVERSE_SUPPORT_NOT_SHORT_INTERVAL`。`GLOBAL_MOVING_UNIT_CAUCHY`
仍是另一条 moving-product unit全域 shortcut的 historical STOP，不因本段重开。
本轮 claim ceiling仍是
`CONTRACT_LEVEL_PARTIAL_ARITHMETIC_RANGE_ADVANCE_AND_CORE_SHRINK`：actual-atom
membership、all-`D`、exact cover、tails、A/B selection、original/global
normalization与 provenance仍 OPEN；TPC-207=false、fixed-atom credit=`0`、global
strict `1/400=UNPAID`、`L2=NONE`。岛屿图上仅 Bridge A1红色断点前移；A2、
Bridge B与 Hénon辅助岛保持独立。

第 58 节从已发布 V10 commit
`cd8b798492bc399cae84358dc1dc3ba68b7c015f`继续同一个 canonical
`TPC_FM_EXACT_HALF_AND_HB4xHB2_VORONOI_GATE`。V11 第一次在 V10 exact
conductor/projector normal form上闭合非空 analytic range。令 projector
`rho~P`、complementary conductor `t~T`，冻结
...
平方自由 `g`与 literal `(a,g)=1` mask给
`|c_g(ak)|=phi((g,k))`；又因 `K=F/A>>G`，完整 `k`行满足
`sum_|k|<<K |c_g(ak)|<<K F^o(1)`。复合模 Weil bound及全部 outer
normalization随后给出
...
因此全部 squarefree projector在 `P<=F^(4/3-delta)`有
`eta_D<3delta/4`；`T>=F^(2/3+delta)`在把 `delta`缩小任意 fixed margin、
吸收 dyadic constants后也自动落入该段。局部 strict
`1/400`预算须 `delta>1/150`加 ledger margin。

对 prime projector，V11 又把
[Fouvry--Kowalski--Michel--Sawin Theorem 1.3(2)](https://arxiv.org/html/2511.09459v3)
合法附着到两条 actual `e_1,e_2~F`行。取 `l=3`、monomial exponents
`(-1,-1)`，source length window精确为
`10rho^(1/2)<=F<=rho^(3/4)`；只有在 `rho~P`及 fixed margin下才转成 `P`指数窗口。
恢复未归一化 `Kl_2`因子并支付 `u,k`及全部 outer
ledger后，
...
这闭合了非空 prime区间
`F^(4/3)<=P<=F^(42/31-epsilon)`，其中
`eta_D<31epsilon/36`；在 `P=F^(4/3)`的 supremal local budget为 `1/54`，
即任意固定 `eta_D<1/54`可取，局部 strict
endpoint须 `epsilon>9/1550`加 margin。两段 restricted proof都未给
`mu(g)mu(rho)mu(b)`任何符号 credit。

因此 Bridge A1 的红色断点已实质向前移动。当前 reduced core由三部分组成：
`P=F^(4/3-o(1))<F^(4/3)`的 all-squarefree lower collar、transition以上的
large composite projector，以及 `P>=F^(42/31-o(1))`的 prime upper collar/core。
canonical first subgate改为
`HB4_EXACT_HALF_REDUCED_PROJECTOR_CORE_DISPERSION`。Earnst direct large-`T`
transfer、AFE coefficient replacement、phase-blind primitive large sieve及在 character
side预选 projector complement均已精确 `STOP_SCOPED`。这是一项
`CONTRACT_LEVEL_PARTIAL_ARITHMETIC_RANGE_ADVANCE_AND_CORE_SHRINK`，不是 full
physical theorem：actual-atom membership、all-`D`、exact cover、tails、A/B
selection、original normalization与 provenance仍 OPEN。TPC-207仍为 false；
fixed-atom credit=`0`、global strict `1/400=UNPAID`、`L2=NONE`。Bridge A2、
Bridge B及 Hénon辅助岛均保持独立。
...
2026-08-07 的 Bridge B V20 terminal-innovation audit新增且仅新增一个 broad cell：
...
它停止把 canonical source innovation直接登记为 automatic small error、cancellative
stage telescope、uniform low-rank family或 independently easier dynamical target。第一处
fatal是 terminal no-wrap norm floor `(p-3)/(p-2)`；对 actual combined raw row，其
physical evaluation与原 raw MASTER target只差 `O_K(x^(1/2)log^C x)`，所以 fixed
log-power saving等价。growing-horizon weights还是非负 partition of one；exact path-state
isometry保持 full terminal dimension，finite component union rank也从 raw 65升到 76，
不是自动塌缩。

本 cell不停止真正的 signed terminal-innovation theorem、source-backed all-horizon
physical quotient、target-independent affine input-output theorem、exact symbolic/dynamical
coding或独立 A1/A2。它也不否定 V19 source port；它只证明该 port不免费降低算术难度。
surviving primary为
...
2026-08-07 的 Bridge B V22 centered-projector firewall新增且仅新增三个 scoped cells：

```text
DECLARED_TPC_BRIDGE_B_20260807_CENTERED_ACTUAL_FIBER_RESIDUE_FOURIER_
OR_DIRECT_KLOOSTERMAN_ATTACHMENT_V1
  = STOP_SCOPED_EXACT_ZERO_ALL_RESIDUE_MARGINALS

DECLARED_TPC_BRIDGE_B_20260807_COMPLETE_MESOSCOPIC_COUNTING_PROJECTOR_
DIRECT_DISPERSION_OR_LOW_RANK_COMPRESSION_V1
  = STOP_SCOPED_PAID_MEAN_BRANCH_AND_LARGE_IDENTITY_SPACE

DECLARED_TPC_BRIDGE_B_20260807_LITERAL_DIAGONAL_NAIVE_FULL_Q_FOURIER_
COMPLETION_TO_BP_BALANCED_BLOCKS_V1
  = STOP_SCOPED_BLACK_BOX_CAUCHY_CERTIFIES_NO_NET_GAIN_WITHOUT_BLOCK_STRUCTURE
```

第一 cell只停止把 `r_q=(I-E_q)w`的 mod-`q` residue transform直接登记为 centered
covariance或 Kloosterman input：逐 actual fiber的和为零，所以全部 residue Fourier
marginals exactly为零。finite information-loss witness取 literal `beta_166^raw`但设置
synthetic `w_syn=beta_166^raw`，给 centered energy `2359675/77616`；它不是 actual
prime residual covariance，也不给 arithmetic credit，只证明 direct transform确实丢失
within-fiber information。

第二 cell只停止从完整 `q asymp x^(1/3)` counting-projector ensemble本身获得新
dispersion或 automatic compression。exact identity是 `Cbar_Q=S_x-Hbar_Q`，而 `Hbar_Q`
已由 V21支付；`rank(Pbar_Q)<=sum_q q`，所以 `I-Pbar_Q`有
`x/2-O(x^(2/3))`维 exact identity space。任何对 projector kernel的展开只重写 paid
branch，不能删除 original `S_x` diagonal。

第三 cell只停止一种明确的 post-Poisson实现：先作 full-`q` Fourier completion，再把
长度 `q`的 coefficient切成 `B asymp q^(1/2)` blocks，并且只用 black-box Cauchy
`sum_j||a_j||_2<=B^(1/2)||a||_2`汇总。该 uniform proof引入可能 sharp 的
`q^(1/4)` factor，故无额外 block structure时不能认证 Blomer--Pascadi local
`q^(-1/32)`的 net gain。full support本身不推出 literal coefficient达到 worst case。
本 cell不停止从 literal SHB-D2 determinant diagonal出发、源端已有 short
`sqrt(q)` support、额外 block orthogonality或 collective assembler的 exact compiler。

三个 cells均不停止 V21 paid mean、genuine centered-covariance theorem、literal diagonal
post-Poisson compiler、positive-measure transversal common-return construction、A1/A2、
O161 parents、pair-native、H1或 global architecture。surviving analytic gate与独立
dynamical gate分别为

```text
V22_LITERAL_SHBD2_DIAGONAL_POST_POISSON_COMPILER_GATE = OPEN_CONDITIONAL
...
2026-08-07 的 Bridge B V24 literal Jutila--Farey atom compiler 新增且仅新增以下
三个 scoped cells：
...
第一项停止按字面使用 source (2.4)--(2.5) 的 `max` interval与 fixed-plus inverse phase；
exact Farey geometry要求 `min`与左右半弧 `sigma(z)`。它不停止仓库内已经独立证明的
corrected signed Farey identity。第二项只停止把 Blomer--Li 的 `A(n,1)tau(m)`、
divisor/GL(3) Voronoi chain、原生 parameter clock和一维 complete character sum直接
改名为 literal TPC Möbius/log coefficient或 BP two-array input。第三项只停止把 V23
prime-only shell与 source 的 factorable `q=pt` weight拼成同一个 theorem；原 source
明确要求 `t` 不受 prime 等 arithmetic restriction，才能执行后续 Poisson。

这三个 cells 均不停止 Blomer--Li Lemma 1 Jutila interface、V24仓库内证明的 corrected
determinant-to-Farey atomization、完整 signed `(q_J,c_F,b_F,sigma)` physical
emitter、独立 source-native factorable auxiliary ensemble、V23 dynamics reserve、
A1/A2、O161 parents、pair-native、H1或 global architecture。surviving gates为
...
2026-08-08 的 Bridge B V28 Euler-zero-axis/reduced-radical/kernel-carrier 审计新增且仅
新增以下四个 scoped STOP cells：
...
第一项不停止 occurrence-native \(M_x^{\rm loc}(0)=0\) carrier；它只停止把
`M(0)=S_x` 的 generic smooth interpolant称为新 main。第二项不停止 MRT
Proposition 3.1作为 abstract reduction；它停止用单边 local energy支付 literal
product flatness。第三项只停止 stationary mixing factor到 rotation/odometer；
nonautonomous stage diagram仍 OPEN。第四项停止从 positive Haar mean直接推出指定
seed positivity；low-norm target-independent Riesz kernel仍 OPEN。
...
BC corridor的 `1891/1920` 与 `121/9600` margin是 conditional local engine，
...
第一项只停止把 A 描述成 B 之后更容易的 preliminary，不停止 terminal major theorem；
第二、第四、第五项只停止错误 source transfer；第三项要求 hard major预声明；第六项
只停止 full-coordinate translation-equivariant exact compression；第七项只停止
local admissibility自动推出 prime mass。surviving primary为
`V30_B_CELL_LINF_CROSS_FLATNESS` 与 `V30_B_MLOC_PLUS_A_ATTACHMENT`；terminal
analytic gate为 `V30_TAGGED_QLOCAL_RESIDUAL_MAJOR_COVARIANCE`。完整 theorem出现前，
arithmetic advance仍为 `NO`。精确 proof、checker与 release boundary见第 77 节。
...
BRIDGE_B_SHBD2_LONG_HORIZON_SOURCE_INNOVATION_RETURN = SUPERSEDED_BY_V20_TERMINAL_EQUIVALENCE_GATE
BRIDGE_B_TERMINAL_INNOVATION_NORM_FLOOR = PROVED_EXACT_RATIO_P_MINUS_3_OVER_P_MINUS_2
BRIDGE_B_TERMINAL_INNOVATION_RAW_TARGET_EQUIVALENCE = PROVED_AT_FIXED_LOG_POWER_SCALE
...
BRIDGE_B_SHBD2_Q_AVERAGED_CENTERED_PHYSICAL_COVARIANCE = SUPERSEDED_BY_V22_OPERATOR_FIREWALL
BRIDGE_B_CENTERED_ACTUAL_FIBER_RESIDUE_FOURIER = PROVED_EXACT_ZERO_ALL_MARGINALS
BRIDGE_B_DIRECT_CENTERED_RESIDUE_KLOOSTERMAN_ATTACHMENT = STOP_SCOPED_KERNEL_TYPE
BRIDGE_B_COMPLETE_PROJECTOR_MEAN_SPAN_RANK = PROVED_AT_MOST_SUM_q
BRIDGE_B_COMPLETE_PROJECTOR_IDENTITY_MULTIPLICITY = PROVED_AT_LEAST_H_MINUS_SUM_q
BRIDGE_B_DIRECT_PROJECTOR_DISPERSION = STOP_SCOPED_PAID_HBAR_ONLY
BRIDGE_B_BP_BALANCED_LOCAL_SAVING = SOURCE_BACKED_CONDITIONAL_q_MINUS_1_OVER_32
BRIDGE_B_BP_LOCAL_X_SAVING = CONDITIONAL_X_MINUS_1_OVER_96_BEFORE_COMPILER_LOSSES
BRIDGE_B_NAIVE_FULL_q_COMPLETION_TO_BP = STOP_SCOPED_BLACK_BOX_CAUCHY_CERTIFIES_NO_NET_GAIN_WITHOUT_BLOCK_STRUCTURE
V22_LITERAL_SHBD2_DIAGONAL_POST_POISSON_COMPILER_GATE = OPEN_CONDITIONAL
...
V24_LITERAL_DETERMINANT_JUTILA_FAREY_ATOMIZATION = PROVED_EXACT_L0
V24_BLOMER_LI_LEMMA1_JUTILA_INTERFACE = PROVED_SOURCE_BACKED
V24_BLOMER_LI_LEMMA2_AS_PRINTED_MAX_FIXED_PLUS = STOP_SCOPED_LITERAL_FAREY_COUNTEREXAMPLES
V24_CORRECTED_SIGNED_FAREY_IDENTITY = PROVED_EXACT_L0_REPOSITORY_DERIVATION
V24_BARE_FAREY_B_SUM_TO_COMPLETE_KLOOSTERMAN_BILINEAR = PROVED_EXACT_L0
V24_SIGNED_q_c_b_COLLECTIVE_PHYSICAL_EMITTER = OPEN_NEW_CONSTRUCTION
V24_DIRECT_BLOMER_LI_GL3_DIVISOR_TO_LITERAL_TPC_TRANSFER = STOP_SCOPED_COEFFICIENT_VORONOI_CLOCK_AND_REASSEMBLY_MISMATCH
V24_PRIME_ONLY_JUTILA_SHELL_AS_BLOMER_LI_FACTORIZABLE_WEIGHT = STOP_SCOPED_UNRESTRICTED_SMOOTH_t_REQUIRED_FOR_POISSON
V24_PRIME_SHELL_JUTILA_ERROR_SIGNED_FAREY_KLOOSTERMAN_THEOREM = OPEN_NEW_THEOREM
V24_PRIME_SHELL_JUTILA_MAIN_TO_BP_COLLECTIVE_EMITTER = OPEN_NEW_THEOREM
...
ARITHMETIC_ROUTE_ADVANCE = CONTRACT_LEVEL_PARTIAL_ARITHMETIC_RANGE_ADVANCE_AND_CORE_SHRINK
FIXED_ATOM_CREDIT = 0
STRICT_1_OVER_400 = UNPAID
L2 = NONE
...
model又独立支付 `x^(95/96+o(1))`；二者都不得重复计算成 whole-scalar credit。动力学只允许
target-independent low-Christoffel quotient与 exact
...
V20证明 terminal innovation在 fixed log-power scale与原 target等价；V21又对 complete
prime ensemble `x^(1/3)<q<=2x^(1/3)`完成 exact wrapped split，并支付全部 mean branch。
这些正面节点不得写回缺失，也不得升级成 separated SHB-D2 saving。

V22新增的 firewall也不得重开：对 `r_q=(I-E_q)w`，全部 mod-`q` residue Fourier
marginals exactly为零；完整 projector平均只作用于 rank至多 `sum_q q`的 mean span，
而 `I-Pbar_Q`保留 `x/2-O(x^(2/3))`维 identity space。exact identity
`Cbar_Q=S_x-Hbar_Q`说明任何 direct projector/congruence expansion只重写 V21已付的
`Hbar_Q`，original `S_x` diagonal原样留下。不得把这个 off-diagonal branch、zero
marginal、martingale命名或 Kloosterman符号包装成 centered saving。

V24已经从 exact Fourier diagonal继续到 corrected signed Farey representation，并把
bare identity branch逐 atom化成 complete `S(D,sigma(z)u;c)`。source printed
`max`/fixed-plus formula已有 exact counterexamples；合法路线只使用仓库从 Farey neighbors
独立证明的 `min`/`sigma(z)` identity。因此下一 construction不能再笼统写成“寻找 second
refinement”：prime-shell error支必须控制完整 signed
`(q_J,c_F,b_F,d_J,ell_J,sigma)` emitter；main支必须真正发射 BP-compatible short arrays。
所有 primal/dual lengths、unit/nonunit/axes、hybrid zero mode、smooth/Mellin norms、tails、
`1/L`、`1/delta`、whole prime ensemble及 exactly-once outer reassembly仍须逐项登记。
固定 `eta=1/32`后，BP ledger在 compiler losses前给 `x^(-11/1536)`，相对 strict
...
1. 全部 squarefree `rho<=F^(4/3-delta)`由
   `F^4rho^(3/2+o(1))` absolute Weil dyadic theorem闭合；large
   `t>=F^(2/3+delta)`自动属于此段。
2. prime `F^(4/3)<=rho<=F^(42/31-epsilon)`由 FKMS Theorem 1.3(2)、`l=3`、
   literal `e_1,e_2` grouping闭合，完整 raw bound为
   `F^(11/3)rho^(31/18+o(1))`。
3. 全部 squarefree `F^(4/3)<=rho<=F^(3/2-delta)`由 exact inverse-residue
...
5. 在 top collar写 `rho=fm`、`f=cond(chi)`。对第 54.18、57.3 节冻结的
   common-coefficient source contract，primitive multiplicative large sieve给
   `V_(m>=M)<<[F^4+F^2P^2/M^2]X^o(1)`。所以 `M=P^kappa`删除全部
   `f<=P^(1-kappa)`，局部 saving为 `D^(-min(1,2kappa)+o(1))`；支付被删 sector
   的 strict `1/200`须
...
本轮仍固定 V19/V21/V27--V29 的同一物理对象：`h0=2`、ordered `+2,-1`
occurrence、deterministic MASTER/H2 route、`Lambda(.+2)-b_z`、prime shell
`q~x^(1/3)`、`H=x^(21/32)` 以及原始 normalization。没有替换 coefficient，
没有删除 zero/nonunit/axis，也没有把 formal Euler model升级为 prime theorem。
...
route advance为 `YES`；arithmetic advance为 `NO`。

### 77.2 Exact J scalar 与 endpoint-equivalence

令

~~~text
e_x(h)=r_x(h)-Mloc_x(h),                 Mloc_x(0)=0,
...
~~~text
S_x^physical=e_x(0)=J(e_x)+E(e_x).                       (77.1)
~~~

`J(r_x)` 的 circle form必须使用 `chi_x(-alpha)B_x(alpha)conj(W_x(alpha))`；
reflection由既定 Fourier sign强制。若 B 门已给
`|E(e_x)|<<x^(399/400-eta_E)`，则 `J(e_x)` 有 strict endpoint saving当且仅当
`S_x^physical` 有 strict endpoint saving，方向分别由
`S=J+E` 与 `J=S-E` 得到。因此 A 门仍是必需的 terminal theorem，但不是更容易的
preliminary。

### 77.3 q-local density tensor

对充分大的 `x`、`q>z` 及 `a mod q`，定义

~~~text
F_(q,a)(h)=q/(q-1) * 1_(a+h+2 != 0 mod q),
G_(q,a)(h)=q/(q-1)                  if a+h=0 mod q,
            q(q-2)/(q-1)^2          otherwise,
Delta_(q,a)=F_(q,a)-G_(q,a).
~~~

写 `u=a+h`，则 `Delta` 在 `u=-2,0,other` 三类上分别为

~~~text
-q(q-2)/(q-1)^2,  0,  q/(q-1)^2.                        (77.2)
~~~

exact finite identities为

~~~text
q^(-1) sum_(h mod q) Delta_(q,a)(h)=0,
q^(-1) sum_(h mod q) c_q(h)Delta_(q,a)(h)=Delta_(q,a)(0)=delta_q(a),
sum_(a mod q)delta_q(a)=0,
sum_(a mod q)|delta_q(a)|=2q(q-2)/(q-1)^2<2.             (77.3)
~~~

这只是 formal local shifted-prime density与 external `q>z` hybrid Euler density的
exact difference，不是 weighted prime-distribution theorem。

### 77.4 Poisson extraction 与已付 `x^(95/96)`

因 `H>2Q` 且 `supp psi subset [-1,1]`，非零 period aliases精确消失：

~~~text
sum_h hatpsi_+(h/H)c_q(h)Delta_(q,a)(h)
  = H psi(0) delta_q(a).                                (77.4)
~~~

常数是 `H psi(0)`，不是 `H hatpsi_+(0)`。后者只保证 `kappa_x(0)=1`。
对每个 labelled occurrence逐项加减该 profile，得到 exact

~~~text
J(r_x)=M_q-local+R_q-local.                              (77.5)
~~~

由 `sum_(o:t_o=t)|a_o|<=3d4(t)` 与
`|delta_q(t)|<=1_(q|t+2)+2/q`，

~~~text
M_q-local << x^(95/96+o(1)),
399/400-95/96=19/2400.                                  (77.6)
~~~

V29 的 exact boundary cover给 `x^(47/48+epsilon+o(1))`，
`epsilon<11/1920`；interior tail由 Schwartz decay支付。因此

~~~text
J(e_x)=R_q-local+O(x^(95/96+o(1))).                     (77.7)
~~~

### 77.5 Unit、diagonal 与 terminal residual

`c_q(0)=q-1` 已包含全部 unit residues。证明 (77.4) 时允许为 Fourier inversion
补上 zero numerator，但必须在同一 identity中精确减回一次；不得借此删除 physical
diagonal。q-local model在 diagonal上的绝对量为 `x^(2/3+o(1))`，而 residual
diagonal仍为

~~~text
S_x^physical+O(x^(2/3+o(1))).                           (77.8)
~~~

所以 first major-side fatal仍是同一 literal signed q-local residual covariance，
不是 formal model、BV/BDH residue mean或已付 local Bettin--Chandee carrier。

### 77.6 B 门：cell-product MRT reduction

固定预声明 hard major `M`，令 `m` 为其 circle complement。对 dyadic `H<=Y<=x` 把 circle划成
`2Y` 个 half-open cells，并设

~~~text
u_j=||B_x||_(L2(I_j intersect m)),
v_j=||W_x||_(L2(I_j intersect m)),
c_j=u_j v_j.
~~~

cellwise Cauchy--Schwarz 与一个长度 `1/Y` 的 circular interval至多交三个 cells给

~~~text
P0<=||c||_1,  PY<=3||c||_infinity,
sum_(|h-h*|<=Y)|r_x(h)-MT_(M,h)|^2
  << 3Y||c||_1||c||_infinity.                           (77.9)
~~~

literal divisor envelopes给 `||c||_1<<x^(1+o(1))`。因此最窄当前 theorem应同时证明

~~~text
||c||_infinity << x^(1+2theta+2epsilon_N)/Y,
theta+epsilon_N<13/4800,
MT_(M,h)=Mloc_x(h)+a_x(h),
~~~

并支付同一 `a_x` 的 weighted L2。`u=(R,R^-1)`、
`v=(R^-1,R)`、`c=(1,1)` 证明 cross-flatness严格弱于两边各自 flatness。
major set必须预声明；事后把共同 spike移入 major只是给 target改名。

### 77.7 Primary-source ceiling

Blomer--Li Lemma 1只给 Jutila/Ramanujan interface；MRT
`arXiv:1707.01315v3` Proposition 3.1只给 (77.9) reduction；MRT
`arXiv:1812.01224v1` Theorem 1.2只给 Liouville 的 averaged additive Fourier
uniformity；一般 1-bounded 版本须读作 Theorem 1.4 pretentiousness inverse theorem，
并附带 non-pretentiousness 条件；
Guth--Maynard `arXiv:2405.20552v2` 是 multiplicative-frequency large values；
Bettin--Chandee已只支付 V29 local `1891/1920` corridor。BV/BDH corpus也不接收
prime-shell moving-residue signed covariance。当前 primary corpus没有给
`MT=Mloc+a` 加同一 tagged cross-flatness，也没有给 terminal residual major theorem。

### 77.8 Equivariant quotient no-go 与 q=5 survivor

若 `Q:l2(Z/NZ)->Z` translation-equivariant且 point evaluation
`ev_0=ell o Q`，则 `ker Q` translation-invariant且包含于 `ker ev_0`。对
`f in ker Q` 的全部 translates在零点求值，得到 `f=0`，故 `Q` 必须 injective。
full coordinate point kernel满足

~~~text
kappa=N,  kappa0=N-1.                                  (77.10)
~~~

这排除 translation-equivariant whole-shell compression；distinguished arithmetic seed、
restricted source class或 nonautonomous tag仍 scoped open。

在 `Z/5Z` 上 `A5={1,2,4}`、`K5=(5/3)1_A5` 给
`int K5=1`、`||K5||_2^2=5/3`、`||K5-1||_2^2=2/3`。
它是 genuine low-Christoffel finite carrier，但 `f=0` 也满足相同 admissible support，
故 local admissibility不产生 positive prime mass。

### 77.9 路线排序与 first fatal
...
A 与 B 都必需；排序只表示研究杠杆。当前 overall first fatal为
`MISSING_LITERAL_MT_EQUALS_MLOC_PLUS_A_AND_TAGGED_CELL_CROSS_FLATNESS`。

### 77.10 Canonical registry、checker 与发布边界
...
`49/52/7/6` contract/registry/source/dependency rows，执行
`100/107/16/14/155` mutations，共 `392` 个 unique reject actions，registry SHA-256为
`acead73d0c6e12b03d30d40f35ea345c32d859bea5106456f33b4724fdf23563`。

closure前状态为：

~~~text
V30_FINAL_RELEASE_QA = PASS
V30_MATH_SOURCE_FORMULA_QA = PASS
V30_CHECKER_ADVERSARIAL_QA = PASS
V30_CROSS_DOCUMENT_RELEASE_SCOPE_QA = PASS
V30_PRE_CLOSURE_HANDOFF_SHA256_RAW = 512c061acda8b91310316cfbc4916e38d432e59bb20aa8079c3207b960e713fa
V30_PRE_CLOSURE_HANDOFF_SHA256_CANONICAL_LF = 48849be7aeb0f84026cb8d91db6ac517f0ff911ff2a13582e81ad8545bcc8962
V30_COMPASS_SHA256_RAW = 3f630341f9376351b1f1634961d42f40089bb4312257f8bb179110bdc7ce47b0
V30_COMPASS_SHA256_CANONICAL_LF = c8974aa5de47c26e01f4c88f25c0a402d271351490215565c60813c960af09d3
V30_README_SHA256_RAW = bdee87e4d80059babb894575bf62466bbd6f1c37ce996660fe1640afa08618f3
V30_README_SHA256_CANONICAL_LF = 40dcc575b9f63619a6606fe888a7e9fb3685ea541683492c89bc07ea22831652
V30_PROOF_SHA256_RAW_CANONICAL_LF = 5c3d59e3b324a8c67109566c5e54dd3d3fc381b295b2c6cce15c49762ea4bbf6
V30_CHECKER_SHA256_RAW_CANONICAL_LF = 662dbe9259f8a6176894711d692470608115b9df89f255e6c53dbd493e11cfcf
V30_CHECKER_STDOUT_SHA256_RAW_CRLF = 8187fd0e2719ec554d305314666a7af9b49818e5121d1c890a8e147ae5b1492c
V30_CHECKER_STDOUT_SHA256_CANONICAL_LF = 76f4269353b7670e9103961f0449ca7c11161dd5f0021268f6bd1b811e7a135a
V30_CHECKER_STDOUT_PAYLOAD_SHA256 = 09780e8aca48430b79cb69991ffc87736985d9d5c079682e7e7a855a95aeadf7
V30_CHECKER_CONTRACT_REGISTRY_SOURCE_DEPENDENCY = 49/52/7/6
V30_CHECKER_MUTATIONS = 100/107/16/14/155
V30_CHECKER_ADVERSARIAL_ACTIONS = 392/392_UNIQUE_REJECTED
V30_CHECKER_REGISTRY_SHA256 = acead73d0c6e12b03d30d40f35ea345c32d859bea5106456f33b4724fdf23563
STARTUP_REGRESSION = 22/22
SUPPLEMENTAL_TPC111_TPC124_TPC126_TPC127 = 4/4
V23_TO_V30_DEPENDENCY_PROCESSES = 16/16
V23_TO_V30_DEPENDENCY_STDOUT_IDENTITIES = 8/8
BIG_ROAD_CHECKER_PROCESSES = 36/36
BIG_ROAD_CHECKER_STDOUT_IDENTITIES = 18/18
RELEASE_ALLOWLIST = EXACT_FIVE_PATHS
CACHED_DIFF = EMPTY
DIFF_CHECK = PASS
PROTECTED_UNTRACKED_COUNT = 130
PROTECTED_UNTRACKED_MANIFEST_SHA256 = 9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1
MARKDOWN_FENCES = HANDOFF_BACKTICK_2838_TILDE_144__COMPASS_BACKTICK_224_TILDE_42__README_BACKTICK_378_TILDE_38__PROOF_BACKTICK_0_TILDE_8
EQUATION_TAGS = HANDOFF_78_OF_78_UNIQUE__PROOF_76_OF_76_UNIQUE
BASELINE_HEAD_ORIGIN_REMOTE = dadf2e896a1bf083b7f91a03e39e0a7c308c9831
FILES_CHANGED = EXACT_FIVE_RELEASE_PATHS
GENERATED_OUTPUTS = NONE
V30_ROUTE_ADVANCE = YES
V30_ARITHMETIC_ADVANCE = NO
...
V30_L2 = NONE
V30_TPC_207_TRIGGER = false
V30_SEALED_FOR_NEW_SESSION = true
NUMBERED_RELEASE = NO
TPC_207_TRIGGER = false
~~~
...
本轮从已发布 V27 基线
`a2d82be3375b9592ad86b107c56745e1a7f325cd`启动，完整保留 protected
untracked、TPC-105 `__pycache__`、TPC-63 build products与 `tmp/`。三条只读
scout分别审核：

1. occurrence-native local Euler main、zero axis与 composite DFT/CRT；
2. MRT/BC/current weighted-shift source corpus与 strict endpoint；
3. Logistic/Hénon stationary factor、nonautonomous escape与 low-norm seed kernel。

主控另行完成 exact radical envelope、proof artifact与 fail-closed checker。本节不编号，
不创建 TPC-207、paper、PDF或 build。

路线结论是：V27 的 generic smooth-main STOP保持，但 source-native Euler tensor具有
精确零轴；其 inverse-residue corridor已有 Bettin--Chandee conditional power engine。
整体首墙前移到 literal smooth emitter、same-residual joint J/E reassembly与
two-sided product-local flatness。动力学 stationary odometer factor被谱论否定，只保留
nonautonomous compressed-kernel reserve。

### 75.2 Occurrence-native Euler tensor

固定 V19 同一个 physical scalar、`h0=2`、`x=2X`、`Q=x^(1/3)`、
`H=x^(21/32)` 与 comparison cutoff `z=(log x)^K`。对每个 MASTER
occurrence \(o\)，令
...
并令 \(m(o)\) 为 actual deterministic routing选出的 group，满足

\[
(x/2)^{133/400}<m(o)\le x^{1/2},\qquad
D(o)=\operatorname{rad}m(o).
\tag{75.2}
\]
...
\[
\Delta_{m,z}(h)=
\prod_{p\mid m}F_p(h)-\prod_{p\mid m}G_{p,z}(h).
\tag{75.5}
\]

source-native formal carrier为

\[
M_x^{\rm loc}(h)=
\sum_{\substack{o:\ {\rm MASTER}\\t(o),t(o)+h\in I_x}}
a_o\Delta_{m(o),z}(h),
\qquad e_x(h)=r_x(h)-M_x^{\rm loc}(h).
\tag{75.6}
\]

这是 exact decomposition和 source-native local tensor定义；尚未证明
\(M_x^{\rm loc}\) 是 hard finite interval上的 actual weighted AP main。

### 75.3 Zero、mean、Ramanujan 与 noncircular J/E

对 \(D=\operatorname{rad}m\) 逐 tensor精确有

\[
\Delta_{m,z}(0)=0,\qquad
\sum_{h\bmod D}\Delta_{m,z}(h)=0,
\tag{75.7}
\]
...
\(q\nmid D\) 时这是 CRT independence；\(q\mid D\) 时必须保留完整 unit-frequency
sum并使用 \(F_q(0)=G_{q,z}(0)\)，不得逐 row取绝对值。

令

\[
\kappa_x(h)=\widehat\psi_+(h/H)A_Q(h)/L_{\rm pr},\quad
J(f)=\sum_h\kappa_x(h)f(h),\quad E(f)=f(0)-J(f).
\tag{75.9}
\]

则 \(J(f)+E(f)=f(0)\)。generic interpolant若强制 \(M(0)=S_x\)，就以系数一返回
目标；而 (75.7) 给出

\[
\boxed{M_x^{\rm loc}(0)=0,\qquad
J(M_x^{\rm loc})+E(M_x^{\rm loc})=0.}
\tag{75.10}
\]

这个 cancellation只在 tagged Jutila main/error共同重组时有用；它本身不估计
\(e_x\)。

### 75.4 Reduced-radical CRT normal form

采用 normalized negative-sign DFT。对 \(q\nmid D\) 的 determinant row令
...
local frequency满足 \(k_p=0\) for \(p\mid g\)，而 \(p\mid R\) 时
\(k_p=a\bar q\,\overline{R/p}\pmod p\)。因此 \(g\) 精确消失：

\[
\widehat P_D(n\bar q)
=\frac{\mu(R)}{\varphi(R)}e_R(2a\bar q).
\tag{75.12}
\]

写 \(R=R_0R_1\)，其中 \(p\mid R_0\) iff \(p\le z\)，则

\[
\widehat B_{D,z}(n\bar q)
=\frac{\mu(R_0)}
{\varphi(R_0)\varphi(R_1)^2}
e_{R_0}(2a\overline{qR_1}).
\tag{75.13}
\]

\(R_1=1\) 时两项相同、difference为零。active rows满足

\[
0<|a|\le qR/H,\quad (a,qR)=1,\quad
R\ge H/q=x^{31/96+o(1)},\quad
qR/H\le x^{17/96+o(1)}.
\tag{75.14}
\]

fixture \(D=30,q=11,n=2\) 给
\(g=2,R=15,a=1\) 与 phase \(e_{15}(7)\)；错误保留 \(g\) 会给
\(e_{15}(14)\)，checker必须拒绝。

### 75.5 Radical envelope 与 Bettin--Chandee conditional engine

actual selected group的 ordered occurrence mass满足

\[
\sum_{o:m(o)=M}|a_o|\ll x^{1+o(1)}/M,
\qquad
\sum_{\operatorname{rad}M=D}1/M\le1/\varphi(D).
\tag{75.15}
\]

所以 dyadic \(R\asymp S\) 上

\[
\|B_F\|_2\ll x^{1+o(1)}S^{-3/2},
\tag{75.16}
\]

而固定 \(R_1\) 的 hybrid branch满足
...
对 \(R=x^\rho\)，Bettin--Chandee
`arXiv:1502.00769v1` Theorem 1给条件指数

\[
\mathfrak E_F(\rho)\le
\begin{cases}
2077/1920-3\rho/10,&31/96\le\rho\le1/3,\\
639/640-\rho/20,&1/3\le\rho\le1/2.
\end{cases}
\tag{75.18}
\]
...
\[
\boxed{\mathfrak E_F(31/96)=1891/1920,\qquad
399/400-1891/1920=121/9600.}
\tag{75.19}
\]

这里的 source theorem 参数与外部 normalization 不能省略。对 \(F\) branch，
...
prime-\(q\)、short-\(a\) arrays 的 norms分别是
\(Q^{1/2+o(1)}\)、\(A^{1/2+o(1)}\)，第三个 array使用
(75.16)--(75.17)，外部 normalization统一为 \(H/L_{\rm pr}\)。
coprimality由 Mobius split保留。若
\(R_0=x^\sigma,R_1=x^\lambda\)，则 \(G\) 的 second
Bettin--Chandee term满足

\[
\mathfrak E_{G,2}(\sigma,\lambda)\le
\begin{cases}
1-\sigma/8-\lambda/2,&1/3+\lambda\ge\sigma,\\
23/24-5\lambda/8,&\sigma\ge1/3+\lambda,
\end{cases}
\tag{75.19c}
\]

两支分别至多 \(737/768\) 与 \(23/24\)。\(F\) 的 second term也分别至多
\(737/768\)、\(23/24\)；\(G\) first term的 worst endpoint仍为
\((\sigma,\lambda)=(31/96,0)\)，即 \(1891/1920\)。因此两个 source terms、
两个 \(F/G\) branches均严格越过 endpoint。这个 source-backed local engine仍以
exact hard-shell smoothing、moving \(R_0/R_1\) emitter、coprimality Mobius cells、
both signs、\(q\mid D\) aggregate cancellation与 one outer reassembly为条件。
因此 `ARITHMETIC_ADVANCE=NO`。

### 75.6 Residual product-local gate

MRT `arXiv:1707.01315v3` Proposition 3.1是 abstract finite-sequence
product-local L2 reduction。一个充分的新 theorem是对每个 dyadic \(Y\ge H\)

\[
\int_{\mathfrak m}|B_xW_x|\le x^{1+o(1)},\qquad
\sup_\alpha\int_{\mathfrak m\cap I(\alpha,1/Y)}|B_xW_x|
\le x^{1+2\theta+o(1)}/Y
\tag{75.20}
\]

with one fixed \(\theta<13/4800\)，并 exact重组为同一个 tagged \(e_x\)。
现有 MRT只给 one-sided logarithmic input；即使理想化 one-sided flatness，输出仍有
\(H^{1/4}=x^{21/128}\) loss，超过 endpoint

\[
21/128-13/4800=1549/9600.
\tag{75.21}
\]

因此 product-local flatness与 same-residual J/E reassembly仍是 analytic primary。

### 75.7 Stationary factor no-go 与 compressed kernel

若 mixing probability system exact factor到 finite rotation或 odometer，则 nontrivial
quotient character拉回 root-of-unity eigenfunction，其 correlations不趋零，矛盾。
所以 stationary Logistic/Hénon-to-odometer factor STOP；nonautonomous stage diagram
不受这个 theorem直接排除。

若指定 seed evaluation有 target-independent Riesz representer \(K_j\)，则

\[
S_j(a^\dagger)=\int K_jS_j\,d\nu,\quad
\int K_jd\nu=1,\quad
|S_j(a^\dagger)-m_j|\le\|K_j\|_2V_j.
\tag{75.22}
\]

要从 \(m_j\gg x_j/\log^2x_j\) 推出 positivity，充分条件是
...
在 full primorial quotient上，point evaluation的唯一 kernel为
\(P{\bf1}_{0}\)，norm \(=\sqrt P\)。取 \(P=\prod_{p\le\sqrt x}p\) 时为
\(\exp((1/2+o(1))\sqrt x)\)，generic variance无法支付。Wang--Young Hénon
dense-tree/natural-extension只登记为 topology；arithmetic stage/event/measure/seed
diagram仍 OPEN。

### 75.8 Status atlas 与路线顺序

~~~text
V28_MAXIMUM_CLAIM
  = EXACT_OCCURRENCE_NATIVE_EULER_ZERO_AXIS_AND_REDUCED_RADICAL_CORRIDOR_PLUS_SOURCE_BACKED_CONDITIONAL_BETTIN_CHANDEE_ENGINE_PLUS_STATIONARY_FACTOR_NO_GO_AND_COMPRESSED_KERNEL_ROUTE
V28_ROUTE_ADVANCE = YES
V28_ARITHMETIC_ADVANCE = NO
V28_FIXED_ATOM_CREDIT = 0
V28_STRICT_1_OVER_400 = UNPAID
...
1. analytic primary：same tagged \(e_x\) 的 joint Jutila main/error reassembly与
   product-local flatness；
2. analytic second road：完成 reduced-radical \(F/G\) smooth emitter并集体应用
   Bettin--Chandee；
3. dynamics reserve：nonautonomous target-independent low-norm kernel compiler。

三条路线互不借 theorem credit。O161 parents、pair-native/H1、A1/A2与 global
architecture继续独立 OPEN。

### 75.9 Artifact、checker 与 release 状态

proof为
`research/tpc-big-road/bridge_b_euler_zero_axis_and_kernel_carrier.md`；
checker为
`research/tpc-big-road/tpc_bridge_b_euler_kernel_checker.py`。checker冻结
49-field contract、36-row registry与 SHA-256

~~~text
2926e4dc94080ff3179970dc134c1a1edb76bcb5b7f64be783b4bc747d5c7a0b
~~~

149 contract、75 registry、12 source、15 dependency与149 result mutations必须
...
### 75.10 Final QA pending

~~~text
V28_FINAL_RELEASE_QA = PASS
V28_MATH_SOURCE_FORMULA_QA = PASS
V28_CHECKER_ADVERSARIAL_QA = PASS
V28_CROSS_DOCUMENT_RELEASE_SCOPE_QA = PASS
V28_PRE_CLOSURE_HANDOFF_SHA256_RAW = 3180cf9f08e835391dc523d812faf3b794812d8c80c58723010a4c3225467e1a
V28_PRE_CLOSURE_HANDOFF_SHA256_CANONICAL_LF = 619e1c3ec6fd7397feefbefd6c75c2851c7963de1b91d976279b7dd300935718
V28_COMPASS_SHA256_RAW = 00534a02ee7812ad5383728e031af69058c63130084f48a461baf03f0232c297
V28_COMPASS_SHA256_CANONICAL_LF = 325c3f1ec4a64fd36e40038fda778d72682af19b5b26f0ca7c10a72683218e63
V28_README_SHA256_RAW = d03e223ff3fdea50731dce029d2e985a2fd0b30d0e49ef9fbfc061d4a30f57df
V28_README_SHA256_CANONICAL_LF = d2f35ca8e1791b4037f60085b187ba7eab0002c6f5e19edf2f01b16373696f13
V28_PROOF_SHA256_RAW_CANONICAL_LF = 922d5601b088a8a3a8dd52d3e9d186c85e7fea00ca670f3c6f324c1d433da464
V28_CHECKER_SHA256_RAW_CANONICAL_LF = dddff8b09472fe9fc563caa3f6c204c24895aa45613781efba0877ae3fc421a7
V28_CHECKER_STDOUT_SHA256_RAW_CRLF = 920584fdf53ed8ea792af358e7d5ededd08d690eeecaa8f30207a74181958acd
V28_CHECKER_STDOUT_SHA256_CANONICAL_LF = 568df7eda9b9141e6a99731de8399bca4b7ffc765fd15ca8b365d546ac57458d
V28_CHECKER_STDOUT_PAYLOAD_SHA256 = 9a71441fd68c53b24acad40f71902722e80a73df70183b75842896c89eddedea
V28_CHECKER_CONTRACT_REGISTRY_SOURCE_DEPENDENCY = 49/36/5/6
V28_CHECKER_MUTATIONS = 149/75/12/15/149
V28_CHECKER_REGISTRY_SHA256 = 2926e4dc94080ff3179970dc134c1a1edb76bcb5b7f64be783b4bc747d5c7a0b
STARTUP_REGRESSION = 22/22
SUPPLEMENTAL_TPC111_TPC124_TPC126_TPC127 = 4/4
V23_TO_V28_DEPENDENCY_PROCESSES = 12/12
V23_TO_V28_DEPENDENCY_STDOUT_IDENTITIES = 6/6
BIG_ROAD_CHECKER_PROCESSES = 32/32
BIG_ROAD_CHECKER_STDOUT_IDENTITIES = 16/16
RELEASE_ALLOWLIST = EXACT_FIVE_PATHS
CACHED_DIFF = EMPTY
DIFF_CHECK = PASS
PROTECTED_UNTRACKED_COUNT = 130
PROTECTED_UNTRACKED_MANIFEST_SHA256 = 9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1
MARKDOWN_FENCES = HANDOFF_BACKTICK_2838_TILDE_44__COMPASS_BACKTICK_224_TILDE_22__README_BACKTICK_378_TILDE_20__PROOF_BACKTICK_0_TILDE_28
EQUATION_TAGS = HANDOFF_78_OF_78_UNIQUE__PROOF_77_OF_77_UNIQUE
BASELINE_HEAD_ORIGIN_REMOTE = a2d82be3375b9592ad86b107c56745e1a7f325cd
FILES_CHANGED = EXACT_FIVE_RELEASE_PATHS
GENERATED_OUTPUTS = NONE
V28_ARITHMETIC_ADVANCE = NO
V28_FIXED_ATOM_CREDIT = 0
...
V28_L2 = NONE
V28_TPC_207_TRIGGER = false
V28_SEALED_FOR_NEW_SESSION = true
NUMBERED_RELEASE = NO
TPC_207_TRIGGER = false
~~~
...
### 71.1 基线、任务与 source lock

V24从已发布 V23 commit
`1c3d13302818206104d6ff2396c035b4ec0c091d`继续。它不再搜索一个未定义的
“second refinement”，而只问：对同一个 literal fixed-`h0=2` physical scalar，现有
source identities能无损走到哪一层，第一处真正需要新 theorem 的位置在哪里。

两个 read-only agents分别完成 Blomer--Li/Blomer--Pascadi primary-source lock与
formula-level atom/reassembly审计；全部正式写入仍由主控完成。冻结来源为：

1. Ford--Maynard Lemma 5.2 的 ordered `+2,-1` V19 emitters；
2. Blomer--Li `arXiv:2511.03294v1` §2.1 Lemma 1、(2.2)、Lemma 2 printed
   formula与其 Farey-neighbor proof；其中 Lemma 2 displayed `max`/fixed-plus不是
   source-backed可用结论，V24另给 corrected repository proof；
3. Blomer--Pascadi `arXiv:2607.24311v1` Theorems 1.1、5.2、5.5；
4. V19 literal raw row、V21 hybrid residual、V23 prime-shell parameter clock。

TPC-27--32 legacy writers与 TPC-122 writer均未执行；全部 protected untracked保持原样。

### 71.2 Literal physical determinant atoms

对每个 ordered V19 MASTER occurrence `o`，写
...
prime branch 与 hybrid branch 在 enforcing diagonal之前分别发射：

```text
nu_p=(o,d_phys,r_p),
u_p=d_phys r_p-2 in I_x,
A_x(nu_p)=a_o mu(d_phys)log(r_p),
D_x(nu_p)=d_phys r_p-T_o-2;                           (71.2)

nu_h=(o,d_rough,e_sieve,r_h),
e_sieve|P(z), u_h=e_sieve r_h-2 in I_x, d_rough|u_h,
A_x(nu_h)=-a_o C_z W_z g_z(d_rough)mu(e_sieve),
D_x(nu_h)=e_sieve r_h-T_o-2.                          (71.3)
```

所有 shell masks、rough-divisor restrictions、occurrence multiplicities与 original
normalization保持。令

```text
G_x(alpha)=sum_nu A_x(nu)e(alpha D_x(nu)).            (71.4)
```
...
S_x=int_0^1 G_x(alpha)dalpha
   =sum_(nu:D_x(nu)=0)A_x(nu).                        (71.5)
```

这是 literal determinant equality，不是 `D_x congruent 0 (mod q)`。

以下 label必须保持异型：

```text
d_phys,d_rough,e_sieve;
q_J,a_J,beta_J,d_J,ell_J;
c_F,b_F,z_F,u_F,t_F;
d_BL;
m_BP,n_BP,a_BP,c_BP.                                 (71.6)
```

特别地 `d_phys!=d_J!=d_BL`、`q_J!=c_F`；只有额外 theorem允许的 local slice才可把
某个 BP modulus与其中一个 modulus对齐。

### 71.3 Exact Jutila main/error split

Blomer--Li Lemma 1 假设 `omega:[1,Q_src]->[0,infinity)`、
`L=sum_q phi(q)omega(q)!=0`、smooth `psi:R->[0,1]` supported on `[-1,1]`且
`int psi=1`、`0<delta<1/2`，并定义

```text
chi(alpha)=1/(delta L)sum_q omega(q)sum_(a mod q)^*
           sum_(k in Z)psi((alpha-a/q+k)/delta).      (71.7)
```

V23 specialization继续冻结为

```text
Q_mes=x^(1/3), Q_src=2Q_mes,
omega(q)=1_(q prime,Q_mes<q<=2Q_mes),
L=sum_q(q-1), eta=1/32,
delta=Q_mes^(-63/32)=x^(-21/32).                     (71.8)
```
...
```text
S_x=M_x+E_x,
M_x=int chi(alpha)G_x(alpha)dalpha,
E_x=int(1-chi(alpha))G_x(alpha)dalpha.                (71.9)
```
...
K_chi(D)=L^(-1)sum_q omega(q)c_q(D)hatpsi(-delta D). (71.10)
```

对 prime `q`，`c_q(D)=q-1`若 `q|D`，否则为 `-1`；所以 main仍含全部 determinant
copies。写成一个 `q`-average不会创造 free `1/R_x` gain。

### 71.4 Complete Farey geometry与 Kloosterman atoms

Blomer--Li v1 Lemma 2 的 displayed formula按字面不成立。它把 interval写成
`max(1/(c|z|)-c,C)`，但 Farey-neighbor denominator必须同时满足
`C-c<t<=C`与`t<=1/(c|z|)-c`，所以正确上界是 `min`。原式还在两侧都用
`e_c(u t)e_c(u inverse(b))`；然而左、右邻分母分别满足
`c' congruent inverse(b)`与`c'' congruent -inverse(b)`。

finite falsifiers为：`C=c=1,z=1/4`时 printed interval有3个整数，corrected interval
只有1个；`C=5,c=3,b=1,z=-1/22`时 corrected interval为`{3,4}`，fixed-plus detector
为0而 negative signed detector为3。由 source自身的 exact Farey partition与 neighbor
congruences，V24独立定义

```text
I_C^corr(c,z)
 =(C-c,min(1/(c|z|)-c,C)] intersection Z,
sigma(z)=+1 (z>0), -1 (z<0).                        (71.11)
```

`z=0`为 measure-zero convention。对任意 smooth one-periodic `f`，仓库内逐 Farey arc
证明 exact corrected identity；应用于 `f=(1-chi)G_x`给

```text
E_x=sum_(c<=C)int_(-1/(cC))^(1/(cC)) 1/c
    sum_(u mod c)sum_(t_F in I_C^corr(c,z))e_c(u t_F)
    sum_(b mod c)^*e_c(sigma(z)u inverse(b))f(b/c+z)dz. (71.12)
```

把 `f`拆成 identity与`-chi` branches；对一枚 mismatch `D`，identity branch的完整
unit sum为

```text
sum_(b mod c)^*e_c(bD+sigma(z)u inverse(b))
  =S(D,sigma(z)u;c).                                 (71.13)
```

因此 literal determinant 到 complete Farey/Kloosterman atoms 已
`PROVED_EXACT_L0`，但 status必须写作 repository derivation而不是 printed Lemma 2
source transfer。这一步没有使用 Weil/BP bound，也没有给 physical saving。

### 71.5 Rational `chi` 与 signed collective emitter

Blomer--Li (2.2) 的第二行精确为

```text
chi(b/c+z)=1/(delta L)sum_q omega(q)sum_(d_J|q)mu(q/d_J)
 sum_(ell_J congruent b d_J mod c)
 psi(delta^(-1)(ell_J/(c d_J)+z)).                   (71.14)
```

这里没有额外 `q` factor。对 prime `q`，`d_J=1`支在 `chi`内带负号，`d_J=q`支
带正号。代回 corrected (71.12) 后，合法 emitter必须在任何 outer absolute value之前
同时保留

```text
identity / -chi,
d_J=1 / d_J=q,
q_J,c_F,b_F,u_F,t_F,ell_J,z_F,sigma(z_F),
prime / hybrid,
zero / nonzero / nonunit / axes / tails.             (71.15)
```

这给两个明确 theorem targets：

```text
V24_PRIME_SHELL_JUTILA_ERROR_SIGNED_FAREY_KLOOSTERMAN_THEOREM
  = OPEN_NEW_THEOREM
V24_PRIME_SHELL_JUTILA_MAIN_TO_BP_COLLECTIVE_EMITTER
  = OPEN_NEW_THEOREM.                                 (71.16)
```

第一项必须直接控制完整 literal error integral；第二项必须发射 source-valid short BP
arrays并支付全部 coefficient、interval、norm、unit、outer、hybrid、tail与 cover losses。

### 71.6 Source-transfer与 factorability firewalls

Blomer--Li 的最终 theorem对象是 `A(n,1)tau(m)`或 smooth divisor convolution，不是
V19 Möbius/log row。其 proof对 divisor side作 Voronoi、对 `A(n,1)`作 GL(3) Voronoi，
并在 (3.14)选择

```text
q=pt, p prime, t unrestricted smooth.                (71.17)
```

原文明确说明后续对 `d_BL`作 Poisson需要 `t`没有 arithmetic restriction，尤其不能
限制为 prime。其 native clock为 `Q=x^(4/7)`、`delta=x^(-1)`；强塞 V23
`Q=x^(1/3)`时 (3.25) 第一项至少为 `x^(7/6+o(1))`，不是 saving。其 complete
`b`-sum又是一维 GL(3)-Voronoi character sum，不是 BP arbitrary two-array bilinear。

因此 §6 的三个 V24 STOP cells 精确成立：printed Lemma 2 literal transfer、direct
GL(3)-divisor transfer及 prime-only/factorable splice分别停止。它们不停止 Lemma 1
Jutila interface、corrected signed Farey repository identity或新的 collective theorem。
一个 source-native factorable reserve仍可另行构造：

```text
V24_FACTORIZABLE_AUXILIARY_JUTILA_ENSEMBLE_WITH_LITERAL_PHYSICAL_REASSEMBLY
  = OPEN_NEW_CONSTRUCTION.                            (71.18)
```

但 `q_aux=pt`必须与 V21/V23 prime shell分开声明，并重新支付自己的 `L`、error、clock、
literal attachment与 provenance。

### 71.7 Finite diagnostic、checker与 claim ceiling

checker先锁定 source correction：`C=c=1,z=1/4`给 printed-max/corrected-min counts
`3/1`；`C=5,c=3,b=1,z=-1/22`给 negative signed/fixed-plus detector values `3/0`。
它再从 V19重算 `x=166` literal beta：support=`30`、mass=`839/42`。再取 source-backed
local `d_7` profile作为 typed synthetic diagnostic，而不是 actual residual theorem；其
support=`71`、shell mass=`-7/36`。在 `c=5`的 residue correlation为

```text
(275/36,-943/108,53/27,821/108,-2669/216),          (71.19)
```

support=`5/5`、zero axis=`275/36`；在 `c=7` support=`7/7`、zero axis=`407/108`。
这证明 finite literal periodization是 full且 axes存活，只认证类型，不认证 growing bound。

proof为
`research/tpc-big-road/bridge_b_literal_jutila_farey_atom_compiler.md`；checker为
`research/tpc-big-road/tpc_bridge_b_literal_jutila_farey_atom_checker.py`。checker冻结
58-field contract、55-row registry与 SHA-256

```text
6ce7f2c3d2eb3a2e30900aaa7285d8e99bca139711db04915bdd9453a0a1808e. (71.20)
```

176 contract mutations、170 registry mutations与7个 coordinated dependency-lock
attacks必须全部拒绝；normal/optimized `--check`必须 stdout byte-identical。maximum
claim为
...
STRICT_1_OVER_400 = UNPAID
L2 = NONE
TPC_207_TRIGGER = false.                              (71.21)
```

### 71.8 Release QA

```text
V24_FINAL_RELEASE_QA = PASS
V24_MATH_SOURCE_FORMULA_QA = PASS
V24_CHECKER_ADVERSARIAL_QA = PASS
V24_CROSS_DOCUMENT_RELEASE_SCOPE_QA = PASS
STARTUP_READ_ONLY_REGRESSION = 22/22 PASS
SUPPLEMENTAL_TPC111_124_126_127 = 4/4 PASS
CURRENT_BIG_ROAD_NORMAL_OPTIMIZED = 24/24 PASS
CURRENT_BIG_ROAD_STDOUT_BYTE_IDENTITIES = 12/12 PASS
V24_CONTRACT_FIELDS_MUTATIONS = 58/176
V24_REGISTRY_ROWS_MUTATIONS = 55/170
V24_SOURCE_LOCK_MUTATIONS = 7/7 REJECTED
V24_REGISTRY_SHA256 = 6ce7f2c3d2eb3a2e30900aaa7285d8e99bca139711db04915bdd9453a0a1808e
V24_CHECKER_STDOUT_SHA256 = 0e92ed32291676de6b334213cb0ce41b32d5c2f4881a1246bdf4117a08b5a3c7
V24_PROOF_RAW_AND_CANONICAL_LF_SHA256 = d8fb384e8f3dea30ea9c525805ec12878905e8424ad878aab27ab1f3bffc5a69
V24_CHECKER_RAW_AND_CANONICAL_LF_SHA256 = ab499b2b809e446747baf817f48c11c70ca9ac9d8726271df22892e00c5ed54d
MARKDOWN_FENCES = HANDOFF_2780_COMPASS_202_README_358_PROOF_106_BALANCED
EXPECTED_RELEASE_PATHS = 5
CACHED_DIFF_PATHS = 0
PROTECTED_UNTRACKED = 130
PROTECTED_MANIFEST_SHA256 = 9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1
REMOTE_OVERLAP_AUDIT = 5_COMMITS_63_PATHS_RH_ONLY_ZERO_TPC_OR_POLICY_OVERLAP
PRE_RELEASE_BASE_HEAD = 1c3d13302818206104d6ff2396c035b4ec0c091d
OBSERVED_ORIGIN_MAIN = cd46509c231094780757e9542a057e259fc2d508
NUMBERED_RELEASE = NO
TPC207_CREATED = false
NUMBERED_PAPER_PDF_BUILD_CREATED = NO
TPC27_TO_32_LEGACY_WRITERS_EXECUTED = NO
...
三路 read-only QA已对最终同一快照给出 PASS。source/formula agent把 printed Lemma 2
严格限制为 counterexample-backed STOP，只保留 Lemma 1及 underlying Farey partition/
neighbor relations的 source credit；corrected `min`/`sigma(z)` identity明确为 repository
derivation。checker agent逐项拒绝全部 formal-token、provider-decoy、digest/path rebind与
false-release attacks。release-scope agent复算五文件结构、exact allowlist、protected
manifest与 claim firewall。远端在本轮期间增加的5个 commits、63条 unique paths全部仅为
`RH_HANDOFF.md`或 `papers/RH-382/**`、`papers/RH-383/**`，与 TPC及 shared policy零重叠。
只允许提交本轮五个预期文件；rebase后仍须重跑受影响 checks并验证最终远端 hash一致。
...
V22从已发布 V21 commit
`a6d3cbc8cabff382ce0f4c1d3d8ee2e86d79ca3b`启动。启动时 local `HEAD`、
`origin/main`与 remote `refs/heads/main`一致；working tree只有原 130个 protected
untracked files，canonical manifest仍为

```text
9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1. (69.1)
```

启动 `TPC_HANDOFF.md` raw SHA-256为
`6e550471a0b03bf8524afef3e513fddd3b2ab13684808461af91d11c62ec6a6d`。
三个 read-only agents分别审核：direct mesoscopic projector/dispersion algebra，
Blomer--Pascadi及相邻 primary sources的 literal attachment，以及 exact symbolic/dynamical
return与 distinguished-seed gate。全部正式写入由主控完成；agents没有改仓库文件。
TPC-27--32 legacy writers与 TPC-122 writer均未执行。

本轮不是再造一个 centered-covariance微变体，而是回答两个大路问题：

1. `q`-fiber centering本身是否已经生成一个可送进 Fourier/Kloosterman engine的对象；
2. exact symbolic orbit-sum是否已经把 prescribed arithmetic seed交给标准混合定理。

两问的直接答案都是否定，但各自留下一个更精确、可证伪的 compiler gate。

### 69.2 Exact projector normal form

冻结 V21 physical object：
...
beta_x^raw=V19 combined raw MASTER covector,
Q_x={q prime:x^(1/3)<q<=2x^(1/3)},
R_x=#Q_x.                                             (69.2)
```

对 `f in C^(I_x)`，令 `E_q`为 counting-measure到 actual residue fibers的正交投影：

```text
(E_qf)(t)=1/n_(q,t mod q) sum_(u in I_x,u=t mod q)f(u),
Pbar_Q=R_x^(-1)sum_(q in Q_x)E_q.                    (69.3)
```

actual `n_(q,a)`始终保留；empty fibers不参与除法。V21 exact split现在写成

```text
Hbar_Q=<beta_x^raw,Pbar_Q w_x^(z)>,
Cbar_Q=<beta_x^raw,(I-Pbar_Q)w_x^(z)>,
S_x=Hbar_Q+Cbar_Q.                                   (69.4)
```

V21已经 source-backed证明 `Hbar_Q<<_(A,K)x/log^A x`。因此

```text
Cbar_Q=S_x-Hbar_Q.                                   (69.5)
```

式 (69.5) 是 firewall，不是 saving：如果对 `Pbar_Q`或 `E_q`作 additive congruence
展开，所触及的是已付 `Hbar_Q`；identity branch `S_x`没有因此消失。

### 69.3 Residue Fourier annihilation 与 literal falsifier

令 `r_q=(I-E_q)w_x^(z)`。逐 residue fiber exact有

```text
sum_(t in I_x,t=a mod q)r_q(t)=0.                    (69.6)
```

所以对每个 `h mod q`，

```text
sum_(t in I_x)r_q(t)e_q(ht)
 =sum_(a mod q)e_q(ha)sum_(t in I_x,t=a mod q)r_q(t)
 =0.                                                 (69.7)
```

这说明 residue-only Fourier transform完全看不见 centered directions。它不说明
`<beta,r_q>=0`，因为 `beta_x^raw`在同一 fiber内不是常数。为证明 operator information
loss，checker从 V19 source-locked literal row重算 `x=166,q=7`，但刻意定义
...
所有 7个 fiber sums及全部 7个 Fourier marginals为零，而

```text
<beta_166^raw,r_syn>=2359675/77616.                  (69.8)
```

这里 second vector是 synthetic，不是 actual prime residual `w_166^(z)`，也不给 arithmetic
credit。它只证明 residue transform并非 within-fiber covariance的充分统计量。因此把
(69.7)直接改名为 covariance cancellation是一个 exact type error，而非估计不够强。

### 69.4 Complete ensemble 的大 identity space

每个 `E_q`为正半定正交投影，故 `Pbar_Q`正半定，并且

```text
ker(Pbar_Q)=intersection_(q in Q_x)ker(E_q),
rank(Pbar_Q)<=sum_(q in Q_x)rank(E_q)<=sum_(q in Q_x)q,
dim ker(Pbar_Q)>=|I_x|-sum_(q in Q_x)q.               (69.9)
```

由 elementary prime count upper bound，`sum_q q=O(x^(2/3))=o(x)`；所以
`I-Pbar_Q`有至少 `x/2-O(x^(2/3))`个 eigenvalue `1`。完整 predeclared ensemble没有
带来 automatic smoothing或 low-rank compression。

checker的 exact rational fixture取 `x=1000`、`Q={11,13,17,19}`、shell dimension
`500`；four mean spaces的 union rank为 `57`，common centered identity multiplicity为
`443`。它还用多项式

```text
z^t0 product_(q in Q)(1-z^q)                         (69.10)
```

给出 expanded support `15`的显式 common-kernel witness；这里
`11+19=13+17=30`使两个同号 subset terms合并，故不是把空 subset人为删去。这个 witness只是 geometry
certificate，不冒充 literal residual的饱和定理。

### 69.5 Shift comb、diagonal 与 paid off-diagonal

对一个 nonempty `q`-fiber、`n=n_(q,a)`，逐 `t` exact有

```text
(I-E_q)F(t)
 =1/n[(n-1)F(t)-sum_(u!=t,u=t mod q)F(u)].           (69.11)
```

comb的 normalized `l1`为 `2(n-1)/n`，normalized squared `l2`为 `(n-1)/n`；没有
contraction。令

```text
Abar_Q=R_x^(-1)sum_(q in Q_x)sum_(t in I_x)
 beta_x^raw(t)w_x^(z)(t)/n_(q,t mod q).              (69.12)
```

则 exact

```text
Cbar_Q=(S_x-Abar_Q)+(Abar_Q-Hbar_Q).                 (69.13)
```

由 `n_(q,a) asymp x/q`、`q asymp x^(1/3)`与 V19 divisor bounds，
`Abar_Q=O_K(x^(1/3+o(1)))`。第二括号可由 (69.5)与 paid `Hbar_Q`反解；第一括号
仍以系数 `1-O(x^(-2/3))`包含 original diagonal `S_x`。所以 direct projector
dispersion没有创造新的 off-diagonal主路。

在 literal determinant variables中，prime side的 congruence是

```text
d r-t=q ell+2,                                       (69.14)
```

`ell=0`就是 original determinant-two diagonal。hybrid side还必须保留
`D s-t=q ell`、`e r-D s=2`、Möbius/log signs与 rough tail。任何 post-Poisson
route都必须从这些 literal equations开始，而不是从已经 annihilated的 residue marginal
倒推物理信息。

### 69.6 Blomer--Pascadi local engine 与缺失 compiler

source lock为 Blomer--Pascadi `arXiv:2607.24311v1` Theorems 1.1、5.2、5.5。对 prime
`q`、unit phase及 balanced `M,N asymp q^(1/2)`，其 local bilinear Kloosterman estimate
相对 unnormalized trivial scale给 `q^(-1/32+o(1))` gain。由于 `q=x^(1/3)`，这对应

```text
q^(-1/32)=x^(-1/96),
1/96-1/400=19/2400.                                  (69.15)
```

这是在 compiler losses之前的 conditional exponent budget，不是 current physical
saving。direct attachment有三处 fatal：

1. `(I-E_q)w`的 residue transform由 (69.7)恒等为零；
2. natural fiber quotient长度 `x/q asymp q^2`，不在 source的 `M,N<=q`输入窗口；
3. 从 literal determinant完成 coprime inversion/Poisson之后的 coefficient一般有 full
   `q` frequency support，而不是自动 `sqrt(q)` support。

若先 full-`q` completion，再切 `B asymp q^(1/2)`个 `sqrt(q)` blocks，且只用
black-box Cauchy，则 uniform proof给
...
这个 possible factor在 equal block masses时 sharp；full support本身并不证明 literal
coefficient饱和 worst case。准确 STOP是：没有额外 block structure时，Cauchy-only
compiler不能相对 `q^(-1/32)`认证 net gain。唯一保留的解析 highway是证明一个 exact
identity

```text
S_x=R_x^(-1)sum_(q in Q_x)sum_j
 kappa_(x,q,j) S(a_(x,q,j)m,n;q)+Err_x,              (69.16)
```

其中 source端原生提供可控的 short/balanced supports，完整保留 zero/nonunit/diagonal/
tails、Möbius/log coefficients、actual ranges与 normalization，且

```text
sum_(q,j) ||kappa_(x,q,j)||_phys * q^(-1/32)
```

在 full physical-loss ledger后仍支付 strict `1/400`。当前记为

```text
V22_LITERAL_SHBD2_DIAGONAL_POST_POISSON_COMPILER_GATE
  = OPEN_CONDITIONAL.                                (69.17)
```

Zheng arbitrary-modulus exponent `7/36<1/3`且 well-factorable slots不匹配 whole prime
shell；现有 Ford--Maynard、BDH、Maynard/Pascadi families也没有给出 (69.16)。这只是
declared source corpus的 no-survivor审计，不是未来 theorem的 nonexistence claim。

### 69.7 Exact odometer return 与动力学 first fatal

在 profinite odometer `T(r)=r+1`、distinguished seed `0`上，把 shell-supported
observable定义为

```text
Phi_x(T^t0)=beta_x^raw(t)R_x^(-1)
             sum_(q in Q_x)(I-E_q)w_x^(z)(t).        (69.18)
```

则逐 coefficient exact有

```text
Cbar_Q=sum_(t in I_x)Phi_x(T^t0).                    (69.19)
```

这是 lossless `L0` orbit-sum/Bratteli encoding，满足用户所强调的 symbolic/nonautonomous
视角；它没有给 cancellation。第一处 dynamics fatal是

```text
NO_SOURCE_BACKED_EXACT_PHYSICAL_COVARIANCE_TO_BOUNDED_COMPLEXITY_
MIXING_OBSERVABLE_INTERTWINER.                       (69.20)
```

`E_q`只保证 `w`逐 fiber mean zero，不保证 `beta*w` mean zero。`Phi_x`的 support、
variation、rank与 parameter dependence都随 `x`增长；普通 Logistic/Henon ergodicity、
a.e.-seed ASIP/DBC或 a.e.-parameter typicality均不控制同一个 prescribed arithmetic seed
的 triangular array。把 Hénon当 natural extension也必须先给 exact stage-preserving
semiconjugacy，不能靠几何图像取得 theorem credit。

### 69.8 保留的 transversal common-return highway

动力学仍有一条大胆但精确的 conditional highway。预先声明 positive-measure parameter
family `{F_lambda}`与 common critical section，要求：

1. 对每个 physical stage，存在 coefficientwise exact map把 (69.18)送到同一个
   stage-preserving carrier；
2. arithmetic return word/time独立于 `lambda`，而不是逐 stage重新选择参数；
3. carrier mean由独立 theorem证明为零或足够小；
4. 对某个同一 good `lambda`及全部 sufficiently large stages，uniform triangular
   ASIP/LIL或 stronger pointed theorem给
   `O(x^(1-delta)log^C x)`；
5. 编码、exceptional set、return multiplicity、normalization及 physical coefficient
   ledger exactly once闭合。

任意 fixed `delta>0`都足以支付 arbitrary log powers。当前 gate为

```text
V22_TRANSVERSAL_COMMON_RETURN_CENTERED_PHYSICAL_CARRIER_GATE
  = OPEN_NEW_CONSTRUCTION.                           (69.21)
```

下一 finite falsifier锁定 adjacent `x=166,168` stages及全部 active mesoscopic residues：
检查一个 open parameter interval上是否存在 exact common return、return time是否独立于
parameter、以及 carrier evaluation的 parameter derivative是否 nondegenerate。失败只会
停止该 finite ansatz；成功也只进入 uniform/all-stage theorem gate，不自动给 arithmetic
credit。

### 69.9 STOP/OPEN atlas 与 checker

V22新增的三个 narrow stops为

```text
DIRECT_CENTERED_RESIDUE_FOURIER_OR_KLOOSTERMAN
 = STOP_SCOPED_EXACT_ZERO_MARGINAL,

COMPLETE_MESOSCOPIC_PROJECTOR_DIRECT_DISPERSION_OR_COMPRESSION
 = STOP_SCOPED_PAID_MEAN_BRANCH_AND_LARGE_IDENTITY_SPACE,

NAIVE_FULL_q_FOURIER_COMPLETION_TO_BP_BALANCED_BLOCKS
 = STOP_SCOPED_BLACK_BOX_CAUCHY_CERTIFIES_NO_NET_GAIN_WITHOUT_BLOCK_STRUCTURE. (69.22)
```

它们不停止 (69.16) 的 literal diagonal compiler、(69.21) 的 transversal construction、
真正新的 centered theorem、A1/A2、两个 O161 parents、pair-native、H1或 global
architecture。V21 `F12-only` arithmetic subgate仍为正；V22本身 arithmetic advance=`NO`。

proof与 checker为

```text
research/tpc-big-road/bridge_b_centered_projector_firewall.md
research/tpc-big-road/tpc_bridge_b_centered_projector_checker.py. (69.23)
```

checker只读锁定 V21 canonical dependency、20-field exact contract、36-row exact registry、
actual ragged projectors、zero marginals、finite ranks、common-kernel polynomial、
synthetic `w_syn=beta_166^raw` information-loss witness、odometer return及 exponent
ledger。该 witness不是 actual physical residual。registry SHA-256为

```text
19c228b356cf1b8034eb3e018c0ef0e2d8363515062f9472c4857b54b1f1d8c7. (69.24)
```

normal与 optimized运行必须 stdout byte-identical；contract/registry validators把 semantic
literals与 digest内嵌于 validator，拒绝 coordinated provider self-attestation。built-in
matrix须实际执行 `41`个 contract mutations与 `76`个 registry mutations；mutation bases
来自已经独立验证的 `CONTRACT/REGISTRY_ITEMS`，不由可重绑定 fixture provider自证。

### 69.10 Release closure

V22最大 claim仅为

```text
EXACT_L0_CENTERED_PROJECTOR_FIREWALL_AND_CONDITIONAL_FORK.          (69.25)
```

它不是 centered-covariance saving，不是 Logistic/Hénon mixing theorem，也没有支付
fixed atom或 strict endpoint。最终 independent math/source/dynamics、checker-adversarial
与 release-scope agents均返回 `PASS`、first fatal=`NONE`、`files_changed=[]`。冻结的
proof与 checker raw/canonical-LF SHA-256分别为

```text
d283efb1162ee90b91d084c492a75c7bbbfa3ddf06bcd80dd1a735269bf2699e,
013aea5a5975c65c3dda9be0df335dd672fec7776096f4edff13b1d7946637a2. (69.26)
```

checker normal/optimized均 exit `0`、stderr为空、stdout byte-identical；867-byte stdout
SHA-256为

```text
c7adb8363a432b718137993ccaf73f17cc90421634bf8c8222cd093ef4cc8650. (69.27)
```

20-field contract、36-row registry、41个 contract mutations与76个 registry mutations
全部实际执行；provider-decoy、coordinated contract/registry/digest false release、dependency
path/hash与 rank-engine attacks均 fail closed。synthetic witness和 black-box Cauchy-only
STOP的精确类型已由四文档与 checker交叉锁定。

主控随后完成：

1. 第 1 节 22项启动回归：`22/22 PASS`；
2. TPC-111/124/126/127 supplemental checks：`4/4 PASS`；
3. 全部10个 big-road checkers normal/optimized：`20/20 PASS`；
4. normal/optimized stdout：`10/10 byte-identical`；
5. exact five-file allowlist、cached diff=`0`、`git diff --check=PASS`；
6. Markdown headings无重复、fences全部配对、§69 equation definitions唯一；
7. original protected untracked files=`130`，canonical manifest仍为
   `9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1`；
8. 没有新 TPC-207、paper、PDF或 build output。

release-scope agent曾误用一次 `py_compile`生成唯一 V22 `.pyc`；主控先验证该文件不在
启动 protected set且目录没有其他内容，再删除该可重建 output与空目录。最终 untracked
精确为 two V22 candidates加原 protected 130，manifest未漂移。TPC-27--32 legacy与
TPC-122 writers均未执行。

最终数学状态为

```text
V22_FINAL_RELEASE_QA = PASS,
ARITHMETIC_ADVANCE = NO,
FIXED_ATOM_CREDIT = 0,
STRICT_1_OVER_400 = UNPAID,
L2 = NONE,
TPC_207_TRIGGER = false.                              (69.28)
```

封印后只允许 stage本轮 exact five-file allowlist。commit后须执行
`git pull --rebase origin main`、post-rebase只读复核与 push，并在会话报告中验证 local
`HEAD`、`origin/main`、remote `refs/heads/main`三 hash一致；这些 operational hashes不能
自指写入同一个 commit。任一步失败即不得宣称 remote publish完成。
...
V20从已发布 V19 commit
`a96bac95db63b9fcbc99b98fb24c90c0ac6e37fb`继续。冻结的 physical object仍是
...
z=(log x)^K,
w_x^(z)(t)=Lambda(t+2)-b_x^(z)(t),
beta_x^raw=V19 combined raw MASTER covector.          (67.1)
```

本 gate只判断 canonical source innovation是否提供比原 signed MASTER target更小的
对象；不搜索 201st micro-paper，不创建 TPC-207。三个 read-only audits分别核对 exact
fiber/horizon公式、affine-state/physical-quotient边界、finite rank与 terminal falsifier；
所有正式写入仍由主会话完成。

### 67.2 Exact fiber projection 与 physical evaluation

固定 `gcd(P,p)=1`，逐 parent fiber有两条 deleted copies与 `p-2` survivors。写

```text
Pi_p=alpha_p^(-1)R_pR_p^*,
alpha_p=(p-2)/p,
eta_p(V)=(I-Pi_p)V,
Vbar_r=(p-2)^(-1)sum_(j surviving)V(q_(r,j)).          (67.2)
```

则 exact coordinate formula为

```text
eta_p(V)(q_(r,j))
 =V(q_(r,j)),                    j deleted,
 =V(q_(r,j))-Vbar_r,             j surviving.         (67.3)
```

对 algebraic raw functional `L_beta(V)=sum beta(q)V(q)`，没有缺失的 Haar scalar：

```text
L_beta(eta_p(V))
 =sum_r [
    sum_(j deleted)beta_j V_j
   +sum_(j surviving)beta_j V_j
   -(p-2)^(-1)(sum_surviving beta_j)(sum_surviving V_j)
  ].                                                    (67.4)
```

normalized-Haar Riesz vector是 `pP conjugate(beta)`；complete-frequency Parseval或
averaged covariance不能替换 (67.4) 的 prescribed physical scalar。

### 67.3 Terminal no-wrap norm floor

取 terminal prime clock
...
对 sufficiently large stage，V19的 primorial growth给 `P_(s-1)>p_(s+1)^2>x`，
physical shell每个 fiber至多一个 nonzero child。由 (67.3)逐 fiber exact求和得

```text
||eta_p(V)||_(pP)^2
 =1/(pP)[
    sum_(t deleted)|V(t)|^2
   +(p-3)/(p-2)sum_(t surviving)|V(t)|^2
  ],                                                    (67.6)

||eta_p(V)||_(pP)^2
 >=(p-3)/(p-2)||V||_(pP)^2.                            (67.7)
```

support也会在 survivor fiber展开：

```text
#supp eta
 =#deleted active+(p-2)#surviving active.              (67.8)
```

由于 `p asymp sqrt(x)`，(67.7) 的 ratio趋于一。ambient Haar dilution不是 coordinate-
sum saving，不能支付 physical normalization。

### 67.4 Combined raw target的 log-power equivalence

在同一 terminal no-wrap stage，(67.4)对 V19 literal combined raw row与 residual给

```text
L_(beta_x^raw)(eta_p(w_x^(z)))
 =S_x-(p-2)^(-1)
   sum_(x/2<t<=x, p does not divide t(t+2))
      beta_x^raw(t)w_x^(z)(t),                         (67.9)

S_x=sum_(x/2<t<=x)beta_x^raw(t)w_x^(z)(t).
```

两条 source-locked ordered HB2 types与 literal constants `+2,-1`给
`|beta_x^raw(t)|<=3d_4(t)`。committed residual pointwise bound、fixed divisor moments
及 Bertrand给

```text
L_(beta_x^raw)(eta_p(w_x^(z)))
 =S_x+O_K(x^(1/2)log^C x).                             (67.10)
```

所以对每个 fixed `A,K`，terminal-innovation `x/log^A x` saving当且仅当原 combined
raw MASTER target有同样 saving。这一 equivalence不 materialize缺失的 separated
`Xi,Kappa` template registry，也不支付 transform tails。

### 67.5 Growing horizon没有 automatic telescope

在 no-wrap horizon定义

```text
q_s(t)=1_(p_s does not divide t(t+2))/(p_s-2),
omega_base(t)=product_(a<u<=b)q_u(t),
omega_s(t)=[1-q_s(t)]product_(s<u<=b)q_u(t).           (67.11)
```

逐 physical coordinate exact有

```text
omega_base+sum_(a<s<=b)omega_s=1,
omega_base>=0,
omega_s>=0.                                            (67.12)
```

terminal deleted coordinate有 `omega_b=1`；terminal survivor有
`omega_b=(p_b-3)/(p_b-2)`。因此 Duhamel expansion只是 nonnegative partition of one，
不是 alternating/cancellative telescope。跨 coordinate的 arithmetic signs仍可能取消，
但必须由新 theorem证明。

### 67.6 Exact path state、quotient firewall 与 coherence witness

令 `E_s=ker R_(s-1)^*`。递归 pseudoinverse给 unique exact decomposition

```text
W_b=R_(b:a)W_a+sum_(a<s<=b)R_(b:s)eta_s,

||W_b||_b^2
 =alpha_(b:a)||W_a||_a^2
  +sum_(a<s<=b)alpha_(b:s)||eta_s||_s^2.              (67.13)
```

该 weighted synthesis从 `V_a direct_sum E_(a+1)...direct_sum E_b`到 `V_b`是 isometry，
且

```text
dim V_a+sum_(a<s<=b)(dim V_s-dim V_(s-1))=dim V_b=P_b. (67.14)
```

所以 abstract affine/path carrier是 full-dimensional re-coordinatization，不自动降秩或
改善 Cauchy--Schwarz。whole innovation space也不能 quotient掉：actual V19 row满足

```text
beta_168^raw(90)=2,
L_(beta_168)((I-Pi_13)e_90)=20/11.                    (67.15)
```

changing-scale rows不是单一 pullback orbit：

```text
beta_166^raw(84)=1,
84 survives p=13,
but 84 is outside the strict x=168 shell.              (67.16)
```

故 single-terminal telescope不能升级为 cross-scale coboundary theorem。

### 67.7 Exact finite rank falsifier

V19 `k=5,b=7` rows去掉各 nonzero prime-log column scalar后，独立 exact rational
elimination给

```text
raw source rows / coordinates / partitions = 120 / 110 / 3984,
raw source rank                              = 65,
base component rank                         = 56,
terminal eta rank                           = 50,
all eta rank                                = 54,
all eta plus base union rank                = 76.      (67.17)
```

component union从 raw rank 65增至 76，不是 automatic low-rank collapse。这是 finite
falsifier，不是 all-horizon asymptotic rank theorem；数值 energy diagnostics不进入正式
claim。

### 67.8 Scoped stop 与 surviving theorem
...
DECLARED_TPC_BRIDGE_B_20260807_SHBD2_LONG_HORIZON_SOURCE_INNOVATION_
SMALL_NORM_AUTOMATIC_TELESCOPE_OR_LOW_RANK_BYPASS_V1
  = STOP_SCOPED_EXACT_TERMINAL_NEAR_IDENTITY.          (67.18)
```

它只停止 automatic small norm、stagewise cancellation、uniform low rank或 independently
easier dynamical error的推断。合法 enlarged state、complete physical quotient、真正
signed dynamical theorem、separated analytic proof及 A1/A2均未停止。

surviving Bridge B theorem精确为

```text
BRIDGE_B_SHBD2_TERMINAL_INNOVATION_SIGNED_PHYSICAL_EVALUATION
  = OPEN_NEW_ARITHMETIC_THEOREM.                       (67.19)
```

它必须直接、uniform地证明 source-locked combined raw MASTER scalar或 (67.10) 的等价
terminal-innovation scalar有 arbitrary log-power saving。必须保留 fixed physical
`h0=2`、`x=2X`、ordered physical coefficients、actual masks、normalization、全部
parameter ranges、uniform constants、separation/reassembly与完整 physical-loss ledger。
若由 Logistic/symbolic dynamics提供，还须给 target-independent affine input-output law、
exact arithmetic event coding与 distinguished-seed deterministic return；positive measure、
a.e. genericity、complete-frequency mean或 recurrence均不够。

### 67.9 Artifact、checker 与 publication boundary

正式 V20 artifacts为

```text
research/tpc-big-road/bridge_b_terminal_innovation_floor.md
research/tpc-big-road/tpc_bridge_b_terminal_innovation_checker.py. (67.20)
```

checker先锁 V19 canonical final-LF checker SHA-256，再以 exact integer/rational arithmetic
复核 fiber formulas、terminal floor/support/evaluation、299个 horizon partition cases、
path isometry、两个 actual witnesses与 (67.17)。canonical registry有 37 unique rows，
final-LF SHA-256为

```text
0408cb3e4fd0bbfb7815df0df24902d4cc9fa1b75875e66f41482c30768652ee. (67.21)
```

baseline run拒绝 19 contract与 39 registry mutations；normal与 optimized Python必须输出
byte-identical。最大 claim仅

```text
EXACT_L0_TERMINAL_INNOVATION_EQUIVALENCE_AND_BYPASS_STOP. (67.22)
```
...
research/tpc-big-road/README.md
research/tpc-big-road/bridge_b_terminal_innovation_floor.md
research/tpc-big-road/tpc_bridge_b_terminal_innovation_checker.py. (67.23)
```

独立 QA 与最终只读回归 closure为

```text
V20_MATH_PROOF_QA = PASS / FIRST_FATAL_NONE
V20_CHECKER_ADVERSARIAL_QA = PASS
V20_EXTERNAL_CONTRACT_ATTACKS = 50/50 REJECTED
V20_EXTERNAL_REGISTRY_ATTACKS = 83/83 REJECTED
V20_INTERNAL_MUTATIONS = 19 CONTRACT / 39 REGISTRY
V20_RELEASE_ENTRY_QA = PASS
V20_STARTUP_REGRESSION = 22/22 PASS
V20_SUPPLEMENTAL = 4/4 PASS
V20_BIG_ROAD_NORMAL_AND_OPTIMIZED = 16/16 PASS / BYTE_IDENTICAL
V20_RELEASE_ALLOWLIST = 5/5 EXACT
V20_PROTECTED_UNTRACKED = 130 / 9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1
V20_PAPER_PDF_BUILD_TPC207_OUTPUT = NONE.                 (67.24)
```

V20 proof与 checker committed canonical final-LF bytes的 SHA-256分别为

```text
bridge_b_terminal_innovation_floor.md
  2f11526ae5c17371de3d51c9990abcd7723834df07cfa7b631aaf96ebb9b4bef
tpc_bridge_b_terminal_innovation_checker.py
  ce62b12023a4d65a8eb7ff2e01db50110d7d96183c5db771e11728da10ce50a7. (67.25)
```

这些 hash按 final-LF canonical bytes计算；Windows working-tree CRLF不改变冻结对象。
三项 read-only audits的 `files_changed=[]`，cached diff为空。无 arithmetic advance、无
fixed-atom credit、strict `1/400`仍 UNPAID、`L2=NONE`、TPC-207=false；不创建 numbered
paper、PDF或 build artifact。
...
### 61.1 source lock、top geometry与 claim ceiling

V14从已发布且本地/remote三 hash一致的 V13 commit启动：

```text
ea1db5210f7db34a30bce372084dc2d4eee1b4a0
```

启动 tracked/cached diff为空；130个 protected untracked files原样保留，canonical
manifest仍为
`9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1`。
三个 `READ_ONLY_FROZEN` agents分别审核 top modulus dispersion、secondary
conductor family及 actual outer spectrum；所有正式写入仍只由主控完成。

本节继续 `(60.1.1)`的同一个 theorem-valid source contract，并只处理

```text
P=F^(2-epsilon_F), epsilon_F=o(1),
GPTAB asymp F^2.                                      (61.1.1)
```

所以 `G,T,A,B,v=F^o(1)`、`U=F/G`、`J=F/(Av)`。V13的 exact product fibers继续
逐字保留 `W^ell(avj/F)`、fixed physical `h0=2`、全部 masks、正负 `j` shells及
physical `c_v=-2v conjugate(gab^2t^2)_rho`。

本节最高结论是
`CONTRACT_LEVEL_PARTIAL_ARITHMETIC_RANGE_ADVANCE_AND_CORE_SHRINK`。它只给
common-coefficient source contract上的 positive conductor-tail compression；actual
atom membership、all-`D`、exact cover、tails、A/B selection、original/global
normalization与 provenance仍分别 OPEN。

### 61.2 exact induced-conductor decomposition

写 `rho=fm`，令 `chi=Ind_f^(fm)psi`且 `psi` primitive mod `f`。odd squarefree
induction给

```text
tau_(fm)(conjugate chi)
 =mu(m)conjugate(psi)(m)tau_f(conjugate psi).          (61.2.1)
```

因此 V13 top cell精确变成

```text
sum_(fm~P) mu(f)mu(m)/[phi(f)phi(m)]
 sum_(psi mod f)^*
 psi(c_(v,f))conjugate(psi)(m)^2 tau_f(conjugate psi)^2
 E_m(conjugate psi)L_(v,m)(psi),                       (61.2.2)
```

其中 `c_(v,f)=-2v conjugate(gab^2t^2)_f`。source atom分离时，最后一行是

```text
E_(1,m)(conjugate psi)E_(2,m)(conjugate psi)
U_m(psi)V_(v,m)(psi).                                  (61.2.3)
```

`(61.2.1)`平方只令内部 `mu(m)^2=1`；outer `mu(m)`仍在 `(61.2.2)`。primitive
character zero extension代管 `f`-unit mask，但每条 polynomial必须显式保留
`(n,m)=1`。checker以 `rho=35,f=7,m=5`的 complex sextic character exact锁定
`conjugate(psi)(m)^2`、physical `psi(c_v)`、outer sign及 orientation，并检测删 mask、
错共轭、漏 cofactor-square和错误保留第三枚 `mu(m)`等 mutations。

### 61.3 common-coefficient primitive large sieve

固定 `m`。因 `|tau_f(conjugate psi)|^2=f`，Cauchy给

```text
|T_(v,m)|<=1/phi(m)[S_E(m)S_L(m)]^(1/2),              (61.3.1)
```

两项的 primitive-large-sieve权均为 exact `f/phi(f)`。第 54.18、57.3 节已冻结：
coprimality expansion后，fixed outer variables与 transform parameter的 arithmetic
arrays不依赖 varying primitive modulus `f`；残余 smooth modulus scalar由 BV partial
summation移除。joint `E` block按 `n=e_1e_2` grouping后由 divisor bound保持
`L2^2<<F^2X^o(1)`。所以
[Pascadi Lemma 3.4](https://doi.org/10.1112/S0010437X2500747X)的 standard primitive
multiplicative large sieve合法给

```text
S_E(m)<<[F^2+P^2/m^2]F^2X^o(1),
S_L(m)<<[F^2+P^2/m^2]F^2/(GAv)X^o(1),

|T_(v,m)|
 <<F^2/[phi(m)sqrt(GAv)] [F^2+P^2/m^2]X^o(1).          (61.3.2)
```

若只有逐 `rho`给出的 `A_rho(e_1,e_2)`与 uniform `L2`，而 arrays可随 modulus
自适应，则 `(61.3.2)`非法；不得把 V13 arbitrary joint-`L2` superclass升级成
common coefficient。

对 `m>=M`求和，使用
`sum 1/phi(m)<<log P`与
`sum_(m>=M)1/[phi(m)m^2]<<M^(-2)P^o(1)`，再乘回 physical `v`与 top outer
ledger，得到

```text
V_(m>=M)(P)
 <<[F^4+F^2P^2/M^2]X^o(1).                            (61.3.3)
```

`M^-2`来自 fixed-`m`的 `1/phi(m)`与 conductor-family `(P/m)^2`共同形成
`m^-3` tail，不是把 pointwise operator gain重复计算。

### 61.4 range、strict budget与 near-primitive剩余核

在 `P=F^2=D`，top baseline为 `F^2P^2=F^6`。取 `M=P^kappa`，则

```text
V_(m>=P^kappa)/F^6
 <<P^(-1+o(1))+P^(-2kappa+o(1))
 =D^(-min(1,2kappa)+o(1)).                             (61.4.1)
```

所以对每个 fixed `kappa>0`，全部

```text
m>=P^kappa  ==>  f<<P^(1-kappa),
f<=P^(1-kappa)  ==>  m>=P^kappa.                      (61.4.2)
```

已在 contract level闭合。若 downstream损 `D^lambda_D`，被删 sector支付 local
strict `1/200`须

```text
min(1,2kappa)-lambda_D>1/200.                          (61.4.3)
```

在 `0<kappa<=1/2`时，`(61.4.3)`等价于
`kappa>1/400+lambda_D/2`；`kappa>=1/2`时 saving饱和于 `D^(-1+o(1))`。
等号不能吸收 `X^o(1)`。`(61.4.3)`不给剩余 core任何 credit。“subpower-cofactor
collar”只表示每个 fixed-power-separated tail均已闭合，不声称 `kappa->0`有 uniform
常数。

### 61.5 两个 phase-blind STOP与真正的 surviving angle

剩余核不能命名为 primitive-only。取 `rho=3p,f=p,m=3`、`p=1 (mod 24)`与
primitive quadratic character，normalized Gauss-square coefficient为

```text
p/[2(p-1)] ->1/2.                                     (61.5.1)
```

checker的 exact最小方便实例 `rho=51,f=17`给 `17/32`。这只否定 conductor
projection自身提供 fixed power；它是 coefficient-class/operator fixture，不是 actual
Möbius packet lower bound。

另取 prime `p~F^2`与长度 `O(F)` row
`A(chi)=sum_n a(n)chi(n)`。支撑模 `p`单射时 exact有

```text
sum_(chi!=chi_0)|A(chi)|^2
 =(p-1)||a||_2^2-|sum a|^2
 >=(p-1-|supp a|)||a||_2^2.                            (61.5.2)
```

prime nonprincipal characters全部 primitive且 `|tau_p(chi)|^2=p`。在 frozen source
envelope允许的 literal Möbius/smooth equal-row subfamily `E_1=E_2=E`中，Cauchy再给

```text
sum_(chi!=chi_0)|E(chi)|^4 >>pF^2.                    (61.5.3)
```

smooth equal `U=V`同样达到 endpoint scale。因此 frozen superclass上的 primitive
eigenspace avoidance与 phase-blind fourth moment/large sieve精确 `STOP_SCOPED`；
actual-atom-specific avoidance仍为 `OPEN_ATTACHMENT`。`mu(p)=-1`在 prime sector恒定，
outer Möbius sign单独只给 `F^6/log F`级 logarithmic density，不给 fixed power。

合法 surviving theorem必须保留 root-number-square phase，证明 actual fixed-prime/
near-primitive angle，或证明完整 varying-squarefree outer-`mu` collective bound。
current first subgate因此为
...
它必须同时接受 literal physical coefficients、fixed `h0=2`、summation/prefix domain、
`X/N/q` ranges、uniform constants、normalization、全部 masks/signed shells、physical
`c_v`、non-Hermitian four-polynomial orientation与完整 physical-loss ledger。

### 61.6 finite primary screen

[Pascadi Theorem 10.3](https://arxiv.org/abs/2304.11696)把 moving coefficient放入
level-dependent `L2` array时抹去 sign且在 top geometry返回 endpoint；若把 `rho`放入
smooth modulus slot，则不接受 literal `mu(rho)`。
[Blomer--Risager--Shparlinski Theorem 1.3](https://arxiv.org/html/2411.17823v3#S1.Thmtheorem3)
的 literal statement在 sharp unweighted modulus sum中保持 fixed inner index，而
physical index随 `rho`取 modular inverse；smooth-weight版本只能由相关
Theorem 1.1/Remark 1.2或 partial summation另行导出，仍不接受 literal `mu(rho)`。
[Earnst Theorem 1.6](https://arxiv.org/html/2603.22124v1#S1.Thmtheorem6)证明 prime-even
root-number-square `|L(1/2,chi)|^2` moment的 fixed saving，是正面的 mechanism
evidence；但其 AFE/fixed-twist coefficients不是 actual `E_1E_2UV`，不能替换。
以上只是列明 statements的 attachment screen，不声称全局文献不存在。

### 61.7 canonical V14 registry、岛屿地图与 release边界
...
岛屿图上 Bridge A1断点缩到 near-primitive phase collar；A2 paired-Voronoi、Bridge B
distinguished-seed genericity与 Hénon辅助岛保持 source-lock独立。第 6 节全部旧
method cells继续 `STOP_SCOPED`，尤其 TPC193 V1、common-`k` V1、tail-failure/A/B V1
与 full-`r_Rr_R` ultra-complement V1。两个 O161 pointwise parents、pair-native
reroute、H1与 global architecture继续 OPEN。

本轮没有创建 TPC-207、paper directory、论文、PDF或 build log。fixed-atom
credit=`0`、global strict `1/400=UNPAID`、`L2=NONE`、
`TPC207_TRIGGER=false`。

### 61.8 post-write fail-closed audit

正式写入后，主控与三个 read-only agents分别完成数学、schema/mutation和 release
审计。共同系数量词下的 `M^-2` tail、strict budget、prime diagonal floor及
`OPEN_ATTACHMENT`/`STOP_SCOPED`边界均通过复核；agents没有修改、暂存或生成文件。
当前 registry在六个文档块与 checker的两个 dict中均为 `56 raw / 56 unique`，键序、
键值完全一致，canonical LF-final SHA-256为
`d5844f93a3653ea24c36820beee899b46c742c5b89fb3e783a2b978a74889de2`。

```text
STARTUP_REGRESSION = 22/22 PASS
TPC111_124_126_127_SUPPLEMENTAL = 4/4 PASS
BIG_ROAD_READ_ONLY_CHECKERS = 3/3 PASS
FM_LOCAL_COMPARISON_NORMAL_AND_OPTIMIZED = PASS
GIT_DIFF_CHECK = PASS
EXPECTED_TRACKED_RELEASE_FILES = 5
PROTECTED_UNTRACKED = 130 FILES
PROTECTED_UNTRACKED_MANIFEST_SHA256
  = 9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1
SUBAGENT_FILES_CHANGED = 0
...
Git的 LF-to-CRLF提示不影响 `git diff --check`，本轮也没有修改任何以 worktree raw
bytes作证书的 tracked artifact。发布仍只允许 stage本节列出的五个预期文件；其余
既有 untracked不得纳入提交。
...
### 59.1 发布基线、路线选择与 claim ceiling

V12 从已发布且 local `HEAD`、`origin/main`、remote main三重一致的 V11 commit
启动：

```text
5a3f0325a67f09a7bb80f06b88f8c3ab9f2a18dc
```

启动 tracked/cached diff为空；130 个 protected untracked files保持原样，canonical
manifest仍为
`9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1`。
主控继续按根 `AGENTS.md`与 ARS source/proof/integrity流程使用 read-only agents审计
composite projector、prime upper grouping与 transition collar；正式写入只由主控
完成。没有运行 TPC-27--32 legacy writers或 TPC-122 writer。

V12 不是 schema补丁。它把 V11 分裂的 composite `4/3`与 prime `42/31`墙统一
推进到 `3/2`。最高 claim仍精确为

```text
CONTRACT_LEVEL_PARTIAL_ARITHMETIC_RANGE_ADVANCE_AND_CORE_SHRINK.
```

它不证明 actual selected atom属于 source envelope，不证明 all-`D`、physical cover、
tail、A/B selection、global normalization或 provenance，也不触发 TPC-207。

### 59.2 exact inverse-residue transfer 与 primary theorem

保持第 57--58 节的 literal normal form：

```text
sum_(g,rho,t,a,b)
 mu(g)mu(rho)mu(b)a/[phi(t)phi(a)phi(b)]
 * sum_(e_1,e_2,u,k) mu(e_1)mu(e_2)W(...)c_g(ak)
 * S(1,-2uk conjugate(g e_1e_2 a b^2t^2);rho),

g~G, rho~P, t~T, a~A, b~B,
GPTAB asymp F^2, PT>=F,
e_1,e_2~F, u~F/G, |k|<<F/A.
```

冻结 `(g,rho,t,a,b,u,k)`并令

```text
C=-2uk conjugate(g a b^2t^2)_rho.                    (59.2.1)
```

actual masks给 `(C e_1e_2,rho)=1`。在 V11 reduced core的
`P>=F^(4/3)`区域，fixed dyadic constants与 `rho~P`, `e_i~F`保证对充分大
`F`有 actual supports短于并位于 `rho`以下。因此

```text
e_1 -> m=conjugate(e_1)_rho,
e_2 -> n=conjugate(e_2)_rho                           (59.2.2)
```

逐支撑单射。对每个 product atom，将两条 coefficients按 `(59.2.2)`置于
`{1,...,rho}`并在其他 residues补零。于是

```text
||alpha_tilde||_2=||alpha||_2,
||beta_tilde||_2=||beta||_2,                          (59.2.3)
```

而 source envelope已冻结 common parameters并给
`||alpha||_2,||beta||_2<<F^(1/2)X^o(1)`，总 atom `L1`为 `X^o(1)`。physical
kernel逐式成为

```text
S(1,C conjugate(e_1e_2);rho)
 =S(1,Cmn;rho)=S(Cm,n;rho).                           (59.2.4)
```

[Blomer--Pascadi, Lemma 5.1, (5.1)](https://arxiv.org/abs/2607.24311)接受
任意 positive modulus、unit multiplier、ordinary intervals上的 arbitrary complex
arrays以及 unnormalized `S(am,n;c)`。合法取 `M=N=c=rho`并使用上述 zero-padded
arrays，得到 complete bilinear bound。这里还可把 uniform constant完全显式化：若

```text
A(r)=sum_m alpha_tilde(m)e_rho(rm),
B(s)=sum_n beta_tilde(n)e_rho(sn),
```

则打开 Kloosterman sum给

```text
B_rho=sum_(x mod rho)^* A(Cx)B(conjugate(x)).          (59.2.5)
```

Cauchy与 complete additive Parseval逐字推出

```text
|B_rho|<=rho||alpha_tilde||_2||beta_tilde||_2
        <<F P X^o(1).                                 (59.2.6)
```

这对 prime/composite `rho`一视同仁。`(59.2.6)`使用 physical unnormalized `S`；
不得删掉或重复加入 `rho^(1/2)`。

### 59.3 完整 outer ledger 与新 `3/2` transition

第 58 节已经证明 squarefree `g`及 `(a,g)=1`下

```text
sum_(0<|k|<<F/A)|c_g(ak)| << (F/A)F^o(1),
a/[phi(t)phi(a)phi(b)] << F^o(1)/(TB).                (59.3.1)
```

把 `(59.2.6)`、`u`长度、`(59.3.1)`与全部 outer cells相乘，且不给
`mu(g)mu(rho)mu(b)`任何 cancellation credit：
...
故对每个 fixed `0<delta<1/6`，

```text
F^(4/3)<=P<=F^(3/2-delta)
 ==> |V(P)|<<F^(6-2delta+o(1))
              =F^6D^(-eta_D),
     every fixed eta_D<delta.                         (59.3.3)
```

在旧 `P=F^(4/3)` transition处 supremal local `eta_D=1/6`；在旧
`P=F^(42/31)` endpoint处为 `9/62`。仓库的 strict physical ledger要求
`eta_D>1/200`，所以若 later loss为 `D^(lambda_D)`，合法条件是

```text
delta>1/200+lambda_D.                                 (59.3.4)
```

等号不能吸收 `X^o(1)`。这是 local restricted-window payment；fixed-atom credit
仍为零，global strict endpoint仍未支付。

### 59.4 scoped STOP 与真正剩余核心

Blomer--Pascadi 的 nontrivial short-interval theorem要求 coefficient indices落在
ordinary intervals。取逆后的 actual support散布于完整 residue hull；其 cardinality
为 `F`不等于 interval length `F`。用合法 ambient length `rho`不会改善 complete
bound，把 length强改成 `F`则是禁止的 block/support substitution。因此

```text
BP2607_NONTRIVIAL_INTERVAL_BOUND_AFTER_INVERSION
 = STOP_SCOPED_INVERSE_SUPPORT_NOT_SHORT_INTERVAL.     (59.4.1)
```

这不构成对所有未来 inverse-set theorem的 absence claim。V11 已 source-lock 的
FKMS `l=3` prime window被 `(59.3.3)`完整覆盖；本轮没有为 FKMS Theorem 1.4
建立 literal packet/range/outer-ledger attachment，故不给它新 credit，也不登记任何
由它导出的数值 endpoint。普通 Kuznetsov/Pascadi special-sequence theorems要求
level-independent arguments或 Fourier-concentrated coefficients，不能接受随
`rho`变化的两条 inverse Möbius rows。

`GLOBAL_MOVING_UNIT_CAUCHY`继续是 `STOP_SCOPED_EXACT_ENDPOINT_PRODUCT_RESONANCE`：
它控制另一条完整 moving-product unit轴并企图一次覆盖全域；V12只在 two literal
`e` rows上做 exact inverse transfer和 restricted dyadic ledger，不重开旧 cell。

在 `P=F^(3/2)`，`(59.3.2)`恰为 `F^(6+o(1))`。所以统一后的 first missing是

```text
HB4_EXACT_HALF_THREE_HALVES_PROJECTOR_CORE_DISPERSION
 = SELECTED_CORE_OPEN_NEW_THEOREM.                    (59.4.2)
```

它必须覆盖 `P=F^(3/2-o(1))` lower collar及全部 `P>=F^(3/2)` squarefree
projectors，并真正利用 varying `rho`、`u/k` families或 literal outer signs之一。
complete-frequency mean、phase-blind large sieve、ordinary averaged-residue theorem或
BP interval/support偷换均不得升级成该 theorem。

### 59.5 canonical V12 registry、岛屿地图与 release边界
...
岛屿图上 Bridge A1 的红色断点从 split `4/3`/`42/31`前移到 common `3/2`。
Bridge A2 paired-Voronoi、Bridge B distinguished-seed genericity与 Hénon辅助岛均
保持 source-lock独立，不给本段 analytic ledger任何 credit。

本轮没有创建 TPC-207、paper directory、论文、PDF或 build log。actual-atom、
all-`D`、exactly-once cover、tails、A/B、original/global normalization与完整
provenance继续 OPEN；fixed-atom credit=`0`、global strict `1/400=UNPAID`、
`L2=NONE`、`TPC207_TRIGGER=false`。

### 59.6 post-write fail-closed verification

```text
POST_WRITE_STARTUP_REGRESSION = 22/22 PASS
POST_WRITE_TPC111_124_126_127_SUPPLEMENTAL = 4/4 PASS
POST_WRITE_BIG_ROAD_CHECKS = 3/3 PASS
POST_WRITE_INDEPENDENT_AUDITS = 3/3 MATHEMATICAL/SOURCE/RELEASE_QA PASS
POST_WRITE_EXPECTED_TRACKED_DIFF = 5/5 ONLY; CACHED DIFF EMPTY
POST_WRITE_V12_REGISTRY
  = 39 ROWS x 6 CURRENT DOC COPIES + 2 CHECKER DICTS IDENTICAL
POST_WRITE_V12_REGISTRY_SHA256
  = 8129fb18e73dee52ee11824ea8033a9eabecaacccafab2dfdbafb1f9477379aa
POST_WRITE_CHECKER_FIXTURES
  = INVERSE_TRANSFER 40/40 PASS; COMPLETE_PARSEVAL 10/10 PASS
POST_WRITE_MARKDOWN_FENCES
  = HANDOFF 2086; COMPASS 88; README 166; COMPILER 580; ALL EVEN
POST_WRITE_GIT_DIFF_CHECK = PASS
POST_WRITE_PROTECTED_UNTRACKED = 130 FILES PRESERVED
POST_WRITE_PROTECTED_MANIFEST
  = 9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1 MATCH
...
## 58. 2026-08-06 V11：small-projector closure 与首段 prime-projector trace 大路

### 58.1 启动事实与 claim ceiling

V11 从已发布并三重 hash一致的 V10 commit启动：

```text
HEAD = origin/main = remote main
     = cd8b798492bc399cae84358dc1dc3ba68b7c015f.
```

启动时 tracked/cached diff为空；130 个既有 protected untracked paths保持原样，
canonical manifest仍为
`9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1`。
本轮没有运行 TPC-27--32 legacy writers或 TPC-122 writer，没有创建 paper、PDF、
numbered directory或 build log。

V11 的 claim ceiling为

```text
CONTRACT_LEVEL_PARTIAL_ARITHMETIC_RANGE_ADVANCE_AND_CORE_SHRINK.
```

它证明 V10 exact source contract的两个 projector dyadic窗口，不证明 actual selected
atom已经属于该 contract，也不完成 all-`D` physical reassembly。checker只验证 exact
algebra、finite identities与 exponent geometry；analytic input分别是 classical composite
Weil bound与 source-locked FKMS Theorem 1.3(2)。

### 58.2 dyadic source lock与 Ramanujan absolute ledger

从 `(57.4.2)`冻结

```text
g~G, rho~P, t~T, a~A, b~B,
GPTAB asymp F^2, PT>=F, K=F/A.                        (58.2.1)
```

`P`只表示 projector `rho`，不得与 primitive conductor `r=rho t`混写。全部
pairwise-coprime odd-squarefree条件、`e_i~F`、`u~F/G`、`|k|<<F/A`及 unit masks
保持 `(57.4.2)`原样。

由 `GAB<<F`得 `K>>G`。平方自由 `g`和 `(a,g)=1`给 exact identity

```text
|c_g(ak)|=phi((g,k)).                                 (58.2.2)
```
...
checker对 960 个 squarefree/unit有限 fixtures exact验证 `(58.2.2)`，并用 `g=9`
mutation确认 squarefree mask不能删除。

### 58.3 全 squarefree small-projector theorem与 large-T transfer

literal masks保证 Kloosterman第二参数为 `rho`-unit，所以 composite Weil给

```text
|S(1,n;rho)| << P^(1/2+o(1)),                         (58.3.1)
```

`rho=1`按 `S mod 1=1`处理。固定 outer variables后，`e_1,e_2,u,k`和为
...
又有 `a/[phi(t)phi(a)phi(b)]<<F^o(1)/(TB)`；乘 outer `GPTAB` count得到

```text
|V(P)| << F^4P^(3/2+o(1)).                            (58.3.3)
```
...
P<=F^(4/3-delta)
 ==> |V(P)|<<F^(6-3delta/2+o(1))
     =F^6D^(-eta_D),
     any eta_D<3delta/4.                              (58.3.4)
```

`T>=F^(2/3+delta)`只严格推出 `P<<F^(4/3-delta)`；把 `delta`缩小任意 fixed
margin即可吸收 dyadic constant并进入 `(58.3.4)`，所以 large complementary
conductor整翼不是新墙。局部 strict endpoint存在 `eta_D>1/200`当且仅当
`delta>1/150`并保留 fixed margin；等号不能吸收 `F^o(1)`。若后续还有 fixed
physical loss `lambda_D`，条件为
`delta>1/150+(4/3)lambda_D`。

### 58.4 FKMS prime-projector exact attachment

对 prime `rho`，冻结 `(g,rho,t,a,b,u,k)`，只把两条 literal Möbius rows
`e_1,e_2~F`送入
[Fouvry--Kowalski--Michel--Sawin Theorem 1.3(2)](https://arxiv.org/html/2511.09459v3)。
令
...
C=-2uk conjugate(gab^2t^2)_rho.                       (58.4.1)
```
...
```text
S(1,C conjugate(e_1e_2);rho)
 =rho^(1/2)Kl_2(Ce_1^(-1)e_2^(-1);rho).              (58.4.2)
```

标准 `Kl_2` sheaf的 geometric monodromy为 `SL_2`，满足 source的 gallant/light
条件；unit scaling `C`有 uniform complexity，negative monomial powers合法，actual
masks保证两变量都在 `F_rho^*`。第 55.1 节 source-envelope已冻结 common transform
parameter并把 weight分成 product atoms，所以 `e_1,e_2`确为两个 independent
one-variable arrays，不是未分离 joint weight的切片。两条系数的 `L2` norm均
`<<F^(1/2)X^o(1)`，总 product-atom `L1`只损 `X^o(1)`。

取 source中的 `l=3`，literal length hypotheses化为

```text
10rho^(1/2)<=F<=rho^(3/4).                            (58.4.3)
```

在 `rho~P`且缩小任意 fixed margin后，`(58.4.3)`给 dyadic transition
`P>=F^(4/3)`。normalized theorem给 `F^(5/3)P^(2/9+o(1))`；恢复 `(58.4.2)`中不能遗漏的
exact `rho^(1/2)`，再用 dyadic `rho~P`，local unnormalized `e_1,e_2` sum为

```text
<<F^(5/3)P^(13/18+o(1)).                              (58.4.4)
```

用 `(58.2.3)`支付 `u,k`、再完成 outer count与 normalization，且不给
`mu(g)mu(rho)mu(b)`任何 cancellation credit，得到

```text
|V_prime(P)| << F^(11/3)P^(31/18+o(1)).               (58.4.5)
```
...
F^(4/3)<=P<=F^(42/31-epsilon), rho prime
 ==> |V_prime(P)|<<F^(6-31epsilon/18+o(1))
     =F^6D^(-eta_D),
     any eta_D<31epsilon/36.                          (58.4.6)
```

在 lower transition `P=F^(4/3)`的 supremal local budget为 `1/54`，即任意固定
`eta_D<1/54`可取，所以这是非空 genuine source-backed arithmetic sector。局部 strict endpoint须
`epsilon>9/1550`加 ledger margin。source displayed bound的 `l=2`在 `4/3`前
已耗尽；`l=4`需从 `16/11`开始，但 outer saving在 `288/211`已耗尽。因此 `l=3`
是相邻 displayed choices中唯一跨过 transition的直接 grouping；这不声明更强 trace
theorem普遍不存在。

### 58.5 root-number支路的精确 STOP 与 reduced core

V11没有把 large `T`改写为 character-family free parameter。在 `(57.2.7)` character
坐标中，`t=r/rho`只在 primitive projector展开后出现；展开前不能预选。展开后 large
`T`已由 `(58.3.4)`闭合。

[Earnst Theorem 1.6](https://arxiv.org/abs/2603.22124)是真正的 prime/even
root-number-square central moment saving，但其 proof使用 asymmetric AFE的专用
coefficients、prime-field hyper-Kloosterman trace estimate与 fixed twists。它不接受
actual `E_1E_2HB`、两种 parity、varying/composite conductor、`c_g,c_s`或 outer
reassembly。普通 primitive large sieve经 Cauchy抹去 root-number phase，且第 12.8
节 substitution在 exact-half第一、第四项饱和。故
...
HB4_EXACT_HALF_LARGE_T_CHARACTER_SIDE_PROJECTOR_COMPLEMENT_SELECTION
 = STOP_SCOPED_T_NOT_INTRINSIC_BEFORE_PROJECTOR_EXPANSION.       (58.5.1)
```

剩余 core为：

```text
composite squarefree P above the 4/3 transition;
prime P at or above the 42/31 transition;
critical boundary bands with no fixed margin.                   (58.5.2)
```

其 first fatal分别是 FKMS prime-modulus hypothesis，以及 outer `P` count与当前
`u,k` L1 ledger耗尽 local trace saving。下一条 selected theorem为

```text
HB4_EXACT_HALF_REDUCED_PROJECTOR_CORE_DISPERSION
 = SELECTED_CORE_OPEN_NEW_THEOREM.                    (58.5.3)
```

合法方向是把 varying/composite modulus、`u,k`或三条 exact signs编入真正 collective
dispersion/trace theorem。不得把 fixed-prime result提升到 composite modulus，也不得把
Kuznetsov complete-frequency mean改写成 prescribed physical cancellation。

### 58.6 canonical V11 registry、岛屿地图与裁决
...
按用户的 TPC 岛屿地图，Bridge A1现在已有两段 theorem-backed实桥：all-squarefree
small-projector span与 prime-projector FKMS span；红色断点移到 `(58.5.2)`。
Bridge A2 paired-Voronoi、Bridge B distinguished-seed genericity及 Hénon/几何岛仍是
不可拼接的 independent routes。

V11正式 verdict为

```text
TPC_BIG_ROAD_V11_20260806_SMALL_PROJECTOR_RAMANUJAN_L1_COMPOSITE_WEIL_
FOUR_THIRDS_RANGE_LARGE_COMPLEMENTARY_T_TRANSFER_PRIME_PROJECTOR_FKMS_L3_
LITERAL_E1E2_ATTACHMENT_FOUR_THIRDS_TO_FORTY_TWO_OVER_THIRTY_ONE_NONEMPTY_
SOURCE_BACKED_SECTOR_LOCAL_STRICT_BUDGETS_CRITICAL_COLLARS_REDUCED_COMPOSITE_AND_LARGE_PRIME_
CORE_OPEN_NO_GLOBAL_TRIGGER_CONTRACT_LEVEL_PARTIAL_ARITHMETIC_RANGE_ADVANCE
```

即 `CONTRACT_LEVEL_PARTIAL_ARITHMETIC_RANGE_ADVANCE_AND_CORE_SHRINK`。actual atom、
tails、all-`D`、exactly-once cover、A/B selection、original normalization与完整
provenance仍未闭合；fixed-atom credit=`0`、global strict `1/400=UNPAID`、
`L2=NONE`、`TPC207_TRIGGER=false`。没有创建 TPC-207、paper、PDF或下一编号。

### 58.7 post-write fail-closed verification
...
POST_WRITE_EXPECTED_DIFF_ONLY = 5/5 PASS
POST_WRITE_CANONICAL_V11_REGISTRY = 33/33 x 6 BYTE_IDENTICAL PASS
POST_WRITE_CANONICAL_V11_REGISTRY_SHA256
  = a842f1726e5c463bd1a22a55a7a41688e959c6bd6404c9a323c46738ef7e9ee5
POST_WRITE_MARKDOWN_FENCES
  HANDOFF = 2040 EVEN
  COMPASS = 78 EVEN
  README = 152 EVEN
  COMPILER = 554 EVEN
POST_WRITE_V11_RAMANUJAN_ABSOLUTE_FIXTURES = 960 PASS
POST_WRITE_PROTECTED_UNTRACKED = 130 FILES
POST_WRITE_PROTECTED_UNTRACKED_MANIFEST
  = 9c46e2112b0c71d0fbfae0282f3bf7ecc7d8ea5f2437a06dfbcee8a7909230e1
ACCIDENTAL_PYCOMPILE_CACHE = REMOVED_OWN_GENERATED_ONLY_BASELINE_MANIFEST_RESTORED
SUBAGENT_FILES_CHANGED = 0
PAPER_PDF_NUMBERED_RELEASE_CREATED = NO
FINAL_SYNC_ORIGIN_MAIN = READY_FOR_FINAL_PULL_REBASE
FINAL_HEAD_ORIGIN_REMOTE_HASH_EQUALITY = READY_FOR_POST_PUSH_VERIFICATION
TPC27_TO_32_LEGACY_WRITERS_EXECUTED = NO
TPC122_WRITER_EXECUTED = NO
```
