"""Verify RH-388 archive membership, hashes, and immutable source identity."""
...
def verify_manifest(manifest: object) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-388_fixed_publication_manifest":
    ...
    expected_remote = [source_locks.JY_CANONICAL_SHA256, source_locks.MAYNARD_CANONICAL_SHA256]
    if not exact_equal(manifest.get("remote_source_lock_sha256"), expected_remote):
    ...
    try:
        ...
        remote_pass = exact_equal(remote.get("canonical_digests"), expected_remote)
    ...
    semantic_match = (
        ...
        and local_entries.get("main.pdf") == local_entries.get("rank-one-p2-tail-resummation.pdf")
    )
    current = {
    ...
    return {
        "status": "RH-388_archive_verified" if not failures else "RH-388_archive_failed",
