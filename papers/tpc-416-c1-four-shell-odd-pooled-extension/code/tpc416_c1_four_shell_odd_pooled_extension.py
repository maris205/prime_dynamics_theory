#!/usr/bin/env python3
"""Exact four-shell pooled CRT replay with explicit odd parity counts."""
from __future__ import annotations
import argparse,hashlib,json,math,sys
from fractions import Fraction
from pathlib import Path
sys.set_int_max_str_digits(1_000_000)
PROJECT=Path(__file__).resolve().parents[1]; RESULT=PROJECT/'results/tpc416_certificate.json'; SCHEMA='TPC416_C1_FOUR_SHELL_ODD_POOLED_EXTENSION_V1'; STATUS='PROVED_EXACT_FINITE_FOUR_SHELL_ODD_POOLED_EXTENSION'; QS,H,N,B=(65536,131072,262144,524288),66,264,1_000_000; COUNTS=(5709,10749,20390,38635)
...
def payload():
    items=shells(); return {'schema':SCHEMA,'status':STATUS,'Q_scales':list(QS),'H':H,'N':N,'origin_lower_bound':B,'shell_rule':'pooled full shells Q<p<=2Q','shell_counts':list(COUNTS),'shell_sha256':{str(q):hashlib.sha256(canonical([p for p,x in items if x==q])).hexdigest() for q in QS},'window_rule':'N=264=4H','normalization':'pooled complete-shell local diagonal','theorem_domain':{'H_and_N':'fixed integer H=66,N=264 with N=4H','Q_and_shell':'four full odd shells Q=65536,131072,262144,524288 pooled without deletion','profile':'pooled primes ordered increasingly; even i residue 0, odd i residue -N','amplitude':'a_i=p_i^3/[Q_i^2(p_i-1)] using each prime declared shell Q_i','parity':'pooled cardinality r=75483 is odd, with m_minus=37741 and m_plus=37742','origin':'o is a CRT solution above the declared origin lower bound','proxy':'complete-shell masked local geometry from TPC-404'},'theorem':{'exact_sharp_bound':'0<=z<=t1/(a_min*sqrt(S0*S1))','coarse_uniform_bound':'z<=4/(a_min*H)<=4/H','proof_steps':['P_minus^2<=m_minus*V_minus','G1>=V_minus*S1','V_minus>=m_minus*a_min^2','S0>=H/4','S1>=H/4','a_min>=1'],'scope':'one four-shell pooled adjacent normalized proxy entry, not the full operator'},'cases':[row(items)],'claim_firewall':{'FOUR_SHELL_ODD_POOLED_EXTENSION':'PROVED_EXACT_FINITE','POOLED_DECIMAL':'NUMERICAL_OBSERVATION','FULL_OPERATOR_NORM':'OPEN','NORMALIZED_GROWING_THEOREM':'OPEN','ARITHMETIC_SIGN_IDENTIFICATION':'OPEN','ARITHMETIC_ADVANCE':'NO','FIXED_POWER_CREDIT':0,'FULL_GATE_B':'OPEN','TWIN_PRIME_RESULT':'NONE'},'round2_clue':'TEST_C1_FOUR_SHELL_ODD_POOLED_EXTENSION'}
def write():
...
def main():
    ...
    if x.write: write(); print('TPC416_CERTIFICATE=WRITTEN')
    elif x.check:
        d=json.loads(RESULT.read_bytes(),object_pairs_hook=nodup,parse_constant=noconst); need(d['claim_status']==STATUS and d['payload_sha256']==hashlib.sha256(canonical(d['payload'])).hexdigest(),'certificate'); need(canonical(d['payload'])==canonical(payload()),'exact four-shell certificate'); print('TPC416_CERTIFICATE=PASS cases=1 shells=4 shell_count=75483 odd_parity=37741/37742 literal_domain=PASS')
