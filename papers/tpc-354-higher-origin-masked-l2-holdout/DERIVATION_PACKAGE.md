...
they are not discarded in this release.  TPC-354 uses exactly the TPC-353
operator definition; only the origin panel changes.
...
identity about twin primes.  The holdout origins are
`21001,23001,25001`, with counts `256,512,1024`; the parent origins are
`6001,8001,10001`.
...
## 5. Origins-only transfer ledger

Let `S_353(e)` denote the TPC-353 law summary and `S_354(e)` the corresponding
summary on the holdout.  The certificate records, for every law `e`,

```text
Delta_min(e)  = min kappa_A(S_354(e)) - min kappa_A(S_353(e)),
Delta_mean(e) = mean kappa_A(S_354(e)) - mean kappa_A(S_353(e)),
Delta_max(e)  = max kappa_A(S_354(e)) - max kappa_A(S_353(e)).
```

The parent certificate is hash-locked and the protocol comparison requires
that counts, shells, exponents, height, laws, and source model agree; origins
are the sole declared change.  This makes a positive census a transfer audit,
not a second source-fitted experiment.
