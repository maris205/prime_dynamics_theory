# RH-MVP2 theorem and status ledger
...
| Same-clock head transport | OPEN / NOT_TESTABLE | `D_(4k)(R)=sum_(2<=n<4k)|h_(sigma,n)-s_(k,n)|R^n/n -> 0` is assumed by RH-355--RH-360 and proved by none. |
| Actual full-trace `q/E_off` closure | OPEN / NOT_TESTABLE | No same-clock, same-type, unnormalised aggregate theorem is present. |
| RH-241 all-order noisy bridge | OPEN | The moving noisy envelope and no-over-extraction coefficient bridge remain absent. |
| Gates A--E | FALSE/OPEN | No Gate is promoted by this synthesis. |
| RH-362 activation | NOT ACTIVATED | A synthesis document is not a numbered theorem edge; new numbering still requires an admissible trigger in `RH_HANDOFF.md`. |

The ledger deliberately distinguishes `VERIFIED` (machine-readable audit),
`SUMMARISED` (source-backed compression), `PROVED UPSTREAM`, `OPEN`, and
`NOT_TESTABLE`.  These labels are not interchangeable.
