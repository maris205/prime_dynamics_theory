...
\begin{longtable}{p{1.4cm}p{4.0cm}p{8.0cm}}
\toprule
...
\begin{center}
\begin{tabular}{p{1.0cm}p{4.0cm}p{2.0cm}p{6.2cm}}
\toprule
...
\section{One umbrella and three thematic volumes}

The present manuscript is the short umbrella: it fixes vocabulary,
provenance, phases, Gates, and the current stop.  If a longer publication set
is wanted, the source crosswalk supports three largely independent thematic
volumes without changing any theorem status:
\begin{enumerate}[leftmargin=2.2em]
\item RH-161--RH-241: physical Riesz packets, temporal clouds, relative
 determinants, and the trace-envelope frontier;
\item RH-242--RH-281: deterministic numerator anchors, selectors, analytic
 tails, and counterloops;
\item RH-282--RH-361: noisy heads, weighted/annular endpoints, first alias,
 and actual-versus-deterministic signed completion.
\end{enumerate}
RH-MVP1 remains the dedicated synthesis for RH-1--RH-160.  This division is
preferable to one purported mega-theorem because each volume has one data
type, one family of clocks, and an independently auditable boundary.
