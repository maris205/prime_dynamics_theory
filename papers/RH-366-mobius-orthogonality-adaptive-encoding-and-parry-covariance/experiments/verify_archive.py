"""Independently verify the RH-366 fixed publication manifest."""
...
from experiments.build_archive import EXTERNAL_INPUTS, LOCAL_MEMBERS
...
EXPECTED_PUBLICATION_MEMBERS = set(LOCAL_MEMBERS)
EXPECTED_EXTERNAL_INPUTS = set(EXTERNAL_INPUTS)
...
def validate_manifest(manifest: dict[str, object]) -> None:
    if manifest.get("status") != "RH-366_fixed_publication_manifest":
    ...
    publication_count = manifest.get("publication_file_count")
    external_count = manifest.get("external_input_count")
    if type(publication_count) is not int:
        raise RuntimeError("publication file count is not an integer")
    if type(external_count) is not int:
        raise RuntimeError("external input count is not an integer")
    if publication_count != len(EXPECTED_PUBLICATION_MEMBERS):
        raise RuntimeError("publication file count changed")
    if external_count != len(EXPECTED_EXTERNAL_INPUTS):
