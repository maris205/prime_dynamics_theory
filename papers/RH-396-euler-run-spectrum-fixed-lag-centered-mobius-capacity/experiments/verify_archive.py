"""Verify RH-396 archive membership, hashes, sources, and replay gates."""
...
def verify_manifest(manifest: object) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-396_fixed_publication_manifest":
    ...
    if type(external_entries) is not dict or external_count != 160:
    ...
    exact_counts = {
        ...
        "external_git_input_count": 160,
        "remote_logical_input_count": 4,
        "logical_input_total": 164,
    }
    ...
    return {
        "status": "RH-396_archive_verified" if not failures else "RH-396_archive_failed",
        ...
        "logical_input_total": 164 if fresh is not None else 0,
        **current_counts,
