"""Verify RH-380 manifest membership, hashes, and source-lock identity."""
...
def verify_manifest(manifest: dict[str, object]) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-380_fixed_publication_manifest":
    ...
    try:
        ...
        result_files = (
            {entry["path"]: entry["sha256"] for entry in result_entries}
            if isinstance(result_entries, list)
            else None
        )
        release_pass = result.get("source_locks", {}).get(
            "release_blob_identity_pass"
        ) is True
    ...
    return {
        "status": "RH-380_archive_verified" if not failures else "RH-380_archive_failed",
