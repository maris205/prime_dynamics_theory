...
def test_archive_membership_contract() -> None:
    ...
    assert len(build_archive.SOURCE_FILES) == 24
    assert len(set(build_archive.SOURCE_FILES)) == 24
...
def test_archive_full_regeneration_if_present() -> None:
    ...
    assert stored_report["status"] == "RH-380_archive_verified"
    assert stored_report["failure_count"] == 0
    assert stored_report["publication_file_count"] == 28
    assert stored_report["external_input_count"] == 24
