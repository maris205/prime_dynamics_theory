"""Verify RH-394 archive membership, hashes, sources, and replay gates."""
...
def verify_manifest(manifest: object) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-394_fixed_publication_manifest":
    ...
    if type(external_entries) is not dict or external_count != 128:
    ...
    exact_counts = {
        ...
        "external_git_input_count": 128,
        "remote_logical_input_count": 4,
        "logical_input_total": 132,
    }
    ...
    return {
        "status": "RH-394_archive_verified" if not failures else "RH-394_archive_failed",
        ...
        "logical_input_total": 132 if fresh is not None else 0,
        **current_counts,
