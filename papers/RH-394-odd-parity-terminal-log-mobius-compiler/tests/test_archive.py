...
from odd_parity_compiler.core import exact_equal, loads_strict  # noqa: E402
...
def test_manifest_is_exact_fresh_regeneration() -> None:
    ...
    require(stored["publication_file_count"] == len(build_archive.LOCAL_MEMBERS) == 39)
    require(stored["release_stage_file_count"] == 41)
    require(stored["external_git_input_count"] == 128)
    require(stored["remote_logical_input_count"] == 4)
    require(stored["logical_input_total"] == 132)
...
def test_archive_report_is_exact_and_failure_free() -> None:
    ...
    require(exact_equal(stored, fresh))
    require(stored["status"] == "RH-394_archive_verified")
...
def test_frozen_stage_digests_are_exact() -> None:
    require(len(build_archive.FROZEN_STAGE_DIGESTS) == 10)
    require(all(
...
def test_manifest_hash_membership_and_path_attacks_fail_closed() -> None:
    ...
    for relative in ("unexpected.pdf", "../outside", "x" * 5000):
        ...
        require(report["status"] == "RH-394_archive_failed")
...
def test_exact_integer_boolean_commit_and_hygiene_attacks_fail() -> None:
    ...
    for key, value in (
        ...
        attacks.append(attacked)
    attacked = deepcopy(manifest)
    attacked["source_commits"]["rh393_release"] = "0" * 40
    attacks.append(attacked)
    for payload in attacks:
        require(verify_archive.verify_manifest(payload)["failure_count"] > 0)
...
value = build_result.build_payload(); emit(build_result.pretty_json_bytes(value)); del value; gc.collect()
value = build_schema.build_schema(); emit(build_result.pretty_json_bytes(value)); del value; gc.collect()
manifest = build_archive.build_payload(); emit(build_archive.serialized_payload(manifest).encode())
value = verify_archive.verify_manifest(manifest); emit(verify_archive.serialized_report(value).encode())
"""
...
def test_official_schema_is_closed_and_accepts_result() -> None:
    ...
    require(list(jsonschema.Draft202012Validator(schema).iter_errors(result)) == [])


def test_outer_verifier_replays_without_writing() -> None:
    manifest = loads_strict((ROOT / "results" / "dependency_manifest.json").read_text(encoding="utf-8"))
    report = verify_archive.verify_manifest(manifest)
    require(report["status"] == "RH-394_archive_verified")
    require(report["failure_count"] == 0)
