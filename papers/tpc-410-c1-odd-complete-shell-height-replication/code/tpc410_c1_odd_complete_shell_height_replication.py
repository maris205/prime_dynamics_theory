#!/usr/bin/env python3
"""Exact odd complete-shell height replication for the TPC local proxy entry."""
...
RESULT = PROJECT / "results/tpc410_certificate.json"
...
Q, HEIGHTS, B = 131072, (16, 32, 66, 128), 1_000_000
EXPECTED_SHELL_COUNT = 10749
...
def payload():
    ...
    return {
        ...
        "theorem_domain": {
            ...
            "Q_and_shell": "Q=131072>N and the complete odd shell Q<p<=2Q with r=10749 primes",
            "profile": "all shell primes, indexed increasingly; even i residue 0, odd i residue -N",
            "parity": "r odd; m_plus=ceil(r/2)=5375 and m_minus=floor(r/2)=5374",
        ...
        "theorem": {
            ...
            "scope": "one adjacent normalized proxy entry across a finite odd-shell height replication, not the full operator",
        },
        "cases": [row(H, shell) for H in HEIGHTS],
        "claim_firewall": {
            "ODD_COMPLETE_SHELL_HEIGHT_REPLICATION": "PROVED_EXACT_FINITE",
            "HEIGHT_REPLICATION_DECIMALS": "NUMERICAL_OBSERVATION",
...
def main():
    ...
    if args.write:
        write(); print("TPC410_CERTIFICATE=WRITTEN")
    elif args.check:
        check_document(json.loads(RESULT.read_bytes(), object_pairs_hook=no_duplicates,
                                  parse_constant=no_constants))
        print("TPC410_CERTIFICATE=PASS cases=4 heights=4 odd_complete_shell=PASS")
    else:
