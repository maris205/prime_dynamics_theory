# TPC-410 proof package

## Theorem (finite odd complete-shell height ladder)

Fix `Q=131072`. For each `H` in `{16,32,66,128}`, set `N=4H` and retain all
primes in the complete shell `Q<p<=2Q`. The shell has `r=10749` primes, so
`m_-=floor(r/2)=5374` odd-index terms and `m_+=ceil(r/2)=5375` even-index
