#!/usr/bin/env python3
"""Independent exact literal replay for TPC-410's odd-shell height replication."""
...
def replay():
    ...
    need(len(shell) == 10749, "shell census")
...
def main():
    ...
    need(payload["heights"] == list(HEIGHTS) and payload["shell_count"] == 10749, "height census")
    need(payload["theorem_domain"]["parity"].startswith("r odd"), "odd-shell domain")
    need(payload["claim_firewall"]["ODD_COMPLETE_SHELL_HEIGHT_REPLICATION"] == "PROVED_EXACT_FINITE", "firewall")
    print("TPC410_INDEPENDENT_CHECK=PASS cases=4 heights=4 literal_crt_masks=PASS odd_shell=PASS")
