...
2048-point intervals are disjoint from the declared TPC-376--379 intervals by
integer endpoint comparison.  The q=8 anchor `[1300014,1300027)` has shell
`{11,13}`; its geometry is a positive rational sum and every law matrix is
symmetric.  The offset is a fixed finite anchor repair and is not a panel
selection rule.
...
## 3. Count-replay result

The 36-row `N=2048` panel has the same failure profiles as TPC-379:
...
all-plus band maximum is about `0.66694556699`; the three signed maxima are
about `0.00776463827`, `0.01203821432`, and `0.21613429441` in declaration
order.  Thus the finite high-Q signature persists at the larger count while
remaining strongly law-dependent.

## 4. Boundary

This result is finite count persistence only.  It does not establish law,
