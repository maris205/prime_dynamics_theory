"""Verify RH-384 manifest membership, hashes, and source-lock identity."""
...
def verify_manifest(manifest: dict[str, object]) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-384_fixed_publication_manifest":
    ...
    if external_count != len(SOURCE_FILES) or type(external_entries) is not dict or set(external_entries) != set(SOURCE_FILES):
    ...
    semantic_match = type(local_entries) is dict and (
        local_entries.get("main.pdf") == local_entries.get("prime-tail-scale-separation.pdf")
    )
    checks = (
    ...
    return {
        "status": "RH-384_archive_verified" if not failures else "RH-384_archive_failed",
...
def main() -> None:
    try:
        ...
        payload = {
            "status": "RH-384_archive_failed", "publication_file_count": 0,
