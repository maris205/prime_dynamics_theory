#!/usr/bin/env python3
"""TPC-395: cross-family holdout of the TPC-394 origin signal.

TPC-394 found a large alternating-index origin spread on a fresh same-count
family.  TPC-395 freezes that family's cell means as a read-only baseline and
tests a third, disjoint affine family with three calibration and three
holdout origins.  The panel is finite and response-blind; it is not an
...
RESULT = PROJECT / "results/tpc395_certificate.json"
PARENT_CODE = ROOT / (
...
CLAIM_FIREWALL = {
    ...
    "TPC395_CROSS_FAMILY_PANEL":
        "NUMERICALLY_CERTIFIED_FINITE_48_ROWS",
    "TPC395_CROSS_FAMILY_MEAN_TRANSFER":
        "NUMERICALLY_CERTIFIED_FINITE_SCOPED",
    "TPC395_WITHIN_FAMILY_ORIGIN_AUDIT":
        "NUMERICALLY_CERTIFIED_FINITE_SCOPED",
    "TPC395_SPECTRAL_ENVELOPE":
        "REFUTED_ON_DECLARED_FINITE_PANEL",
    "TPC395_SCHUR_ENVELOPE":
...
def load_parent_baseline() -> dict[tuple[str, str, int], float]:
    ...
    need(payload.get("schema") ==
         "TPC394_C1_ORIGIN_UNIFORMITY_LADDER_V1" and
         payload.get("status") ==
    ...
    need(set(baseline) == {(norm, law, Q_ANCHORS[0])
...
def origin_summary(rows: list[dict[str, Any]],
                   baseline: dict[tuple[str, str, int], float]) -> dict[str, Any]:
    definitions = {
    ...
    return {
        ...
        "holdout_origin_count": 3, "parent_family": "TPC394",
...
def build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "parent_lock": {
            ...
            "parent_schema": "TPC394_C1_ORIGIN_UNIFORMITY_LADDER_V1",
            "parent_status":
                "NUMERICALLY_CERTIFIED_FINITE_C1_ORIGIN_UNIFORMITY_LADDER_AUDIT",
        ...
        "protocol": {
            ...
            "cross_family_definition": "new-family cohort mean / TPC394 mean - 1",
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            ...
            print("TPC395_CERTIFICATE=WRITTEN")
        else:
            ...
            print(f"TPC395_CERTIFICATE=PASS rows={ROW_COUNT} cells={CELL_COUNT} "
        ...
        print("TPC395_CERTIFICATE=FAIL " + str(error))
        return 1
