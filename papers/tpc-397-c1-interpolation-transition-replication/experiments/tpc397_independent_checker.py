#!/usr/bin/env python3
"""Independent reverse-shell replay for TPC-397.

The implementation is intentionally separate from the producer.  It validates
the TPC-396 endpoint baseline, rebuilds the new family in descending prime
order, forms the four exact finite matrix interpolants, and recomputes all
cohort aggregates.
"""
...
def parent_endpoints() -> dict[tuple[str, str, int], float]:
    ...
    need(payload.get("schema") == "TPC396_C1_SIGNED_LAW_INTERPOLATION_V1" and
         payload.get("status") ==
...
def parent_baseline() -> dict[tuple[str, str, int], float]:
    ...
    for norm in NORMALIZATIONS:
        plus = endpoints[norm, "all_plus", 8192]
        alternating = endpoints[norm, "alternating_index", 8192]
        for law, (numerator, denominator) in LAMBDA_FRACTIONS.items():
            lam = numerator / denominator
            result[norm, law, 8192] = ((1.0 - lam) * plus +
                                       lam * alternating)
    return result
    ...
    for norm in NORMALIZATIONS:
        for law in LAWS:
            ...
            need(cell.get("origins") == [r["origin"] for r in selected] and
                 ...
                 cell.get("parent_family") == "TPC396" and
                 cell.get("interpolation_lambda") ==
                 f"{LAMBDA_FRACTIONS[law][0]}/{LAMBDA_FRACTIONS[law][1]}",
                 "cell protocol")
...
def check_document(document: dict[str, Any], recompute: bool = True) -> None:
    ...
    need(lock.get("parent_code_sha256") == PARENT_CODE_SHA256 and
         ...
         lock.get("parent_schema") == "TPC396_C1_SIGNED_LAW_INTERPOLATION_V1" and
...
def main() -> int:
    ...
    try:
        ...
        print(f"TPC397_INDEPENDENT_CHECK=PASS rows={ROW_COUNT} cells={CELL_COUNT} "
        ...
        print("TPC397_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
