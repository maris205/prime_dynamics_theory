#!/usr/bin/env python3
"""Mutation firewall for the TPC-397 interpolation certificate."""
...
import tpc397_independent_checker as independent  # noqa: E402
...
def main() -> int:
    ...
    try:
        ...
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC397_ARITHMETIC_ADVANCE", "YES"))
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC397_FIXED_POWER_CREDIT", 1))
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC397_FULL_GATE_B", "CLOSED"))
        ...
        print("TPC397_STRESS=PASS mutations=28")
        return 0
    except Exception as error:
        print("TPC397_STRESS=FAIL " + str(error), file=sys.stderr)
        return 1
