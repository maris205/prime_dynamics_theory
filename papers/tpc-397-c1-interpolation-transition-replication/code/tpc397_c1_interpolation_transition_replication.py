#!/usr/bin/env python3
"""TPC-397: finite interpolation transition replication on a fresh c=1 family.

TPC-396 localized a finite endpoint transition: coefficients through 2/3 were
stable on its panel, while the alternating endpoint was not.  TPC-397 freezes
TPC-396's endpoint means and tests coefficients inside the remaining interval
on a fifth disjoint family.  The panel is finite and response-blind; the
intermediate laws are modeling probes, not new arithmetic sign laws.
"""
...
RESULT = PROJECT / "results/tpc397_certificate.json"
PARENT_CODE = ROOT / (
    "papers/tpc-396-c1-signed-law-interpolation/code/"
    "tpc396_c1_signed_law_interpolation.py")
PARENT_CERT = ROOT / (
    "papers/tpc-396-c1-signed-law-interpolation/results/"
    "tpc396_certificate.json")
PARENT_CODE_SHA256 = (
...
LAMBDA_FRACTIONS = {
    "blend_3_4": Fraction(3, 4),
    "blend_5_6": Fraction(5, 6),
    "blend_11_12": Fraction(11, 12),
...
def load_parent_endpoints() -> dict[tuple[str, str, int], float]:
    ...
    need(payload.get("schema") ==
         "TPC396_C1_SIGNED_LAW_INTERPOLATION_V1" and
         payload.get("status") ==
         "NUMERICALLY_CERTIFIED_FINITE_C1_SIGNED_LAW_INTERPOLATION_AUDIT",
         "parent header")
    cells = payload.get("origin_summary", {}).get("cells", [])
    baseline: dict[tuple[str, str, int], float] = {}
    for cell in cells:
        current_law = cell.get("law")
        if current_law not in ("blend_0", "blend_1"):
            continue
        endpoint = "all_plus" if current_law == "blend_0" \
            else "alternating_index"
...
def load_parent_baseline() -> dict[tuple[str, str, int], float]:
    ...
    baseline: dict[tuple[str, str, int], float] = {}
    for norm in NORMALIZATIONS:
        plus = endpoints[norm, "all_plus", Q_ANCHORS[0]]
        alternating = endpoints[norm, "alternating_index", Q_ANCHORS[0]]
        for law in LAWS:
            lam = float(LAMBDA_FRACTIONS[law])
            baseline[norm, law, Q_ANCHORS[0]] = (
                (1.0 - lam) * plus + lam * alternating)
    return baseline
...
def origin_summary(rows: list[dict[str, Any]],
    ...
    for norm in NORMALIZATIONS:
        for law in LAWS:
            for q0 in Q_ANCHORS:
                ...
                coefficient = LAMBDA_FRACTIONS[law]
                parent_plus = parent_endpoints[norm, "all_plus", q0]
                parent_alternating = parent_endpoints[
                    norm, "alternating_index", q0]
                ...
                cells.append({
                    ...
                    "count": WINDOW_COUNT,
                    "parent_family": "TPC396",
                    "parent_interpolation_definition":
                        "(1-lambda)*TPC396 blend_0 + lambda*TPC396 blend_1",
                    "interpolation_lambda":
                        f"{coefficient.numerator}/{coefficient.denominator}",
                    "parent_endpoint_means": {
                        "all_plus": show(parent_plus),
                        "alternating_index": show(parent_alternating),
                    },
    ...
    return {
        ...
        "holdout_origin_count": 3, "parent_family": "TPC396",
        "parent_interpolation":
            "linear interpolation of the two frozen TPC396 endpoint means",
    ...
    return {
        ...
        "protocol": {
            ...
            "cross_family_definition":
                "new-family cohort mean / linearly interpolated TPC396 endpoint mean - 1",
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            ...
            print("TPC397_CERTIFICATE=WRITTEN")
        else:
            ...
            print(f"TPC397_CERTIFICATE=PASS rows={ROW_COUNT} cells={CELL_COUNT} "
        ...
        print("TPC397_CERTIFICATE=FAIL " + str(error))
        return 1
