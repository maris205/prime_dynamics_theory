#!/usr/bin/env python3
"""Independent reverse-shell audit for TPC-357.

The producer is deliberately not imported.  This checker rebuilds the prime
shell, divisibility masks, unsigned geometry, four signed matrices, Schur and
Frobenius envelopes, and the all-plus spectral rows in reverse shell order.
"""
...
def reverse_components(origin: int, count: int, q0: int, exponent: int
    ...
    for index, prime in reversed(list(enumerate(primes))):
        ...
        for law in LAW_NAMES:
            matrices[law] += sign_map[law][index] * block
    for law in LAW_NAMES:
...
def exact_anchor() -> dict[str, Any]:
    ...
    need(all(matrix[i][j] == matrix[j][i]
         ...
         "exact reverse symmetry")
    need(all(value > 0 for value in geometry), "exact reverse geometry")
...
def load_certificate() -> tuple[dict[str, Any], dict[str, Any]]:
...
def check_static(payload: dict[str, Any]) -> None:
    ...
    need(lock.get("TPC355_code_sha256") == BASE_CODE_SHA256 and
         lock.get("TPC356_code_sha256") == PARENT_CODE_SHA256 and
         lock.get("TPC356_certificate_sha256") == PARENT_CERT_SHA256,
         "parent lock")
    ...
    need(protocol.get("origins") == list(ORIGINS) and
         ...
         protocol.get("source_response_used") is False,
         "protocol")
    ...
    need(len(keys) == 288, "row key uniqueness")
                        ...
                        for family, actual in (("raw_metrics", actual_raw),
                            ...
                            if spectrum:
                                ...
                                need(recorded["spectral_norm"] is None,
                                     "unexpected non-spectrum row")
                        need(float(actual_norm["schur_row_sum_bound"]) < 0.83,
                             "reverse normalized Schur cap")
    anchor = exact_anchor()
    recorded_anchor = payload.get("exact_anchor", {})
    for field in ("row_sums_digest", "geometry_digest", "shell"):
        need(anchor[field] == recorded_anchor.get(field),
             "exact anchor " + field)
...
def main() -> int:
    ...
    try:
        _, payload = load_certificate()
        check_static(payload)
        replay(payload)
        print("TPC357_INDEPENDENT_CHECK=PASS rows=288 spectral_rows=72 "
              "reverse_shell=1 normalized_schur_cap=0.83")
        return 0
    except (Failure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError, np.linalg.LinAlgError) as error:
        print("TPC357_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
