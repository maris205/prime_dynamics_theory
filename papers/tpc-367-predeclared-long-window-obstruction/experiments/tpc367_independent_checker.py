#!/usr/bin/env python3
"""Independent reverse-order replay for the TPC-367 certificate.

This checker deliberately does not import the TPC-367 producer.  It rebuilds
the prime shell, masked components, weighted geometry, four sign laws, and
the exact anchor in a different accumulation order, then compares every
recorded finite row.
"""
...
ORIGINS = tuple(CANDIDATE_ORIGINS[i] for i in ORIGIN_INDICES)
COUNTS = (512, 1024)
...
def reverse_components(values: np.ndarray, q0: int, exponent: int,
                       beta: int):
    """Rebuild all components with a descending shell traversal."""
    ...
    for beta in BETAS:
        ...
        need(item.get("rows") == 144 and
             item.get("spectral_cap_violations") == sum(
                 x > SPECTRAL_CAP for x in spectra) and
             item.get("schur_cap_violations") == sum(
                 x > SCHUR_CAP for x in schurs) and
             item.get("spectral_min") == format(min(spectra), ".17g") and
             item.get("spectral_max") == format(max(spectra), ".17g") and
             item.get("schur_max") == format(max(schurs), ".17g"),
             "phase beta " + str(beta))
...
def main() -> int:
    ...
    try:
        ...
        print("TPC367_INDEPENDENT_CHECK=PASS rows=288 beta2_rows=144 "
              "beta2_violations=6 baseline_beta0_violations=36")
        return 0
    except (Failure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError, np.linalg.LinAlgError) as error:
        print("TPC367_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
