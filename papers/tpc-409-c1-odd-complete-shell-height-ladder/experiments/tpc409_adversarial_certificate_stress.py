#!/usr/bin/env python3
"""Strict mutation tests for the TPC-409 height-ladder contract."""
...
def validate(document):
    ...
    need(payload.get("Q") == 65536 and payload.get("heights") == [16, 32, 66, 128], "scales")
    need(payload.get("shell_count") == 5709, "shell count")
    ...
    need(all(c.get("N") == 4 * c.get("H") and c.get("uniform_bound_exact") is True and
             c.get("m_minus") + c.get("m_plus") == 5709 and
...
def main():
    ...
    print(f"TPC409_STRESS=PASS mutations={len(mutations)} strict_contract=PASS")
