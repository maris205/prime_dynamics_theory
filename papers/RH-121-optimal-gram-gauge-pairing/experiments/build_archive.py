...
def main() -> None:
    external = {
        "rh114_audit": ROOT.parent / "RH-114-psd-rayleigh-directional-tail/results/psd_rayleigh_audit.json",
        "rh120_summary": ROOT.parent / "RH-120-gauge-covariant-rayleigh-transfer/results/summary.json",
    }
    ...
    publications = [ROOT / name for name in (
        ...
        "pyproject.toml", "requirements.txt", "figures/optimal_gram_gauge_pairing.pdf",
        "figures/optimal_gram_gauge_pairing.png", "main.pdf", "optimal-gram-gauge-pairing.pdf",
    )]
    dependency = {
        "status": "all_rh121_inputs_sources_and_publication_artifacts_hashed",
    ...
    audit = json.loads((ROOT / "results" / "optimal_gauge_audit.json").read_text(encoding="utf-8"))
    results = [ROOT / "results" / name for name in ("optimal_gauge_audit.json", "optimal_gauge_smoke.json", "dependency_manifest.json")]
    summary = {
        "status": "rh121_optimal_gram_gauge_pairing_archived",
        "theorem": {"optimal_exact_gram_gauge": True, "ordered_generalized_eigenframe_matching": True},
