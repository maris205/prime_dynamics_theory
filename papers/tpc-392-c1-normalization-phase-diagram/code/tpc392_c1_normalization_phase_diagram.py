#!/usr/bin/env python3
"""TPC-392: a scalar-normalization phase diagram.
...
Everything here is a c=1 proxy calculation; no arithmetic or asymptotic
claim is made.
"""
...
RESULT = PROJECT / "results/tpc392_certificate.json"
PARENT_CODE = ROOT / (
    "papers/tpc-391-c1-recursive-horizon-localization/code/"
    "tpc391_recursive_horizon_localization.py")
PARENT_CERT = ROOT / (
...
CLAIM_FIREWALL = {
    ...
    "TPC392_NORMALIZATION_PANEL":
        "NUMERICALLY_CERTIFIED_FINITE_256_ROWS",
    "TPC392_SCALAR_DEFINITIONS":
        "PROVED_EXACT_FINITE_DECLARED",
    "TPC392_PHASE_COMPARISON":
        "NUMERICALLY_CERTIFIED_FINITE_SCOPED",
    "TPC392_CALIBRATION_FORECAST":
        "NUMERICALLY_CERTIFIED_FINITE_SCOPED",
...
def parent_reference() -> dict[str, Any]:
    ...
    need(payload.get("schema") ==
         "TPC391_C1_RECURSIVE_HORIZON_LOCALIZATION_V1", "parent schema")
    need(payload.get("status") ==
         "NUMERICALLY_CERTIFIED_FINITE_C1_RECURSIVE_HORIZON_LOCALIZATION_AUDIT",
         "parent status")
    return {
...
def _legacy_transfer_summary(rows: list[dict[str, Any]],
    ...
    """Retained TPC-391 template, intentionally unused by TPC-392.
    ...
    """
    raise CheckFailure("retired TPC-391 helper is not part of TPC-392")
...
def phase_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    """Summarize the predeclared normalization phase diagram."""
    cells: list[dict[str, Any]] = []
    definitions = {
        "local_diagonal":
    ...
    need(len(cells) == 32, "phase cell census")
    local_means = {}
    for cell in cells:
...
def build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "parent_lock": {
            ...
            "parent_round2_clue": "TEST_C1_NORMALIZATION_PHASE_DIAGRAM",
        ...
        "finite_audit": {
            ...
            "calibration_counts": list(CALIBRATION_COUNTS),
            "holdout_count": HOLDOUT_COUNT,
            "band_mode_count": 1, "q_count": 2, "law_count": 4,
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            ...
            print("TPC392_CERTIFICATE=WRITTEN")
        else:
            ...
            print("TPC392_CERTIFICATE=PASS rows=256 cells=32 "
        ...
        print("TPC392_CERTIFICATE=FAIL " + str(error), file=sys.stderr)
        return 1
