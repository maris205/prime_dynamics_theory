#!/usr/bin/env python3
"""Independent reverse-shell replay for TPC-392.

This file intentionally does not import the TPC-392 producer.  It rebuilds
the finite c=1 matrices in descending prime-shell order and checks the
normalization phase diagram against the sealed certificate.
"""
...
PARENT_CERT = ROOT / (
    "papers/tpc-391-c1-recursive-horizon-localization/results/"
    "tpc391_certificate.json")
PARENT_CERT_SHA256 = (
...
def _legacy_check_summary_from_rows(rows: list[dict[str, Any]],
    ...
    raise Failure("retired TPC-391 helper is not part of TPC-392")
...
def check_phase_summary(rows: list[dict[str, Any]],
    ...
    recorded_cells = summary.get("cells", [])
    need(len(recorded_cells) == 32, "recorded phase cell count")
    recorded_map = {}
    for cell in recorded_cells:
        key = (cell.get("band_mode"), cell.get("normalization"),
               cell.get("law"), cell.get("Q"))
    ...
    terminal_means = {
        norm: sum(recomputed[("fixed_c3", norm, law, q0)][2][1536]
                  for law in LAWS for q0 in Q_ANCHORS) / 8.0
        for norm in NORMALIZATIONS
    }
    terminal_order = [
...
def check_document(document: dict[str, Any], recompute: bool = True) -> dict[str, Any]:
    ...
    need(finite_audit.get("rows") == 256 and
    ...
    need(firewall.get("TPC392_ARITHMETIC_ADVANCE") == "NO" and
         ...
         firewall.get("TPC392_TWIN_PRIME_RESULT") == "NONE", "firewall")
...
def main() -> int:
    ...
    try:
        ...
        print("TPC392_INDEPENDENT_CHECK=PASS rows=256 cells=32 "
              ...
              f"stable_holdout={summary['stable_cells']['1536']}/32 "
              f"terminal_order={summary['terminal_mean_ordering']}")
        return 0
    except (Failure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError) as error:
        print("TPC392_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
