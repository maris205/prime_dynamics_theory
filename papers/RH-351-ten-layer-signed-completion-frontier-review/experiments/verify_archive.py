"""Verify all manifests and write only the RH-351 verification record."""
...
def main() -> None:
    ...
    for directory in paper_directories():
        ...
        if manifest.get("status") != f"{directory.name}_publication_manifest":
            failures.append(
                {"file": "dependency_manifest.json", "reason": "status_mismatch"}
            )
        if manifest.get("file_count") != len(expected):
            failures.append(
                {"file": "dependency_manifest.json", "reason": "file_count_mismatch"}
            )
        ...
        if directory == ROOT:
            ...
            if existing != payload:
                failures.append(
                    {
                        "file": "archive_verification.json",
                        "reason": "upstream_verification_mismatch",
                    }
                )
