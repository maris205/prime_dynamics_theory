"""Verify RH-379 manifest membership, hashes, and source-lock identity."""
...
def verify_manifest(manifest: dict[str, object]) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-379_fixed_publication_manifest":
    ...
    try:
        ...
        result_files = result.get("source_locks", {}).get("files")
    ...
    return {
        "status": "RH-379_archive_verified" if not failures else "RH-379_archive_failed",
