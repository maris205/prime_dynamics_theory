...
def main() -> None:
    ...
    publications = [ROOT / name for name in (
        ...
        "pyproject.toml", "requirements.txt", "figures/outward_loewner_transport_guards.pdf",
        "figures/outward_loewner_transport_guards.png", "main.pdf", "outward-loewner-transport-guards.pdf",
    )]
    dependency = {
        "status": "all_rh127_sources_and_publication_artifacts_hashed",
        "external_inputs": {},
    ...
    audit = json.loads((ROOT / "results" / "outward_guard_audit.json").read_text(encoding="utf-8"))
    results = [ROOT / "results" / name for name in ("outward_guard_audit.json", "outward_guard_smoke.json", "dependency_manifest.json")]
    summary = {
        "status": "rh127_outward_loewner_transport_guards_archived",
        "theorem": {"outward_loewner_guard": True, "sharp_radius_amplification": True},
