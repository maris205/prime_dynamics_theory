...
TPC-385 transferred a high-bandwidth phase across new origins at a fixed
window count.  This project freezes that phase reference and changes only
the count: three N=512 calibration windows define the pooled geometry, while
two coordinate-disjoint N=1024 windows are read out as holdouts.  The fixed
three-block band and the full relative band are both reported, so a failure
of a finite spectral cap cannot be hidden inside a bandwidth convention.

Everything here is a finite numerical certificate for the proxy.  It is not
an asymptotic operator theorem and it makes no arithmetic claim about twin
primes.
"""
...
RESULT = PROJECT / "results/tpc386_certificate.json"
PARENT_CODE = ROOT / (
...
PARENT_CODE_SHA256 = (
    "68825812bfffd90733472103fd4de200adb7b81ed3d02b57f992cc8d0d21e4b0")
PARENT_CERT_SHA256 = (
    "ecac4403e2f803fd36c764509f2cd7cbb385e8c45aa5bba103f5b734341f391e")
...
COUNT_BY_ORIGIN = {origin: (512 if origin in CALIBRATION_ORIGINS else 1024)
...
# These are copied from the hash-locked TPC-385 certificate.  They are
# forecasts/reference values, never fitted to the TPC-386 holdout.
...
CLAIM_FIREWALL = {
    ...
    "TPC386_COUNT_HOLDOUT_PANEL":
        "NUMERICALLY_CERTIFIED_FINITE_160_ROWS",
    "TPC386_ALL_PLUS_COUNT_TRANSFER":
        "NUMERICALLY_CERTIFIED_FINITE_SCOPED",
    "TPC386_FIXED_SPECTRAL_CAP_TRANSFER":
        "REFUTED_FINITE_SCOPED",
...
def coordinate_disjointness() -> bool:
    ...
    current = [(origin, origin + COUNT_BY_ORIGIN[origin])
               for origin in ORIGINS]
...
def mask_for(mode: str, count: int) -> tuple[np.ndarray, int]:
    ...
    cutoff = 3 if mode == "fixed_c3" else block_count - 1
    block_ids = np.arange(count) // BLOCK_LENGTH
    return np.abs(block_ids[:, None] - block_ids[None, :]) <= cutoff, cutoff


def make_row(origin: int, q0: int, law: str, norm: str, mode: str,
    ...
    return {
        ...
        "normalization": norm, "band_mode": mode,
        "effective_cutoff": effective_cutoff, "count": count,
        "interval": [origin, origin + count],
        ...
        "pooled_train_scalar": show(pooled),
...
def build_rows() -> tuple[list[dict[str, Any]], dict[int, float]]:
    ...
    packs_by_q: dict[int, list[tuple[int, Any]]] = {}
    for q0 in Q_ANCHORS:
        packs = []
        for origin in ORIGINS:
            count = COUNT_BY_ORIGIN[origin]
        ...
        for origin, (primes, matrices, geometry, weights) in packs:
            ...
            for mode in BAND_MODES:
                mask, effective_cutoff = mask_for(mode, count)
                for norm in NORMALIZATIONS:
                    for law in LAWS:
                        rows.append(make_row(
                            origin, q0, law, norm, mode, matrices[law],
                            geometry, pooled[q0], primes, weights, mask,
                            effective_cutoff))
            ...
            for law in LAWS:
                for q0 in Q_ANCHORS:
                    ...
                    cells.append({
                        ...
                        "calibration_count": 512, "holdout_count": 1024,
                        "all_origin": stats(all_values),
                        "calibration": stats(cal_values),
                        "holdout": stats(hold_values),
                        "holdout_to_calibration_ratio": show(ratio),
                        "count_log2_ratio": show(math.log(ratio, 2)),
                        "within_count_transfer_cap": bool(
                            abs(ratio - 1.0) <= COUNT_TRANSFER_CAP),
                    })
    ...
    for mode in BAND_MODES:
        for norm in NORMALIZATIONS:
            ...
            forecasts.append({
                ...
                "within_count_transfer_cap": bool(
                    abs(ratio - 1.0) <= COUNT_TRANSFER_CAP),
                "within_parent_reference_cap": bool(
                    abs(error) <= COUNT_TRANSFER_CAP),
            })
    return {
        ...
        "count_transfer_cap": show(COUNT_TRANSFER_CAP),
...
def build_payload() -> dict[str, Any]:
    ...
    need(parent_doc.get("payload", {}).get("schema") ==
         "TPC385_C1_BANDWIDTH_ORIGIN_HOLDOUT_V1", "parent schema")
    ...
    return {
        ...
        "parent_lock": {
            ...
            "parent_schema": "TPC385_C1_BANDWIDTH_ORIGIN_HOLDOUT_V1",
            "parent_round2_clue": "TEST_C1_HOLDOUT_COUNT_BANDWIDTH",
        ...
        "protocol": {
            ...
            "calibration_origins": list(CALIBRATION_ORIGINS),
            "holdout_origins": list(HOLDOUT_ORIGINS),
            "counts_by_origin": {str(o): COUNT_BY_ORIGIN[o] for o in ORIGINS},
            "calibration_count": 512, "holdout_count": 1024,
            ...
            "fixed_band_definition": "three adjacent blocks for both counts",
            "full_band_definition": "all block pairs at each count",
            "pooled_train_scalar_definition":
                "mean coordinate geometry over N=512 calibration origins only at fixed Q",
            "parent_forecast_definition":
                "locked TPC385 all-plus Q=8192 c=3 reference; no TPC386 response fit",
        },
        "rows": rows,
        "row_digest": hashlib.sha256(canonical(rows)).hexdigest(),
        "pooled_train_scalar_by_Q": {str(q): show(value)
                                      for q, value in pooled.items()},
