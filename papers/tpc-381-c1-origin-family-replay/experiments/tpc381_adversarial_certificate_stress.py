#!/usr/bin/env python3
"""Adversarial schema/firewall mutations for the TPC-381 certificate."""
...
def validate_shape(document: dict[str, Any]) -> None:
    ...
    need(parent.get("parent_schema") ==
        "TPC380_C1_LAW_CONTROL_COUNT_REPLAY_V1" and
         parent.get("parent_round2_clue") ==
        "TEST_C1_LAW_CONTROL_ORIGIN_FAMILY_REPLAY" and
    ...
    need(firewall.get("TPC381_ARITHMETIC_ADVANCE") == "NO" and
         firewall.get("TPC381_FIXED_POWER_CREDIT") == 0 and
         firewall.get("TPC381_FULL_GATE_B") == "OPEN" and
         firewall.get("TPC381_TWIN_PRIME_RESULT") == "NONE", "firewall")
...
def mutations(document: dict[str, Any]) -> list[dict[str, Any]]:
    paths = [
        ...
        ("grid_start", lambda d: d["payload"]["selection_protocol"].__setitem__("grid_start", 1400002)),
        ("grid_step", lambda d: d["payload"]["selection_protocol"].__setitem__("grid_step", 400)),
        ("candidate_origins", lambda d: d["payload"]["selection_protocol"].__setitem__("candidate_origins", [1400001] * 41)),
        ...
        ("row_origin", lambda d: d["payload"]["rows"][0].__setitem__("origin", 1400002)),
        ...
        ("arithmetic", lambda d: d["payload"]["claim_firewall"].__setitem__("TPC381_ARITHMETIC_ADVANCE", "YES")),
...
def main() -> int:
    ...
    try:
        ...
        print("TPC381_STRESS=PASS mutations=25")
        return 0
    except (Failure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError) as error:
        print("TPC381_STRESS=FAIL " + str(error), file=sys.stderr)
        return 1
