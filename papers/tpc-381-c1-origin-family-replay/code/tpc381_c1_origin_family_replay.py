#!/usr/bin/env python3
"""TPC-381: a finite c=1 law-control origin-family replay."""
...
RESULT = PROJECT / "results/tpc381_certificate.json"
PARENT_CODE = ROOT / (
...
SPECTRAL_CAP = 0.64
SCHUR_CAP = 0.83
# The first 13-point subinterval is positive for the q=8 shell at this
# predeclared origin family, so no response-dependent anchor repair is needed.
...
def load_parent_module():
    spec = importlib.util.spec_from_file_location(
        "tpc380_parent_for_tpc381", PARENT_CODE)
...
def load_parent_payload() -> dict[str, Any]:
    ...
    need(isinstance(payload, dict) and
         ...
         payload.get("status") ==
         "NUMERICALLY_CERTIFIED_FINITE_C1_LAW_CONTROL_COUNT_REPLAY",
         "parent payload")
    return payload
...
def prior_intervals() -> list[tuple[int, int]]:
    return [
        ...
        (1300001, 1300001 + 2048),
        (1308021, 1308021 + 2048),
...
def main() -> int:
    ...
    try:
        if args.write:
            ...
            print("TPC381_CERTIFICATE=WRITTEN")
        else:
            ...
            compact = ";".join(
                ...
                for law in LAWS)
            print("TPC381_CERTIFICATE=PASS rows=" + str(audit["rows"]) +
                  " failures=" + str(audit["spectral_cap_violations"]) +
                  " profiles=" + compact)
        return 0
    except (CheckFailure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError) as error:
        print("TPC381_CERTIFICATE=FAIL " + str(error), file=os.sys.stderr)
        return 1
