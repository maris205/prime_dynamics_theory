#!/usr/bin/env python3
"""TPC-396: finite signed-law interpolation across a fresh c=1 family.

TPC-395 found a stable all-plus/unstable alternating split on a third
coordinate-disjoint family.  TPC-396 freezes the two TPC-395 endpoint means
and tests four exact linear combinations of the corresponding finite matrices
on a fourth disjoint family.  The panel is finite and response-blind; the
...
RESULT = PROJECT / "results/tpc396_certificate.json"
PARENT_CODE = ROOT / (
    "papers/tpc-395-c1-origin-cross-family-holdout/code/"
    "tpc395_c1_origin_cross_family_holdout.py")
PARENT_CERT = ROOT / (
    "papers/tpc-395-c1-origin-cross-family-holdout/results/"
    "tpc395_certificate.json")
PARENT_CODE_SHA256 = (
...
LAMBDA_FRACTIONS = {
    "blend_0": Fraction(0, 1),
    "blend_1_3": Fraction(1, 3),
    "blend_2_3": Fraction(2, 3),
...
def load_parent_endpoints() -> dict[tuple[str, str, int], float]:
    ...
    need(payload.get("schema") ==
         "TPC395_C1_ORIGIN_CROSS_FAMILY_HOLDOUT_V1" and
         payload.get("status") ==
         "NUMERICALLY_CERTIFIED_FINITE_C1_ORIGIN_CROSS_FAMILY_HOLDOUT_AUDIT",
         "parent header")
    ...
    for cell in cells:
        ...
        need(key not in baseline, "duplicate parent cell")
...
def origin_summary(rows: list[dict[str, Any]],
        ...
        for law in LAWS:
            for q0 in Q_ANCHORS:
                ...
                cells.append({
                    ...
                    "count": WINDOW_COUNT,
                    "parent_family": "TPC395",
                    "parent_interpolation_definition":
                        "(1-lambda)*TPC395 all_plus + lambda*TPC395 alternating_index",
                    "interpolation_lambda":
                        f"{coefficient.numerator}/{coefficient.denominator}",
    ...
    return {
        ...
        "holdout_origin_count": 3, "parent_family": "TPC395",
        "parent_interpolation":
            "linear interpolation of the two frozen TPC395 endpoint means",
    ...
    return {
        ...
        "protocol": {
            ...
            "cross_family_definition":
                "new-family cohort mean / linearly interpolated TPC395 endpoint mean - 1",
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            ...
            print("TPC396_CERTIFICATE=WRITTEN")
        else:
            ...
            print(f"TPC396_CERTIFICATE=PASS rows={ROW_COUNT} cells={CELL_COUNT} "
        ...
        print("TPC396_CERTIFICATE=FAIL " + str(error))
        return 1
