#!/usr/bin/env python3
"""Independent reverse-shell replay for TPC-396.

The implementation is intentionally separate from the producer.  It validates
the TPC-395 endpoint baseline, rebuilds the new family in descending prime
...
def parent_endpoints() -> dict[tuple[str, str, int], float]:
    ...
    need(payload.get("schema") == "TPC395_C1_ORIGIN_CROSS_FAMILY_HOLDOUT_V1" and
         payload.get("status") ==
    ...
    for norm in NORMALIZATIONS:
        for law in LAWS:
            ...
            need(cell.get("origins") == [r["origin"] for r in selected] and
                 ...
                 cell.get("parent_family") == "TPC395" and
...
def check_document(document: dict[str, Any], recompute: bool = True) -> None:
    ...
    need(lock.get("parent_code_sha256") == PARENT_CODE_SHA256 and
         ...
         lock.get("parent_schema") == "TPC395_C1_ORIGIN_CROSS_FAMILY_HOLDOUT_V1" and
...
def main() -> int:
    ...
    try:
        ...
        print(f"TPC396_INDEPENDENT_CHECK=PASS rows={ROW_COUNT} cells={CELL_COUNT} "
        ...
        print("TPC396_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
