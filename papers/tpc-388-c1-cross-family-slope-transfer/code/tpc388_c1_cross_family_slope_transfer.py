#!/usr/bin/env python3
"""TPC-388: cross-family transfer of a finite count-ladder slope.

TPC-387 fitted a logarithmic 512 -> 768 slope on one fresh coordinate
family.  This release freezes those slopes and applies them, without any
refitting, to a second coordinate-disjoint family.  A same-family fit is
kept as a finite control.  Everything here is a c=1 proxy calculation; no
...
RESULT = PROJECT / "results/tpc388_certificate.json"
PARENT_CODE = ROOT / (
    "papers/tpc-387-c1-count-ladder-renormalization/code/"
    "tpc387_c1_count_ladder_renormalization.py")
PARENT_CERT = ROOT / (
    "papers/tpc-387-c1-count-ladder-renormalization/results/"
    "tpc387_certificate.json")
PARENT_CODE_SHA256 = (
    "35a5f739e709ba14fc218fa7e52d9f587e8d0b7cbea76a22eb3cbedea775b2d7")
PARENT_CERT_SHA256 = (
...
CLAIM_FIREWALL = {
    ...
    "TPC388_CROSS_FAMILY_PANEL":
        "NUMERICALLY_CERTIFIED_FINITE_256_ROWS",
    "TPC388_PARENT_SLOPE_TRANSFER":
...
def parent_cells() -> dict[tuple[str, str, str, int], dict[str, Any]]:
    ...
    need(payload.get("schema") ==
         "TPC387_C1_COUNT_LADDER_RENORMALIZATION_V1", "parent schema")
    cells = payload.get("ladder_summary", {}).get("cells", [])
    result = {}
    for cell in cells:
...
def transfer_summary(rows: list[dict[str, Any]],
            ...
            for law in LAWS:
                for q0 in Q_ANCHORS:
                    ...
                    means = {count: float(stats[count]["mean"])
                             for count in (512, 768, 1024)}
                    local_alpha = math.log(means[768] / means[512]) / math.log(1.5)
                    parent_alpha = float(parents[(mode, norm, law, q0)][
                        "calibration_log2_slope"])
                    local_prediction = means[768] * (4.0 / 3.0) ** local_alpha
                    parent_prediction = means[768] * (4.0 / 3.0) ** parent_alpha
                    local_ratio = means[1024] / local_prediction
                    parent_ratio = means[1024] / parent_prediction
                    cells.append({
                        ...
                        "calibration_counts": [512, 768],
                        "holdout_count": 1024,
                        "N512": stats[512], "N768": stats[768],
                        "N1024_holdout": stats[1024],
                        "parent_family_log2_slope": show(parent_alpha),
                        "local_family_log2_slope": show(local_alpha),
                        "parent_prediction_N1024_mean": show(parent_prediction),
                        "local_prediction_N1024_mean": show(local_prediction),
                        "parent_holdout_to_prediction_ratio": show(parent_ratio),
                        "local_holdout_to_prediction_ratio": show(local_ratio),
                        "parent_transfer_error": show(parent_ratio - 1.0),
                        "local_control_error": show(local_ratio - 1.0),
                        "within_parent_transfer_cap": bool(
    ...
    parent_errors = [abs(float(c["parent_transfer_error"])) for c in cells]
    local_errors = [abs(float(c["local_control_error"])) for c in cells]
    stable = {
        str(count): sum(bool(c[f"N{count}"]["within_one_percent"])
                        for c in cells)
        for count in (512, 768)
    }
    stable["1024_holdout"] = sum(
        bool(c["N1024_holdout"]["within_one_percent"]) for c in cells)
    failures = {
        f"{mode}_{norm}": {
    ...
    return {
        ...
        "parent_transfer_pass_count": sum(
            bool(c["within_parent_transfer_cap"]) for c in cells),
        "local_control_pass_count": sum(
            bool(c["within_local_control_cap"]) for c in cells),
        "parent_transfer_max_abs_error": show(max(parent_errors)),
...
def build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "parent_lock": {
            ...
            "parent_schema": "TPC387_C1_COUNT_LADDER_RENORMALIZATION_V1",
            "parent_round2_clue": "TEST_C1_COUNT_LADDER_SECOND_HOLDOUT",
        ...
        "protocol": {
            ...
            "parent_transfer_definition":
                "current-family N=768 mean times (4/3) to parent TPC387 slope",
            "local_control_definition":
                "current-family N=768 mean times (4/3) to current-family slope",
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            ...
            print("TPC388_CERTIFICATE=WRITTEN")
        else:
            ...
            print("TPC388_CERTIFICATE=PASS rows=256 cells=32 "
        ...
        print("TPC388_CERTIFICATE=FAIL " + str(error), file=sys.stderr)
        return 1
