...
def test_built_payload_passes_sources_foundation_and_finite_checks() -> None:
    ...
    assert finite["trace_formula_match"] is True
    assert finite["expected_primitive_counts_match"] is True
    assert finite["first_trace_ledger"] == [1, 1, 4]
    assert finite["first_primitive_ledger"] == [1, 0, 1]
    assert finite["all_scalar_rows_Q1_Q2_match"] is True
    assert finite["all_scalar_rows_Q3_exceed_one"] is True


def test_exact_route_and_claim_firewall() -> None:
    payload = build_payload()
    route = payload["route"]
