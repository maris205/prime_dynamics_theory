#!/usr/bin/env python3
"""Exact three-shell pooled CRT replay for the TPC local proxy."""
from __future__ import annotations
import argparse,hashlib,json,math,sys
from fractions import Fraction
from pathlib import Path
sys.set_int_max_str_digits(1_000_000)
PROJECT=Path(__file__).resolve().parents[1]; RESULT=PROJECT/'results/tpc414_certificate.json'; SCHEMA='TPC414_C1_THREE_SHELL_POOLED_EXTENSION_V1'; STATUS='PROVED_EXACT_FINITE_THREE_SHELL_POOLED_EXTENSION'; QS,H,N,B=(65536,131072,262144),66,264,1_000_000; COUNTS=(5709,10749,20390)
...
def main():
    ...
    if x.write: write(); print('TPC414_CERTIFICATE=WRITTEN')
    elif x.check:
        d=json.loads(RESULT.read_bytes(),object_pairs_hook=nodup,parse_constant=noconst); need(d['claim_status']==STATUS and d['payload_sha256']==hashlib.sha256(canonical(d['payload'])).hexdigest(),'certificate'); need(canonical(d['payload'])==canonical(payload()),'exact three-shell certificate'); print('TPC414_CERTIFICATE=PASS cases=1 shells=3 shell_count=36848 literal_domain=PASS')
