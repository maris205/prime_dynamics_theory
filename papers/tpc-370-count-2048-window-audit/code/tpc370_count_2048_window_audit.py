#!/usr/bin/env python3
"""TPC-370: audit the count-2048 window."""
...
RESULT = PROJECT / "results/tpc370_certificate.json"
BASE_CODE = ROOT / (
...
def load_base():
    spec = importlib.util.spec_from_file_location("tpc355_base_tpc370",
                                                  BASE_CODE)
...
def load_parent() -> dict[str, Any]:
    ...
    need(isinstance(payload, dict) and
         payload.get("schema") == "TPC369_THIRD_ORIGIN_FAMILY_AUDIT_V1",
         "parent payload")
    return payload
...
def build_payload() -> dict[str, Any]:
    ...
    parent_failure_keys = [
        tuple(key) for key in parent["finite_audit"]["replicated_failure_keys"]
    ]
    parent_signature = sorted((key[0], key[2], key[3], key[4])
                              for key in parent_failure_keys)
    replicated_signature = sorted((key[0], key[2], key[3], key[4])
                                  for key in beta2_failures)
    pattern_matches_parent = replicated_signature == parent_signature
    return {
        ...
        "protocol": {
            ...
            "parent_panel": "TPC-369 third predeclared origin-family audit",
        },
        "exact_theorem": {
            "origin_protocol": "The three origins are inherited deterministic grid points declared before replay and use no response or geometry score.",
            "exact_anchor_rule": "The repaired unsigned exact anchor [1010346,1010359) is inherited unchanged from TPC-369 and is fixed before count-2048 response replay.",
            "anchor_inheritance": {
            ...
            "replication_scope": "The count-2048 audit is restricted to count 2048, Q in {512,2048,8192}, exponent one, and the three inherited origins.",
        },
        "finite_audit": {
            "rows": len(rows), "settings_per_beta": 36,
            ...
            "parent_failure_keys": [list(key) for key in parent_failure_keys],
            "parent_failure_signature": [list(key) for key in parent_signature],
            "replicated_failure_signature": [list(key) for key in replicated_signature],
            "failure_signature_matches_parent": pattern_matches_parent,
