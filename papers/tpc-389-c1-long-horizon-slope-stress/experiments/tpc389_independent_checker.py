#!/usr/bin/env python3
"""Independent reverse-shell replay for TPC-389.

This file intentionally does not import the TPC-389 producer.  It rebuilds
the finite c=1 matrices in descending prime-shell order and checks the
long-horizon forecast against the sealed certificate.
"""
...
PARENT_CERT = ROOT / (
    "papers/tpc-388-c1-cross-family-slope-transfer/results/"
    "tpc388_certificate.json")
PARENT_CERT_SHA256 = (
    "6808eb81b5f18c1add88685f82f1df61681abc4e9e61e432067eb88d7d5b67b1")
...
def make_row(origin: int, count: int, q0: int, law: str, norm: str,
    ...
    return {
        "origin": origin, "origin_role":
            f"calibration_{count}" if origin in CALIBRATION_ORIGINS else "holdout_1280",
...
def replay_rows() -> list[dict[str, Any]]:
    ...
    for q0 in Q_ANCHORS:
        ...
        gamma = math.log(train[1024] / train[768]) / math.log(1024.0 / 768.0)
        extrapolated = train[1024] * (1280.0 / 1024.0) ** gamma
            ...
            for mode in BAND_MODES:
                ...
                for norm in NORMALIZATIONS:
                    if norm == "local_diagonal":
                        ...
                        denominator, role = extrapolated, "calibration_extrapolated_1280"
                    for law in LAWS:
...
def parent_cells() -> dict[tuple[str, str, str, int], dict[str, Any]]:
    ...
    need(parent_payload.get("schema") ==
         "TPC388_C1_CROSS_FAMILY_SLOPE_TRANSFER_V1", "parent schema")
...
def check_summary_from_rows(rows: list[dict[str, Any]],
    ...
    local_errors = []
    recursive_errors = []
    stable = {"768": 0, "1024": 0, "1280_holdout": 0}
                ...
                for q0 in Q_ANCHORS:
                    ...
                    by_count = {
                        count: [float(row["band_spectral"]) for row in selected
                                if row["count"] == count]
                        for count in (768, 1024, 1280)
                    }
                    stats = {count: finite_stats(by_count[count])
                             for count in (768, 1024, 1280)}
                    means = {count: stats[count]["mean"]
                             for count in (768, 1024, 1280)}
                    local_alpha = math.log(means[1024] / means[768]) / math.log(1024.0 / 768.0)
                    parent_alpha = float(parents[key]["parent_family_log2_slope"])
                    local_prediction = means[1024] * (1280.0 / 1024.0) ** local_alpha
                    parent_prediction = means[1024] * (1280.0 / 1024.0) ** parent_alpha
                    recursive_prediction = means[768] * (1280.0 / 768.0) ** parent_alpha
                    errors = {
                        "parent_horizon_error": means[1280] / parent_prediction - 1.0,
                        "local_control_error": means[1280] / local_prediction - 1.0,
                        "recursive_parent_error": means[1280] / recursive_prediction - 1.0,
                    }
                    cell = recorded_map[key]
                    need(cell.get("calibration_counts") == [768, 1024] and
                         cell.get("holdout_count") == 1280,
                         "cell count roles")
                    for count, field in ((768, "N768"), (1024, "N1024"),
                                         (1280, "N1280_holdout")):
                        ...
                        if count == 768 and stats[count]["within_one_percent"]:
                            stable["768"] += 1
                        if count == 1024 and stats[count]["within_one_percent"]:
                            stable["1024"] += 1
                        if count == 1280 and stats[count]["within_one_percent"]:
                            stable["1280_holdout"] += 1
                    for name, actual in (
                            ("parent_horizon_log2_slope", parent_alpha),
                            ("local_horizon_log2_slope", local_alpha),
                            ("parent_prediction_N1280_from_N1024", parent_prediction),
                            ("local_prediction_N1280_from_N1024", local_prediction),
                            ("parent_prediction_N1280_from_N768", recursive_prediction),
...
def check_document(document: dict[str, Any], recompute: bool = True) -> dict[str, Any]:
    ...
    expected = {(o, n, q, law, norm, mode)
                for o in CALIBRATION_ORIGINS for n in (768, 1024)
                for q in Q_ANCHORS for law in LAWS for norm in NORMALIZATIONS
                for mode in BAND_MODES}
    expected |= {(o, 1280, q, law, norm, mode)
    ...
    for key, row in row_map.items():
        ...
        role = f"calibration_{count}" if origin in CALIBRATION_ORIGINS else "holdout_1280"
    ...
    need(sum(int(v.get("spectral", -1)) for v in failures.values()) == 64 and
         ...
         "failure census")
    need(summary.get("stable_cells", {}).get("1280_holdout") == 24,
         "holdout stability")
    ...
    need(firewall.get("TPC389_ARITHMETIC_ADVANCE") == "NO" and
         ...
         firewall.get("TPC389_TWIN_PRIME_RESULT") == "NONE", "firewall")
    anchor = payload.get("exact_anchor", {})
    need(anchor.get("interval") == [2800001, 2800014] and
...
def main() -> int:
    ...
    try:
        ...
        print("TPC389_INDEPENDENT_CHECK=PASS rows=256 cells=32 "
              ...
              f"stable_holdout={summary['stable_cells']['1280_holdout']}/32")
        return 0
    except (Failure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError) as error:
        print("TPC389_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
