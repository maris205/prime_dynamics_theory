#!/usr/bin/env python3
"""Mutation firewall for the TPC-389 certificate contract."""
...
import tpc389_independent_checker as independent  # noqa: E402
...
def main() -> int:
    ...
    try:
        ...
        mutate(lambda d: d["payload"]["transfer_summary"]["stable_cells"]
               .__setitem__("1280_holdout", 27))
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC389_ARITHMETIC_ADVANCE", "YES"))
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC389_FIXED_POWER_CREDIT", 1))
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC389_FULL_GATE_B", "CLOSED"))
        ...
        print("TPC389_STRESS=PASS mutations=25")
        return 0
    except Exception as error:
        print("TPC389_STRESS=FAIL " + str(error), file=sys.stderr)
        return 1
