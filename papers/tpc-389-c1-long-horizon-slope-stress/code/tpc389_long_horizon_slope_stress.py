#!/usr/bin/env python3
"""TPC-389: a longer-horizon stress test for a frozen count slope.

TPC-388 transferred a frozen count slope over the 512 -> 768 -> 1024
ladder.  This release moves to a third coordinate-disjoint family, calibrates
only at 768 and 1024, and tests the next endpoint 1280.  Both a frozen parent
slope and a same-family two-level control are reported.  Everything here is
a c=1 proxy calculation; no arithmetic or asymptotic claim is made.
"""
...
RESULT = PROJECT / "results/tpc389_certificate.json"
PARENT_CODE = ROOT / (
    "papers/tpc-388-c1-cross-family-slope-transfer/code/"
    "tpc388_c1_cross_family_slope_transfer.py")
PARENT_CERT = ROOT / (
    "papers/tpc-388-c1-cross-family-slope-transfer/results/"
    "tpc388_certificate.json")
PARENT_CODE_SHA256 = (
    "ee659c08d3af5d0ddd300ca89fa5cf7f4c7c9f7630d00577e943716966b6e411")
PARENT_CERT_SHA256 = (
...
def build_rows() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for q0 in Q_ANCHORS:
        ...
        gamma = math.log(train_scalars[1024] / train_scalars[768]) / math.log(1024.0 / 768.0)
        extrapolated = train_scalars[1024] * (1280.0 / 1024.0) ** gamma
        scalar_info[str(q0)] = {
            "by_calibration_count": {str(k): show(v)
                                     for k, v in train_scalars.items()},
            "geometry_log2_slope": show(gamma),
            "extrapolated_1280": show(extrapolated),
        }
                ...
                for norm in NORMALIZATIONS:
                    if norm == "pooled_train_scalar":
                        if count in train_scalars:
                            ...
                            denominator, denominator_role = extrapolated, "calibration_extrapolated_1280"
                    else:
...
def transfer_summary(rows: list[dict[str, Any]],
            ...
            for law in LAWS:
                for q0 in Q_ANCHORS:
                    ...
                    parent_alpha = float(parents[(mode, norm, law, q0)][
                        "parent_family_log2_slope"])
                    local_prediction = means[1024] * (1280.0 / 1024.0) ** local_alpha
                    parent_prediction = means[1024] * (1280.0 / 1024.0) ** parent_alpha
                    recursive_parent_prediction = means[768] * (1280.0 / 768.0) ** parent_alpha
                    local_ratio = means[1280] / local_prediction
                    parent_ratio = means[1280] / parent_prediction
                    recursive_parent_ratio = means[1280] / recursive_parent_prediction
                    cells.append({
                        ...
                        "calibration_counts": [768, 1024],
                        "holdout_count": 1280,
                        "N768": stats[768], "N1024": stats[1024],
                        "N1280_holdout": stats[1280],
                        "parent_horizon_log2_slope": show(parent_alpha),
                        "local_horizon_log2_slope": show(local_alpha),
                        "parent_prediction_N1280_from_N1024": show(parent_prediction),
                        "local_prediction_N1280_from_N1024": show(local_prediction),
                        "parent_prediction_N1280_from_N768": show(recursive_parent_prediction),
    ...
    stable = {
        str(count): sum(bool(c[f"N{count}"]["within_one_percent"])
                        for c in cells)
        for count in (768, 1024)
    }
    stable["1280_holdout"] = sum(
        bool(c["N1280_holdout"]["within_one_percent"]) for c in cells)
    failures = {
        f"{mode}_{norm}": {
...
def build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "parent_lock": {
            ...
            "parent_schema": "TPC388_C1_CROSS_FAMILY_SLOPE_TRANSFER_V1",
            "parent_round2_clue": "TEST_C1_CROSS_FAMILY_SLOPE_STRESS",
        ...
        "protocol": {
            ...
            "parent_transfer_definition":
                "current-family N=1024 mean times (5/4) to parent TPC388 slope",
            "local_control_definition":
                "current-family N=1024 mean times (5/4) to current-family 768 -> 1024 slope",
            "recursive_parent_definition":
                "current-family N=768 mean times (1280/768) to parent TPC388 slope",
        ...
        "finite_audit": {
            ...
            "calibration_counts": [768, 1024], "holdout_count": 1280,
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            ...
            print("TPC389_CERTIFICATE=WRITTEN")
        else:
            ...
            print("TPC389_CERTIFICATE=PASS rows=256 cells=32 "
        ...
        print("TPC389_CERTIFICATE=FAIL " + str(error), file=sys.stderr)
        return 1
