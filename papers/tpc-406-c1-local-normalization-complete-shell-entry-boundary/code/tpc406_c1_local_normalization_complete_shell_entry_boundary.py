#!/usr/bin/env python3
"""Exact complete-shell audit for the TPC-405 normalized proxy entry."""
...
RESULT = PROJECT / "results/tpc406_certificate.json"
...
def shell_primes() -> list[int]:
    shell = [p for p in primes(2 * Q) if p > Q]
    need(len(shell) == 872 and shell[0] == 8209 and shell[-1] == 16381, "complete shell census")
    return shell


def row(H: int, shell: list[int]) -> dict:
    N = 4 * H
    ...
    need(z2 <= sharp2 <= coarse2, "complete-shell uniform bound")
    return {
        "H": H, "N": N, "m": len(shell) // 2, "Q": Q, "origin_lower_bound": B,
        "shell_count": len(shell), "selected_primes": shell, "residues": residues,
        "crt_residue": residue, "crt_period": period, "origin": origin,
        "S0": text_fraction(S0), "S1": text_fraction(S1), "a_min": text_fraction(amin),
        "P_minus": text_fraction(Pminus), "V_minus": text_fraction(Vminus),
        "V_plus": text_fraction(Vplus), "G0": text_fraction(G0), "G1": text_fraction(G1),
        "direct": text_fraction(direct), "normalized_square": text_fraction(z2),
        "sharp_bound_square": text_fraction(sharp2), "coarse_bound_square_4_over_H": text_fraction(coarse2),
...
def payload() -> dict:
    ...
    return {
        ...
        "shell_rule": "all primes Q<p<=2Q", "shell_count": len(shell),
        "shell_sha256": hashlib.sha256(canonical(shell)).hexdigest(),
        "heights": list(HEIGHTS), "multiplicity": len(shell) // 2,
        "window_rule": "N=4H", "normalization": "complete-shell local diagonal",
        "theorem_domain": {
            "H_and_N": "integers H,N with H>=1 and N>=H+2",
            "Q_and_shell": "integer Q>N and an even complete prime shell Q<p<=2Q with 2m primes",
            ...
            "proxy": "selected-prime masked local geometry from TPC-404, with the complete shell selected",
        },
        "theorem": {
            ...
            "scope": "one adjacent normalized proxy entry with every shell prime selected, not the full operator",
        },
        "cases": [row(H, shell) for H in HEIGHTS],
        "claim_firewall": {
            "COMPLETE_SHELL_LOCAL_ENTRY_BOUND": "PROVED_EXACT_FINITE",
...
def check_document(document: dict) -> None:
    ...
    need(canonical(document["payload"]) == canonical(payload()), "exact complete shell")
...
def main() -> None:
    ...
    if args.write:
        write(); print("TPC406_CERTIFICATE=WRITTEN")
    elif args.check:
        check_document(json.loads(RESULT.read_bytes(), object_pairs_hook=no_duplicates, parse_constant=no_constants))
        print("TPC406_CERTIFICATE=PASS cases=5 shell_primes=872 exact_bound=PASS")
    else:
