...
def test_exact_106_plus_3_source_closure() -> None:
    closure = locks.build_source_closure()
    assert closure["pass"] is True
    assert (closure["git_count"], closure["remote_count"], closure["logical_count"]) == (106, 3, 109)
    ...
    assert closure["git"]["path_unique_count"] == 106
...
def test_group_digest_and_remote_object_mutations_fail(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    wrong_groups["rh389_standard8"] = "0" * 64
