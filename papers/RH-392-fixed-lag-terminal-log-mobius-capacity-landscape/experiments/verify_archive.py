"""Verify RH-392 archive membership, hashes, sources, and replay gates."""
...
from fixed_lag_capacity.core import exact_equal, loads_strict  # noqa: E402
...
def verify_manifest(manifest: object) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-392_fixed_publication_manifest":
    ...
    if type(external_entries) is not dict or external_count != 106:
    ...
    exact_counts = {
        ...
        "external_git_input_count": 106,
        "remote_logical_input_count": 3,
        "logical_input_total": 109,
    }
    ...
    return {
        "status": "RH-392_archive_verified" if not failures else "RH-392_archive_failed",
        ...
        "remote_logical_input_count": 3 if fresh is not None else 0,
        "logical_input_total": 109 if fresh is not None else 0,
        **current_counts,
