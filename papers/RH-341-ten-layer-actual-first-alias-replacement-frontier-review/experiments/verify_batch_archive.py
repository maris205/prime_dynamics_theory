"""Verify the RH-332--RH-341 batch publication manifest."""
...
def main() -> None:
    manifest = json.loads(
        (ROOT / "results/batch_dependency_manifest.json").read_text()
    )
    paths = []
    ...
    failures = []
    if manifest.get("status") != "rh332_341_batch_publication_manifest":
        failures.append(
            {"file": "batch_dependency_manifest.json", "reason": "status_mismatch"}
        )
    if manifest.get("paper_numbers") != list(NUMBERS):
        failures.append(
            {"file": "batch_dependency_manifest.json", "reason": "paper_numbers_mismatch"}
        )
    if manifest.get("file_count") != len(expected):
        failures.append(
            {"file": "batch_dependency_manifest.json", "reason": "file_count_mismatch"}
        )
    ...
    payload = {
        "status": "rh332_341_batch_archive_verified",
