#!/usr/bin/env python3
"""Adversarial schema/firewall mutations for the TPC-379 certificate."""
...
def validate_shape(document: dict[str, Any]) -> None:
    ...
    need(parent.get("parent_schema") ==
         "TPC378_C1_SCALE_ORIGIN_CROSSHOLDOUT_V1" and
         parent.get("parent_round2_clue") ==
         "TEST_C1_CROSSHOLDOUT_LAW_CONTROL" and
    ...
    need(firewall.get("TPC379_ARITHMETIC_ADVANCE") == "NO" and
         firewall.get("TPC379_FIXED_POWER_CREDIT") == 0 and
         firewall.get("TPC379_FULL_GATE_B") == "OPEN" and
         firewall.get("TPC379_TWIN_PRIME_RESULT") == "NONE", "firewall")
...
def mutations(document: dict[str, Any]) -> list[dict[str, Any]]:
    paths = [
        ...
        ("grid_start", lambda d: d["payload"]["selection_protocol"].__setitem__("grid_start", 1200002)),
        ("grid_step", lambda d: d["payload"]["selection_protocol"].__setitem__("grid_step", 400)),
        ("candidate_origins", lambda d: d["payload"]["selection_protocol"].__setitem__("candidate_origins", [1200001] * 41)),
        ...
        ("window_count", lambda d: d["payload"]["selection_protocol"].__setitem__("window_count", 2048)),
        ...
        ("row_origin", lambda d: d["payload"]["rows"][0].__setitem__("origin", 1200002)),
        ...
        ("arithmetic", lambda d: d["payload"]["claim_firewall"].__setitem__("TPC379_ARITHMETIC_ADVANCE", "YES")),
...
def main() -> int:
    ...
    try:
        ...
        print("TPC379_STRESS=PASS mutations=25")
        return 0
    except (Failure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError) as error:
        print("TPC379_STRESS=FAIL " + str(error), file=sys.stderr)
        return 1
