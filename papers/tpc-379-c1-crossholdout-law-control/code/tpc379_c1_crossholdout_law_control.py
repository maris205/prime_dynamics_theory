#!/usr/bin/env python3
"""TPC-379: a finite c=1 cross-holdout law-control audit."""
...
RESULT = PROJECT / "results/tpc379_certificate.json"
PARENT_CODE = ROOT / (
...
def load_parent_module():
    spec = importlib.util.spec_from_file_location(
        "tpc378_parent_for_tpc379", PARENT_CODE)
...
BASE = PARENT.PARENT.PARENT.ENGINE.BASE
...
def record(origin: int, q0: int, law: str) -> dict[str, Any]:
    values = np.arange(origin, origin + WINDOW_COUNT, dtype=np.int64)
    primes, matrices, geometry, weights = weighted_components(values, q0)
...
def build_rows() -> list[dict[str, Any]]:
    jobs = [(origin, q0, law)
            for origin in ORIGINS for q0 in Q_ANCHORS for law in LAWS]
...
def build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "protocol": {
            ...
            "block_length": BLOCK_LENGTH, "block_count": BLOCK_COUNT,
            "partition": "four contiguous 256-point blocks",
            "band_cutoff": BAND_CUTOFF,
...
def main() -> int:
    ...
    try:
        if args.write:
            ...
            print("TPC379_CERTIFICATE=WRITTEN")
        else:
            ...
            compact = ";".join(
                ...
                for law in LAWS)
            print("TPC379_CERTIFICATE=PASS rows=" + str(audit["rows"]) +
                  " failures=" + str(audit["spectral_cap_violations"]) +
                  " profiles=" + compact)
        return 0
    except (CheckFailure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError) as error:
        print("TPC379_CERTIFICATE=FAIL " + str(error), file=os.sys.stderr)
        return 1
