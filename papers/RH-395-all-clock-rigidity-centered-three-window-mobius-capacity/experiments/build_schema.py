"""Generate the recursively closed Draft 2020-12 RH-395 result schema."""
...
SCHEMA_ID = "https://example.invalid/schemas/RH-395-result.schema.json"
SCHEMA_TITLE = "RH-395 exact Stage-1 result"
RESULT_BUILDER_BYTES = 36_225
RESULT_BUILDER_SHA256 = "9679d20d219c8e4f5c828b193a13061ae2d9c02b6a2f706001ded130d088f5d7"
RESULT_TEST_BYTES = 8_216
RESULT_TEST_SHA256 = "06ac58d5f8b5d505ea0af84b8471de4ceca4cea7099745d9a7dac8d9db8ef54f"
RESULT_PRETTY_BYTES = 148_331
RESULT_PRETTY_SHA256 = "7557bcc78811b29d7ac9f155fd8553c75d70b659748a37cf2fef427af4958f27"
RESULT_CANONICAL_BYTES = 101_772
RESULT_CANONICAL_SHA256 = "9377280e97c1c92f92f492abb10edb72ae2b4b08b90b2ded1c30cf57e2904c9b"
SCHEMA_FIXTURE_BYTES = 265_717
SCHEMA_FIXTURE_SHA256 = "1958e593b29b5095efc15eb3a447db12236504d81f447bc6626fde75978d2849"


SCHEMA_MUTATION_NAMES = (
    "draft", "id", "title", "root_extra", "root_required_missing",
    "root_additional_true", "property_removed", "property_extra",
    "scalar_type", "scalar_const", "integer_float_const", "boolean_integer_const",
    "array_items_open", "array_min", "array_max", "array_prefix_removed",
    "array_prefix_reordered", "nested_object_open", "nested_required_missing",
    "nested_keyword_extra", "payload_title", "payload_q2", "payload_source_count",
    "payload_float",
)
...
def _validate_result_seals() -> None:
    expected = {
        ...
        "id": "https://example.invalid/schemas/RH-395-result.schema.json",
        "title": "RH-395 exact Stage-1 result",
        "builder_bytes": 36225,
        "builder_sha": "9679d20d219c8e4f5c828b193a13061ae2d9c02b6a2f706001ded130d088f5d7",
        "test_bytes": 8216,
        "test_sha": "06ac58d5f8b5d505ea0af84b8471de4ceca4cea7099745d9a7dac8d9db8ef54f",
        "pretty_bytes": 148331,
        "pretty_sha": "7557bcc78811b29d7ac9f155fd8553c75d70b659748a37cf2fef427af4958f27",
        "canonical_bytes": 101772,
        "canonical_sha": "9377280e97c1c92f92f492abb10edb72ae2b4b08b90b2ded1c30cf57e2904c9b",
    }
    actual = {
        "draft": SCHEMA_DRAFT, "id": SCHEMA_ID, "title": SCHEMA_TITLE,
        "builder_bytes": RESULT_BUILDER_BYTES, "builder_sha": RESULT_BUILDER_SHA256,
        "test_bytes": RESULT_TEST_BYTES, "test_sha": RESULT_TEST_SHA256,
        "pretty_bytes": RESULT_PRETTY_BYTES, "pretty_sha": RESULT_PRETTY_SHA256,
    ...
    def visit(value: object, node: object, current: str) -> None:
        ...
        if kind == "object":
            ...
            if current == "$" and (
                ...
                or node["$id"] != "https://example.invalid/schemas/RH-395-result.schema.json"
                or node["title"] != "RH-395 exact Stage-1 result"
            ):
...
def _frozen_payload(*, compare_fresh: bool = True) -> dict[str, object]:
    ...
    test_path = ROOT / "tests" / "test_result.py"
    result_path = ROOT / "results" / "result.json"
    for path, size, digest in (
        (builder_path, RESULT_BUILDER_BYTES, RESULT_BUILDER_SHA256),
        (test_path, RESULT_TEST_BYTES, RESULT_TEST_SHA256),
        (result_path, RESULT_PRETTY_BYTES, RESULT_PRETTY_SHA256),
    ):
        raw = path.read_bytes()
...
def _make_schema_validator():
    ...
    schema_bytes_literal = 265717
    schema_sha_literal = "1958e593b29b5095efc15eb3a447db12236504d81f447bc6626fde75978d2849"
    result_bytes_literal = 101772
    result_sha_literal = "9377280e97c1c92f92f492abb10edb72ae2b4b08b90b2ded1c30cf57e2904c9b"
    ...
    def semantic(schema: object, payload: object) -> bool:
        ...
        expected["$id"] = "https://example.invalid/schemas/RH-395-result.schema.json"
        expected["title"] = "RH-395 exact Stage-1 result"
...
def mutate_schema(schema: dict[str, object], payload: dict[str, object], name: str) -> tuple[dict[str, object], dict[str, object]]:
    if type(name) is not str or name not in SCHEMA_MUTATION_NAMES:
    ...
    actions = {
        ...
        "scalar_const": lambda: properties["paper"].__setitem__("const", "RH-394"),
        ...
        "array_min": lambda: properties["core_mutation_audit"].__setitem__("minItems", 56),
        "array_max": lambda: properties["core_mutation_audit"].__setitem__("maxItems", 58),
        ...
        "payload_q2": lambda: changed_payload["summary"].__setitem__("C2", "K2-K3"),
        "payload_source_count": lambda: changed_payload["source_closure"].__setitem__("git_count", 147),
...
def main() -> None:
    ...
    validate_exact_instance(payload, schema)
    if SCHEMA_FIXTURE_BYTES and validate_schema_artifact(schema, payload, compare_fresh=False) is not True:
