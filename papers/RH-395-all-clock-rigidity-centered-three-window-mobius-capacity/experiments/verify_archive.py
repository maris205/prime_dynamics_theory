"""Verify RH-395 archive membership, hashes, sources, and replay gates."""
...
def verify_manifest(manifest: object) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-395_fixed_publication_manifest":
    ...
    if type(external_entries) is not dict or external_count != 148:
    ...
    exact_counts = {
        ...
        "external_git_input_count": 148,
        "remote_logical_input_count": 4,
        "logical_input_total": 152,
    }
    ...
    return {
        "status": "RH-395_archive_verified" if not failures else "RH-395_archive_failed",
        ...
        "logical_input_total": 152 if fresh is not None else 0,
        **current_counts,
