...
Table [1](#tab:summary) gives the law-wise spectral summary. The missing long-count spectra for the three non-all-plus laws are intentional protocol choices, not imputed values.
...
The finite envelope inequalities in [\[eq:envelopes\]](#eq:envelopes) are proved exactly for finite matrices. The finite selection and its response independence are also exact consequences of the declared algorithm. The numerical caps, tightness ratios, law census, and transition census are certified only for this finite protocol.
