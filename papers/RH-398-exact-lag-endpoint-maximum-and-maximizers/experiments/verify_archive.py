"""Verify RH-398 archive membership, hashes, sources, and replay gates."""
...
def _failure_payload(failure: str) -> dict[str, object]:
    ...
    normalized = loads_strict(json.dumps(payload, sort_keys=True, allow_nan=False))
    if type(normalized) is not dict:
        raise RuntimeError("normalized failure report is not an object")
    return normalized
...
def verify_manifest(manifest: object) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-398_fixed_publication_manifest":
    ...
    if type(external_entries) is not dict or external_count != 184:
    ...
    exact_counts = {
        ...
        "external_git_input_count": 184,
        "remote_logical_input_count": 4,
        "logical_input_total": 188,
    }
    ...
    if not exact_equal(manifest.get("source_commits"), dict(sorted(SOURCE_COMMITS.items()))):
    ...
    if not exact_equal(
        manifest.get("git_group_sizes"),
        dict(sorted(SOURCE_GROUP_SIZES.items())),
    ):
        failures.append("manifest:git_group_sizes")
    if not exact_equal(
        manifest.get("git_group_digests"),
        dict(sorted(SOURCE_GROUP_DIGESTS.items())),
    ):
    ...
    serialized_fresh = (
        loads_strict(serialized_payload(fresh)) if fresh is not None else None
    )
    rebuild_match = serialized_fresh is not None and exact_equal(manifest, serialized_fresh)
    ...
    payload = {
        "status": "RH-398_archive_verified" if not failures else "RH-398_archive_failed",
        ...
        "logical_input_total": 188 if fresh is not None else 0,
        **current_counts,
    ...
    normalized = loads_strict(json.dumps(payload, sort_keys=True, allow_nan=False))
    if type(normalized) is not dict:
        raise RuntimeError("normalized archive report is not an object")
    return normalized
