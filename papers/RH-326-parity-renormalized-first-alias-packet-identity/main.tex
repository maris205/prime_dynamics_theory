...
defect is $C_*C_M\lambda^\eta$.  Scalar balance is possible only at
$\eta_*=3.0609149137\ldots$; throughout the usual floor/ceil/nearest window
$|\eta|\le1$, parity supplies at most $0.3438880199\ldots$ of the alias.
Thus scalar parity subtraction alone cannot close the first-alias equation.
This is a scoped negative result: the raw physical boundary packet and the
neighboring shell may still provide the missing signed contribution.  We
finish with a phase- and coordinate-preserving interface for that calculation.
No joint full-trace replacement is proved.
\end{abstract}
...
For every $k,n\ge1$, the counterloop defect is exactly
\begin{equation}
 \boxed{
...
\begin{theorem}[Phase law and unique scalar balance]
\label{thm:phase-law}
Along any subsequence on which $\eta_\sigma\to\eta$,
\begin{equation}
 \boxed{
 \frac{\cP_{\sigma,2k_\sigma}}{\cA_{k_\sigma,2k_\sigma}}
 \longrightarrow C_*C_M\lambda^\eta.}
 \label{eq:ratio-law}
\end{equation}
The leading scalar packets balance only at
\begin{equation}
 \eta_*=-\frac{\log(C_*C_M)}{\log\lambda}
 =3.060914913721564\ldots .
 \label{eq:balance-phase}
\end{equation}
For every phase with $|\eta|\le1$,
\begin{equation}
 C_*C_M\lambda^\eta
 \le C_*C_M\lambda
 =0.343888019998872\ldots<1.
 \label{eq:canonical-gap}
\end{equation}
Therefore, on such a subsequence, if
$\cT_{\sigma,2k}=o(\cA_{k,2k})$, then
\begin{equation}
 |e_{\sigma,k,2k}|
 \ge\{0.656111980001127\ldots+o(1)\}\cA_{k,2k},
...
\eqref{eq:ratio-law}.  Monotonicity in $\eta$ gives the unique balance phase
and \eqref{eq:canonical-gap}.  Substitute the ratio into
...
This is deliberately a scalar-only negative result.  It proves that omitting
an alias-scale raw boundary/shell packet cannot work on common
floor/ceil/nearest clocks.  It does not bound the actual raw trace packet and
does not prove divergence of the combined physical residual.

The same phase controls the local clearance.  If the archived boundary-cycle
clearance is
\begin{equation}
 \Delta_k^{\rm clr}=C_{\rm b}\lambda^{-2k}\{1+o(1)\},
 \qquad C_{\rm b}=0.4608051492\ldots,
 \label{eq:clearance}
\end{equation}
then
\begin{equation}
...
\end{equation}
At scalar balance,
$d_*=C_{\rm b}(C_*C_M)^2=0.019340633095025\ldots$, whereas
$|\eta|\le1$ corresponds to
$0.163544744607256\ldots\le d\le1.298368749415710\ldots$.
Thus suppressing the clock phase would discard both the scalar balance ratio
and the RH-322 entrance profile.
...
law.  It evaluates the following constant ledger:
\begin{center}\small
\begin{tabular}{rrr}
\toprule
...
The exact identity combines only known algebraic packets.  It does not
