#!/usr/bin/env python3
"""Exact complete-shell Q-scale extension of the TPC-407 proxy entry."""
...
RESULT = PROJECT / "results/tpc408_certificate.json"
SCHEMA = "TPC408_C1_COMPLETE_SHELL_Q_SCALE_EXTENSION_V1"
STATUS = "PROVED_EXACT_FINITE_COMPLETE_SHELL_Q_SCALE_EXTENSION"
Q_SCALES = (65536, 131072)
H, N, B = 66, 264, 1_000_000
EXPECTED_COUNTS = (5709, 10749)
...
def shell_for(Q):
    ...
    need(len(shell) > 0, "nonempty shell")
    return shell


def row(Q, shell):
    # The extension permits odd complete shells: the alternating profile has
    # m_minus=floor(r/2) and m_plus=ceil(r/2), with no artificial deletion.
...
def payload():
    ...
    return {
        "schema": SCHEMA, "status": STATUS, "Q_scales": list(Q_SCALES),
        "H": H, "N": N, "origin_lower_bound": B,
        "shell_rule": "all primes Q<p<=2Q, with the full shell retained regardless of parity",
        "shell_counts": list(EXPECTED_COUNTS),
        "shell_sha256": {str(Q): hashlib.sha256(canonical(s)).hexdigest()
                         for Q, s in zip(Q_SCALES, shells)},
        "window_rule": "N=264=4H", "normalization": "complete-shell local diagonal",
        "theorem_domain": {
            "H_and_N": "fixed integers H=66,N=264 with N=4H",
            "Q_and_shell": "Q>N and a complete prime shell Q<p<=2Q with r>=2 primes",
            "profile": "all shell primes, indexed increasingly; even i residue 0, odd i residue -N",
            "parity": "r may be odd; m_plus=ceil(r/2), m_minus=floor(r/2), with no shell deletion",
        ...
        "theorem": {
            ...
            "scope": "one adjacent normalized proxy entry across a finite Q scale extension, not the full operator",
        },
        "cases": [row(Q, s) for Q, s in zip(Q_SCALES, shells)],
        "claim_firewall": {
            "COMPLETE_SHELL_Q_SCALE_EXTENSION": "PROVED_EXACT_FINITE",
            "SCALE_EXTENSION_DECIMALS": "NUMERICAL_OBSERVATION",
...
def check_document(document):
    ...
    need(canonical(document["payload"]) == canonical(payload()), "exact Q extension")
...
def main():
    ...
    if args.write:
        write(); print("TPC408_CERTIFICATE=WRITTEN")
    elif args.check:
        check_document(json.loads(RESULT.read_bytes(), object_pairs_hook=no_duplicates,
                                  parse_constant=no_constants))
        print("TPC408_CERTIFICATE=PASS cases=2 q_scales=2 odd_complete_shells=PASS")
    else:
