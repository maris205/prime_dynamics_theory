...
def main() -> None:
    ...
    publications = [ROOT / name for name in (
        ...
        "pyproject.toml", "requirements.txt", "figures/gauge_covariant_rayleigh_transfer.pdf",
        "figures/gauge_covariant_rayleigh_transfer.png", "main.pdf", "gauge-covariant-rayleigh-transfer.pdf",
    )]
    dependency = {
        "status": "all_rh120_sources_and_publication_artifacts_hashed",
        "external_inputs": {},
    ...
    audit = json.loads((ROOT / "results" / "gauge_transfer_audit.json").read_text(encoding="utf-8"))
    results = [ROOT / "results" / name for name in ("gauge_transfer_audit.json", "gauge_transfer_smoke.json", "dependency_manifest.json")]
    summary = {
        "status": "rh120_gauge_covariant_rayleigh_transfer_archived",
        "theorem": {"gauge_covariant_gamma_transfer": True, "four_volume_transfer": True, "simultaneous_sharpness": True},
