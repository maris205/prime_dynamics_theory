...
Table [1](main.tex#L109){reference-type="ref" reference="tab:profile"} reports the range over the three fresh origins and the number of spectral-cap failures. The two rows in each entry are not independent random samples; they are the fixed finite protocol above.
