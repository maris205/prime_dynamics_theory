#!/usr/bin/env python3
"""Exact four-height replay for the pooled two-shell TPC local proxy."""
...
PROJECT=Path(__file__).resolve().parents[1]; RESULT=PROJECT/'results/tpc412_certificate.json'
SCHEMA='TPC412_C1_POOLED_COMPLETE_SHELL_EXTENSION_V1'; STATUS='PROVED_EXACT_FINITE_POOLED_COMPLETE_SHELL_EXTENSION'
QS,HEIGHTS,B=(65536,131072),(16,32,66,128),1_000_000; COUNTS=(5709,10749)
...
def crt(rs,ms):
    period=math.prod(ms); residue=sum(r*(period//p)*pow(period//p,-1,p) for r,p in zip(rs,ms))%period
...
def shells():
    ...
    return out
def row(h,items):
    ps=[p for p,_ in items]; qs=[q for _,q in items]; n=4*h; need(n<min(ps),'pooled domain')
    rs=[0 if i%2==0 else -n for i in range(len(ps))]; residue,period=crt(rs,ps); origin=residue+((B-residue)//period+1)*period
    ...
    aa=[Fraction(p**3,q*q*(p-1)) for p,q in items]; minus,plus=aa[1::2],aa[::2]; vm=sum((a*a for a in minus),Fraction(0)); vp=sum((a*a for a in plus),Fraction(0)); pm=sum(minus,Fraction(0)); amin=min(aa)
    g0=vm*s0; g1=vm*s1+vp*(s1-t(1)**2); direct=t(1)*pm; z2=direct*direct/(g0*g1); sharp=t(1)**2/(amin*amin*s0*s1); coarse=Fraction(16,h*h)
    need(origin>B and z2<=sharp<=coarse,'bound')
...
def check_document(d):
    need(type(d) is dict and set(d)=={'certificate_version','claim_status','payload','payload_sha256'},'document'); need(d['certificate_version']==1 and d['claim_status']==STATUS,'header'); need(d['payload_sha256']==hashlib.sha256(canonical(d['payload'])).hexdigest(),'digest'); need(canonical(d['payload'])==canonical(payload()),'exact height extension')
def main():
    a=argparse.ArgumentParser(); a.add_argument('--write',action='store_true'); a.add_argument('--check',action='store_true'); x=a.parse_args()
    if x.write: write(); print('TPC412_CERTIFICATE=WRITTEN')
    elif x.check: check_document(json.loads(RESULT.read_bytes(),object_pairs_hook=no_duplicates,parse_constant=no_constants)); print('TPC412_CERTIFICATE=PASS cases=4 heights=4 pooled_shells=2 literal_domain=PASS')
