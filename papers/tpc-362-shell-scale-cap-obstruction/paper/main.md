...
Table [1](main.tex#L104){reference-type="ref" reference="tab:q"} reports the extrema at each shell value. Each row of the table aggregates 48 law-level records.
...
The envelope inequalities in [\[eq:envelope\]](main.tex#L73){reference-type="eqref" reference="eq:envelope"} are proved for finite matrices. The following statuses deliberately separate the finite positive low-$Q$ result from the scoped negative extension:
