#!/usr/bin/env python3
"""Adversarial mutation stress for the TPC-362 shell-scale certificate."""

from __future__ import annotations

import copy
import hashlib
import json
import sys
from pathlib import Path
from typing import Any

ROOT = Path(__file__).resolve().parents[3]
CERTIFICATE = ROOT / "papers/tpc-362-shell-scale-cap-obstruction/results/tpc362_certificate.json"
SCHEMA = "TPC362_SHELL_SCALE_CAP_OBSTRUCTION_V1"
STATUS = "NUMERICALLY_CERTIFIED_FINITE_SHELL_SCALE_CAP_OBSTRUCTION"


class Rejected(Exception):
    pass


def canonical(value: Any) -> bytes:
    return (json.dumps(value, ensure_ascii=True, sort_keys=True,
                       separators=(",", ":")) + "\n").encode("ascii")


def validate(document: dict[str, Any]) -> None:
    if document.get("certificate_version") != 1 or document.get("claim_status") != STATUS:
        raise Rejected("header")
    payload = document.get("payload")
    if not isinstance(payload, dict) or payload.get("schema") != SCHEMA:
        raise Rejected("schema")
    if document.get("payload_sha256") != hashlib.sha256(canonical(payload)).hexdigest():
        raise Rejected("payload hash")
    protocol = payload.get("protocol", {})
    if (protocol.get("origins") != [313030, 311166, 321651] or
            protocol.get("counts") != [256, 512] or
            protocol.get("q_anchors") != [12, 24, 36, 54, 80, 128, 256, 512] or
            protocol.get("kernel_exponents") != [1, 2] or
            protocol.get("laws") != ["all_plus", "alternating_index",
                                       "mod4_character", "half_split"] or
            protocol.get("spectra_for_all_laws") is not True or
            protocol.get("source_response_used") is not False):
        raise Rejected("protocol")
    rows = payload.get("rows")
    if not isinstance(rows, list) or len(rows) != 384:
        raise Rejected("rows")
    if len({(r.get("origin"), r.get("count"), r.get("Q"),
             r.get("kernel_exponent"), r.get("law")) for r in rows}) != 384:
        raise Rejected("row keys")
    audit = payload.get("finite_audit", {})
    try:
        if (audit.get("rows") != 384 or audit.get("settings") != 96 or
                audit.get("laws") != 4 or audit.get("spectral_rows") != 384 or
                audit.get("first_schur_cap_failure_Q") != 128 or
                audit.get("first_spectral_cap_failure_Q") != 128 or
                float(audit["low_q_normalized_schur_max"]) >= 0.83 or
                float(audit["low_q_normalized_spectral_max"]) >= 0.64 or
                int(audit["schur_cap_violations"]) != 33 or
                int(audit["spectral_cap_violations"]) != 30 or
                audit.get("finite_schur_violations") != 0 or
                audit.get("finite_frobenius_violations") != 0 or
                audit.get("fixed_power_credit") != 0 or
                audit.get("arithmetic_advance") != "NO"):
            raise Rejected("audit")
    except (KeyError, TypeError, ValueError) as error:
        raise Rejected("audit values") from error
    if payload.get("law_winner_audit", {}).get("winner_counts") != {
            "all_plus": 78, "alternating_index": 4,
            "mod4_character": 14, "half_split": 0}:
        raise Rejected("winner census")
    if payload.get("q_transition_audit", {}).get("counts") != {
            "increase": 200, "decrease": 136, "flat": 0}:
        raise Rejected("Q transitions")
    expected = {
        "TPC362_SHELL_SCALE_REPLAY": "NUMERICALLY_CERTIFIED_FINITE_384_ROWS",
        "TPC362_FINITE_SCHUR_ENVELOPE": "PROVED_EXACT_FINITE",
        "TPC362_FINITE_FROBENIUS_ENVELOPE": "PROVED_EXACT_FINITE",
        "TPC362_LOW_Q_CAP": "NUMERICALLY_CERTIFIED_FINITE_SCOPED",
        "TPC362_HIGH_Q_CAP_EXTENSION": "REFUTED_SCOPED_ON_DECLARED_Q_LADDER",
        "TPC362_LAW_WINNER_CENSUS": "NUMERICALLY_CERTIFIED_FINITE_SCOPED",
        "TPC362_GROWING_OPERATOR_BOUND": "OPEN",
        "TPC362_SOURCE_UNIFORM_L2": "OPEN",
        "TPC362_ARITHMETIC_ADVANCE": "NO",
        "TPC362_FIXED_POWER_CREDIT": 0,
        "TPC362_FULL_GATE_B": "OPEN", "TPC362_TWIN_PRIME_RESULT": "NONE",
    }
    for key, value in expected.items():
        if payload.get("claim_firewall", {}).get(key) != value:
            raise Rejected("firewall " + key)


def main() -> int:
    if sys.argv[1:] != ["--check"]:
        raise SystemExit("--check is the only argument")
    try:
        document = json.loads(CERTIFICATE.read_bytes())
        validate(document)
        baseline = hashlib.sha256(canonical(document)).hexdigest()
        mutations = [
            (("payload", "schema"), "MUTATED"),
            (("payload", "protocol", "q_anchors"), [24, 54, 80]),
            (("payload", "protocol", "source_response_used"), True),
            (("payload", "rows"), []),
            (("payload", "finite_audit", "rows"), 383),
            (("payload", "finite_audit", "first_spectral_cap_failure_Q"), 256),
            (("payload", "finite_audit", "low_q_normalized_spectral_max"), "0.9"),
            (("payload", "finite_audit", "schur_cap_violations"), 0),
            (("payload", "finite_audit", "spectral_cap_violations"), 0),
            (("payload", "law_winner_audit", "winner_counts", "all_plus"), 77),
            (("payload", "q_transition_audit", "counts", "increase"), 199),
            (("payload", "claim_firewall", "TPC362_HIGH_Q_CAP_EXTENSION"), "PROVED"),
            (("payload", "claim_firewall", "TPC362_ARITHMETIC_ADVANCE"), "YES"),
            (("payload", "claim_firewall", "TPC362_FIXED_POWER_CREDIT"), 1),
            (("payload_sha256",), "0" * 64),
        ]
        rejected = 0
        for path, value in mutations:
            item = copy.deepcopy(document)
            target: Any = item
            for key in path[:-1]:
                target = target[key]
            target[path[-1]] = value
            try:
                validate(item)
            except Rejected:
                rejected += 1
        if (rejected != len(mutations) or
                hashlib.sha256(canonical(document)).hexdigest() != baseline):
            raise Rejected("mutation census")
        print("TPC362_STRESS=PASS exact_baseline=1 mutations=15")
        return 0
    except (Rejected, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError) as error:
        print("TPC362_STRESS=FAIL " + str(error), file=sys.stderr)
        return 1


if __name__ == "__main__":
    raise SystemExit(main())
