#!/usr/bin/env python3
"""Independent reverse-shell replay for the TPC-371 block audit."""
...
def parent_failure_snapshot() -> list[list[Any]]:
    ...
    need(payload.get("schema") == "TPC370_COUNT_2048_WINDOW_AUDIT_V1",
         "parent schema")
...
def sign_patterns(primes: list[int]) -> dict[str, np.ndarray]:
    return {
        ...
        "half_split": np.asarray(
            [1.0 if i < len(primes) / 2 else -1.0 for i in range(len(primes))],
            dtype=np.float64),
    }
...
def validate_header(document: dict[str, Any]) -> dict[str, Any]:
    ...
    parent_keys = parent_failure_snapshot()
    ...
    need(lock.get("base_code_sha256") == BASE_SHA256 and
         ...
         "TPC370_COUNT_2048_WINDOW_AUDIT_V1" and
         lock.get("parent_failure_keys") == parent_keys, "parent lock")
    origin = payload.get("origin_protocol", {})
    need(origin.get("candidate_origins") == list(CANDIDATE_ORIGINS) and
         origin.get("candidate_count") == 41 and
    ...
    need(isinstance(rows, list) and len(rows) == 576, "rows")
    keys = {(row.get("origin"), row.get("block_index"), row.get("Q"),
             row.get("kernel_exponent"), row.get("beta"), row.get("law"))
            for row in rows}
    need(keys == expected, "row keys")
    ...
    need(audit.get("rows") == 576 and
         ...
         audit.get("window_count") == WINDOW_COUNT and
         audit.get("block_count_fixed") == BLOCK_COUNT and
    ...
    for beta, prefix in ((2, "beta2"), (0, "baseline_beta0")):
        ...
        need(audit.get(prefix + "_spectral_cap_violations") ==
             item["spectral_cap_violations"] and
    ...
    actual = [[row["origin"], row["block_index"], row["Q"],
              ...
              float(row["normalized"]["spectral"]) > SPECTRAL_CAP]
    need(audit.get("beta2_failure_keys") == actual and
         audit.get("beta2_all_declared_blocks_pass") == (not actual) and
...
def main() -> int:
    ...
    try:
        ...
        need(payload.get("exact_anchor") == exact_anchor_expected(),
             "exact anchor")
        recorded = {(row["origin"], row["block_index"], row["Q"],
        ...
        for beta in reversed(BETAS):
            for origin in reversed(ORIGINS):
                for block_index in reversed(BLOCK_INDICES):
                    start = origin + block_index * BLOCK_COUNT
                    values = np.arange(start, start + BLOCK_COUNT,
                                       dtype=np.int64)
                    for q0 in reversed(Q_ANCHORS):
                        for exponent in reversed(EXPONENTS):
                            ...
                            for law in reversed(LAWS):
                                key = (origin, block_index, q0, exponent, beta, law)
                                row = recorded[key]
                                need(row["interval"] == [start, start + 255] and
                                     row["window_interval"] ==
                                     [origin, origin + WINDOW_COUNT - 1] and
                                ...
                                for label, matrix in (("raw", matrices[law]),
                                                      ("normalized", matrices[law] /
                                                       scale)):
                                    actual_metrics = metrics(matrix)
                                    target = row[label]
                                    for field in ("schur", "frobenius", "spectral",
                                        ...
                                        close(actual_metrics[field], target[field],
                                              label + " " + field)
                                    need(actual_metrics["schur_row_index"] ==
        ...
        need(checked == 576 and violations == {
        ...
        print("TPC371_INDEPENDENT_CHECK=PASS rows=" + str(checked) +
        ...
        print("TPC371_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
