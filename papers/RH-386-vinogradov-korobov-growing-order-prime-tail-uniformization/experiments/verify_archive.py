"""Verify RH-386 archive membership, hashes, and immutable source identity."""
...
from experiments.build_archive import (  # noqa: E402
    ...
    validated_remote_lock,
    validated_result_git_source_map,
)
from experiments import build_result, build_schema  # noqa: E402
from vk_prime_tail.core import exact_equal, loads_strict, payload_sha256  # noqa: E402
...
def verify_manifest(manifest: object) -> dict[str, object]:
    ...
    expected_keys = {
        ...
        "external_git_input_count", "external_git_inputs",
        "remote_logical_input_count", "remote_source_lock_sha256",
        "source_commits", *BOOLEAN_KEYS,
    }
    if set(manifest) != expected_keys:
        failures.append("manifest:top_level_membership")
    if manifest.get("status") != "RH-386_fixed_publication_manifest":
    ...
    if external_count != 59:
        failures.append("manifest:external_git_membership")
    if type(manifest.get("remote_logical_input_count")) is not int or manifest.get("remote_logical_input_count") != 1:
        failures.append("manifest:remote_logical_input_count")
    if not exact_equal(manifest.get("source_commits"), build_result.SOURCE_COMMITS):
        failures.append("manifest:source_commits")

    result_rebuild = schema_rebuild = result_match = False
    release_pass = digest_pass = certificate_pass = remote_pass = False
    ...
    try:
        ...
        release_pass = git_locks.get("release_blob_identity_pass") is True
        digest_pass = git_locks.get("digest_contract_pass") is True
        certificate = result.get("certificate")
        certificate_pass = type(certificate) is dict and payload_sha256(certificate) == build_result.CERTIFICATE_FIXTURE_SHA256
        lock = validated_remote_lock(result)
        remote_sha = payload_sha256(lock)
        remote_pass = remote_sha == manifest.get("remote_source_lock_sha256")
    ...
    return {
        "status": "RH-386_archive_verified" if not failures else "RH-386_archive_failed",
        "publication_file_count": local_count,
        "external_git_input_count": external_count,
        "remote_logical_input_count": 1 if remote_pass else 0,
...
def main() -> None:
    try:
        manifest = loads_strict(MANIFEST.read_text())
