"""Generate the recursively closed official Draft 2020-12 RH-386 schema."""
...
def exact_schema(value: object) -> dict[str, object]:
    ...
    raise TypeError(f"unsupported JSON schema primitive: {type(value).__name__}")


def build_schema() -> dict[str, object]:
    schema = exact_schema(build_payload())
    schema["$schema"] = "https://json-schema.org/draft/2020-12/schema"
    schema["$id"] = "https://example.invalid/schemas/RH-386-result.schema.json"
    schema["title"] = "RH-386 exact release result"
    return schema
