#!/usr/bin/env python3
"""Independent replay for the TPC-331 mean/centered response decomposition.
...
def locked_parent_checks(payload: dict[str, Any]) -> None:
    ...
    need(lock["TPC330_producer_sha256"] == PARENT_CODE_SHA256 and
         lock["TPC330_certificate_sha256"] == PARENT_CERT_SHA256,
         "parent lock fields")
...
def check() -> None:
    raw, document = load_json(CERTIFICATE)
    need(document.get("certificate_version") == 1 and
         document.get("claim_status") == STATUS, "TPC331 certificate header")
    payload = document.get("payload")
    need(isinstance(payload, dict) and payload.get("schema") == SCHEMA,
         "TPC331 schema")
    need(document.get("payload_sha256") == hashlib.sha256(
         canonical(payload)).hexdigest(), "TPC331 payload hash")
    need(payload.get("round2_clue") ==
    ...
    need(isinstance(protocol, dict) and
         ...
         protocol.get("laws") == list(LAW_NAMES), "TPC331 protocol")

    rows = payload.get("rows")
    need(isinstance(rows, list) and len(rows) == 32, "TPC331 row census")
    expected_keys = {(o, n, q, s) for o in ORIGINS for n in SCALES
                     for q in Q_ANCHORS for s in EXPONENTS}
    seen: set[tuple[int, int, int, int]] = set()
    ...
    for row in rows:
        key = (row["origin"], row["scale"], row["Q"],
               row["kernel_exponent"])
        need(key not in seen and key in expected_keys, "TPC331 row key")
        seen.add(key)
        origin, scale, q0, exponent = key
        lo, hi = origin, origin + scale // 2 - 1
        sh = shell(q0)
        ...
        # The inherited identity row and the two positive component controls
        # are replayed as guardrails for the same finite source/matrix.
        for law in LAW_NAMES:
            check_metric(row["laws"][law],
                         metric(matrices[law], residual),
                         "TPC331 inherited " + law)
        check_metric(row["component_controls_all_plus"]["lambda"],
                     metric(matrices["all_plus"], lam),
                     "TPC331 lambda control")
        check_metric(row["component_controls_all_plus"]["comparison"],
                     metric(matrices["all_plus"], comp),
                     "TPC331 comparison control")
        records.append(local)
    need(seen == expected_keys, "TPC331 complete row keys")
        ...
        for component in DECOMPOSITION_COMPONENTS:
            for key_name in ("ratio_min", "ratio_max"):
                close(want["component_ratio_ranges"][component][key_name],
                      ...
                      "TPC331 summary ratio " + law + " " + component)
    ...
    need(isinstance(finite, dict) and finite.get("rows") == 32 and
         ...
         "TPC331 finite audit firewall")

    anchor = payload.get("exact_anchor")
    exact = exact_decomposition_anchor()
    need(isinstance(anchor, dict) and anchor.get("interval") == [36001, 36016]
    ...
    }, "TPC331 claim firewall")
    print("TPC331_INDEPENDENT_CHECK=PASS rows=32 origins=2 scales=2 laws=4 "
...
def main() -> int:
    ...
    try:
        ...
        print("TPC331_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
    return 0
