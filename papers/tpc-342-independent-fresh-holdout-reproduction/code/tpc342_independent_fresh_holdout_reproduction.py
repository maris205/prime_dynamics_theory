#!/usr/bin/env python3
"""TPC-342: fresh holdout test for nuisance orthogonalization.
...
def build_payload() -> dict[str, Any]:
    ...
    need(len(rows) == 3 and len(raw) == 108 and len(nonempty) == 81 and
    ...
    return {
        "schema": SCHEMA,
        "parent_lock": {"TPC341_protocol_producer_sha256": PROTOCOL_CODE_SHA256,
                         "TPC341_protocol_certificate_sha256": PROTOCOL_CERT_SHA256,
        ...
        "claim_firewall": {
            "TPC342_PROJECTION_IDENTITY":
                "PROVED_EXACT_FINITE_DECLARED_MODEL",
            "TPC342_INDEPENDENT_FRESH_REPLAY":
                "NUMERICALLY_CERTIFIED_FINITE_108_RAW_RECORDS",
            "TPC342_IN_SAMPLE_PROJECTION":
                "NUMERICALLY_CERTIFIED_FINITE_3_ROWS",
            "TPC342_HOLDOUT_OBSTRUCTION":
                "NUMERICALLY_CERTIFIED_FINITE_27_RECORDS",
            "TPC342_IN_SAMPLE_RETENTION":
                "NUMERICAL_OBSERVATION_0.270_TO_0.296",
            "TPC342_HOLDOUT_RETENTION":
                "NUMERICAL_OBSERVATION_0.589_TO_0.943",
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            ...
            print("TPC342_CERTIFICATE=WRITTEN")
        else:
            raw = RESULT.read_bytes()
            stored = json.loads(raw)
            need(raw == canonical(stored), "certificate canonicality")
            need(stored == document, "TPC342 certificate does not replay")
            print("TPC342_CERTIFICATE=PASS rows=3 controls=9 raw_records=108 "
                  "holdout_records=27 rank_failures=0 exact_anchor=1")
        return 0
    except (Failure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError) as error:
        print("TPC342_CERTIFICATE=FAIL " + str(error), file=sys.stderr)
        return 1
