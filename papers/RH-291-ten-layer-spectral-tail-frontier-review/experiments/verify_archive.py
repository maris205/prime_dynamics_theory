"""Verify publication manifests for RH-282 through RH-291."""
...
def main() -> None:
    ...
    for directory in paper_directories():
        manifest = json.loads((directory / "results/dependency_manifest.json").read_text())
        expected = manifest["files"]
        current = {str(path.relative_to(directory)): path for path in publication_files(directory)}
        failures = []
        ...
        summaries.append({"paper": directory.name, "file_count": manifest["file_count"], "failure_count": len(failures)})
    print(json.dumps({"paper_count": len(summaries), "failure_count": total_failures, "papers": summaries}, sort_keys=True))
    if total_failures:
        raise SystemExit(1)
