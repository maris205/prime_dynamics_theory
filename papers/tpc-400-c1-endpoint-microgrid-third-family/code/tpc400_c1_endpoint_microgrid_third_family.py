#!/usr/bin/env python3
"""TPC-400: third-family replication of the C1 endpoint microgrid.

TPC-399 established a finite cross-family transfer certificate on the second
fresh coordinate-disjoint family.  TPC-400 repeats the same predeclared
protocol on a third fresh family and compares each same-law cohort mean with
the frozen all-origin mean from TPC-399.  The parent values are imported by
hash and are not refit from the current readout.  The matrix interpolation
identity is exact at the finite anchor, while every transfer statement is
finite and diagnostic.  Nothing here is an arithmetic sign law or an
asymptotic operator statement.
"""
...
RESULT = PROJECT / "results/tpc400_certificate.json"
PARENT_CODE = ROOT / (
...
CLAIM_FIREWALL = {
    ...
    "TPC400_INTERPOLATION_IDENTITY":
        "PROVED_EXACT_FINITE_LINEAR_MATRIX_IDENTITY",
    "TPC400_INTERPOLATION_PANEL":
        "NUMERICAL_OBSERVATION_FINITE_FLOAT64_96_ROWS",
    "TPC400_ORIGIN_PHASE":
        "NUMERICAL_OBSERVATION_FINITE_FLOAT64_SCOPED",
    "TPC400_PARENT_CROSS_FAMILY_TRANSFER":
        "NUMERICAL_OBSERVATION_FINITE_FLOAT64_SCOPED",
    "TPC400_SPECTRAL_ENVELOPE":
        "NUMERICAL_OBSERVATION_FINITE_FLOAT64_SCOPED_ONLY",
    "TPC400_SCHUR_ENVELOPE":
        "NUMERICAL_OBSERVATION_FINITE_FLOAT64_SCOPED_ONLY",
...
def load_parent_means() -> dict[tuple[str, str, int], float]:
    ...
    need(payload.get("schema") ==
         "TPC399_C1_ENDPOINT_MICROGRID_CROSS_FAMILY_V1" and
         payload.get("status") ==
    ...
    for cell in cells:
        ...
        if current_law not in LAWS:
            continue
        key = (cell.get("normalization"), current_law, cell.get("Q"))
        need(key not in baseline, "duplicate parent mean")
...
def origin_summary(rows: list[dict[str, Any]],
    ...
    return {
        ...
        "holdout_origin_count": 3, "parent_family": "TPC399",
...
def prior_intervals() -> list[tuple[int, int]]:
    return [
        (7200001, 7200001 + 1024), (7203209, 7203209 + 1024),
        (7206417, 7206417 + 1024), (7209625, 7209625 + 1024),
        (7212833, 7212833 + 1024), (7216041, 7216041 + 1024),
        (6800001, 6800001 + 1024), (6803209, 6803209 + 1024),
        (6806417, 6806417 + 1024), (6809625, 6809625 + 1024),
        (6812833, 6812833 + 1024), (6816041, 6816041 + 1024),
...
def build_payload() -> dict[str, Any]:
    ...
    summary = origin_summary(rows, parent)
    return {
        "schema": SCHEMA, "status": STATUS,
        "parent_lock": {
            "parent_code_sha256": PARENT_CODE_SHA256,
            "parent_certificate_sha256": PARENT_CERT_SHA256,
            "parent_schema": "TPC399_C1_ENDPOINT_MICROGRID_CROSS_FAMILY_V1",
            "parent_status":
                "NUMERICALLY_CERTIFIED_FINITE_C1_ENDPOINT_MICROGRID_CROSS_FAMILY_AUDIT",
            "parent_interface_frozen": True,
            "parent_interface_used_for_current_fit": False,
            "parent_means_used_as_response_blind_baseline": True,
            "parent_endpoint_laws": list(LAWS),
            "parent_segment": "lambda in {7/8,15/16,31/32,1}",
            "parent_baseline_definition":
                "direct same-law TPC399 all-origin mean",
            "parent_interpolation_is_modeling_choice": False,
        },
        "selection_protocol": {
        ...
        "protocol": {
            ...
            "cross_family_definition":
                "new-family cohort mean / frozen same-law TPC399 all-origin mean - 1",
            ...
            "parent_segment_coordinate_definition":
                "none: direct same-law parent means are used; no segment refit",
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            ...
            print("TPC400_CERTIFICATE=WRITTEN")
        else:
            ...
            print(f"TPC400_CERTIFICATE=PASS rows={ROW_COUNT} cells={CELL_COUNT} "
        ...
        print("TPC400_CERTIFICATE=FAIL " + str(error))
        return 1
