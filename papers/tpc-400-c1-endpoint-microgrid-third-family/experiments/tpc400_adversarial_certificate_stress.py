#!/usr/bin/env python3
"""Mutation firewall for the TPC-400 cross-family microgrid certificate."""
...
import tpc400_independent_checker as independent  # noqa: E402
...
def main() -> int:
    ...
    try:
        ...
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC400_ARITHMETIC_ADVANCE", "YES"))
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC400_FIXED_POWER_CREDIT", 1))
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC400_FULL_GATE_B", "CLOSED"))
        ...
        print("TPC400_STRESS=PASS mutations=28")
        return 0
    except Exception as error:
        print("TPC400_STRESS=FAIL " + str(error), file=sys.stderr)
        return 1
