"""Verify RH-387 archive membership, hashes, and immutable source identity."""
...
from integral_resummation.core import exact_equal, loads_strict, payload_sha256  # noqa: E402
...
def verify_manifest(manifest: object) -> dict[str, object]:
    ...
    expected_keys = {
        ...
        "logical_source_digest", "source_commits", *BOOLEAN_KEYS,
    }
    if set(manifest) != expected_keys:
        failures.append("manifest:top_level_membership")
    if manifest.get("status") != "RH-387_fixed_publication_manifest":
    ...
    if (
        ...
        or manifest.get("logical_source_digest") != build_result.EXPECTED_LOGICAL_SOURCE_DIGEST
    ):
    ...
    try:
        ...
        logical_pass = (
            source_locks.get("logical_source_digest") == build_result.EXPECTED_LOGICAL_SOURCE_DIGEST
        ...
        remote_pass = payload_sha256(lock) == manifest.get("remote_source_lock_sha256")
    ...
    semantic_match = (
        ...
        and local_entries.get("main.pdf")
        == local_entries.get("all-order-prime-tail-integral-resummation.pdf")
    )
    current = {
    ...
    return {
        "status": "RH-387_archive_verified" if not failures else "RH-387_archive_failed",
