...
from integral_resummation.core import exact_equal, loads_strict  # noqa: E402
if str(ROOT / "experiments") not in sys.path:
    sys.path.insert(0, str(ROOT / "experiments"))
from build_result import RH386_DIRECTORY, RH386_RELEASE, git_blob  # noqa: E402


def remote_source_lock() -> dict[str, object]:
    """Return the exact upstream RH-386 lock without any network request."""
    return loads_strict(
        git_blob(
            RH386_RELEASE,
            f"{RH386_DIRECTORY}/results/external_source_lock.json",
        ).decode("utf-8")
    )
...
def _fetch_exact(
    ...
    *,
    ...
    request = Request(requested_url, headers={"User-Agent": "RH-387-source-lock-verifier/1.0"})
    try:
...
def _pdf_pages_pdfinfo(data: bytes) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="rh387-pdf-") as directory:
