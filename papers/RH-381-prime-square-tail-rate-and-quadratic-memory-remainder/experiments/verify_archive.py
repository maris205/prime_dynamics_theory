"""Verify RH-381 manifest membership, hashes, and source-lock identity."""
...
def verify_manifest(manifest: dict[str, object]) -> dict[str, object]:
    ...
    expected_keys = {
        ...
        "independent_interval_digest_pass",
        "semantic_pdf_match",
    }
    if set(manifest) != expected_keys:
        failures.append("manifest:top_level_membership")
    if manifest.get("status") != "RH-381_fixed_publication_manifest":
    ...
    elif isinstance(external_entries, dict) and set(external_entries) != set(SOURCE_FILES):
    ...
    try:
        ...
        interval_pass = result.get("certificate", {}).get(
            "interval_digest_matches_independent_plan"
        ) is True
    ...
    if not interval_pass or manifest.get("independent_interval_digest_pass") is not True:
        failures.append("manifest:independent_interval_digest_pass")

    semantic_pdf_match = False
    if isinstance(local_entries, dict):
        semantic_pdf_match = local_entries.get("main.pdf") == local_entries.get(
    ...
    return {
        "status": "RH-381_archive_verified" if not failures else "RH-381_archive_failed",
        ...
        "independent_interval_digest_pass": interval_pass,
...
def main() -> None:
    try:
        ...
        payload = {
            ...
            "independent_interval_digest_pass": False,
            "semantic_pdf_match": False,
        }
    else:
