"""Build the fixed RH-381 publication and dependency manifest."""
...
LOCAL_MEMBERS = (
    ...
    "main.pdf",
    "main.tex",
    "prime-square-tail-rate-and-quadratic-memory-remainder.pdf",
    "pyproject.toml",
    "references.bib",
    "requirements.txt",
    "results/result.json",
    "results/result.schema.json",
    "src/prime_square_tail/__init__.py",
    "src/prime_square_tail/core.py",
...
def validated_result_source_map(result: dict[str, object]) -> dict[str, str]:
    ...
    if type(entries) is not list or len(entries) != len(SOURCE_FILES):
        raise RuntimeError("result source-lock entries are not the frozen 25 rows")
    paths: list[str] = []
    for entry in entries:
        if type(entry) is not dict:
            raise RuntimeError("a result source-lock row is not an object")
        path = entry.get("path")
    ...
    if type(source_locks.get("count")) is not int or source_locks.get("count") != len(
        SOURCE_FILES
    ):
        raise RuntimeError("result source-lock count is not the exact integer 25")
...
def build_payload() -> dict[str, object]:
    ...
    if source_lock_object.get("digest_contract_pass") is not True:
        raise RuntimeError("result did not certify the 25-source digest contract")
    if result.get("certificate", {}).get("interval_digest_matches_independent_plan") is not True:
        raise RuntimeError("result did not certify the independent interval digest")
    main_pdf = publication["main.pdf"]
    semantic_pdf = publication["prime-square-tail-rate-and-quadratic-memory-remainder.pdf"]
    if main_pdf != semantic_pdf:
        raise RuntimeError("semantic PDF is not byte-identical to main.pdf")
    return {
        "status": "RH-381_fixed_publication_manifest",
        ...
        "independent_interval_digest_pass": True,
        ...
        json.dumps(
            {
                ...
                "independent_interval_digest_pass": payload["independent_interval_digest_pass"],
