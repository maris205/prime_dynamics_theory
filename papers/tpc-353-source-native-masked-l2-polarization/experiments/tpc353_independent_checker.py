#!/usr/bin/env python3
"""Independent reverse-shell replay for TPC-353.

This checker does not import the TPC-353 producer.  It rebuilds the V59
source midpoints, the literal endpoint-masked matrices, all four sign laws,
and every operator-polarization metric.  Shells are accumulated in reverse
order to vary the floating-point path.
"""
...
EXACT_RIGHT_DIGEST = (
    "2cb0dcdd035384e86826ea289941f54bf53e5a1d299faab0946139babeb3f8e3")
EXACT_CROSS_DIGEST = (
    "f48e62bc814feda9a33b565f337e038fd25808bc9863087fbbd61d9da67237ed")
EXACT_RESIDUAL_DIGEST = (
    "80aea8f6e49dbc6cf33b36508c9071ca4b19db5ba0c7bfab98d5e04dd12bd9ef")
...
def check() -> None:
    ...
    for path, expected, label in (
            (PARENT_CODE, PARENT_CODE_SHA256, "TPC328 code"),
            (PARENT_CERT, PARENT_CERT_SHA256, "TPC328 certificate"),
    ...
    need(lock.get("TPC328_code_sha256") == PARENT_CODE_SHA256 and
         lock.get("TPC328_certificate_sha256") == PARENT_CERT_SHA256 and
    ...
    need(firewall.get("TPC353_FINITE_OPERATOR_POLARIZATION") ==
         ...
         firewall.get("TPC353_FIXED_POWER_CREDIT") == 0 and
         firewall.get("TPC353_FULL_GATE_B") == "OPEN" and
         firewall.get("TPC353_TWIN_PRIME_RESULT") == "NONE",
         "claim firewall")
    print("TPC353_INDEPENDENT_CHECK=PASS rows=216 "
...
def main() -> int:
    ...
    try:
        ...
        print("TPC353_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
    return 0
