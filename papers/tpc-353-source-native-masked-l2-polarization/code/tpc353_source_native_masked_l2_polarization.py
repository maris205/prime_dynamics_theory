#!/usr/bin/env python3
"""TPC-353: operator-level polarization of the source-native residual.

The preceding finite papers left two pieces separate: the V59 residual was
audited at source level, while the literal divisibility-masked operator was
audited as a matrix.  This release joins those pieces.  For
...
The resulting polarization coefficient is a finite diagnostic only.  No
source-uniform estimate, asymptotic statement, or twin-prime conclusion is
claimed.
"""
...
try:
    ...
    raise SystemExit("TPC353 requires numpy: " + str(error))
...
RESULT = PROJECT / "results/tpc353_certificate.json"
...
ROUND2_CLUE = (
    "TEST_SOURCE_NATIVE_L2_CROSS_TERM_ON_DISJOINT_HIGHER_ORIGINS_OR_"
    "BUILD_POSITION_AWARE_MASKED_BOUND")

# This panel is below the inherited 50,000 source cutoff and is disjoint from
# the TPC-328--TPC-352 active panels.  Counts are the actual interval lengths.
...
def build_payload() -> dict[str, Any]:
    ...
    return {
        "schema": SCHEMA,
        "parent_lock": {
            "TPC328_code_path": str(PARENT_CODE.relative_to(ROOT)),
            "TPC328_code_sha256": PARENT_CODE_SHA256,
            "TPC328_certificate_path": str(PARENT_CERT.relative_to(ROOT)),
        ...
        "protocol": {
            ...
            "panel_scope": "fresh low-origin panel disjoint from TPC328--TPC352 active panels",
        },
        "exact_theorem": {
        ...
        "claim_firewall": {
            ...
            "TPC353_STRONGEST_OBSTRUCTION": (
                "FINITE_OPERATOR_ALIGNMENT_DOES_NOT_PROVE_SOURCE_UNIFORMITY"),
        },
...
def main() -> int:
    ...
    try:
        if args.write:
            write_certificate()
            print("TPC353_CERTIFICATE=WRITTEN")
        else:
            check_certificate()
            print("TPC353_CERTIFICATE=PASS rows=216 positive_alignment=216/216 "
                  "unresolved=0 fixed_power_credit=0")
    except (CheckFailure, OSError, json.JSONDecodeError, ValueError,
            TypeError) as error:
        print("TPC353_CERTIFICATE=FAIL " + str(error), file=sys.stderr)
        return 1
    return 0
