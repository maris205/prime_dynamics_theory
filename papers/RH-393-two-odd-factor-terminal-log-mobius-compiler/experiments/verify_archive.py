"""Verify RH-393 archive membership, hashes, sources, and replay gates."""
...
def verify_manifest(manifest: object) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-393_fixed_publication_manifest":
    ...
    if type(external_entries) is not dict or external_count != 117:
    ...
    return {
        "status": "RH-393_archive_verified" if not failures else "RH-393_archive_failed",
        ...
        "logical_input_total": 120 if fresh is not None else 0,
        **current_counts,
