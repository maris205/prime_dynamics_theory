"""Independently verify the RH-362 fixed publication manifest."""
...
def validate_manifest(manifest: dict[str, object]) -> None:
    if manifest.get("status") != "RH-362_fixed_publication_manifest":
...
def verification_payload() -> dict[str, object]:
    ...
    expected_publication = manifest.get("publication_file_count")
    expected_external = manifest.get("external_input_count")
    if expected_publication != publication_count:
        failures.append({"path": "publication_file_count", "reason": "count_mismatch"})
    if expected_external != external_count:
        failures.append({"path": "external_input_count", "reason": "count_mismatch"})
    return {
        "status": "RH-362_archive_verified",
