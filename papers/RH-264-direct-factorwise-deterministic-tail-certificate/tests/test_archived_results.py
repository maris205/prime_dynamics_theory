...
def test_archived_direct_tail():
    payload = json.loads((ROOT / "results/direct_tail_audit.json").read_text())
    assert len(payload["replays"]) == 3
    assert all(all(row["comparisons"].values()) for row in payload["replays"])
