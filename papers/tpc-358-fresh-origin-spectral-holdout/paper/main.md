...
TPC-357 froze the numerical thresholds $S(A_\varepsilon^\#)<0.83$ on its all-law panel and $\lVert\,\cdot\,\rVert_2{A_{+}^\#}<0.64$ on its all-plus panel `\cite{tpc357}`. We call the fresh test a parent-cap transfer when both fresh maxima remain below these thresholds and within $0.001$ of the corresponding parent maxima. This is a finite comparison rule, not a statistical confidence statement or an asymptotic assertion.

# Results

Table [1](main.tex#L123){reference-type="ref" reference="tab:extrema"} reports the envelope extrema over the 72 rows of each law. The normalized scales are close to the preceding panel despite the large origin separation, while the raw scales remain much larger.
