#!/usr/bin/env python3
"""TPC-365: response-blind fresh holdout for the TPC-364 beta=2 rule."""
...
RESULT = PROJECT / "results/tpc365_certificate.json"
BASE_CODE = ROOT / (
    "papers/tpc-355-position-aware-mask-energy-normalization/code/"
    "tpc355_position_aware_mask_energy_normalization.py")
PARENT_CODE = ROOT / (
    "papers/tpc-364-shell-tilt-phase-diagram/code/"
    "tpc364_shell_tilt_phase_diagram.py")
PARENT_CERT = ROOT / (
...
PARENT_CODE_SHA256 = (
    "8b7428b02294cf1a7daca7317b2d312af44f64bf153b86f99f4a18cb27e02dd2")
PARENT_CERT_SHA256 = (
    "5b6aa0e6f97a746d27a36c7866810b2450a84096d911734bd2065784486a4058")
...
CANDIDATE_ORIGINS = tuple(410001 + 257 * j for j in range(51))
PILOT_COUNT = 256
SELECTED_COUNT = 3
MIN_SEPARATION = 2048
...
def load_base():
    spec = importlib.util.spec_from_file_location("tpc355_base_tpc365",
                                                  BASE_CODE)
...
def load_parent() -> dict[str, Any]:
    ...
    need(isinstance(payload, dict) and
         payload.get("schema") == "TPC364_SHELL_TILT_PHASE_DIAGRAM_V1",
         "parent payload")
    return payload
...
def exact_anchor() -> dict[str, Any]:
    # The interval is recorded in the repository's half-open convention
    # [left, right), matching the finite-window rows above.
...
def build_payload() -> dict[str, Any]:
    ...
    holdout_max = float(phase["by_beta"]["2"]["spectral_max"])
    parent_max = float(parent_phase["spectral_max"])
    need(holdout_max <= SPECTRAL_CAP and
         abs(holdout_max - parent_max) <= TRANSFER_TOL,
         "finite beta2 transfer")
    return {
        ...
        "protocol": {
            ...
            "parent_panel": "TPC-364 beta=2 finite phase point",
        },
        "exact_theorem": {
            "selection": "The declared greedy rule reads only unsigned weighted geometry and is finite deterministic.",
            ...
            "scope": "The holdout transfer is finite and limited to the declared candidate rule and panel.",
        },
        "finite_audit": {
            "rows": len(rows), "settings_per_beta": 48,
            "beta_count": len(BETAS), "spectral_rows": len(rows),
            "beta2_holdout_rows": 192, "beta2_holdout_cap_violations": 0,
            "baseline_beta0_cap_violations": 30,
            "parent_beta2_max_spectral": show(parent_max),
            "holdout_beta2_max_spectral": show(holdout_max),
            "holdout_minus_parent_max_spectral": show(holdout_max - parent_max),
...
def main() -> int:
    ...
    try:
        ...
        print("TPC365_CERTIFICATE=FAIL " + str(error), file=sys.stderr)
        return 1
    return 0
