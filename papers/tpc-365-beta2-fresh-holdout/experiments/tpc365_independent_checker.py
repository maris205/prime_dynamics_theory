#!/usr/bin/env python3
"""Independent reverse-order replay for the TPC-365 certificate.

This checker deliberately does not import the TPC-365 producer.  It rebuilds
the sieve, response-blind selection rule, weighted blocks, and matrix metrics
with a reverse prime accumulation order.
"""
...
def validate_header(document: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_phase = {0: 30, 2: 0}
    for beta, violations in expected_phase.items():
        item = phase.get("by_beta", {}).get(str(beta), {})
        need(item.get("rows") == 192 and
...
def main() -> int:
    ...
    try:
        ...
        need(CERTIFICATE.read_bytes() == canonical(document),
                        ...
                        for exponent in EXPONENTS:
                            ...
                            for law in LAWS:
                                ...
                                if float(row["normalized"]["spectral"]) > \
                                    ...
                                    violations[beta] += 1
                                checked += 1
        need(checked == 384 and violations == {0: 30, 2: 0},
             "phase census")
        print("TPC365_INDEPENDENT_CHECK=PASS rows=384 beta2_holdout=192 "
              "beta2_violations=0 baseline_beta0_violations=30")
        return 0
    except (Failure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError, np.linalg.LinAlgError) as error:
        print("TPC365_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
