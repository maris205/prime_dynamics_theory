#!/usr/bin/env python3
"""Independent reverse-shell replay for TPC-399.

The implementation is intentionally separate from the producer.  It validates
the hashed TPC-398 all-origin same-law baseline, rebuilds the new coordinate
...
def parent_means() -> dict[tuple[str, str, int], float]:
    ...
    need(payload.get("schema") == "TPC398_C1_INTERPOLATION_ENDPOINT_MICROGRID_V1" and
         payload.get("status") ==
...
def check_summary(rows: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    for norm in NORMALIZATIONS:
        for law in LAWS:
            ...
            need(cell.get("origins") == [r["origin"] for r in selected] and
                 ...
                 cell.get("parent_family") == "TPC398" and
    ...
    need(summary.get("spectral_failures_by_normalization") == spectral and
         ...
         "summary totals")
    need(summary.get("parent_family") == "TPC398" and
...
def check_document(document: dict[str, Any], recompute: bool = True) -> None:
    ...
    need(lock.get("parent_code_sha256") == PARENT_CODE_SHA256 and
         ...
         lock.get("parent_schema") == "TPC398_C1_INTERPOLATION_ENDPOINT_MICROGRID_V1" and
    ...
    need(firewall.get("TPC399_ARITHMETIC_ADVANCE") == "NO" and
         ...
         "NUMERICALLY_CERTIFIED_FINITE_SCOPED_ONLY" and
         firewall.get("TPC399_PARENT_CROSS_FAMILY_TRANSFER") ==
         "NUMERICALLY_CERTIFIED_FINITE_SCOPED", "firewall")
...
def main() -> int:
    ...
    try:
        ...
        print(f"TPC399_INDEPENDENT_CHECK=PASS rows={ROW_COUNT} cells={CELL_COUNT} "
        ...
        print("TPC399_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
