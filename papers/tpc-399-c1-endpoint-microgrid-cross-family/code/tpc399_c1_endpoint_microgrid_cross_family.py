#!/usr/bin/env python3
"""TPC-399: cross-family replication of the C1 endpoint microgrid.

TPC-398 established a finite endpoint microgrid on one coordinate-disjoint
family.  TPC-399 repeats the same predeclared protocol on a second fresh
family and compares each same-law cohort mean with the frozen all-origin mean
from TPC-398.  The parent values are imported by hash and are not refit from
the current readout.  The matrix interpolation identity is exact at the
finite anchor, while every transfer statement is finite and diagnostic.
...
RESULT = PROJECT / "results/tpc399_certificate.json"
PARENT_CODE = ROOT / (
    "papers/tpc-398-c1-interpolation-endpoint-microgrid/code/"
    "tpc398_c1_interpolation_endpoint_microgrid.py")
PARENT_CERT = ROOT / (
    "papers/tpc-398-c1-interpolation-endpoint-microgrid/results/"
    "tpc398_certificate.json")
PARENT_CODE_SHA256 = (
...
def load_parent_means() -> dict[tuple[str, str, int], float]:
    ...
    need(payload.get("schema") ==
         "TPC398_C1_INTERPOLATION_ENDPOINT_MICROGRID_V1" and
         payload.get("status") ==
         "NUMERICALLY_CERTIFIED_FINITE_C1_INTERPOLATION_ENDPOINT_MICROGRID_AUDIT",
         "parent header")
...
def origin_summary(rows: list[dict[str, Any]],
        ...
        for law in LAWS:
            for q0 in Q_ANCHORS:
                ...
                cells.append({
                    ...
                    "count": WINDOW_COUNT,
                    "parent_family": "TPC398",
                    "parent_interpolation_definition":
                        "direct same-law TPC398 all-origin mean, frozen "
    ...
    return {
        ...
        "holdout_origin_count": 3, "parent_family": "TPC398",
        "parent_interpolation":
            "direct same-law all-origin TPC398 means (no refit)",
...
def build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "parent_lock": {
            ...
            "parent_schema": "TPC398_C1_INTERPOLATION_ENDPOINT_MICROGRID_V1",
            "parent_status":
                "NUMERICALLY_CERTIFIED_FINITE_C1_INTERPOLATION_ENDPOINT_MICROGRID_AUDIT",
            ...
            "parent_baseline_definition":
                "direct same-law TPC398 all-origin mean",
        ...
        "protocol": {
            ...
            "cross_family_definition":
                "new-family cohort mean / frozen same-law TPC398 all-origin mean - 1",
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            ...
            print("TPC399_CERTIFICATE=WRITTEN")
        else:
            ...
            print(f"TPC399_CERTIFICATE=PASS rows={ROW_COUNT} cells={CELL_COUNT} "
        ...
        print("TPC399_CERTIFICATE=FAIL " + str(error))
        return 1
