"""Verify RH-383 manifest membership, hashes, and source-lock identity."""
...
from euler_tail_normal_form import CERTIFICATE_FIXTURE_SHA256, payload_sha256  # noqa: E402
...
def verify_manifest(manifest: dict[str, object]) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-383_fixed_publication_manifest":
    ...
    if type(local_entries) is dict:
        semantic_pdf_match = local_entries.get("main.pdf") == local_entries.get("exact-euler-tail-partition-normal-form.pdf")
    ...
    return {
        "status": "RH-383_archive_verified" if not failures else "RH-383_archive_failed",
