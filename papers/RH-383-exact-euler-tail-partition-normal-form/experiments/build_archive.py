"""Build the fixed RH-383 publication and dependency manifest."""
...
LOCAL_MEMBERS = (
    ...
    "main.pdf",
    "main.tex",
    "exact-euler-tail-partition-normal-form.pdf",
    "pyproject.toml",
    "references.bib",
    "requirements.txt",
    "results/result.json",
    "results/result.schema.json",
    "src/euler_tail_normal_form/__init__.py",
    "src/euler_tail_normal_form/core.py",
...
def validated_result_source_map(result: dict[str, object]) -> dict[str, str]:
    ...
    if type(entries) is not list or len(entries) != len(SOURCE_FILES):
        raise RuntimeError("result source-lock entries are not the frozen 41 rows")
    paths: list[str] = []
    ...
    if type(source_locks.get("count")) is not int or source_locks.get("count") != len(SOURCE_FILES):
        raise RuntimeError("result source-lock count is not the exact integer 41")
...
def build_payload() -> dict[str, object]:
    ...
    if source_lock_object.get("digest_contract_pass") is not True:
        raise RuntimeError("result did not certify the 41-source digest contract")
    ...
    semantic_pdf = publication["exact-euler-tail-partition-normal-form.pdf"]
    if main_pdf != semantic_pdf:
        raise RuntimeError("semantic PDF is not byte-identical to main.pdf")
    return {
        "status": "RH-383_fixed_publication_manifest",
