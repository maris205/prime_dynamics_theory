"""Publication-manifest and archive mutation tests for RH-383."""
...
def test_archive_membership_contract() -> None:
    ...
    assert len(build_archive.SOURCE_FILES) == len(set(build_archive.SOURCE_FILES)) == 41
...
def test_archive_source_lock_rows_fail_closed() -> None:
    ...
    assert len(build_archive.validated_result_source_map(result)) == 41

    duplicate = copy.deepcopy(result)
    duplicate["source_locks"]["entries"].append(copy.deepcopy(duplicate["source_locks"]["entries"][0]))
    with pytest.raises(RuntimeError, match="frozen 41 rows"):
    ...
    with pytest.raises(RuntimeError, match="exact integer 41"):
...
def test_manifest_verifier_rejects_membership_commit_boolean_and_pdf_mutations() -> None:
    ...
    rebound_commit["source_commits"]["rh382_release"] = "0" * 40
    ...
    semantic_drift = copy.deepcopy(manifest)
    semantic_drift["publication_artifacts"]["exact-euler-tail-partition-normal-form.pdf"] = "0" * 64
    cases.append(semantic_drift)
    for candidate in cases:
        report = verify_archive.verify_manifest(candidate)
        assert report["failure_count"] > 0
        assert report["status"] == "RH-383_archive_failed"
...
def test_archive_full_regeneration_if_present() -> None:
    ...
    assert stored_report["status"] == "RH-383_archive_verified"
    assert stored_report["failure_count"] == 0
    assert stored_report["publication_file_count"] == 29
    assert stored_report["external_input_count"] == 41
