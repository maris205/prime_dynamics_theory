"""Source-lock, closed-schema, and result-ledger tests for RH-383."""
...
from euler_tail_normal_form import (  # noqa: E402
...
def test_result_full_regeneration_and_41_release_blob_locks() -> None:
    ...
    assert locks["count"] == len(SOURCE_FILES) == 41
    assert len(set(SOURCE_FILES)) == 41
    assert [len(SOURCE_GROUPS[group]) for group in SOURCE_GROUPS] == [7, 8, 8, 8, 8, 2]
...
def test_source_commit_membership_path_duplicate_and_digest_mutations_fail() -> None:
    ...
    bad_commits["rh382_release"] = "0" * 40
    ...
    bad_groups["rh382_release"] = bad_groups["rh382_release"][:-1]
    ...
    mutable = copy.deepcopy(entries)
    mutable[0]["path"] = "prime_dynamics_theory/RH_HANDOFF.md"
    with pytest.raises(ValueError, match="mutable"):
        source_digest_lines(mutable)
...
def test_schema_rejects_count_gate_type_and_redundancy_mutations() -> None:
    schema = build_schema()
    payload = load_json(ROOT / "results/result.json")
    mutations = (
        lambda value: value["source_locks"].__setitem__("count", True),
        lambda value: value["certificate"]["counts"].__setitem__("q_signs", 72),
        lambda value: value["certificate"]["sections"]["negative_mutations"].__setitem__("rejected", 19),
        lambda value: value["certificate"]["sections"]["gamma_equivalence"].__setitem__("m2_redundancy_note", "1151 distinct theorems"),
        lambda value: value["gates"].__setitem__("E_completed_zeta_divisor_equality", True),
    )
...
def test_certificate_fixture_and_boundary_language() -> None:
    payload = load_json(ROOT / "results/result.json")
    certificate = verify_certificate()
    assert len(canonical_json_bytes(certificate)) == CERTIFICATE_FIXTURE_BYTES == 12245
    assert payload_sha256(certificate) == CERTIFICATE_FIXTURE_SHA256
    assert payload["certificate_fixture"]["sha256"] == CERTIFICATE_FIXTURE_SHA256
    ...
    assert payload["gates"]["D_von_mangoldt_weighted_prime_power_traces"] is False
    assert payload["gates"]["E_completed_zeta_divisor_equality"] is False
    assert "92*rho_y^(D+1)/(3*pi^2)" in payload["theorem"]["remainder"]


def test_builder_optimized_mode_and_ambient_decimal_context() -> None:
    original_precision = getcontext().prec
    try:
        getcontext().prec = 6
        assert build_payload()["certificate"]["all_pass"] is True
    finally:
    ...
    assert completed.stdout.strip() == "41 True"


def test_json_round_trip_has_no_numeric_alias_or_nonfinite_constant() -> None:
