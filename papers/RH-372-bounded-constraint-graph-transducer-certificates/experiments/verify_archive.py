"""Verify every RH-372 manifest hash and fixed membership count."""
...
# Permit both ``python experiments/verify_archive.py`` and the Makefile's
# ``PYTHONPATH=src:.`` invocation without relying on the caller's cwd.
...
def _verify_map(
    base: Path, entries: object, label: str, failures: list[str]
) -> int:
...
def main() -> None:
    ...
    if manifest.get("status") != "RH-372_fixed_publication_manifest":
        failures.append("manifest:status")

    local_count = _verify_map(
        ROOT, manifest.get("publication_artifacts"), "local", failures
    )
    external_count = _verify_map(
        WORKSPACE, manifest.get("external_inputs"), "external", failures
    )
    ...
    # The manifest is a fixed release contract.  A shortened but internally
    # self-consistent map must not be accepted as a publication archive.
    ...
    payload = {
        "status": "RH-372_archive_verified" if not failures else "RH-372_archive_failed",
