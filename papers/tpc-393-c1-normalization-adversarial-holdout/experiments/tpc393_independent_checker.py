#!/usr/bin/env python3
"""Independent reverse-shell replay for TPC-393.

This file intentionally does not import the TPC-393 producer.  It rebuilds
the finite c=1 matrices in descending prime-shell order and checks the
targeted normalization holdout against the sealed certificate.
"""
            ...
            for mode in BAND_MODES:
                ...
                for norm in NORMALIZATIONS:
                    if norm == "local_diagonal":
                        ...
                        denominator, role = float(geometry.mean()), \
                            f"origin_{origin}_{count}"
                    elif norm == "frozen_train_1024_scalar":
                        denominator, role = frozen, "calibration_1024_frozen"
                    else:
...
def _legacy_check_summary_from_rows(rows: list[dict[str, Any]],
                                    summary: dict[str, Any]) -> None:
    """Retained template code; the active audit uses check_phase_summary."""
    raise Failure("retired TPC-391 helper is not part of TPC-393")
    parents = parent_cells()
    recorded_cells = summary.get("cells", [])
    need(len(recorded_cells) == CELL_COUNT, "recorded cell count")
    recorded_map = {}
    ...
    need(summary.get("row_count") == ROW_COUNT and
         summary.get("cell_count") == CELL_COUNT,
         "summary dimensions")
...
def check_phase_summary(rows: list[dict[str, Any]],
                        summary: dict[str, Any]) -> None:
    """Recompute normalization cells, forecasts, and phase aggregates."""
    definitions = {
        ...
        "origin_scalar": "current-origin mean geometry at each count",
        "frozen_train_1024_scalar":
            "single calibration-origin mean geometry at N=1024 for every count",
    }
    recorded_cells = summary.get("cells", [])
    need(len(recorded_cells) == CELL_COUNT, "recorded phase cell count")
    recorded_map = {}
    for cell in recorded_cells:
        key = (cell.get("band_mode"), cell.get("normalization"),
               cell.get("law"), cell.get("Q"))
        need(key not in recorded_map, "duplicate phase cell")
        recorded_map[key] = cell
    expected_keys = {("fixed_c3", norm, law, q0)
                     for norm in NORMALIZATIONS for law in LAWS
                     for q0 in Q_ANCHORS}
    need(set(recorded_map) == expected_keys, "phase cell keys")

    recomputed = {}
    for norm in NORMALIZATIONS:
        for law in LAWS:
            for q0 in Q_ANCHORS:
                key = ("fixed_c3", norm, law, q0)
                selected = [row for row in rows
                            if (row["band_mode"], row["normalization"],
                                row["law"], row["Q"]) == key]
                selected.sort(key=lambda row: (row["count"], row["origin"]))
                by_count = {
                    count: [float(row["band_spectral"]) for row in selected
                            if row["count"] == count]
                    for count in COUNT_LEVELS
                ...
                stats = {count: finite_stats(by_count[count])
                         for count in COUNT_LEVELS}
                means = {count: stats[count]["mean"] for count in COUNT_LEVELS}
                slope = math.log(means[1280] / means[1024]) / math.log(
                    1280.0 / 1024.0)
                forecast = means[1024] * (1536.0 / 1024.0) ** slope
                error = means[1536] / forecast - 1.0
                cell = recorded_map[key]
                need(cell.get("normalization_definition") == definitions[norm],
                     "normalization definition")
                need(cell.get("origins") == [row["origin"] for row in selected] and
                     cell.get("calibration_origins") ==
                     list(CALIBRATION_ORIGINS) and
                     cell.get("holdout_origins") == list(HOLDOUT_ORIGINS) and
                     cell.get("calibration_counts") ==
                     list(CALIBRATION_COUNTS) and
                     cell.get("holdout_count") == HOLDOUT_COUNT,
                     "phase cell roles")
                for count in COUNT_LEVELS:
                    field = "N1536_holdout" if count == HOLDOUT_COUNT else f"N{count}"
                    stored = cell.get(field, {})
                    need(stored.get("value_count") == stats[count]["value_count"] and
                         stored.get("within_one_percent") ==
                         stats[count]["within_one_percent"],
                    ...
                    for actual, target in zip(by_count[count], values):
                        close(actual, target, field + ".value")
                expected_spectral = {
                    str(count): sum(bool(row["spectral_failure"])
                                    for row in selected if row["count"] == count)
                    for count in COUNT_LEVELS
                }
                expected_schur = {
                    str(count): sum(bool(row["schur_failure"])
                                    for row in selected if row["count"] == count)
                    for count in COUNT_LEVELS
                }
                need(cell.get("spectral_failures_by_count") ==
                     expected_spectral and
                     cell.get("schur_failures_by_count") == expected_schur,
                     "cell failure counts")
                close(slope, cell.get("local_log2_slope"), "local slope")
                close(forecast, cell.get("forecast_1536"), "forecast")
                close(error, cell.get("forecast_error"), "forecast error")
                need(cell.get("within_forecast_cap") ==
                     (abs(error) <= TRANSFER_ERROR_CAP),
                     "forecast flag")
                recomputed[key] = (selected, stats, means)

    local_means = {}
    for key, (_, stats, means) in recomputed.items():
        if key[1] == "local_diagonal":
            for count in COUNT_LEVELS:
                local_means[key[2], key[3], count] = means[count]
    for key, (_, stats, means) in recomputed.items():
        cell = recorded_map[key]
        ratios = cell.get("mean_ratio_to_local")
        need(isinstance(ratios, dict), "local ratio map")
        for count in COUNT_LEVELS:
            actual = means[count] / local_means[key[2], key[3], count]
            close(actual, ratios.get(str(count)),
                  "mean ratio " + key[1] + "." + str(count))

    forecast_passes = {
        norm: sum(bool(recorded_map[("fixed_c3", norm, law, q0)]
                       ["within_forecast_cap"])
                  for law in LAWS for q0 in Q_ANCHORS)
        for norm in NORMALIZATIONS
    }
    forecast_max = {
        norm: max(abs(float(recorded_map[("fixed_c3", norm, law, q0)]
                           ["forecast_error"]))
                  for law in LAWS for q0 in Q_ANCHORS)
        for norm in NORMALIZATIONS
    }
    terminal_means = {
        norm: sum(recomputed[("fixed_c3", norm, law, q0)][2][1536]
                  for law in LAWS for q0 in Q_ANCHORS) / float(
                      len(LAWS) * len(Q_ANCHORS))
        for norm in NORMALIZATIONS
    }
    terminal_order = [
        norm for norm, _ in sorted(terminal_means.items(),
                                   key=lambda item: (-item[1], item[0]))
    ]
    spectral_failures = {
        norm: sum(bool(row["spectral_failure"]) for row in rows
                  if row["normalization"] == norm)
        for norm in NORMALIZATIONS
    }
    schur_failures = {
        norm: sum(bool(row["schur_failure"]) for row in rows
                  if row["normalization"] == norm)
        for norm in NORMALIZATIONS
    }
    stable = {}
    for count in COUNT_LEVELS:
        stable[str(count)] = sum(
            recomputed[("fixed_c3", norm, law, q0)][1][count]
            ["within_one_percent"]
    ...
    need(summary.get("row_count") == ROW_COUNT and
         ...
         summary.get("calibration_counts") == list(CALIBRATION_COUNTS) and
         summary.get("holdout_count") == HOLDOUT_COUNT and
         summary.get("forecast_cap") == format(TRANSFER_ERROR_CAP, ".17g") and
         summary.get("spectral_cap") == format(SPECTRAL_CAP, ".17g") and
         summary.get("schur_cap") == format(SCHUR_CAP, ".17g"),
         "phase summary header")
    need(summary.get("forecast_pass_counts_by_normalization") ==
         forecast_passes, "forecast pass census")
    for norm, value in forecast_max.items():
        close(value, summary.get("forecast_max_abs_error_by_normalization", {})
              .get(norm), "forecast maximum " + norm)
        close(terminal_means[norm],
              summary.get("terminal_mean_by_normalization", {}).get(norm),
              "terminal mean " + norm)
    need(summary.get("terminal_mean_ordering") == terminal_order and
         summary.get("spectral_failures_by_normalization") == spectral_failures and
         summary.get("schur_failures_by_normalization") == schur_failures and
         summary.get("stable_cells") == stable,
         "phase aggregate census")
...
def check_document(document: dict[str, Any], recompute: bool = True) -> dict[str, Any]:
    ...
    need(isinstance(rows, list) and len(rows) == ROW_COUNT,
         "certificate rows")
    ...
    check_phase_summary(rows, summary)
    finite_audit = payload.get("finite_audit", {})
    need(finite_audit.get("rows") == ROW_COUNT and
         finite_audit.get("cell_count") == CELL_COUNT and
         finite_audit.get("calibration_counts") == list(CALIBRATION_COUNTS) and
         finite_audit.get("holdout_count") == HOLDOUT_COUNT and
         finite_audit.get("parent_interface_frozen") is True and
    ...
    need(firewall.get("TPC393_ARITHMETIC_ADVANCE") == "NO" and
         ...
         "NUMERICALLY_CERTIFIED_FINITE_64_ROWS", "firewall")
...
def main() -> int:
    ...
    try:
        ...
        print(f"TPC393_INDEPENDENT_CHECK=PASS rows={ROW_COUNT} cells={CELL_COUNT} "
        ...
        print("TPC393_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
