#!/usr/bin/env python3
"""Mutation firewall for the TPC-393 certificate contract."""
...
import tpc393_independent_checker as independent  # noqa: E402
...
def main() -> int:
    ...
    try:
        ...
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC393_ARITHMETIC_ADVANCE", "YES"))
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC393_FIXED_POWER_CREDIT", 1))
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC393_FULL_GATE_B", "CLOSED"))
        ...
        print("TPC393_STRESS=PASS mutations=25")
        return 0
    except Exception as error:
        print("TPC393_STRESS=FAIL " + str(error), file=sys.stderr)
        return 1
