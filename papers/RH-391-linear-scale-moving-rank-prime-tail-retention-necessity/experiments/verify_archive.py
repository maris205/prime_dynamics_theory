"""Verify RH-391 archive membership, hashes, sources, and replay gates."""
...
def verify_manifest(manifest: object) -> dict[str, object]:
    ...
    expected_keys = {
        ...
        "remote_logical_input_count", "logical_input_total", "remote_source_lock_sha256",
        "logical_source_digest", "source_commits", "offline_remote_replay",
        "remote_payload_hash_count", "publication_payload_hash_hit_count",
        "tree_payload_hash_hit_count", *BOOLEAN_KEYS,
    }
    if set(manifest) != expected_keys:
        failures.append("manifest:top_level_membership")
    if manifest.get("status") != "RH-391_fixed_publication_manifest":
    ...
    if external_count != 97:
        failures.append("manifest:external_git_membership")
    if type(manifest.get("remote_logical_input_count")) is not int or manifest.get("remote_logical_input_count") != 2:
        failures.append("manifest:remote_logical_input_count")
    if type(manifest.get("logical_input_total")) is not int or manifest.get("logical_input_total") != 99:
        failures.append("manifest:logical_input_total")
    ...
    release_pass = digest_pass = logical_pass = certificate_pass = remote_pass = rights_pass = False
    frozen_pass = False
    try:
        ...
        release_pass = git_locks.get("release_identity_pass") is True and len(result_files) == 97
        ...
        rights_pass = remote.get("redistributable_in_release") is False and all(
            row.get("redistributable_in_release") is False and row.get("pdf_vendored") is False
            for row in remote["objects"]
        )
        frozen_pass = type(local_entries) is dict and all(
            local_entries.get(path) == expected for path, expected in FROZEN_STAGE_DIGESTS.items()
        )
    ...
    try:
        ...
        offline_pass = len(offline_rows) == 2 and all(
    ...
    try:
        ...
        payload_excluded = False
        failures.append(f"payload:exclusion:{type(exc).__name__}")
    for key, value in scan.items():
        if type(manifest.get(key)) is not int or manifest.get(key) != value:
    ...
    if scan != {
        ...
        failures.append("payload:hash_scan")

    semantic_match = (
        type(local_entries) is dict
        and local_entries.get("main.pdf") == local_entries.get("linear-scale-moving-rank-prime-tail-retention-necessity.pdf")
        and local_entries.get("main.pdf") == FROZEN_STAGE_DIGESTS["main.pdf"]
    )
    current = {
        ...
        "remote_rights_nonvendor_pass": rights_pass,
    ...
    return {
        "status": "RH-391_archive_verified" if not failures else "RH-391_archive_failed",
        ...
        "remote_logical_input_count": 2 if remote_pass else 0,
        "logical_input_total": 99 if logical_pass else 0,
        **scan,
