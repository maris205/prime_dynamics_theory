...
from moving_rank_necessity.core import exact_equal, loads_strict  # noqa: E402
...
def test_manifest_is_exact_fresh_regeneration() -> None:
    ...
    assert stored["external_git_input_count"] == 97
    assert stored["remote_logical_input_count"] == 2
    assert stored["logical_input_total"] == 99
    assert stored["logical_source_digest"] == source_locks.EXPECTED_LOGICAL_SOURCE_DIGEST
    assert stored["remote_payload_hash_count"] == 4
    assert stored["publication_payload_hash_hit_count"] == 0
    assert stored["tree_payload_hash_hit_count"] == 0
...
def test_archive_verification_is_failure_free() -> None:
    ...
    assert exact_equal(stored, fresh)
    assert stored["status"] == "RH-391_archive_verified"
...
def test_four_external_payloads_are_excluded_from_members_and_tree() -> None:
    ...
    tree_hashes = {sha256(path) for path in ROOT.rglob("*") if path.is_file() and not path.is_symlink()}
...
def test_frozen_stage_digests_are_exact() -> None:
    assert set(build_archive.FROZEN_STAGE_DIGESTS) == {
        "experiments/source_locks.py", "main.log", "main.pdf", "main.tex",
        "references.bib", "results/result.json", "results/result.schema.json",
        "src/moving_rank_necessity/core.py",
    }
    assert all(sha256(ROOT / path) == expected for path, expected in build_archive.FROZEN_STAGE_DIGESTS.items())
...
def test_manifest_hash_membership_and_path_mutations_fail_closed() -> None:
    ...
    assert result["status"] == "RH-391_archive_failed"
...
def test_remote_count_digest_order_logical_and_offline_mutations_fail() -> None:
    ...
    assert verify_archive.verify_manifest(attacked)["failure_count"] > 0
    attacked = deepcopy(manifest)
    attacked["logical_input_total"] = 98
    assert verify_archive.verify_manifest(attacked)["failure_count"] > 0
    attacked = deepcopy(manifest)
...
def test_boolean_stage_payload_and_source_commit_rebinding_fail() -> None:
    manifest = build_archive.build_payload()
    attacked = deepcopy(manifest)
    attacked["semantic_pdf_match"] = 1
    assert verify_archive.verify_manifest(attacked)["failure_count"] > 0
    attacked = deepcopy(manifest)
    attacked["remote_rights_nonvendor_pass"] = 1
    assert verify_archive.verify_manifest(attacked)["failure_count"] > 0
    attacked = deepcopy(manifest)
    attacked["release_stage_file_count"] = True
    assert verify_archive.verify_manifest(attacked)["failure_count"] > 0
    attacked = deepcopy(manifest)
    attacked["tree_payload_hash_hit_count"] = 1
    assert verify_archive.verify_manifest(attacked)["failure_count"] > 0
    attacked = deepcopy(manifest)
    attacked["source_commits"]["rh390_release"] = "0" * 40
    assert verify_archive.verify_manifest(attacked)["failure_count"] > 0
...
def test_nonobject_manifest_is_standard_fail_closed(value: object) -> None:
    ...
    assert result["logical_input_total"] == 0
...
def test_publication_pdfs_are_byte_identical() -> None:
    assert (ROOT / "main.pdf").read_bytes() == (
        ROOT / "linear-scale-moving-rank-prime-tail-retention-necessity.pdf"
    ).read_bytes()
    assert sha256(ROOT / "main.pdf") == build_archive.FROZEN_STAGE_DIGESTS["main.pdf"]
...
def test_outer_archive_verifier_replays_without_writing() -> None:
    manifest = loads_strict((ROOT / "results" / "dependency_manifest.json").read_text(encoding="utf-8"))
    report = verify_archive.verify_manifest(manifest)
    assert report["status"] == "RH-391_archive_verified"
