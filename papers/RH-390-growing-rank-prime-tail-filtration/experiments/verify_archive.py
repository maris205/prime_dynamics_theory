"""Verify RH-390 archive membership, hashes, sources, and replay gates."""
...
from growing_rank_filtration.core import exact_equal, loads_strict, payload_sha256  # noqa: E402
...
def verify_manifest(manifest: object) -> dict[str, object]:
    ...
    if manifest.get("status") != "RH-390_fixed_publication_manifest":
    ...
    semantic_match = (
        ...
        and local_entries.get("main.pdf") == local_entries.get("growing-rank-prime-tail-filtration.pdf")
    )
    current = {
    ...
    return {
        "status": "RH-390_archive_verified" if not failures else "RH-390_archive_failed",
