"""Verify the RH-272--RH-281 batch publication manifest."""
...
def main() -> None:
    ...
    for directory in paper_directories():
        current_paths.extend(publication_files(directory))
    current = {
        str(path.relative_to(PAPERS)): path for path in sorted(current_paths)
    }
    ...
    payload = {
        "status": "rh272_281_batch_archive_verified",
