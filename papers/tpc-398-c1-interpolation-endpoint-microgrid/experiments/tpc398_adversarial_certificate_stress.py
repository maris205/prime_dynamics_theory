#!/usr/bin/env python3
"""Mutation firewall for the TPC-398 interpolation certificate."""
...
import tpc398_independent_checker as independent  # noqa: E402
...
def main() -> int:
    ...
    try:
        ...
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC398_ARITHMETIC_ADVANCE", "YES"))
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC398_FIXED_POWER_CREDIT", 1))
        mutate(lambda d: d["payload"]["claim_firewall"].__setitem__(
            "TPC398_FULL_GATE_B", "CLOSED"))
        ...
        print("TPC398_STRESS=PASS mutations=28")
        return 0
    except Exception as error:
        print("TPC398_STRESS=FAIL " + str(error), file=sys.stderr)
        return 1
