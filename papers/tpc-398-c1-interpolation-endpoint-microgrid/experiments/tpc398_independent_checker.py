#!/usr/bin/env python3
"""Independent reverse-shell replay for TPC-398.

The implementation is intentionally separate from the producer.  It validates
the TPC-397 segment-endpoint baseline, rebuilds the new family in descending
prime order, forms the four exact finite matrix interpolants, and recomputes
all cohort aggregates.
"""
...
def parent_endpoints() -> dict[tuple[str, str, int], float]:
    ...
    need(payload.get("schema") == "TPC397_C1_SIGNED_LAW_INTERPOLATION_V1" and
         payload.get("status") ==
    ...
    for cell in payload.get("origin_summary", {}).get("cells", []):
        ...
        key = (cell["normalization"], endpoint, cell["Q"])
        need(key not in result, "duplicate parent endpoint")
        result[key] = float(cell["all_origin_stats"]["mean"])
    need(set(result) == {
        (norm, law, 8192) for norm in NORMALIZATIONS
...
def parent_baseline() -> dict[tuple[str, str, int], float]:
    ...
    for norm in NORMALIZATIONS:
        start = endpoints[norm, "segment_start", 8192]
        end = endpoints[norm, "segment_end", 8192]
        for law, (numerator, denominator) in LAMBDA_FRACTIONS.items():
            lam = Fraction(numerator, denominator)
            t = (lam - Fraction(*SEGMENT_START)) / (
                Fraction(*SEGMENT_END) - Fraction(*SEGMENT_START))
            result[norm, law, 8192] = ((1.0 - float(t)) * start +
                                       float(t) * end)
    return result
...
def check_summary(rows: list[dict[str, Any]], summary: dict[str, Any]) -> None:
    ...
    for norm in NORMALIZATIONS:
        for law in LAWS:
            ...
            coefficient = Fraction(*LAMBDA_FRACTIONS[law])
            t = (coefficient - Fraction(*SEGMENT_START)) / (
                Fraction(*SEGMENT_END) - Fraction(*SEGMENT_START))
            ...
            need(cell.get("origins") == [r["origin"] for r in selected] and
                 ...
                 cell.get("parent_family") == "TPC397" and
                 cell.get("interpolation_lambda") ==
                 f"{LAMBDA_FRACTIONS[law][0]}/{LAMBDA_FRACTIONS[law][1]}" and
                 cell.get("parent_segment_coordinate") ==
                 f"{t.numerator}/{t.denominator}",
                 "cell protocol")
            endpoint_means = cell.get("parent_segment_endpoint_means", {})
            close(endpoints[norm, "segment_start", 8192],
                  endpoint_means.get("blend_3_4"), "parent segment start")
            close(endpoints[norm, "segment_end", 8192],
                  endpoint_means.get("blend_1"), "parent segment end")
    ...
    need(summary.get("spectral_failures_by_normalization") == spectral and
         ...
         "summary totals")
    need(summary.get("parent_family") == "TPC397" and
         summary.get("parent_interpolation") ==
         "linear interpolation of frozen TPC397 blend_3_4 and blend_1 means "
         "on the segment lambda in [3/4,1]", "summary parent contract")
...
def check_document(document: dict[str, Any], recompute: bool = True) -> None:
    ...
    need(lock.get("parent_code_sha256") == PARENT_CODE_SHA256 and
         ...
         lock.get("parent_schema") == "TPC397_C1_SIGNED_LAW_INTERPOLATION_V1" and
         lock.get("parent_status") ==
         "NUMERICALLY_CERTIFIED_FINITE_C1_INTERPOLATION_TRANSITION_REPLICATION_AUDIT" and
         lock.get("parent_endpoint_laws") == ["blend_3_4", "blend_1"] and
         lock.get("parent_segment") == "lambda in [3/4,1]" and
...
def main() -> int:
    ...
    try:
        ...
        print(f"TPC398_INDEPENDENT_CHECK=PASS rows={ROW_COUNT} cells={CELL_COUNT} "
        ...
        print("TPC398_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
