#!/usr/bin/env python3
"""Independent reverse-shell replay for TPC-341.
...
def main() -> int:
    try:
        ...
        need(len(all_raw) == 108 and
             sum(item["source_l2"] > 0 for item in all_raw) == 90 and
        ...
        need(summary["rank_failures"] == 0 and summary["rank_values"] == [2, 3]
             and summary["raw_records"] == 108 and
             summary["nonempty_raw_records"] == 90 and
        ...
        need(firewall["TPC341_ARITHMETIC_ADVANCE"] == "NO" and
             firewall["TPC341_FIXED_POWER_CREDIT"] == 0 and
             firewall["TPC341_CONTROL_STABILITY"] == "REFUTED_SCOPED" and
             firewall["TPC341_FULL_GATE_B"] == "OPEN", "firewall")
        print("TPC341_INDEPENDENT_CHECK=PASS rows=3 controls=9 raw_records=108 "
              "holdout_records=27 rank_failures=0 reverse_shell=1 exact_anchor=1")
        return 0
    except (Failure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError) as error:
        print("TPC341_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
