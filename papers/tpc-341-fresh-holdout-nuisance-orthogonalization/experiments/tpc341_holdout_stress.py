#!/usr/bin/env python3
"""Fail-closed mutation stress for TPC-341."""
...
RESULT = Path(__file__).resolve().parents[1] / "results/tpc341_certificate.json"
...
def validate(document: dict[str, Any]) -> None:
    ...
    need(summary.get("rank_failures") == 0 and
         ...
         summary.get("rank_values") == [2, 3], "summary census")
    ...
    need(firewall.get("TPC341_ARITHMETIC_ADVANCE") == "NO" and
         firewall.get("TPC341_CONTROL_STABILITY") == "REFUTED_SCOPED" and
         firewall.get("TPC341_FIXED_POWER_CREDIT") == 0 and
         firewall.get("TPC341_FULL_GATE_B") == "OPEN", "firewall")
...
def main() -> int:
    try:
        ...
        item = copy.deepcopy(original)
        item["payload"]["claim_firewall"]["TPC341_ARITHMETIC_ADVANCE"] = "YES"
        reseal(item)
        mutations.append(item)
        ...
        print("TPC341_STRESS=PASS mutations=6 rejected=6 rank_guard=1 "
              "holdout_guard=1 cutoff_guard=1 semantic_guards=3")
        return 0
    except (Failure, OSError, ValueError, TypeError, KeyError,
            json.JSONDecodeError) as error:
        print("TPC341_STRESS=FAIL " + str(error), file=sys.stderr)
        return 1
