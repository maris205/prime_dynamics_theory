...
TPC-331 showed an exact mean/centered identity on two held-out source windows.
This release moves the same five-control orbit to a disjoint two-origin,
three-scale ensemble.  It asks the hostile replication question: which parts
of the finite response survive a growing source ensemble, and which signs do
not?  For the literal deleted-diagonal prime-shell blocks B_p, it forms
...
contribution.  For w_j=P_j v, wbar=mean_j w_j and z_j=w_j-wbar, the exact
...
try:
    ...
    raise SystemExit("TPC332 requires numpy: " + str(error))
...
RESULT = PROJECT / "results/tpc332_certificate.json"

PARENT_PROJECT = ROOT / "papers/tpc-331-control-average-centered-response-decomposition"
PARENT_CODE = PARENT_PROJECT / (
    "code/tpc331_control_average_centered_response_decomposition.py")
PARENT_RESULT = PARENT_PROJECT / "results/tpc331_certificate.json"
PARENT_CODE_SHA256 = (
...
def load_parent() -> tuple[dict[str, Any], dict[str, Any]]:
    need(PARENT_CODE.is_file() and
         ...
         "TPC331 producer provenance")
    need(PARENT_RESULT.is_file() and
         digest(PARENT_RESULT.read_bytes()) == PARENT_RESULT_SHA256,
         "TPC331 certificate provenance")
    raw = PARENT_RESULT.read_bytes()
    document = json.loads(raw)
    need(raw == canonical(document), "TPC331 certificate canonicality")
    need(document.get("certificate_version") == 1 and
         document.get("claim_status") ==
         "NUMERICALLY_CERTIFIED_FINITE_CONTROL_AVERAGE_CENTERED_RESPONSE_DECOMPOSITION",
         "TPC331 certificate header")
    payload = document.get("payload")
    need(isinstance(payload, dict) and
         payload.get("schema") ==
         "TPC331_CONTROL_AVERAGE_CENTERED_RESPONSE_DECOMPOSITION_V1",
         "TPC331 payload type")
    ...
    need(protocol.get("origins") == [28001, 36001] and
         ...
         protocol.get("placement_rule") == PLACEMENT_RULE,
         "TPC331 protocol lock")
...
def row_record(origin: int, scale: int, q0: int, exponent: int
    ...
    source_lambda_l2 = float(np.dot(lam, lam))
    source_comparison_l2 = float(np.dot(comp, comp))
    source_cross = float(np.dot(lam, comp))
    source_residual_l2 = float(np.dot(residual, residual))
    source_identity_error = abs(
        source_residual_l2 - (source_lambda_l2 + source_comparison_l2 -
                              2.0 * source_cross))
    need(all(math.isfinite(item) and item >= 0.0 for item in (
        source_lambda_l2, source_comparison_l2, source_cross,
        source_residual_l2, source_identity_error)),
         "finite source L2 decomposition")
    source_metrics = {
        "lambda_l2": display(source_lambda_l2),
        "comparison_l2": display(source_comparison_l2),
        "residual_l2": display(source_residual_l2),
        "lambda_comparison_inner_product": display(source_cross),
        "residual_identity_error": display(source_identity_error),
        "lambda_l2_per_source": display(source_lambda_l2 / len(values)),
        "comparison_l2_per_source": display(source_comparison_l2 /
                                              len(values)),
        "residual_l2_per_source": display(source_residual_l2 / len(values)),
        "normalized_cross_correlation": display(
            source_cross / math.sqrt(source_lambda_l2 * source_comparison_l2)),
    }
    decomposition = {
...
def source_ensemble_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    """Summarise source-native norms and the nested scale ladder."""
    expected_per_window = len(Q_ANCHORS) * len(EXPONENTS)
    windows: list[dict[str, Any]] = []
    for origin in ORIGINS:
        for scale in SCALES:
            selected = [row for row in rows
                        if row["origin"] == origin and row["scale"] == scale]
            need(len(selected) == expected_per_window, "source window census")
            source_records = [row["source_l2"] for row in selected]
            ...
            for record in source_records[1:]:
                for key in ("lambda_l2", "comparison_l2", "residual_l2",
                    ...
                    need(abs(float(record[key]) - float(first[key])) < 1.0e-8,
                         "source metrics depend on operator row")
            windows.append({
                "origin": origin,
                "scale": scale,
                "source_count": scale // 2,
                "rows": len(selected),
                "lambda_l2": first["lambda_l2"],
                "comparison_l2": first["comparison_l2"],
                "residual_l2": first["residual_l2"],
                "lambda_comparison_inner_product":
                    first["lambda_comparison_inner_product"],
                "lambda_l2_per_source": first["lambda_l2_per_source"],
                "comparison_l2_per_source":
                    first["comparison_l2_per_source"],
                "residual_l2_per_source": first["residual_l2_per_source"],
                "normalized_cross_correlation":
                    first["normalized_cross_correlation"],
                "max_identity_error": display(max(
                    float(record["residual_identity_error"])
                    for record in source_records)),
            })

    growth_pairs: list[dict[str, Any]] = []
    for origin in ORIGINS:
        for small_scale, large_scale in zip(SCALES, SCALES[1:]):
            small = next(item for item in windows
                         if item["origin"] == origin and
                         item["scale"] == small_scale)
            large = next(item for item in windows
                         if item["origin"] == origin and
                         item["scale"] == large_scale)
            residual_growth = (float(large["residual_l2"]) /
                               float(small["residual_l2"]))
            lambda_growth = (float(large["lambda_l2"]) /
                             float(small["lambda_l2"]))
            comparison_growth = (float(large["comparison_l2"]) /
                                 float(small["comparison_l2"]))
            growth_pairs.append({
                "origin": origin,
                "small_scale": small_scale,
                "large_scale": large_scale,
                "residual_l2_growth": display(residual_growth),
                "lambda_l2_growth": display(lambda_growth),
                "comparison_l2_growth": display(comparison_growth),
                "residual_log2_slope": display(math.log(
                    residual_growth, 2.0)),
                "residual_per_source_ratio": display(
                    float(large["residual_l2_per_source"]) /
                    float(small["residual_l2_per_source"])),
            })

    source_identity_errors = [float(row["source_l2"]["residual_identity_error"])
                              for row in rows]
    all_plus = [row["laws"]["all_plus"] for row in rows]
    return {
        "windows": windows,
        "growth_pairs": growth_pairs,
        "source_l2_observations": len(windows),
        "max_source_identity_error": display(max(source_identity_errors)),
        "all_plus_actual_census": {
            label: sum(item["classification"] == label for item in all_plus)
            for label in LABELS
        },
        "all_plus_ratio_min": display(min(float(item["ratio"])
                                           for item in all_plus)),
        "all_plus_ratio_max": display(max(float(item["ratio"])
                                           for item in all_plus)),
    }


def build_payload(parent_payload: dict[str, Any]) -> dict[str, Any]:
    """Build the TPC-332 certificate from a disjoint growing ensemble."""
    rows = [row_record(origin, scale, q0, exponent)
            for origin in ORIGINS for scale in SCALES
            for q0 in Q_ANCHORS for exponent in EXPONENTS]
    expected_rows = len(ORIGINS) * len(SCALES) * len(Q_ANCHORS) * len(EXPONENTS)
    need(len(rows) == expected_rows == 48, "TPC332 row census")
    ...
    source_summary = source_ensemble_summary(rows)
    expected_census = {
        law: summary["by_law"][law]["component_census"]
        for law in LAW_NAMES
    }
    need(all(item["classification"] != "UNRESOLVED"
             for row in rows for item in row["laws"].values()),
         "residual law separation")
    ...
    need(float(source_summary["max_source_identity_error"]) < 1.0e-8,
         "source L2 identity residual")

    return {
        "schema": SCHEMA,
        "parent_lock": {
            "TPC331_producer_sha256": PARENT_CODE_SHA256,
            "TPC331_certificate_sha256": PARENT_RESULT_SHA256,
        },
        "protocol": {
            "origins": list(ORIGINS),
            "scales": list(SCALES),
            ...
            "laws": list(LAW_NAMES),
            "ensemble_rule": (
                "two disjoint origins; three nested source scales; all windows "
                "remain below the declared 50000 tail cutoff"),
        },
        "exact_theorem": {
        ...
        "source_ensemble_audit": source_summary,
        "exact_anchor": exact_anchor(),
        "claim_firewall": {
...
def check_certificate(parent_payload: dict[str, Any]) -> None:
    ...
    need(raw == canonical(stored), "TPC332 certificate canonicality")
    need(stored == build_document(parent_payload),
         "TPC332 certificate does not replay")
    print("TPC332_CERTIFICATE=PASS rows=48 origins=2 scales=3 laws=4 "
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            ...
            print("TPC332_CERTIFICATE=WRITTEN")
        else:
        ...
        print("TPC332_CERTIFICATE=FAIL " + str(error), file=sys.stderr)
        return 1
    return 0
