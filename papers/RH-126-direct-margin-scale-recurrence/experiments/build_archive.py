...
def main() -> None:
    ...
    publications = [ROOT / name for name in (
        ...
        "pyproject.toml", "requirements.txt", "figures/direct_margin_scale_recurrence.pdf",
        "figures/direct_margin_scale_recurrence.png", "main.pdf", "direct-margin-scale-recurrence.pdf",
    )]
    dependency = {
        "status": "all_rh126_inputs_sources_and_publication_artifacts_hashed",
    ...
    audit = json.loads((ROOT / "results" / "direct_margin_audit.json").read_text(encoding="utf-8"))
    results = [ROOT / "results" / name for name in ("direct_margin_audit.json", "direct_margin_smoke.json", "dependency_manifest.json")]
    summary = {
        "status": "rh126_direct_margin_scale_recurrence_archived",
        "theorem": {"sharp_direct_margin_recurrence": True, "optimal_scalar_profile_fit": True},
