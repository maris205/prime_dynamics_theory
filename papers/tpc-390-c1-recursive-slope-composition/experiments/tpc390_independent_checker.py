#!/usr/bin/env python3
"""Independent reverse-shell replay for TPC-390.

This file intentionally does not import the TPC-390 producer.  It rebuilds
the finite c=1 matrices in descending prime-shell order and checks the
recursive-composition audit against the sealed certificate.
"""
...
def check_summary_from_rows(rows: list[dict[str, Any]],
    ...
    need(set(recorded_map) == {
        (mode, norm, law, q0) for mode in BAND_MODES
        for norm in NORMALIZATIONS for law in LAWS for q0 in Q_ANCHORS
    }, "summary cell keys")
    parent_errors = []
    local_errors = []
    recursive_errors = []
    composition_errors = []
    stable = {"1024": 0, "1280": 0, "1536_holdout": 0}
        ...
        for norm in NORMALIZATIONS:
            for law in LAWS:
                for q0 in Q_ANCHORS:
                    ...
                    stage1_prediction = means[1024] * (1280.0 / 1024.0) ** parent_alpha
                    recursive_prediction = stage1_prediction * (1536.0 / 1280.0) ** parent_alpha
                    direct_prediction = means[1024] * (1536.0 / 1024.0) ** parent_alpha
                    errors = {
                        "parent_horizon_error": means[1536] / parent_prediction - 1.0,
                        "local_control_error": means[1536] / local_prediction - 1.0,
                        "recursive_parent_error": means[1536] / recursive_prediction - 1.0,
                    }
                    composition_error = recursive_prediction / direct_prediction - 1.0
                    cell = recorded_map[key]
                    need(cell.get("calibration_counts") == [1024, 1280] and
                         cell.get("holdout_count") == 1536,
                         "cell count roles")
                    for count, field in ((1024, "N1024"), (1280, "N1280"),
                                         (1536, "N1536_holdout")):
                        stored = cell.get(field, {})
                        need(stored.get("value_count") == stats[count]["value_count"] and
                             stored.get("within_one_percent") == stats[count]["within_one_percent"],
                             field + " census")
                        ...
                        need(isinstance(values, list) and len(values) == len(by_count[count]),
                             field + ".values")
                        for actual, target in zip(by_count[count], values):
                            close(actual, target, field + ".value")
                        if count == 1024 and stats[count]["within_one_percent"]:
                            stable["1024"] += 1
                        if count == 1280 and stats[count]["within_one_percent"]:
                            stable["1280"] += 1
                        if count == 1536 and stats[count]["within_one_percent"]:
                            stable["1536_holdout"] += 1
                    for name, actual in (
                            ("parent_horizon_log2_slope", parent_alpha),
                            ("local_horizon_log2_slope", local_alpha),
                            ("parent_prediction_N1536_from_N1280", parent_prediction),
                            ("local_prediction_N1536_from_N1280", local_prediction),
                            ("parent_stage1_prediction_N1280_from_N1024", stage1_prediction),
                            ("recursive_parent_prediction_N1536_from_N1024", recursive_prediction),
                            ("direct_recursive_prediction_N1536_from_N1024", direct_prediction),
                            ("recursive_composition_error", composition_error),
                            ("parent_holdout_to_prediction_ratio", 1.0 + errors["parent_horizon_error"]),
                            ("local_holdout_to_prediction_ratio", 1.0 + errors["local_control_error"]),
                            ("recursive_parent_holdout_to_prediction_ratio", 1.0 + errors["recursive_parent_error"]),
                            ("parent_horizon_error", errors["parent_horizon_error"]),
                            ("local_control_error", errors["local_control_error"]),
                            ("recursive_parent_error", errors["recursive_parent_error"])):
                        close(actual, cell.get(name), name)
                    need(cell.get("within_parent_horizon_cap") ==
                         (abs(errors["parent_horizon_error"]) <= TRANSFER_ERROR_CAP) and
                         cell.get("within_local_control_cap") ==
                         (abs(errors["local_control_error"]) <= TRANSFER_ERROR_CAP) and
                         cell.get("within_recursive_parent_cap") ==
                         (abs(errors["recursive_parent_error"]) <= TRANSFER_ERROR_CAP),
                         "cell pass flags")
                    parent_errors.append(abs(errors["parent_horizon_error"]))
                    local_errors.append(abs(errors["local_control_error"]))
                    recursive_errors.append(abs(errors["recursive_parent_error"]))
                    composition_errors.append(abs(composition_error))
                    for row in selected:
    ...
    need(summary.get("parent_horizon_pass_count") == sum(
        abs(float(cell["parent_horizon_error"])) <= TRANSFER_ERROR_CAP
        for cell in recorded_cells) and
         summary.get("local_control_pass_count") == sum(
        abs(float(cell["local_control_error"])) <= TRANSFER_ERROR_CAP
        for cell in recorded_cells) and
         summary.get("recursive_parent_pass_count") == sum(
        abs(float(cell["recursive_parent_error"])) <= TRANSFER_ERROR_CAP
        for cell in recorded_cells), "pass census replay")
    close(max(parent_errors), summary.get("parent_horizon_max_abs_error"),
          "parent max error")
    close(max(local_errors), summary.get("local_control_max_abs_error"),
          "local max error")
    close(max(recursive_errors), summary.get("recursive_parent_max_abs_error"),
          "recursive max error")
    close(max(composition_errors), summary.get("recursive_composition_max_abs_error"),
          "composition max error")
...
def check_document(document: dict[str, Any], recompute: bool = True) -> dict[str, Any]:
    ...
    expected = {(o, n, q, law, norm, mode)
                for o in CALIBRATION_ORIGINS for n in (1024, 1280)
                for q in Q_ANCHORS for law in LAWS for norm in NORMALIZATIONS
                for mode in BAND_MODES}
    expected |= {(o, 1536, q, law, norm, mode)
    ...
    for key, row in row_map.items():
        ...
        role = f"calibration_{count}" if origin in CALIBRATION_ORIGINS else "holdout_1536"
    ...
    need(summary.get("row_count") == 256 and summary.get("cell_count") == 32 and
         ...
         summary.get("parent_horizon_max_abs_error") == "0.03633754623843255" and
         summary.get("local_control_max_abs_error") == "0.025804438647033412" and
         summary.get("recursive_parent_max_abs_error") == "0.049074165168337847" and
    ...
    need(sum(int(v.get("spectral", -1)) for v in failures.values()) == 64 and
         sum(int(v.get("schur", -1)) for v in failures.values()) == 0 and
         set(failures) == {f"{mode}_{norm}" for mode in BAND_MODES
                            for norm in NORMALIZATIONS} and
         all(type(v.get("spectral")) is int and v["spectral"] >= 0 and
             type(v.get("schur")) is int and v["schur"] >= 0
             for v in failures.values()), "failure census")
    need(summary.get("stable_cells") ==
         {"1024": 28, "1280": 25, "1536_holdout": 26},
         "stability census")
    ...
    need(firewall.get("TPC390_ARITHMETIC_ADVANCE") == "NO" and
         ...
         firewall.get("TPC390_TWIN_PRIME_RESULT") == "NONE", "firewall")
    anchor = payload.get("exact_anchor", {})
    need(anchor.get("interval") == [3000001, 3000014] and
...
def main() -> int:
    ...
    try:
        ...
        print("TPC390_INDEPENDENT_CHECK=PASS rows=256 cells=32 "
        ...
        print("TPC390_INDEPENDENT_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
