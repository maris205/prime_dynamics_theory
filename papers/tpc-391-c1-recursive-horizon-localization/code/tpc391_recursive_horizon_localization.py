...
TPC-390 found a finite recursive forecast failure on a fourth disjoint family.
This release freezes the TPC-390 slope interface, adds two intermediate
horizons, and separates direct parent, local-control, and recursive forecast
errors along a predeclared count trajectory.  Everything here is a c=1 proxy
...
RESULT = PROJECT / "results/tpc391_certificate.json"
PARENT_CODE = ROOT / (
    "papers/tpc-390-c1-recursive-slope-composition/code/"
    "tpc390_recursive_slope_composition.py")
PARENT_CERT = ROOT / (
    "papers/tpc-390-c1-recursive-slope-composition/results/"
    "tpc390_certificate.json")
PARENT_CODE_SHA256 = (
    "ec9cc88a9b05a7561fc0f8fee41352c6639d3990ae593e1affc91a079ad7e144")
PARENT_CERT_SHA256 = (
...
CLAIM_FIREWALL = {
    ...
    "TPC391_HORIZON_PANEL":
        "NUMERICALLY_CERTIFIED_FINITE_448_ROWS",
    "TPC391_PARENT_HORIZON_TRANSFER":
        "NUMERICALLY_CERTIFIED_FINITE_SCOPED",
    "TPC391_LOCAL_CONTROL": "NUMERICALLY_CERTIFIED_FINITE_SCOPED",
    "TPC391_HORIZON_TRAJECTORY":
        "NUMERICALLY_CERTIFIED_FINITE_SCOPED",
    "TPC391_RECURSIVE_COMPOSITION":
...
def transfer_summary(rows: list[dict[str, Any]],
    ...
    return {
        "cells": cells, "cell_count": 32, "row_count": 448,
...
def build_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "parent_lock": {
            ...
            "parent_schema": "TPC390_C1_RECURSIVE_SLOPE_COMPOSITION_V1",
            "parent_round2_clue": "LOCALIZE_C1_RECURSIVE_HORIZON_OBSTRUCTION",
        ...
        "protocol": {
            ...
            "parent_transfer_definition":
                "current-family N=1024 mean times (N/1024) to frozen parent TPC390 slope",
            "local_control_definition":
                "current-family N=1024 mean times (N/1024) to current-family 1024 -> 1280 slope",
            "recursive_parent_definition":
                "for N >= 1280, current-family N=1024 mean, staged through N=1280, using frozen parent TPC390 slope",
            "direct_recursive_definition":
                "same parent slope applied once from N=1024 to each N >= 1280 as a composition identity control",
            "horizon_localization_definition":
                "first predeclared horizon whose absolute parent forecast error exceeds 0.03",
        ...
        "finite_audit": {
            "rows": 448, "cell_count": 32, "origin_count": 5,
            "calibration_origin_count": 3, "holdout_origin_count": 2,
            "calibration_count_levels": 4, "holdout_count_levels": 1,
            "calibration_counts": list(CALIBRATION_COUNTS),
            "holdout_count": HOLDOUT_COUNT,
            "horizon_levels": list(HORIZON_LEVELS),
...
def main() -> int:
    ...
    try:
        ...
        if args.write:
            ...
            print("TPC391_CERTIFICATE=WRITTEN")
        else:
            ...
            print("TPC391_CERTIFICATE=PASS rows=448 cells=32 "
        ...
        print("TPC391_CERTIFICATE=FAIL " + str(error), file=sys.stderr)
        return 1
