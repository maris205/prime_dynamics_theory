...
## V243 / TPC-390 current anchor

更新时间：2026-09-05

当前入口：proof 为
papers/tpc-390-c1-recursive-slope-composition/PROOF_PACKAGE.md，checker 为
research/tpc-big-road/tpc_bridge_b_tpc390_c1_recursive_slope_composition_checker.py，
编号论文为 papers/tpc-390-c1-recursive-slope-composition/。

TPC-390 在第四个 fresh coordinate-disjoint origin family
`(3000001,3004011,3008021,3012031,3016041)` 上冻结 TPC-389 的 32 个
cell-wise horizon slopes，并测试第二次 recursive composition。前三个 origins
在 `N=1024,1280` calibration，后两个在 `N=1536` holdout；256-row、32-cell
panel 的 one-step parent、local-control、recursive parent forecasts 分别为
`30/32`、`32/32`、`23/32`，最大误差分别为 `0.03633754623843255`、
`0.025804438647033412`、`0.049074165168337847`。两阶段与 direct power
composition residual 仅为 `3.3306690738754696e-16`；stability 为
`28/32,25/32,26/32`，spectral failures 为 `64/256`，Schur failures 为
`0/256`。这是有限 recursive-horizon obstruction audit，不能升级为
source-valid、origin/count-uniform theorem 或 arithmetic result。
...
## V242 / TPC-389 previous anchor

更新时间：2026-09-05

当前入口：proof 为
papers/tpc-389-c1-long-horizon-slope-stress/PROOF_PACKAGE.md，checker 为
research/tpc-big-road/tpc_bridge_b_tpc389_c1_long_horizon_slope_stress_checker.py，
编号论文为 papers/tpc-389-c1-long-horizon-slope-stress/。

TPC-389 在第三个 fresh coordinate-disjoint origin family
`(2800001,2804011,2808021,2812031,2816041)` 上冻结 TPC-388 的 32 个
cell-wise slopes，并把 horizon 从 `N=1024` 推到 `N=1280`。前三个 origins
在 `N=768,1024` 作 calibration，后两个在 `N=1280` 作 holdout；256-row、32-cell
panel 的 anchored parent、local-control 与 recursive parent forecasts 均为
`32/32`，最大误差分别为 `0.017615584096739245`、`0.011997515978539264`、
`0.029949940590637381`。stability census 为 `24/32,27/32,24/32`，64/256
rows 超过 inherited `0.64` spectral diagnostic，Schur failures 为 `0/256`。
这是有限 long-horizon stress certificate，不能升级为 source-valid、origin/count-
uniform theorem 或 arithmetic result。
...
`N=512,768` calibration ladder 与两个 `N=1024` holdout origins。固定
fixed-three-block/full-relative 两种 band mode、`Q=2048,8192`、四个 laws 与
两种 normalization，形成 256-row、32-cell panel。calibration stability 为
`24/32`（N=512）与 `24/32`（N=768），holdout stability 为 `28/32`；32 个
calibration-slope endpoint forecasts 全部通过 3% finite cap，最大绝对误差为
`0.026051162042932119`。40/256 rows 超过 inherited `0.64` spectral diagnostic，
...
`(2000001,2004011,2008021,2012031,2016041)` 上预先划分三个 calibration 与两个
holdout，固定 `N=512`、`c=2,3`、`Q=2048,8192`、四个 laws 与两种 normalization，
形成 160-row、32-cell panel。all-plus `Q=8192` 的四个 holdout forecast errors
均不超过 `2.4194960054838229e-5`；最大 signed holdout spread 为
`0.033223638943350384`。这是有限 origin transfer 与 law obstruction。
...
当前入口：proof 为
research/tpc-big-road/bridge_b_tpc379_c1_crossholdout_law_control.md，checker 为
tpc_bridge_b_tpc379_c1_crossholdout_law_control_checker.py，编号论文为
papers/tpc-379-c1-crossholdout-law-control/。

TPC-379 在新的 coordinate-disjoint affine grid 上冻结 indices `(0,20,40)`，对应
origins `(1200001,1208021,1216041)`，并在 `N=1024`、四个 256-point blocks、
`Q=512,2048,8192`、beta `2`、exponent `1`、同一个 `c=1` mask 和 common
square-energy geometry 下比较四个预声明 laws。完整 36-row panel 中，all-plus
profile 为 `(0,3,3)`（6/9），三个 signed controls 均为 `(0,0,0)`；总计
6/36 spectral failures，Schur failures 为 0/36。

这是有限 response-blind law-control obstruction：all-plus high-Q signature 在三个
signed controls 中全部消失，故不能把这个 signature 提升为 law-invariant operator
property。它不选择 arithmetic law，也不构成 law/origin/window uniformity、cross-block
causality、growing operator bound、source-uniform arithmetic `L2`、fixed-power credit
或 twin-prime endpoint。官方 evaluator files 仍缺失，local Bridge-B 只作 fail-closed
repository evidence；`ARITHMETIC_ADVANCE=NO`、`FIXED_POWER_CREDIT=0`、
...
## V231 / TPC-378 previous anchor

## V230 / TPC-377 previous anchor
...
TPC-375 是当前位置：承接 TPC-374 的 near-block band，冻结同一 count-2048
full-window normalization，并在完整 9-row beta=2/all-plus 面板上比较预声明
block-distance cutoffs `c=0,1,2,3`。spectral failure census 为 `0,6,6,6`，
其中 `c=1` 首次精确复现 parent 的六个高-Q failure keys；四个 cutoff 的 beta=2
Schur failures 均为 `0/9`。这是 finite minimal-cutoff census，不是全局 bandwidth
optimum、causality、uniform theorem 或 asymptotic transfer。

继承 exact anchor `[1010346,1010359)` 仍由 exact rational geometry 复核，未用于 main
panel 选择。producer、独立 descending-shell replay、24-mutation stress、2-page PDF 与
local Bridge-B 均纳入 package；official evaluator files absent，local Bridge-B 仍为
fail-closed fallback。`ARITHMETIC_ADVANCE=NO`、`FIXED_POWER_CREDIT=0`、`FULL_GATE_B=OPEN`；
下一关固定 `c=1`，在新 origin/window holdout 上测试 bandwidth stability。
...
当前入口：proof 为
research/tpc-big-road/bridge_b_tpc372_full_window_offblock_decomposition.md，checker 为
...
`||R||_2 >= ||T||_2-||D||_2` 下界均为正，最大为 `0.19398264343312976`。这说明该
有限面板上的 excess 需要 common-normalization 的 sum/coherence，但不构成 off-block
因果归因。

继承的 exact anchor `[1010346,1010359)` 由 exact rational geometry 复核，未用于 main
panel 选择。producer、独立 descending-shell replay、33-mutation stress、PDF 与 local
Bridge-B 均纳入 package；official evaluator files absent，local Bridge-B 仍为 fail-closed
fallback。`ARITHMETIC_ADVANCE=NO`、`FIXED_POWER_CREDIT=0`、`FULL_GATE_B=OPEN`；下一关
固定同一 full-window normalization，做 extremal eigenmode 的 block-separation audit。
...
panel 选择。producer、独立 descending-shell replay、36-mutation stress、PDF 与 local
Bridge-B 均纳入 package；official evaluator files absent，local Bridge-B 仍为 fail-closed
fallback。`ARITHMETIC_ADVANCE=NO`、`FIXED_POWER_CREDIT=0`、`FULL_GATE_B=OPEN`；下一关
保持 full-window normalization，做固定 block-diagonal/off-block decomposition。
...
## V223 / TPC-370 previous anchor

上一入口：proof 为
research/tpc-big-road/bridge_b_tpc370_count_2048_window_audit.md，checker 为
tpc_bridge_b_tpc370_count_2048_window_audit_checker.py，编号论文为
papers/tpc-370-count-2048-window-audit/。

TPC-370 是上一地图位置：承接 TPC-369 的第三个 response-blind origin family，只把窗口
count 改为 `2048`，保留 origins `(1010001,1018021,1026041)`、`Q=512,2048,8192`、
exponent `1`、四种 sign laws 与 beta `0,2`，完成 `72` 个全真谱 rows。beta=2 在三个
origins 的 `Q=2048,8192`、all-plus rows 上仍有 6 个 spectral violations，`Q=512`
无 violation，36 个 beta=2 rows 无 Schur violation；beta=0 对照有 9 个 spectral 与 9
个 Schur violations。去掉 count 坐标后，六键 origin/`Q`/law support 与 TPC-369
parent 一致，但最大谱由 `0.67410489800609708` 升至 `0.71099989528234753`，有限差
为 `0.036894997276250452`，所以 support persistence 与 magnitude stability 分开记。

继承 exact anchor `[1010346,1010359)` 由 exact rational geometry 复核。该结果仍是
finite scoped evidence，不建立 origin/window uniformity、source-valid normalization、
growing operator bound、source-uniform arithmetic `L2`、prime-shell reassembly、
fixed-power credit、Route-B closure 或 twin-prime result；`ARITHMETIC_ADVANCE=NO`、
`FIXED_POWER_CREDIT=0`、`FULL_GATE_B=OPEN`。下一关是 count-2048 origin/residue/high-Q
phase localization。
...
当前入口：proof 为
research/tpc-big-road/bridge_b_tpc367_predeclared_long_window_obstruction.md，checker 为
...
TPC-367 将 TPC-366 的 beta=2 higher-`Q` 测试改成预声明 origins，并把窗口 count 从
`512` 扩展到 `1024`。在 candidate grid `620001+307j` 的 indices `(0,20,40)` 上固定
origins `(620001,626141,632281)`，不使用 response、source 或 geometry score；随后对
beta `0,2`、counts `512,1024`、`Q=512,2048,8192`、exponents `1,2` 与四种 sign laws
完成 `288` 个全真谱 rows。beta=2 在 count=512 的全部 rows 通过 spectral cap，且全
`144` 行无 Schur violation；但 count=1024 在 `Q=2048,8192` 的 all-plus/exponent-1
rows 出现 `6` 个 spectral-cap violations，最大谱为 `0.67410738070824539`。beta=0
对照有 `36` 个 spectral 与 `36` 个 Schur violations。

这是 finite predeclared long-window obstruction：仅 refute 本声明 panel 上的 transfer
statement，不 refute beta=2 的其他窗口或渐近行为。它不产生 source-valid normalization、
growing operator bound、source-uniform arithmetic `L2`、fixed-power credit、Route-B
reassembly 或 twin-prime result；`ARITHMETIC_ADVANCE=NO`、`FIXED_POWER_CREDIT=0`、
`FULL_GATE_B=OPEN`。official evaluator files 仍 absent，local Bridge-B 仍为 fail-closed。
下一步固定 failing exponent/window scale，在第二个 predeclared origin family 上做独立
replication；若不复现则转入 residue-phase localization。
