...
## V258 / TPC-405 current anchor

更新时间：2026-09-05

当前入口：proof 为
papers/tpc-405-c1-local-normalization-scale-ladder/PROOF_PACKAGE.md，checker 为
research/tpc-big-road/tpc_bridge_b_tpc405_c1_local_normalization_scale_ladder_checker.py，
编号论文为 papers/tpc-405-c1-local-normalization-scale-ladder/。

parameter-uniform finite bound：在整数 H>=1, N>=H+2, Q>N、显式 even/odd
CRT residues 与 selected shell primes 下，一个相邻 normalized proxy entry
满足 0<=z<=t1/(a_min sqrt(S0S1))<=4/(a_min H)<=4/H。五个 heights、
四个 multiplicities 共 20 个 exact rational cases；literal independent CRT
replay 与七项 mutation stress 均通过。它只约束一个 synthetic proxy entry，
...
TPC-404 proof package 位于
papers/tpc-404-c1-local-normalization-boundary/PROOF_PACKAGE.md，checker 为
research/tpc-big-road/tpc_bridge_b_tpc404_c1_local_normalization_boundary_checker.py，
编号论文为 papers/tpc-404-c1-local-normalization-boundary/。

TPC-404 在 TPC-403 的 production proxy 上审计 local diagonal normalization。
`m=1,2,3,4` 的局部能量与相邻系数 identities exact，literal CRT replay 与
五项 mutation stress 全部通过；normalized square-root observations 约为
`0.0136307,0.0136108,0.0135943,0.0135709`。这是 selected-prime finite
boundary structure，不是 full operator norm bound、normalized growing theorem、
arithmetic sign identification、arithmetic `L2` 或 twin-prime result。
...
## V251 / TPC-398 previous anchor

更新时间：2026-09-05

当前入口：proof 为
papers/tpc-398-c1-interpolation-endpoint-microgrid/PROOF_PACKAGE.md，checker 为
research/tpc-big-road/tpc_bridge_b_tpc398_c1_interpolation_endpoint_microgrid_checker.py，
编号论文为 papers/tpc-398-c1-interpolation-endpoint-microgrid/。

TPC-398 承接 TPC-397 的 endpoint microgrid clue，在新的
coordinate-disjoint affine family
`(6800001,6803209,6806417,6809625,6812833,6816041)` 上冻结 3 个
calibration 与 3 个 holdout origins，全部使用 `N=1024`。四个有限 matrix
probes 为 `lambda=7/8,15/16,31/32,1`，并与四种 normalization 组合成
...
`0.044971523016385406`；within-family transfer 均为 `4/4`。谱与 Schur
诊断均为 `0/96` failures，rational anchor 证明四个插值恒等式，但仍不构成
...
TPC-397 在第五个 coordinate-disjoint affine family 上复现了 interior endpoint
transition：`lambda=3/4,5/6,11/12` 的 12/16 cells 通过 origin-spread 门，
`lambda=1` 的 4 个 cells 失败；parent-relative 与 within-family transfer
均为 `4/4`，谱与 Schur failures 为 `0/96`。完整历史与复现实验保留在路线图
及下方的旧段落中。
...
coordinate-disjoint affine family
`(6000001,6003209,6006417,6009625,6012833,6016041)` 上冻结 3 个
calibration 与 3 个 holdout origins，全部使用 `N=1024`。四个有限 matrix
probes 为 `lambda=0,1/3,2/3,1`，并与四种 normalization 组合成 96-row、
16-cell panel。`lambda=0,1/3,2/3` 在每种 normalization 下均通过 1%
origin-spread 门；`lambda=1` 的四个 cells 均失败，最大 spread 为
`0.094070438394687927`。parent-relative holdout 为 `16/16`，最大误差
`0.0033105775404086435`；within-family transfer 为 local `4/4`、三个
scalar normalization 各 `3/4`。spectral failures 为 `24/96`，Schur
failures 为 `0/96`。rational anchor 证明四个插值恒等式；这仍是 finite
proxy phase localization，不是 source-valid growing theorem、arithmetic
...
当前入口：proof 为
papers/tpc-395-c1-origin-cross-family-holdout/PROOF_PACKAGE.md，checker 为
research/tpc-big-road/tpc_bridge_b_tpc395_c1_origin_cross_family_holdout_checker.py，
编号论文为 papers/tpc-395-c1-origin-cross-family-holdout/。

TPC-395 在第三个 coordinate-disjoint affine family 上冻结 3 个 calibration
与 3 个 holdout origins，全部使用 `N=1024`，形成 48-row、8-cell panel。
固定 `fixed_c3`、`Q=8192`、all-plus/alternating-index 两条 laws 与四种
normalization。all-plus 四个 cells 均通过 1% origin-spread 门，alternating
四个均失败，最大 spread 为 `0.068267525703845117`；跨族 calibration 与
holdout transfer 均为 `8/8`，最大 holdout error 为 `0.023289195722825839`，
within-family transfer 也是 `8/8`。spectral failures 为 `24/48`，Schur
failures 为 `0/48`。这是有限 cross-family replication，不是 source-valid
origin-uniform theorem、arithmetic `L2` 或 twin-prime endpoint。
...
## V244 / TPC-391 previous anchor

TPC-391 在第五个 fresh coordinate-disjoint origin family
`(3400001,3404011,3408021,3412031,3416041)` 上冻结 TPC-390 的 32 个
cell-wise horizon slopes，并插入 `N=1152,1280,1408` 三个中间 calibration
horizons，再在 `N=1536` 做预声明 holdout。448-row、32-cell panel 中 parent
forecast 在四个 horizons 的通过数为 `32/32,32/32,32/32,23/32`，local
control 为 `32/32`、`32/32`、`32/32`、`32/32`；9 个 parent cells
首次在终端 `N=1536` 越过 3% cap。parent 最大误差为
...
为 `0/448`。这是有限 horizon-localization audit，不能升级为 source-valid、
origin/count-uniform theorem 或 arithmetic result。
