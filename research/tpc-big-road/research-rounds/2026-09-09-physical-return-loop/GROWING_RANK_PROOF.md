# Growing-rank obstruction for the literal physical operator

Date: 2026-09-09. Unnumbered working proof integrated from the consolidated
Bernoulli report; independent round-3 growing-rank proof audit passed.
All vector spaces, dimensions, ranks, constraints and deflations are over
\(\mathbb C\). This is an operator statement, not a signed arithmetic
cancellation result and not an identification of bad vectors with \(\beta,w\).
The unchanged source clock and coefficient definitions are included below.
...
\(\|A_x-R_x\|\ge|\langle f,A_xf\rangle|\). This proves (4).
The same intersection argument proves the constraint assertions. If \(P\)
is an orthogonal projector with
\(\operatorname{codim}\operatorname{Ran}P=r<k_x\), then
\(W_x\cap\operatorname{Ran}P\) has dimension at least \(k_x-r\),
and on that space \(\langle f,P A_x P f\rangle=\langle f,A_xf\rangle\).
Thus the compression retains at least \(k_x-r\) negative directions. ∎
...
Enlarge this common threshold when using the optional predeclared subspace
to include its prime-count bound below, and when using row normalization
to include the separately proved positivity and upper bound for \(G\).
The latter enlargement and \(C_G\) may depend on the inherited profile.
...
The large Fourier band is conjugation-invariant, but its complex spectral-cutoff
subspace need not be. This proof alone does not reinterpret that subspace as
real. A separately constructed and independently audited real subspace is
given in [REAL_GROWING_RANK_ADDENDUM.md](REAL_GROWING_RANK_ADDENDUM.md), with
its own real-rank convention. The original constant-mode witness is real,
so its previously established norm barrier is unaffected.
...
**Strongest limitation:** this proof does not establish negativity of the
entire large band; the cutoff is needed for this argument. No counterexample
to whole-band negativity or success threshold for larger-rank deflation is proved.
**Remaining mathematical gap:** none identified in this scoped complex-operator
proof; the independent round-3 audit passed and its scope clarifications
