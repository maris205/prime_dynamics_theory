#!/usr/bin/env python3
"""Fail-closed local Bridge-B checker for TPC-331.
...
BRIDGE = ROOT / (
    ...
    "bridge_b_tpc331_control_average_centered_response_decomposition.md")
PRODUCER = PROJECT / (
    "code/tpc331_control_average_centered_response_decomposition.py")
INDEPENDENT = PROJECT / "experiments/tpc331_independent_checker.py"
STRESS = PROJECT / "experiments/tpc331_control_average_stress.py"
...
# Filled after all TPC-331 source and bridge text is final.
PRODUCER_SHA256 = (
    "c96095bd951d80e9147eeba99241761ba31a78b04a6b01bfcd120397f7e0eebc")
INDEPENDENT_SHA256 = (
    "ac12f0734cbe931500b36bebfc68303d7ae31e9e974e6b2f245a035d99181bf9")
STRESS_SHA256 = (
    "05056d343fb378870b10e326f1179c84fca30eb03e82617159c9ba4e4f93bfcf")
CERTIFICATE_SHA256 = (
    "eacd8b5e508956b362cbc0bb3c8da2b245a2155f91d8f48e794121f3e7a4997c")
BRIDGE_SHA256 = (
    "c79d47c61be126bf99ec69cf10f187ba8bab87117eb7dff6abecf50d307ddac4")
...
def _legacy_tpc330_check_files() -> None:
    required = (
...
def _legacy_tpc330_check_bridge_text() -> None:
...
def check_files() -> None:
    required = (
        ".gitignore", "README.md", "PAPER_PLAN.md", "DERIVATION_PACKAGE.md",
        "PROOF_PACKAGE.md", "code/tpc331_control_average_centered_response_decomposition.py",
        "experiments/tpc331_independent_checker.py",
        "experiments/tpc331_control_average_stress.py",
        "results/tpc331_certificate.json", "notes/theorem_ledger.md",
        "notes/claim_firewall.md", "notes/computational_protocol.md",
        "notes/route_evaluation.md", "notes/citation_verification.md",
        "paper/main.tex", "paper/references.bib", "paper/main.pdf",
        "paper/paper.pdf", "paper/compile.log",
    ...
    for path, expected, label in (
        ...
        need(digest(path.read_bytes()) == expected, label + " provenance")
    need(digest(PARENT_CODE.read_bytes()) == PARENT_CODE_SHA256,
         "TPC330 parent producer provenance")
    need(digest(PARENT_CERT.read_bytes()) == PARENT_CERT_SHA256,
         "TPC330 parent certificate provenance")
    ...
    need(payload.get("finite_audit", {}).get("rows") == 32 and
         payload.get("finite_audit", {}).get("laws") == 4 and
         payload.get("finite_audit", {}).get("decomposition_observations") == 128
         and payload.get("finite_audit", {}).get("control_count") == 5,
         "finite audit header")
    expected = payload.get("finite_audit", {}).get("decomposition_census")
    need(expected == EXPECTED_DECOMPOSITION_CENSUS,
         "decomposition census")
    audit = payload.get("decomposition_audit", {})
    need(audit.get("rows") == 32 and audit.get("laws") == 4 and
         audit.get("observations") == 128 and audit.get("control_count") == 5
         and {law: audit.get("by_law", {}).get(law, {}).get(
             "component_census") for law in LAW_NAMES} == expected,
         "decomposition summary")
    anchor = payload.get("exact_anchor", {})
    need(anchor.get("interval") == [36001, 36016] and anchor.get("Q") == 4
         and anchor.get("shell") == [5, 7] and anchor.get("exponent") == 1
         and anchor.get("mean_centered_identity_exact") is True,
         "exact anchor")
    firewall = payload.get("claim_firewall", {})
    need(firewall.get("TPC331_EXACT_MEAN_CENTERED_DECOMPOSITION") ==
         "PROVED_EXACT_FINITE" and
         ...
         "claim firewall")
    need(MAIN_PDF.read_bytes() == PDF.read_bytes(), "PDF copies differ")
    need(PDF.read_bytes().startswith(b"%PDF-") and PDF.stat().st_size > 100_000,
         "PDF integrity")
    log = LOG.read_text(encoding="utf-8", errors="replace")
    for bad in ("Warning:", "undefined", "Overfull \\hbox",
                "Underfull \\hbox", "LaTeX Error"):
...
def main() -> int:
    try:
        ...
        print("TPC331_BRIDGE_CHECK=PASS rows=32 laws=4 "
              "decomposition_observations=128 control_count=5 "
              "all_plus_average=32/32 all_plus_centered=32/32 "
              "all_plus_coherent=31/32 exact_anchor=1")
    except (Failure, DuplicateKey, OSError, json.JSONDecodeError, KeyError,
            TypeError, ValueError) as error:
        print("TPC331_BRIDGE_CHECK=FAIL " + str(error), file=sys.stderr)
        return 1
    return 0
