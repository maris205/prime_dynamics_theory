...
The first thirteen source-priority batches converted the contiguous range
TPC-418–355 from
...
bibliography, so these entries are `full-source-md`, not
`reliable-full-md`; the latter remains zero.

The archive inventory is now `full-source-md=64`,
`reliable-full-md=0`, `partial-or-notes=758`, and
...
| TPC-207 onward | The route map records the Bridge-B moving-hole and physical-observable architecture; the paper directory for TPC-207 is absent and is marked `source-inaccessible` in the inventory | `TPC207_CREATED=false`; no missing paper is inferred from a route-map label |
