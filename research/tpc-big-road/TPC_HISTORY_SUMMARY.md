...
The [TPC315–319 prerequisite audit](TPC_CONVERSION_SCOPE_TPC315_319.md)
distinguishes the selected weighted target from a re-optimized minimum,
the sampled norm sandwich from the true norm, and small spectral gaps from
demonstrated eigenspace instability. It records the strict-Schatten rank
condition, orthogonal-projection requirement, source coefficient pointer,
and preserved notation/working-directory issues. No production replay is claimed.

The [TPC310–314 prerequisite audit](TPC_CONVERSION_SCOPE_TPC310_314.md)
checks the product-choice and positivity assumptions behind aggregation,
modular-rank reduction, and rational primal/dual/interval bounds. It flags
TPC310's pair-table and midpoint discrepancies, TPC311's profile-weighting
wording, TPC312's harmless-on-this-panel cutoff metadata difference, and
TPC313's subtraction prose. Source tables and all originals are preserved.

The [TPC305–309 prerequisite audit](TPC_CONVERSION_SCOPE_TPC305_309.md)
checks tie alignment, positive budget/log quotients, coefficient-selection
sign symmetry, finite Hamming envelopes, and nested profile prefixes. It
flags the TPC305 tie-gauge claim and the TPC306 zero-mean quotient, and
qualifies the general PSD holdout and row-rank wording. The converter now
...
The [TPC290–294 prerequisite audit](TPC_CONVERSION_SCOPE_TPC290_294.md)
checks weighted Gram and Schur identities, nonzero normalized quantities,
sign parity and switching, and finite sign enumeration. It qualifies TPC290's
sparse-only wording and TPC294's tie-selected max-cut comparison. Post-document
TPC294 source is explicitly retained, with all 124 earlier conversion pairs
...
The [TPC280–284 prerequisite audit](TPC_CONVERSION_SCOPE_TPC280_284.md)
checks positive reciprocals, margin orientation, squared norm exponents,
projection/zeroing conventions, and discrete shell controls. It flags the
TPC282–283 printed interval/upper-bound discrepancies while separately
recording agreement of saved parent threshold counts. A saved-endpoint
comparison is not a physical interval replay. All five page maps are unique;
the original source and existing scientific boundaries remain unchanged.

The [TPC275–279 prerequisite audit](TPC_CONVERSION_SCOPE_TPC275_279.md)
checks real/complex packet identities, strict margin hypotheses, geometric
gain floors, finite-control scope, and coherence/deficit conventions. It
records the zero-scalar strictness exception, empty coherence maximum, and
opposite deficit/cross-term signs. Bounded source inspection explains why
comparison cutoff changes need not change the packet gain. All five page
maps are unique; no physical certificate or original source is rewritten.

The [TPC270–274 prerequisite audit](TPC_CONVERSION_SCOPE_TPC270_274.md)
checks powered normalization, absolute correlation, strict epsilon budgets,
interval classifications, and one-sided margin proxies. It records TPC271's
two narrowed/displaced printed intervals and distinguishes TPC273's general
classifier gap from its eleven correctly contained saved middle intervals.
A whitespace-only math-delimiter repair passes 24 tests; all 144 earlier
conversion pairs remain byte-identical. New section maps are unique and
original scientific files remain unchanged.

The [TPC265–269 prerequisite audit](TPC_CONVERSION_SCOPE_TPC265_269.md)
distinguishes actual magnitude bounds from paid-lane labels, three contrasts
from four blocks, and affine output from normalized correlation. It records
TPC267–269's nondirected saved-decimal formatting and TPC268's off-by-one
cutoff metadata without changing beta computations or certificates. Saved
threshold counts agree; this is not a physical interval replay. All five
new source/page maps are unique and all original materials are preserved.

The [TPC260–264 prerequisite audit](TPC_CONVERSION_SCOPE_TPC260_264.md)
checks polygon/DFT normalization, finite-lane quantifiers, phase characters,
and complex Schur geometry. It distinguishes scalar coupling from squared
output energy and records the unpaid-center gap in TPC264's proof-package
corollary. TPC263's source-input excerpts are located without re-proving
their analytic assumptions. All five new page maps are unique; no original
scientific source, certificate, or TPC418 route condition is changed.

The [TPC255–259 prerequisite audit](TPC_CONVERSION_SCOPE_TPC255_259.md)
checks adjoint orientation, combined unit masks, exact divisor-density
cancellation, fixed-epsilon dominance, and projection/residual typing.
It preserves the source's reflected-conjugate and period-mean wording issues,
the exponent comma, and conditional-rate labels. The five new maps are unique;
