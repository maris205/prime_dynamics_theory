...
Current completed endpoint: RH-398

Completed research batch: RH-352 through RH-398

Post-four-volume independent theorem edges: RH-362 through RH-398

Latest completed-paper verdict: RH-398 exact lag endpoint maximum and
maximizers Route A `GO`; `max_(h>=1)B_infinity(h)=B_infinity(1)`, with
equality exactly when `mu^2(h)=1` and `gcd(h,210)=1`; the complement and
joint finite-clock suprema equal `B_infinity(1)` and are not attained;
growing/adaptive, uniform-rate, prelimit, ordinary-Cesaro, causal/online,
lag-monotonicity, operator, trace, zero, RH, and Gate enlargements remain
`STOP_SCOPED` or `NOT_TESTABLE`

Post-RH-398 breadth audit: pending; no RH-399 assigned
...
RH-392 integration commit:
`9768c1cb5f56d959406c19119315afd542b6c30f`.
...
RH-398 integration commit:
`f026f7b5dd3aca499b656d03b1a3edc27b02eb8e`.
...
- `papers/RH-390-growing-rank-prime-tail-filtration/README.md`
- `papers/RH-390-growing-rank-prime-tail-filtration/UPDATED_ROADMAP.md`
...
- `papers/RH-392-fixed-lag-terminal-log-mobius-capacity-landscape/README.md`
- `papers/RH-392-fixed-lag-terminal-log-mobius-capacity-landscape/UPDATED_ROADMAP.md`
- `papers/RH-392-fixed-lag-terminal-log-mobius-capacity-landscape/THEOREM_LEDGER.md`
- `papers/RH-392-fixed-lag-terminal-log-mobius-capacity-landscape/results/result.json`
- `papers/RH-392-fixed-lag-terminal-log-mobius-capacity-landscape/outputs/main.pdf`
- `papers/RH-393-two-odd-factor-terminal-log-mobius-compiler/README.md`
- `papers/RH-393-two-odd-factor-terminal-log-mobius-compiler/UPDATED_ROADMAP.md`
...
- `papers/RH-396-euler-run-spectrum-fixed-lag-centered-mobius-capacity/README.md`
- `papers/RH-396-euler-run-spectrum-fixed-lag-centered-mobius-capacity/UPDATED_ROADMAP.md`
...
- `papers/RH-398-exact-lag-endpoint-maximum-and-maximizers/README.md`
- `papers/RH-398-exact-lag-endpoint-maximum-and-maximizers/UPDATED_ROADMAP.md`
- `papers/RH-398-exact-lag-endpoint-maximum-and-maximizers/THEOREM_LEDGER.md`
- `papers/RH-398-exact-lag-endpoint-maximum-and-maximizers/results/result.json`
- `papers/RH-398-exact-lag-endpoint-maximum-and-maximizers/main.pdf`
...
attainment input, RH-398 as the exact cross-lag fixed-endpoint maximum and
maximizer, complement-supremum, quantitative-gap, and joint finite-clock
nonattainment input, and RH-361 as the immediate endpoint of the still-open
...
RH-398 is now integrated.  Retain RH-396's centered fixed-table interface:
fix `h,q>=1`, every phase table, and an admissible terminal clock before
`X->infinity`, put `d=2h`, take the fixed-table limit, then the finite safe
maximum, then the supremum over finite `q`, and only afterward compare
separately fixed scalar lags.  Define
...
If `p_0(h)` is the least prime not dividing `d`, the vanishing local factor
and the finite second-difference telescope give

```text
B_infinity(h)=sum_(m=1)^(p_0(h)^2-1)(-1)^(m+1)A_m(d).
```

Finite-prime CRT transfer, followed along one common cofinal support, proves
...
On the complement the same value is a nonattained supremum; the fixed lags
`h=p^2`, prime `p>=11`, approach it with a gap in `(0,1/p^2]`.  If
`p_0(h)>=5`, then

```text
B_infinity(1)-B_infinity(h)
 >=B_infinity(1)-B_infinity(3)>1/36750>2/1334025.
```

Together with RH-396's strict finite-clock theorem this gives

```text
sup_(h>=1,q finite)C_h(q)=B_infinity(1),
```

with no finite pair attaining it.  RH-396 is the sole load-bearing analytic
endpoint input; RH-397 is the direct release/provenance predecessor only.
No statement permits growing or adaptive data, a uniform rate, a prelimit
maximum, ordinary Cesaro averaging, a causal rule, lag monotonicity, an
operator, trace, zero model, RH conclusion, or Gate A--E promotion.

RH-397 remains integrated.  Fix `h,q>=1`, a complete `q`-phase family, and
...
Safety is universal distance-`d` safety on every ternary biword.  RH-394 is
the sole analytic terminal-log input, instantiated at the ordered shifts
`L=+h,C=0,R=-h`; all lag-dependent relation, tropical, square-clock,
fresh-prime, and Euler-run arguments are proved locally.  If
`A_r={(x,y):F_r(x,+1,y)=+1}` and `Y_r=Target(A_r)`, positive projection and
nonnegative saturation give

```text
A_r=(T backslash Y_(r-d)) times Y_r.
```

The exact fixed-clock capacity is the full eight-subset tropical trace along
the cycles `r->r+d`.  Four-state compression is proved when `q` does not
divide `d`; self-loop phases `q|d` retain all eight states, with a strict
`h=2,q=4` counterexample to a universal four-state replacement.

For a finite prime set `P`, put `q_P=product_(p in P)p^2`.  On these
square-supported clocks, let `N_h(q_P)` be the number of positive center
phases, `alpha_h(q_P)` the raw step-`d` MWIS cardinality, and
`M_h(q_P)=K_1 alpha_h(q_P)/N_h(q_P)`, where `K_1=6/pi^2`.  Put

```text
p_0(h)=min{p prime:p does not divide 2h}.
```

Once `p_0(h) in P`, equivalently `p_0(h)^2|q_P`, the per-shared-value
marginal identity, including the zero value, proves
`C_h(q_P)=M_h(q_P)`.  If `q_P|Q` and the two moduli have the same prime
support, then `N` and `alpha` scale by the same repetition factor and

```text
C_h(Q)=M_h(Q)=M_h(q_P),
```

without any `gcd(R,d)` hypothesis.  The `h=6` clocks `36->72` show that this
same-support assertion fails before the `p_0^2=25` reset; the clocks
`900->1800` show exact scaling after it.

For finite `J subset Z`, define
...
The exact all-clock endpoint is

```text
B_infinity(h)=3/pi^2+(1/2)sum_(odd ell<p_0(h)^2) R_(ell,h),
sup_(q finite) C_h(q)=B_infinity(h),
C_h(q)<B_infinity(h) for every finite q.
```

Fresh-prime lifting satisfies

```text
N'=(P^2-1)N,
alpha'=P^2 alpha-O=(P^2-1)alpha+E,
B'-B=K_1 E/((P^2-1)N),
```

so a step is strict exactly when an even run is present.  The `h=9` equality
`M_9(36)=M_9(900)=2K_1/3` forbids stepwise strictness; a CRT construction
creates an even run after a finite extension and proves eventual strictness.
Across fixed lags,
...
the declared even clocks.  RH-398 then closed the maximum and exact
maximizers of the RH-396 endpoint, the complement landscape, and the joint
finite-clock endpoint.  No post-RH-398 breadth audit has assigned RH-399.
...
If every monomial has `abs(O(alpha))<=2`, then
...
All channels with one or two odd exponents vanish.  The surviving density
uses the distinct residue set `B_p(E)={a_i mod p^2:i in E}`, with
`nu_p(E)=abs(B_p(E))` and `tau_(p,E)(r)` counting distinct elements of
`B_p(E)` reducing to `r mod p`.  Thus residues are deduplicated modulo
`p^2` before possible collisions modulo `p`.  The exact phase formula is

```text
Theta_(q,r)(E)
 =q^-1 product_(p notdivides q)(1-nu_p(E)/p^2)
       product_(p exactlydivides q)(1-tau_(p,E)(r)/p)
       product_(p^2 divides q)1_(r mod p^2 notin B_p(E)).
```

Its phase sum is `kappa_E=product_p(1-nu_p(E)/p^2)`.  Odd support zero is
paid by finite CRT and cutoff removal; support one uses RH-392 equation (19)
plus terminal Abel transfer; support two uses RH-392 Theorem 2.2 after the
same fixed Boolean square mask.  The order is `P` fixed, `X->infinity`, then
`P->infinity`.

The exact admitted dimension is

```text
D_m=2^m+m2^(m-1)+binom(m,2)2^(m-2),
```

with the last term zero for `m<2`.  At `m=3` this is `26/27`; the missing
coefficient is the signed-cube coefficient `c_111`.  For a distinguished
current score `z f(x,y)`, the criterion becomes the alternating four-corner
condition on `f`, giving exactly `6*2^5=192` of the `512` truth tables.

For `m<=3`, the global squarefree density obeys
`kappa_A>=product_p(1-m/p^2)>0`, with equality exactly when every nonzero
pairwise difference is squarefree.  From `m>=4`, zero is attained exactly
when some prime-square residue system is completely covered.  For fixed
`m>=2`, the supremum is `6/pi^2`, is not attained, and is approached by
primorial-square configurations.  Do not infer odd support at least three,
unrestricted three-coordinate tables, growing data, a rate, ordinary
Cesaro cancellation, a pre-limit maximum, generic multishift capacity,
operators, traces, zeros, RH, or Gates A--E.  Routes A and B are `GO` for
the typed compiler and 192-table corollary; all enlargements are
`STOP_SCOPED`.

RH-392 remains the direct predecessor.  Fix exact integers `q>=1` and
`h>=1`, fixed periodic data, and an admissible terminal window
`1<=omega(X)<=X` with
`omega(X)->infinity`.  The new arbitrary-determinant terminal lemma proves
fixed-data cancellation for two nonparallel affine forms by rerunning the
Boolean squarefree cutoff with reduced determinant

```text
L Delta/(c_D c_V) != 0.
```

TPC-137 supplies only the determinant-two blueprint; Tao's fixed
nonparallel affine theorem is the analytic source.  Cutoff divisors need not
be coprime, and the limit order is: fix the prime cutoff `P`, let
`X->infinity`, then let `P->infinity`.

For `A_p(h)={0,h mod p^2}`, put `nu_p(h)=#A_p(h)` and let
`tau_(p,h)(r)` count the distinct elements of `A_p(h)` reducing to `r mod p`.
The exact phase pair-squarefree density is

```text
theta_(q,r)^(h)
 =q^-1 product_(p notdivides q)(1-nu_p(h)/p^2)
       product_(p exactlydivides q)(1-tau_(p,h)(r)/p)
       product_(p^2 divides q)1_(r mod p^2 notin A_p(h)).
```

Thus `sum_r theta_(q,r)^(h)=kappa_h`, where

```text
kappa_h=product_(p^2 divides h)(1-p^-2)
        product_(p^2 notdivides h)(1-2p^-2).
```

The general fixed finite-shift theorem diagonalizes every total-degree-two
periodic polynomial.  The separate one-lag coordinatewise-biquadratic
compiler pays the higher total-degree square-masked channels locally.  For
universally lag-`h`-safe tables, pointwise projection sends all `512` tables
to eight actions, compatibility is

```text
A_r=empty or +1 notin A_(r+h),
```

and every plus phase charges injectively to the empty predecessor `r-h`.
No coprimality of `q` and `h` is assumed.  The exact post-limit capacity is

```text
G_log(q,h)=6/pi^2-kappa_h/2.
```

It is independent of `q`, is attained positively by table `36` and
negatively by its reflection table `72`, and has the square-divisor spectrum

```text
3/pi^2 < G_log(q,h) <= 6/pi^2-kappa_*/2,
kappa_*=product_p(1-2/p^2).
```

The upper endpoint occurs exactly for squarefree `h`; the lower infimum is
not attained and is approached by squared primorial lags.  All parameters
and tables are fixed before `X->infinity`.  RH-392 proves no growing lag or
period, ordinary-Cesaro law, quantitative rate, growing mask, unrestricted
three-coordinate truth-table theorem, operator, trace, zero model, RH
statement, or Gate A--E conclusion.  Routes A and B are `GO`; all
enlargements are `STOP_SCOPED`.

RH-391 remains the preceding moving-rank theorem.  Maynard's
bounded-consecutive-gap theorem and
...
For exact integers `2<=s<=S_y`, `1<=K<=floor((2s-1)L)`, and
`c in {1,...,7}`, retain the exact prime tails below rank `s` and replace
all higher ranks by

```text
Psi_(c;s,K)
 =sum_(r<s)c^rP_r/r+sum_(r>=s)c^rK_rS_K(a_r)/r,
Psi_(s,K)=(Psi_(1;s,K),...,Psi_(7;s,K)),
Gap_(s,K)=F(Psi_(s,K))/pi^2.
```
...
the safe coordinate ledger is

```text
abs(PhiP_c-Psi_(c;s,K))/K_s
 <=c^s[(4-1/s)A_(s,c)epsilon_x
       +((2s-1)/(2s+1))B_(s,c)/x^2
       +C_c K!/{s((2s-1)L)^K}].
```

Eventually `S_y>=2`.  The common-cube bound `||grad F||_1<126` supplies the
endpoint transfer.  Put `D=(2s-1)L` and `b_k=k!/D^k`.  For exact
`1<=k<floor D`,
...
hence `b_K<=1/D` for every exact `1<=K<=floor D`.  This pays the complete
integer `K` window, while the uniform source, power, and factorial ledgers
pay the growing rank frontier.  Therefore

```text
lim_(y->infinity)
 max_(2<=s<=S_y,1<=K<=floor((2s-1)L))
 abs(GapP-Gap_(s,K))/P_s=0.
```

For every fixed exact integer `s>=2`, put `r=s-1`.  The new all-rank
endpoint coefficient is

```text
gamma_r=(4/r)[3^r u4-2^r u3+5^r u6-4^r u5
              +7^r u8-6^r u7
              +2(u3-u4+u5-u6+u7-u8)]>0.
```

RH-384 outward intervals pay `r=1,...,5`; exact ratio cross-products pay
every `r>=6`.  Maynard's bounded consecutive gaps, strict succession, the
endpoint Hessian/Taylor bounds `224/112`, and the full higher-tail bridge
then give

```text
limsup_y p_y^(2r)abs(P_r-I_(2r))>=1/2,
limsup_y p_y^(2r)pi^2 abs(GapP-GapI_(<r))>=gamma_r/2,
limsup_y p_y^(2r)pi^2 abs(GapP-GapJ_(<r))>=gamma_r/2.
```

Both endpoint errors have infinite limsup after division by `P_s`.  This
necessity theorem is fixed-rank and restricted to the frozen `P/J/I`
hierarchy.  RH-390 proves no growing-rank necessity, arbitrary-surrogate
obstruction, convergent factorial series, complex channel, active `c_11`,
growing clock, `K_N`, operator, trace, zero model, RH statement, or Gate
A--E conclusion.  Routes A and B are `GO`; every proposed enlargement is
`STOP_SCOPED`.
...
regime.  RH-392 then closed every individually fixed nonzero lag at
terminal-log scale, including the exact safe-table capacity and
...
RH-397 then closed the half-span overlap edge.  RH-398 then closed the exact
cross-lag upper endpoint landscape.
...
RH-362 through RH-372 add a separate arithmetic-dynamical branch. RH-362
...
### 3.29 RH-390 growing-rank prime-tail filtration
...
Write `Psi_(s,K)=(Psi_(1;s,K),...,Psi_(7;s,K))` and
`Gap_(s,K)=F(Psi_(s,K))/pi^2`.  Retain
`GapP=B_infinity-G(q_y)=F(PhiP)/pi^2`.  Eventually `S_y>=2`.

The strict Stieltjes source transfer, power-kernel comparison, and exact
Laplace remainder give
...
The factors and the exponent `s+1` in `B_(s,c)` are part of the theorem.
Put `D=(2s-1)L` and `b_k=k!/D^k`.  For exact
`1<=k<floor D`, `b_(k+1)/b_k=(k+1)/D<=1`; hence `b_K<=1/D` for every exact
`1<=K<=floor D`.  This pays the full integer factorial window; finite
fixtures are only regression rows.  The common cube and endpoint bound
`||grad F||_1<126` yield
...
For fixed exact `s>=2`, set `r=s-1`.  Exact differentiation, RH-384
outward intervals for `r<=5`, and cross-product ratio bounds for `r>=6`
prove `gamma_r=grad F(0).(c^r/r)_(c=1)^7>0`.  Maynard succession and the
common-head Taylor ledger then give
...
Thus omitting rank `s-1` has error unbounded relative to `P_s`, but only
for fixed `s` and only within the declared `P/J/I` hierarchy.  Do not infer
growing-rank necessity, an arbitrary-surrogate obstruction, factorial-series
convergence, a larger rank frontier, complex channels, active `c_11`, a
...
### 3.31 RH-392 fixed-lag terminal-log capacity landscape

Fix exact integers `q>=1` and `h>=1`, fixed periodic coefficients, and every
admissible terminal clock `omega` with

```text
1<=omega(X)<=X, omega(X)->infinity.
```

For distinct fixed shifts, all linear and off-diagonal channels of a fixed
total-degree-two polynomial have zero terminal-log limit.  Only the constant
and diagonal squarefree-density terms remain.  For the separate one-lag
compiler, put

```text
x=mu_0(n-h), z=mu(n),
P_r(x,z)=sum_(0<=i,j<=2)c_(ij)(r)x^i z^j.
```

Then its limit is

```text
L_(q,h)(P)=sum_r[c00(r)/q+c20(r)delta_(q,r-h)
                  +c02(r)delta_(q,r)+c22(r)theta_(q,r)^(h)],
```

and the five `c10,c01,c11,c12,c21` channels vanish.  The `c11` channel uses
the new arbitrary-nonzero-determinant terminal lemma; square-masked channels
use local prefix and CRT estimates.  The exact local density is
...
Here `tau` counts distinct residues modulo `p^2`, even when two such residues
collide modulo `p`; this pays the `p divides h` but `p^2 notdivides h` trap.
Summing over phases gives
...
For universally lag-`h`-safe tables, pointwise positive-current projection
gives eight actions.  Translation `r->r-h` is a bijection for every `q`; no
`gcd(q,h)=1` hypothesis is used, and when `q divides h` the self-loop itself
forbids a plus phase.  The predecessor charge proves

```text
G_log(q,h)=max_f abs(L_(q,h)(f))=6/pi^2-kappa_h/2.
```

The maximum over fixed lags occurs exactly at squarefree `h`; the infimum
`3/pi^2` is not attained and is approached by squared primorial lags.  These
are post-limit extrema over already-formed fixed-`(q,h)` limits.  Do not
infer growing `q` or `h`, ordinary Cesaro cancellation, a rate, adaptive
tables, unrestricted multi-lag truth-table capacity, operators, traces,
zeros, RH, or Gates A--E.  Routes A and B are `GO`; enlargements are
`STOP_SCOPED`.

### 3.32 RH-393 two-odd-factor multi-shift terminal compiler

Fix `m>=1`, `q>=1`, pairwise-distinct integer shifts `a_1,...,a_m`, fixed
`q`-periodic coefficients, and every admissible terminal clock.  For a
coordinatewise-quadratic phase polynomial, retain only monomials
`alpha in {0,1,2}^m` satisfying
...
For `B_p(E)={a_i mod p^2:i in E}` as a set, define
`nu_p(E)=abs(B_p(E))` and let `tau_(p,E)(r)` count elements of `B_p(E)`
reducing to `r mod p`.  Then
...
Theta_(q,r)(E)
 =q^-1 product_(p notdivides q)(1-nu_p(E)/p^2)
       product_(p exactlydivides q)(1-tau_(p,E)(r)/p)
       product_(p^2 divides q)1_(r mod p^2 notin B_p(E)),
sum_(r mod q)Theta_(q,r)(E)=product_p(1-nu_p(E)/p^2).
```

The proof treats odd support zero by finite CRT and a union tail, odd support
one by the frozen RH-392 one-form cancellation, and odd support two by its
arbitrary-nonzero-determinant terminal theorem.  Extra square masks become a
fixed periodic weight after fixing the prime cutoff.  The common replacement
error is `O_(m,a)(norm(rho)_infinity(log(omega)/P+1))`, and the order is
`P` fixed, `X->infinity`, then `P->infinity`.
...
At `m=3`, `D_3=26`; the unique excluded monomial is `z_1z_2z_3`, and a
general ternary table is covered exactly when its signed-cube coefficient
vanishes.  For scores `z f(x,y)`, this is the alternating corner condition
on `f`, satisfied by exactly `192/512` truth tables.  The other `320` are
outside the theorem, not proved divergent or nonconvergent.

For a fixed `m`-shift configuration `A`, write
`kappa_A=product_p(1-abs(A mod p^2)/p^2)`.  If `m<=3`, then

```text
kappa_A>=C_m=product_p(1-m/p^2)>0,
```

with equality exactly when every nonzero pairwise difference is squarefree.
For every `m>=4`, zero is attained exactly when `A mod p^2` covers all
`p^2` classes for some prime.  For fixed `m>=2`, the supremum is `6/pi^2`,
is not attained, and is approached by
`A_(m,y)={jQ_y:0<=j<m}`, `Q_y=product_(p<=y)p^2`.

Everything is fixed before the terminal limit.  No odd support at least
three, unrestricted three-coordinate table, growing `m`, `q`, shift, mask,
or coefficient family, effective rate, ordinary Cesaro average,
maximum-before-limit, generic multishift capacity, operator, trace, zero,
...
### 3.35 RH-396 Euler-run spectrum for fixed-lag centered capacity

Fix integers `h,q>=1`, put `d=2h`, and fix a `q`-periodic family
`F_r:T^3->{-1,+1}` before `X->infinity`.  The table reads

```text
(mu_0(n-h),mu(n),mu(n+h)),
```

and the pointwise score is `mu(n)` times the table output.  Safety forbids
two positive outputs at separation `d` for every ternary biword.  RH-394 is
the sole analytic input: its complete fixed-three-shift terminal-log law is
instantiated at `L=+h,C=0,R=-h` for every admissible terminal clock.

For `S subset {L,C,R}`, define the collision-aware local data

```text
B_(p,S)={a_i mod p^2:i in S},
nu_(p,S)=|B_(p,S)|,
tau_(p,S)(r)=|{b in B_(p,S):b mod p=r mod p}|.
```

Deduplication occurs modulo `p^2` before the mod-`p` collision count.  Then

```text
Theta_(h,q,r)(S)=q^(-1)
 product_(p not dividing q)(1-nu_(p,S)/p^2)
 product_(p exactly divides q)(1-tau_(p,S)(r)/p)
 product_(p^2 divides q) 1_(r mod p^2 notin B_(p,S)),
Pi_(h,q,r)(U)=sum_(W subset {L,C,R} backslash U)
              (-1)^|W| Theta_(h,q,r)(U union W).
```

The phase sum is the lag-dependent squarefree density `kappa_h(S)`, not the
collision-free symbol `K_|S|`.  With

```text
Support(x,y)={C} union ({L} if x!=0) union ({R} if y!=0),
lambda_(h,r)(x,y)=2^(-1_(x!=0)-1_(y!=0))
                  Pi_(h,q,r)(Support(x,y)),
```

positive projection and relation saturation give

```text
A_r=(T backslash Y_(r-d)) times Y_r.
```

Thus `C_h(q)` is exactly the sum of the tropical traces of the eight-subset
transition matrices around the `gcd(q,d)` cycles `r->r+d`.  If `q` does not
divide `d`, every cycle is non-self-loop and multi-affinity gives a
four-antipodal-state optimizer.  If `q|d`, the theorem retains all eight
states.  The exact `h=2,q=4` full and four-state-restricted coefficient
vectors `(0,0,1/2,-1/2)` and `(0,0,1,-2)` prove that no all-clock
compression is available.

For a finite prime set `P`, put `q_P=product_(p in P)p^2`.  On these
square-supported clocks, let `N_h(q_P)` be the number of positive center
phases and `alpha_h(q_P)` the raw step-`d` MWIS cardinality.  Only in this
domain set

```text
M_h(q_P)=K_1 alpha_h(q_P)/N_h(q_P), K_1=6/pi^2.
```

Let `p_0(h)=min{p prime:p does not divide d}`.  Once `p_0(h) in P`,
equivalently `p_0(h)^2|q_P`, the per-shared-value marginal identity,
including the zero value, and the path charge prove
`C_h(q_P)=M_h(q_P)`.  For `q_P|Q` with equal prime support,

```text
N_h(Q)=R N_h(q_P), alpha_h(Q)=R alpha_h(q_P),
C_h(Q)=M_h(Q)=M_h(q_P),
```

with no `gcd(R,d)` condition.  Before the `p_0^2` reset this can fail:
for `h=6`, `(alpha,N)` changes from `(9,24)` at `q=36` to `(24,48)` at
`q=72`.  After the reset, `(291,576)` at `q=900` scales exactly to
`(582,1152)` at `q=1800`.

For finite `J subset Z`, put

```text
D_h(J)=product_p(1-|{dj mod p^2:j in J}|/p^2),
R_(ell,h)=D_h([0,ell-1])-D_h({-1} union [0,ell-1])
          -D_h([0,ell])+D_h([-1,ell]).
```

The prime `p_0(h)` makes the run spectrum finite, and absolute convergence
gives
...
If a fresh prime `P` is adjoined after the qualified base and `E` and `O`
are the even- and odd-run MWIS ledgers, then
...
The step is strict exactly when an even run exists.  The fixture
`M_9(36)=M_9(900)=2K_1/3` forbids a stepwise-strict claim.  A CRT extension
creates an exact run of length two, so every qualified finite layer is
strictly improved at some later layer.  Divisibility lift and the
same-support equality then prove
...
Every fixed lag has a positive-density isolated run, while lags with
`2h` divisible by every prime square up to a growing cutoff have only an
outside-prime boundary tail.  Therefore

```text
inf_(h>=1)B_infinity(h)=3/pi^2,
```

and the infimum is not attained.  The result fixes `h`, `q`, and every
table before the terminal limit.  It proves no growing-parameter theorem,
ordinary Cesaro cancellation, rate, pre-limit or adaptive maximum, causal
rule, even-four or larger-window law, generic graph theorem, lag supremum,
...
### 3.37 RH-398 exact lag endpoint maximum and maximizers

RH-398 retains RH-396's fixed centered interface.  Fix `h,q>=1`, every
phase table, and every admissible terminal clock before `X->infinity`, and
put `d=2h`.  The fixed-table terminal limit precedes the finite safe-table
maximum; the supremum over finite clocks and the comparison over separately
fixed scalar lags come afterward.  RH-396 supplies

```text
sup_(q finite)C_h(q)=B_infinity(h),
C_h(q)<B_infinity(h) for every finite q,
inf_(h>=1)B_infinity(h)=3/pi^2, unattained.
```

For each prime define `t_p(d)=p^2/gcd(d,p^2)` and

```text
A_m(d)=product_p(1-min(m,t_p(d))/p^2).
```

If `p_0(h)=min{p prime:p does not divide d}`, then `A_m(d)=0` for
`m>=p_0(h)^2`.  Translation in the RH-396 run formula gives
...
The new comparison works first on finite CRT phase spaces.  If
`a(L)=ceil(L/2)` and `Lambda_T(L)` is the total MWIS deletion loss
`T a(L)` minus the sum over all deleted residue classes modulo `T`, then

```text
T odd,  L odd:  Lambda_T(L)=a(L),
T odd,  L even: Lambda_T(L)=max(0,L/2-(T-1)/2),
T even, L odd:  Lambda_T(L)=min(a(L),T/2),
T even, L even: Lambda_T(L)=0.
```

At collision level `e=min(v_p(d),2)` the local contribution is

```text
V_(p,e)(L)=a(L)-Lambda_(p^(2-e))(L)/p^2.
```

Odd run lengths are equality branches.  The named even-run branches provide
strictness only after a positive-density exact-run CRT cylinder is supplied.
One-prime transfers on a common finite support, followed by one common
cofinal passage, prove

```text
max_(h>=1)B_infinity(h)=B_infinity(1),
argmax_(h>=1)B_infinity(h)
 ={h>=1:mu^2(h)=1 and gcd(h,210)=1}.
```

The conjunction is essential.  At prime `2` the strict branch is
`2|h iff 4|d`, not an ordinary square-factor statement about `h`.  On the
complement the supremum is again `B_infinity(1)` but is not attained.  For
each prime `p>=11`,
...
The sector `p_0(h)>=5` has the uniform strict chain
...
and no finite pair attains it.  RH-396 commit
`cd57086fa90939d56656c3f952a08ffad9aabefe` is the sole load-bearing
theorem and analytic endpoint source: definitions (18)--(21), Theorem 1.3
equation (22), and Corollary 1.4 equation (23).  RH-397 commit
`dd63a109dcfa72365c749e0b183820d2611af733` is the direct release and
provenance predecessor only.  RH-394, RH-392, RH-395, RH-375, and all four
remote locks are inherited provenance or comparison only.

RH-398 proves no `h(X)` or `q(X)` theorem, growing or adaptive table, uniform
rate, prelimit maximum, ordinary Cesaro result, causal rule, monotonicity in
`h`, four-shift or larger-window theorem, generic graph theorem, operator,
trace, zero model, RH statement, or Gate conclusion.  Its finite certificate
is reproduction evidence, not an analytic proof or random-Mobius law.

## 4. Compact conclusions from RH-352 through RH-398
...
- **RH-390:** Simultaneous prime-tail replacement over
  `2<=s<=floor((1-delta)log L/log 7)` and the complete integer factorial
  window, after retaining every exact rank below `s`.  It also proves
  `gamma_r>0` for every rank and sharp fixed-rank necessity inside the
...
- **RH-392:** Fixed finite-shift total-quadratic diagonalization, a separate
  one-lag coordinatewise-biquadratic compiler, exact all-phase pair-squarefree
  densities, and an every-fixed-`(q,h)` safe-table capacity
  `6/pi^2-kappa_h/2`.  Its square-divisor spectrum has a squarefree maximum
  and a nonattained `3/pi^2` infimum.  No growing lag or period, ordinary
  Cesaro theorem, rate, unrestricted multi-lag table, or Gate follows.
- **RH-393:** Coordinatewise-quadratic terminal diagonalization for any fixed
  finite distinct shift set whenever each monomial has at most two odd
  exponents.  It proves the exact multi-shift phase densities, admitted
  dimension, `26/27` signed-cube boundary, `192/512` distinguished-current
...
- **RH-396:** Exact centered/noncausal capacity for every individually fixed
  lag via the full-eight-state `r->r+2h` tropical trace, with four-state
  compression only away from self-loops.  The `p_0(h)`-qualified marginal
  charge, Euler-run spectrum, and fresh-prime recurrence prove the explicit
  endpoint `B_infinity(h)`, strict nonattainment at every finite clock, and
  the unattained cross-lag infimum `3/pi^2`.  No growing lag or clock,
...
- **RH-397:** Exact fixed-`h,q` half-span overlap capacity via two overlap
  ...
  conclusion follows.
- **RH-398:** Exact maximum `B_infinity(1)` of the RH-396 fixed-lag endpoint,
  with equality exactly for squarefree `h` coprime to `210`.  The complement
  has the same nonattained supremum, the `p_0(h)>=5` sector has the recorded
  uniform gap, and `sup_(h>=1,q finite)C_h(q)=B_infinity(1)` has no finite
  maximizer.  All data remain fixed before the terminal limit; no growing,
  causal, Cesaro, monotonicity, operator, zero, RH, or Gate statement follows.

## 5. Route firewall and reopening triggers

Do not:

- turn RH-398's comparison of separately fixed scalar lags into an `h(X)`
  theorem, or compare different finite prime supports before the common-support
  transfer;
- infer RH-398 strictness from an isolated finite word without its
  positive-density exact-run cylinder, or replace the conjunction of
  squarefreeness and coprimality to `210` by either condition alone;
- misstate the RH-398 prime-`2` branch as an ordinary square divisor of `h`,
  infer monotonicity in `h`, or promote its 72-row certificate or auxiliary
  finite CRT probability space to analytic or random-Mobius evidence;
...
- enlarge RH-390's rank frontier beyond
  `floor((1-delta)log L/log 7)` or infer factorial-series convergence from
...
RH-398. Triggers 1--4 remain untouched. RH-365 closes the natural
...
an unbounded-clock max-before-limit, adaptive capacity, or any Gate.
RH-390 closes the simultaneous growing-rank exact-retention filtration and
...
any Gate.  RH-392 closes every individually fixed nonzero lag and fixed
period at terminal-log scale, including the one-lag safe-table capacity and
its square-divisor landscape.  It does not prove a growing lag or period,
ordinary Cesaro cancellation, a quantitative rate, unrestricted multi-lag
truth-table capacity, adaptive capacity, or any Gate.  RH-393 closes the
fixed multi-shift coordinatewise-quadratic compiler only for monomials with
...
capacity, ordinary Cesaro cancellation, or any Gate.  RH-396 closes the same
centered data type at every individually fixed lag, including the exact
lag-dependent full-eight-state optimizer, the `p_0(h)`-qualified
same-support charge, Euler-run endpoint, strict finite-clock nonattainment,
and unattained lag infimum.  Do not replace collision-aware `kappa_h(S)` by
`K_|S|`; compress self-loops to four states; use `M=K_1 alpha/N` outside
square support; omit the `p_0^2` reset; add a false `gcd(R,2h)` condition to
same-support scaling; claim every prime step is strict; or present the
orientation-only nine-digit endpoint decimals as certified.  RH-396 proves
...
complete.  RH-398 closes the exact fixed-lag endpoint maximum and maximizers,
the complement landscape, the quantitative `p_0>=5` gap, and the joint
finite-clock nonattainment theorem.  It proves no growing or adaptive data,
uniform rate, prelimit maximum, ordinary Cesaro result, causal rule,
lag-monotonicity, operator, trace, zero model, RH statement, or Gate.
RH-398 is complete.  No RH-399 is assigned.  The post-RH-398 breadth audit is
pending.
The next breadth audit should distinguish:

1. A causal/end-window graph-coupled capacity.  RH-395 through RH-398 read a
...
3. A growing-parameter compiler.  This is `STOP_SCOPED`: the fixed-data
   ...
   coefficients, or centered profiles.  RH-396 through RH-398 fix all theorem
...
5. A continuum `-1` Riesz projector derived from RH-367 Ulam data.  This is
...
### 6.29 RH-390 growing-rank filtration proof and release (2026-08-09)

The post-RH-389 breadth audit compared a growing-rank prime-tail filtration,
another active-memory clock theorem, and several operator/geometric routes.
The selected edge retains every exact prime tail below a moving threshold
`s`, replaces all ranks `r>=s` by one common factorial order, and proves
uniform next-scale accuracy over the simultaneous `(s,K)` window.  A second
theorem proves positivity of every endpoint direction `gamma_r` and fixed-
rank necessity inside the declared `P/J/I` hierarchy.

The analytic proof pays the strict endpoint, the three exact denominator
factors `A/B/C`, the symbolic full-`K` recurrence, the common cube and
`126` endpoint transfer, RH-386 growing-order uniformity, the outward
intervals for `gamma_1,...,gamma_5`, exact cross-products for every higher
rank, and the Maynard/common-head Taylor bridge.  The growing positive
theorem and fixed-rank negative theorem have separate quantifiers; neither
licenses moving-rank necessity or an arbitrary-surrogate obstruction.

The executable certificate has

```text
72=12+7+15+12+10+10+6
```

rows, is `17571` canonical bytes, and has SHA-256
`e2116abd4aeb910c24ee470a520623f29f1f454bb9b5293840875da091682b3b`.
It rejects `24/24` genuine semantic mutations using independent field
validation.  Its exact role is
`finite_exact_algebra_not_analytic_proof`.

The proof-minimal immutable closure deliberately skips the mathematically
unrelated RH-389/TPC-137/Tao branch.  It contains `87` Git blobs from RH-388,
grouped `77/8/2`, with ordered digest
...
and the resulting `89`-input logical digest is
`2255b26dd68adf09f447e251eb5d38c8b1d31fbaa1c26befd8c04165097ed922`.
Both source verifiers are offline by default and make zero requests.  The
four external payload hashes are absent from both the publication members
and the entire RH-390 tree; no remote payload is redistributed.

The final official suite is `52/52`.  Result and recursively closed Draft
2020-12 schema regenerate byte-exactly in normal and optimized modes.  The
archive contains `34` publication members, `87` Git inputs, two remote
logical locks, and zero failures.  Manifest and report bring the integration
set to exactly `36` RH-390 files in commit
`a3aa5977e9b3338e4c3035c6c42b60d50bc3ac3b`.

The main and semantic PDFs are byte-identical, `362762` bytes, and nine A4
...
pages pass.  The final mechanical reseal changed only two intended equation-
spacing controls: its cross-version rendering is semantically and visually
equivalent, while pages `1` and `3` are correctly not claimed byte-identical
to the pre-reseal raster.  Two independent final audits returned zero
blocker and zero minor.

Routes A and B are `GO`; all enlargements are `STOP_SCOPED`.  RH-390 proves
...
### 6.31 RH-392 fixed-lag terminal-log capacity release (2026-08-10)

The post-RH-391 breadth audit compared fixed-lag terminal-log
diagonalization, growing-period active memory, return-bouquet boundary laws,
and continuum-Ulam projector routes.  The selected theorem closes every
individually fixed nonzero lag without adding a remote beyond RH-389.  It
proves a new arbitrary-determinant terminal lemma locally from Tao's fixed
nonparallel affine input and the TPC-137 cutoff architecture; it does not
misstate TPC-137's determinant-two theorem as an all-lag result.

The proof pays shared cutoff factors with
`lcm(M,k^2,l^2)`, the reduced determinant `L Delta/(c_D c_V)`, the order
`P` fixed, `X->infinity`, then `P->infinity`, and the collision-sensitive
local density.  It separates the total-degree-two finite-shift theorem from
the one-lag coordinatewise-biquadratic compiler.  The safe-table theorem
then reruns the `512 -> 8` projection and predecessor charge with translation
`r->r-h`, including every `gcd(q,h)` cycle and the `q divides h` self-loop.
...
semantic rows, is `220832` canonical bytes, and has SHA-256
`614297795d4d4dfeadfb5667d3e0d405d04fbe8e07e9d87a743faed9cb267a96`.
It rejects `24/24` genuine semantic mutations.  Its false-verification path
is independent of every row, group, and certificate builder and audits all
`262144` ordered truth-table pairs.  Its role is finite reproduction, not
analytic proof.

The proof-minimal closure contains `106` Git blobs from RH-389 release
`8b1a875b4bbefd955a419593951ce2d09987ac6f`, grouped `95/8/3`.  The ordered
group digests are
...
The Johnston--Yang, Maynard, and Tao remote locks bring the closure to `109`
logical inputs with digest
`39bf8e9030b511e85fdf26a7c71722c3e4be5bc74bc738aa253bbc29c94517f9`.
Only Tao is an analytic remote input; Johnston--Yang and Maynard are
inherited provenance.  All three default verifiers make zero requests.  The
five external payload hashes are absent from the publication members and
the entire RH-392 tree.

The final official suite is `60/60`.  Normal and optimized builds regenerate
result, recursively closed Draft 2020-12 schema, manifest, and verification
report byte-exactly.  The archive has `38` publication members, `106` Git
inputs, three remote locks, and zero failures.  Manifest and report bring the
integration set to exactly `40` RH-392 files in commit
`9768c1cb5f56d959406c19119315afd542b6c30f`.

The main and semantic PDFs are byte-identical, `391274` bytes, and eight A4
pages; all `24` font rows are embedded, subsetted, and Unicode-mapped.
Ghostscript, text extraction, LaTeX/BibTeX scans, and all `8/8` rendered
pages pass.  Two independent final audits returned zero blocker and zero
minor.

Routes A and B are `GO`; all enlargements are `STOP_SCOPED`.  RH-392 proves
no growing lag or period, ordinary-Cesaro theorem, rate, adaptive or
unrestricted multi-lag truth-table capacity, three-point Chowla statement,
operator, trace, zero model, RH statement, or Gate A--E conclusion.

### 6.32 RH-393 two-odd-factor multi-shift release (2026-08-10)

The post-RH-392 breadth audit compared a multi-shift odd-support compiler,
higher-coordinate table subclasses, graph-coupled lag capacities, the
RH-365 bouquet radius, and a continuum Ulam projector.  The selected route
extends the frozen one- and two-form terminal inputs through arbitrarily many
fixed square masks, without adding a remote source.  It is not an
unrestricted higher-Chowla theorem.

The proof separates odd support zero, one, and two.  It pays the exact
multi-shift local factors by finite CRT, then removes the square-mask cutoff
with a finite-coordinate union tail.  One odd factor uses RH-392 equation
(19) plus terminal Abel transfer; two odd factors use RH-392 Theorem 2.2.
Every shift, phase coefficient, period, and mask is fixed before the terminal
limit, with order `P` fixed, `X->infinity`, then `P->infinity`.
...
semantic rows, is `117096` canonical bytes, and has SHA-256
`f109da241722796418f39708b16fa162cce0b85a6e448998d3ede593b7bd697b`.
It rejects `32/32` genuine semantic mutations.  Its false-verification path
is independent of all builders and semantic helpers.  It audits the
`192/512` table census, all `27` three-variable monomials, `12` finite-CRT
fixtures, the density landscape, source roles, and every firewall.  Its role
is finite reproduction, not analytic proof.

The proof-minimal closure contains `117` Git blobs from RH-392 release
`9768c1cb5f56d959406c19119315afd542b6c30f`, grouped `106/8/3`.  The
ordered group digests are
...
The Johnston--Yang, Maynard, and Tao locks bring the closure to `120`
logical inputs with digest
`9315d7c01651ed8b4d94f98c3e4019ad11e28469ee6722903721db280b9f92eb`.
Tao is inherited analytic provenance through RH-392; Johnston--Yang and
Maynard are closure-only.  All three default verifiers make zero requests,
and the five external payload hashes are absent from the release members and
the entire RH-393 tree.

The final official normal and optimized suites are each `68/68`.  Result,
recursively closed Draft 2020-12 schema, manifest, and verification report
regenerate byte-exactly.  The archive has `38` publication members, `117`
Git inputs, three remote locks, and zero failures.  Manifest and report bring
the integration set to exactly `40` RH-393 files in commit
`6fed36f44183a2794a3a814493ff602c5dc9314b`.

The main and semantic PDFs are byte-identical, `358432` bytes, and six A4
pages; all `20` font rows are embedded, subsetted, and Unicode-mapped.
Ghostscript, text extraction, LaTeX/BibTeX scans, and all `6/6` rendered
pages pass.  The release gate also rejects cache paths, symlinks, filename
sentinels, literal sealing sentinels, malformed EOFs, unsafe paths, and
external payload inclusion.  Three independent final audits returned zero
blocker and zero minor.

Routes A and B are `GO` at the typed compiler and 192-table scopes; all
enlargements are `STOP_SCOPED`.  RH-393 proves no odd-support-three channel,
...
### 6.35 RH-396 fixed-lag centered Euler-run spectrum release (2026-08-12)

The post-RH-395 breadth audit compared the causal/end-window model, even
four-point cancellation, growing parameters, the RH-365 bouquet radius,
continuum Ulam projectors, and a fixed general centered lag.  Only the last
route passed both the theorem and source gates.  It needs no new remote
source: RH-394 supplies the fixed-three-shift terminal-log law, while RH-395
and RH-375 are finite combinatorial precedents only.

The proof fixes `h`, `q`, and the complete table family before the terminal
limit.  It derives the collision-aware `Theta/Pi/lambda` densities at
`(+h,0,-h)`, the exact full-eight-state tropical trace along `r->r+2h`, and
the four-state reduction only away from self-loops.  The `h=2,q=4`
coefficient vectors give a strict self-loop obstruction.  A per-shared-value
marginal identity, including the zero state, pays the `p_0(h)`-qualified
square-clock equality `C=M`; the `h=6` fixtures show why the reset cannot be
omitted.  The Euler-run density, fresh-prime recurrence, even-run CRT, and
clock-divisibility bridge prove the explicit strict nonattained endpoint at
every fixed lag.  An outside-prime tail then proves the unattained cross-lag
infimum `3/pi^2`.

The exact certificate has

```text
96=12+16+16+12+12+12+8+8
```

semantic rows, is `83309` canonical bytes, and has SHA-256
`7cc0da78ee7e47a22b357d7e8d907bc9d9879caeb82ede30709e8cb1023032ba`.
The builder-independent core rejects `32/32` named semantic mutations; the
result and schema layers reject `65/65` and `28/28`.  Normal and optimized
Stage-1 suites are each `50/50`.  The artifact is finite reproduction, not
the analytic proof.

The proof-minimal source closure contains `160` Git blobs from RH-395 release
`20de7202518f4488cbd9c7d63bf94aaa3dc94476`, grouped `148/8/4`.  The ordered
group digests are

```text
a0ff7451b704aedc6eb839494dc65a9711b1dd7694ec4991c8169e77abafdcae
631dcac47b47865202f13552894a48c7b174575ac893d692bfed575f83120a3e
b1822df0e748c9ebb18c08198840975a378e3e068c08105fd68dbc55be74f79f
```

and their aggregate digest is
`472bf5ce5e352dce0d3a44ad10b22345b98e0e8b9a0cd745be9ecd93dedf0a86`.
The ordered Johnston--Yang, Maynard, Tao, and Tao--Teravainen locks bring the
closure to `164` logical objects with digest
`72040ab3d7a5d98ce308b91d0748d52a8d4886cf245f5079f14c69ee659cc287`.
Redistribution flags are `false,false,true,false`; every external PDF is
nonvendored, the four offline verifiers make zero requests, and all six
payload identities are absent from release members and the whole RH-396
tree.

The release archive has `41` publication members and `43` stage files.  The
offline and archive targeted suites are `7/7` and `23/23`; the complete
normal and optimized suites are each `80/80`.  Result, recursively closed
Draft 2020-12 schema, manifest, and verification report regenerate
byte-exactly.  The manifest is `35406` bytes with SHA-256
`2a7c826355299986331664dc1acfc33faf32527e5bc3c97d2aa16f462db4d2d4`;
the zero-failure report is `1236` bytes with SHA-256
`b0842be38a04f1ee0b13c094ea58ddf1c3af095a7c11947a1e66ff01163ee5e7`.
Integration commit `cd57086fa90939d56656c3f952a08ffad9aabefe` contains exactly
the `43` RH-396 files.

The main and semantic PDFs are byte-identical, `447519` bytes, and `15` A4
pages; all `24` font rows are embedded, subsetted, and Unicode-mapped.
Ghostscript, text extraction, complete LaTeX/BibTeX scans, and all `15/15`
rendered pages pass.  The release gate also rejects caches, bytecode,
symlinks, special or unlisted paths, path and content sentinels, carriage
returns, malformed EOFs, and external payload inclusion.  Independent proof
and source/release final audits returned zero blocker and zero minor.

Route A is `GO` at the fixed-lag centered/noncausal scope.  RH-396 proves no
growing parameter theorem, ordinary-Cesaro theorem, effective rate,
...
### 6.37 RH-398 exact lag endpoint maximum and maximizers release (2026-08-13)

The post-RH-397 breadth audit selected the open RH-396 endpoint maximum and
maximizer problem.  RH-396 is the sole load-bearing theorem and analytic
endpoint source; RH-397 is the direct release and provenance predecessor
only.  RH-398 proves the exact fixed-lag maximum and equality class, the
nonattained complement supremum, the quantitative `p_0>=5` gap, and the
joint finite-clock supremum with no finite maximizer.

The exact certificate has

```text
72=12+12+12+12+8+8+4+4
```

semantic rows, is `36635` canonical bytes, and has SHA-256
`d47de091a8fe5a134ba4bbf8ac4689f53b54786d45dc3bfc7061c99b46bea741`.
Core, result, and schema reject `66/66`, `44/44`, and `32/32` named
mutations.

The source closure contains `184=172+8+4` Git objects and four ordered remote
locks, hence `188` logical objects.  The three group digests are

```text
fd2d749a09316b9c412780e61882e5f1ac050af609cd0a96c0d1aea79ac4c82d
eb7355565e8429765cb967192c00e261998147abb68fb0307517695621bdfd62
e044509ee377c35cea8642b67a75ca5dc4ba861f455228bde7341418791bce20
```
...
Redistribution flags are `false,false,true,false`; every remote replay makes
zero requests, and all six excluded payload identities are absent.

The archive has `41` publication members and `43` release-stage files.
Targeted offline and archive suites pass `7/7` and `23/23`; complete normal
and optimized suites pass `75/75` in both modes.  The `40028`-byte manifest
has SHA-256
`3d9d0463de5cac260bcd624504d3bf32da657bedb9ab18740cde9e741fb899cd`;
the `1236`-byte zero-failure report has SHA-256
`7248e509902acbf45521fd11f56aa48a1fc6e1382c79f4b97d3211f04414d5f8`.

The main and semantic PDFs are byte-identical, `358870` bytes, and eleven A4
pages.  All `22/22` font rows are embedded, subsetted, and Unicode-mapped.
Ghostscript, text extraction, the LaTeX/BibTeX log, and all `11/11` rendered
pages pass.  Independent proof and source/citation/PDF/release audits report
zero blocker and zero minor.  Integration commit
`f026f7b5dd3aca499b656d03b1a3edc27b02eb8e` contains exactly the `43`
RH-398 files.  Route A is `GO`; Gates A--E remain false.
...
- Tests: `52/52`; exact normal and optimized result/schema/manifest/report
  regeneration, independent field validation, strict JSON and exact types,
  official Draft 2020-12 schema, source rebinding, and archive mutations all
  pass or fail closed as declared.
- The canonical certificate is `17571` bytes with SHA-256
  `e2116abd4aeb910c24ee470a520623f29f1f454bb9b5293840875da091682b3b`.
  It contains `72=12+7+15+12+10+10+6` rows and rejects `24/24` genuine
  semantic mutations; its epistemic role is exact finite algebra, not proof.
- Source closure: `87` immutable Git blobs grouped `77/8/2`, plus the
  ordered Johnston--Yang and Maynard remote logical locks.  The logical input
  count is `89`; both default verifiers make zero requests.  All four
  external payload hashes are absent from the release members and tree.
- Individual archive: `34` publication members, `87` Git inputs, two remote
  logical locks, and zero failures.  Integration commit
  `a3aa5977e9b3338e4c3035c6c42b60d50bc3ac3b` contains exactly `36`
  RH-390 files: the `34` members plus manifest and verification report.
- PDF: `9` A4 pages and `362762` bytes, with all `21` font rows embedded,
  subsetted, and Unicode-mapped.  Ghostscript, text extraction, complete
  LaTeX/BibTeX scans, semantic-PDF byte identity, and all `9/9` rendered
  pages pass.  Cross-version spacing is semantically and visually equivalent;
  no false all-page raster-byte identity is asserted.
- Two independent final audits report zero blocker and zero minor.  The
  moving-rank-necessity, arbitrary-surrogate, factorial-convergence,
  complex-channel, active-`c_11`, growing-clock, operator, trace, zero, RH,
  and Gate firewalls remain intact.

RH-390 final hashes:

```text
main.tex
fff2a95195b9c24fef209d1597d0d306833587a792e5952a9b1d43825391d7b4

references.bib
37dfdaa5b99c423beac182e40034d7e4f2572e5d0ec8a6bab51d2d1b5064bc91
...
manifest
c0272433be30d895c5ad1b39231e9ae87d27c2bae47083b01e38af23e3707bec

verification
125aca24c9253f0fc359847ce7fb9ebff18545fa8ca58f73acc715d6d2dba3c8

core
d2ac37aec5d55f85aaed47db3861371f5df1d96709cffdb6fddcc49f4a6b6852
...
integrity audit
a38062eaf6a586e8b6cf87faeb0872b3c4299e456b3b46b7116db0fe61e55d1d

review audit
af21937654276adcf721d83aa37e7d9630cf3144b9f42bd8ae3e7623d91c7f54

format audit
e9da94b05e6b2f748b8717da168b31deba523232ff9a8afe359196af36983e36

replay audit
19c5530a01145ed0bc36c158111b3e863c3ddfbd62bdc413ebca613f0c48596d

visual audit
88a21ec0adf1741510d9e0b2a28d60f769f20a7089018bcd9677a4812c71b8c9
...
- Tests: official `60/60`; fresh normal and optimized
  result/schema/manifest/report regeneration, independent semantic
  validation, official Draft 2020-12 schema, source rebinding, and archive
  mutations all pass or fail closed as declared.
- The canonical certificate is `220832` bytes with SHA-256
  `614297795d4d4dfeadfb5667d3e0d405d04fbe8e07e9d87a743faed9cb267a96`.
  It contains `640=512+8+64+8+9+7+12+8+6+6` semantic rows, rejects `24/24`
  genuine mutations, and audits all `262144` ordered table pairs through an
  independent false-verification path.
- Source closure: `106` immutable Git blobs grouped `95/8/3`, plus ordered
  Johnston--Yang, Maynard, and Tao remote logical locks.  The logical input
  count is `109`; all three default verifiers make zero requests.  All five
  external payload hashes are absent from the release members and tree.
- Individual archive: `38` publication members, `106` Git inputs, three
  remote locks, and zero failures.  Integration commit
  `9768c1cb5f56d959406c19119315afd542b6c30f` contains exactly `40`
  RH-392 files: the `38` members plus manifest and verification report.
- PDF: `8` A4 pages and `391274` bytes, with all `24` font rows embedded,
  subsetted, and Unicode-mapped.  Ghostscript, text extraction, complete
  LaTeX/BibTeX scans, semantic-PDF byte identity, and all `8/8` rendered
  pages pass.
- Two independent final audits report zero blocker and zero minor.  The
  fixed-`q`, fixed-`h`, terminal-log, two-coordinate, no-rate, no-Cesaro,
  and all Gate firewalls remain intact.

RH-392 final hashes:

```text
main.tex
693f38a69b7f6c35d3a89e98191d0dd27789fd9d14e4057684c942f14bf18112

references.bib
5b3073a8381da87a30615e18f0b4ae09f1165acb28ed01bf457058489d9a8bb7

PDF
c8a90aadb7c1f86373e094124cf5e9b279cd764daeab8f014784f0b35345b056

main.log
060e40171624e9c7571e18a01a0025dc3e9f7ccf2aa1ab0e82f3337bc76358c1
...
manifest
c9b937884cea1d80b76b9e9266cf0ab9bd026e76e8f4fc5c2f47ccb12f8597eb

verification
82120af22cfdcd485642eeb4e2807a66c9891d6e12357f92c2ac9b158d395aa4

core
a4e297f4c324ba1fce1829e5c1caf1b8a7e451a9b90c1f78be5f00d44cd97397
...
integrity audit
2d75a2846225966331751202ed5795a6ce12323c8bac67950337939a9bc1a919

review audit
5e78d0582c692302922f84e43e78ddee44bf7886a1566461f187f3a5e57434cf

format audit
e9653529b6b736e064675f1e25753b8e858ba5faeded01c4a94261b31792a700

replay audit
c7b2b7ec06b67829fe0d61bb781377d237a9876eb4460a90e7f17ea634e4ad71

visual audit
f596e762c420b3798e5bfff56d5b26f6a68fe44736b5b4caad2af94f55245bec

remote-source audit
90c1d753c0f7bdbdfd5e67fb7fb0fb3dbac63532db60b30bf217b3560fc296c4
```

Final RH-393 audit:

- Tests: official normal and optimized `68/68`; fresh
  result/schema/manifest/report regeneration, independent semantic
  validation, official Draft 2020-12 schema, source rebinding, literal
  sentinel, and archive mutations all pass or fail closed as declared.
- The canonical certificate is `117096` bytes with SHA-256
  `f109da241722796418f39708b16fa162cce0b85a6e448998d3ede593b7bd697b`.
  It contains `576=512+27+8+12+9+8` semantic rows, rejects `32/32` genuine
  mutations, and has an independent builder-free false-verification path.
- Source closure: `117` immutable Git blobs grouped `106/8/3`, plus ordered
  Johnston--Yang, Maynard, and Tao remote logical locks.  The logical input
  count is `120`; all three default verifiers make zero requests.  All five
  external payload hashes are absent from the release members and tree.
- Individual archive: `38` publication members, `117` Git inputs, three
  remote locks, and zero failures.  Integration commit
  `6fed36f44183a2794a3a814493ff602c5dc9314b` contains exactly `40`
  RH-393 files: the `38` members plus manifest and verification report.
- PDF: `6` A4 pages and `358432` bytes, with all `20` font rows embedded,
  subsetted, and Unicode-mapped.  Ghostscript, text extraction, complete
  LaTeX/BibTeX scans, semantic-PDF byte identity, and all `6/6` rendered
  pages pass.
- Three independent final audits report zero blocker and zero minor.  The
  fixed finite-shift, at-most-two-odd-factor, fixed-period, terminal-clock,
  no-rate, no-Cesaro, no-generic-capacity, and all Gate firewalls remain
  intact.
...
```text
main.tex
d153d9b9597142d42c0d59905534eb57c81b503883ff3a3085d2b32823a1002c

references.bib
b6a458f6d3a31beb78a9a997d2aa0cfe511527079ad1478d898d8b7c58d67040

PDF
20b4b9313c6975d9522ef265bd43680be8dc56eaae4a61a120c932d5dcfeea95

main.log
abf4b7cfad344c697c3f14e95d75a4dc07cf5d45c9d49814e133677a4544fc8f
...
manifest
ae0976f2decaf20798f322c4b3f535b43bcc2c023789eae430a5e1027a8d0ebe

verification
0a802c02b6fac706370e1dd548846a8f842b1c359742d3c16a6ea316484d77fd

core
f92c4f21cd487bff84f40cdc20ca3605d986acfe132fd7e493b126936024a342
...
integrity audit
b61b2bcd2d85b53a04dc27a4d3f759a4c2f69c7d085d7c3f4ee71a3dd44a3a7d

review audit
1957cb3f066304f915b87f204406d41fbab1da71141dfe7f5fb5270f735b976b

format audit
971ba166b170019839cd9cf354dfea02ad587d42a48a71a8a7ba8eb8cb0a0e4c

replay audit
0e87300da20bc9d2bb86efba8d167a0492561beab6b7c62057848a229d889b5a

visual audit
3ec48fdc4a61a4d747919a1c66686503d35e07a099456a54f74e0990d144f8f0
...
- Tests: complete normal and optimized suites `80/80`; targeted offline
  `7/7` and archive `23/23`; normal/optimized fresh
  result/schema/manifest/report regeneration, independent semantic
  validation, official Draft 2020-12 schema, source rebinding, runtime
  sentinels, symlink/content-sentinel attacks, and archive mutations all
  pass or fail closed as declared.
- The canonical certificate is `83309` bytes with SHA-256
  `7cc0da78ee7e47a22b357d7e8d907bc9d9879caeb82ede30709e8cb1023032ba`.
  It contains `96=12+16+16+12+12+12+8+8` semantic rows.  Core, result, and
  schema reject `32/32`, `65/65`, and `28/28` named mutations, with
  independent false-verification paths.
- Source closure: `160` immutable Git blobs grouped `148/8/4`, plus the
  ordered Johnston--Yang, Maynard, Tao, and Tao--Teravainen remote locks.
  The logical input count is `164`; redistribution flags are
  `false,false,true,false`; four offline verifiers make zero requests; all
  six payload identities are absent from release members and the whole tree.
- Individual archive: `41` publication members, `160` Git inputs, four
  remote locks, and zero failures.  Integration commit
  `cd57086fa90939d56656c3f952a08ffad9aabefe` contains exactly `43` RH-396
  files: the `41` members plus manifest and verification report.
- PDF: `15` A4 pages and `447519` bytes, with all `24` font rows embedded,
  subsetted, and Unicode-mapped.  Ghostscript, text extraction, complete
  LaTeX/BibTeX scans, semantic-PDF byte identity, and all `15/15` rendered
  pages pass.
- Independent final proof and source/release audits report zero blocker and
  zero minor.  The fixed-`h`, fixed-`q`, full-eight-state self-loop,
  `p_0(h)` reset, collision-aware density, later-strict rather than
  every-step-strict, no-Cesaro, no-growing-data, and all Gate firewalls remain
  intact.
...
core
728546daa86fac7b51ab06facff2fccc771ad5128a9f7324f2db36d400a3bf0d
...
integrity audit
3b50f67309c398c43f6d74d7d3f2c99bc73ae691f87d5ee1d6d2fc4f278452fa

review audit
1b881dee36569dc060b6c17ffd7dd730be10771c1a7f5e15607a54d17f0c732c

format audit
2d180abe88c32edcf7ac43524a93d69ae43a89e52f44d98878b0cc162be4f611

replay audit
3b9f573c0ef9a92b7819f63d11717c21f82857276a3465adbfa6d775a6b2c8cc

visual audit
01dc9a78176a9f300f8e14306ba39d42f4952245d9b100c9eff156934986f3cf
...
- Tests: targeted offline `7/7`, targeted archive `23/23`, and complete
  normal and optimized suites `75/75`; result, schema, manifest, and report
  regenerate byte-exactly, and the official Draft 2020-12 validator reports
  zero errors.
- Certificate: `72=12+12+12+12+8+8+4+4` rows, `36635` canonical bytes,
  SHA-256
  `d47de091a8fe5a134ba4bbf8ac4689f53b54786d45dc3bfc7061c99b46bea741`;
  core, result, and schema reject `66/66`, `44/44`, and `32/32` mutations.
- Source closure: `184=172+8+4` immutable Git objects plus four ordered
  remote locks, for `188` logical objects.  Rights are
  `false,false,true,false`; offline requests and six payload hits are zero.
- Archive: `41` publication members and exactly `43` release-stage files;
  manifest and zero-failure verification reproduce exactly.  Integration
  commit `f026f7b5dd3aca499b656d03b1a3edc27b02eb8e` contains exactly those
  `43` RH-398 files.
- PDF: eleven A4 pages and `358870` bytes; all `22` font rows are embedded,
  subsetted, and Unicode-mapped.  Ghostscript, text, log, semantic-PDF byte
  identity, and visual inspection `11/11` pass.
- Independent proof, source/citation/PDF, and release audits report zero
  blocker and zero minor.  Fixed-data order, common-support transfer,
  positive-density strictness, exact maximizer conjunction, and all Gate
...
integrity audit
27f45f1ec841415e4f99d254fba7ef842b2c44fd574b1ee702e71512231c1f7d

review audit
b084d41d88126ee5be258b49f23b39a800ce1e7aa4a226467b4da1b626d4dcd3

format audit
c7d881aa02bf6cab30ab1ea81d990bbe132e7751cdff8148f2daaa12f4c16061

replay audit
edb6b3c8f3970be727b246776d963a28610e4d9b6495d331040666b9599a4f6c

visual audit
bea219d04cf9b6a3a7df9ac901928bf39491f8a0cba91c29948196f73e868c8c

remote-source audit
92654693f9f125f9ca255044eb5f41bb393c2d6e71266213348370b9c5d338b6
```
...
the RH-398 README, UPDATED_ROADMAP, THEOREM_LEDGER, result.json, and main.pdf
...
RH-398 as the exact fixed-lag endpoint maximum/maximizer, complement,
quantitative-gap, and joint finite-clock nonattainment input,
...
RH-398 is the current independent trigger-5 theorem edge and does not close
any physical Gate.  Retain RH-396's fixed centered interface: fix `h,q`,
every phase table, and the terminal clock before the limit; put `d=2h`; take
the fixed-table limit, then the finite safe-table maximum, then the supremum
over finite `q`, and only afterward compare separately fixed scalar lags.
With
...
The complement has the same nonattained supremum, prime-square lags approach
it with gap at most `1/p^2`, the `p_0>=5` sector has gap greater than
`2/1334025`, and
...
has no finite maximizer.  RH-396 is the sole load-bearing analytic endpoint
source; RH-397 is the direct release/provenance predecessor only.  Keep the
common-support transfer, positive-density strict cylinders, prime-`2`
branch, exact conjunction, and fixed-data order.  Do not infer growing data,
a rate, prelimit maximum, ordinary Cesaro theorem, causal rule, lag
monotonicity, operator, trace, zero model, RH statement, or Gate conclusion.

RH-397 remains a completed independent trigger-5 theorem edge and does not close
...
Universal safety is at distance `d` for every ternary biword.  RH-394 is the
sole analytic input, with collision-deduplicated shifts `L=+h,C=0,R=-h`.
For `A_r={(x,y):F_r(x,+1,y)=+1}` and `Y_r=Target(A_r)`, retain

```text
A_r=(T backslash Y_(r-d)) times Y_r.
```

The exact capacity is the full-eight-state tropical trace along the cycles
`r->r+d`.  Four-state compression is proved only when `q` does not divide
`d`; retain all eight states on self-loops, including the strict `h=2,q=4`
counterexample.

On square-supported clocks distinguish raw `N_h(q)` and `alpha_h(q)` from
the weighted value `M_h(q)=K_1 alpha_h(q)/N_h(q)`.  The equality `C_h=M_h`
and same-support saturation require a base modulus divisible by
`p_0(h)^2`, where `p_0(h)` is the first prime not dividing `2h`.  Do not add
a `gcd(R,2h)` condition.  Retain the pre-reset `h=6` failure `36->72` and
the post-reset exact scaling `900->1800`.
...
Fresh-prime lifting is strict exactly when an even run exists, not at every
prime step; keep the `h=9` plateau and the CRT eventual-strictness argument.
Across fixed lags, `inf_(h>=1)B_infinity(h)=3/pi^2`, unattained.  Do not claim
a lag supremum, maximum, monotonicity, growing `h`/`q`/tables, ordinary
Cesaro cancellation, a rate, a pre-limit/adaptive maximum, a causal rule,
an even-four/larger-window theorem, a generic graph theorem, an operator,
trace, zero model, RH statement, or any Gate.

RH-395 is the complete direct finite-`h=1` predecessor.  Fix `q>=1` and a
centered, `q`-periodic table family
...
The exact phase density uses the set
`B_p(E)={a_i mod p^2:i in E}` and is
...
It closes every individually fixed nonzero lag, the exact pair density,
`512 -> 8` projection, all-`(q,h)` charge, and square-divisor capacity
landscape.  Do not promote it to growing lag or period, ordinary Cesaro,
rates, or unrestricted multi-lag tables.

RH-391 remains the preceding pairwise moving-rank theorem.  Maynard's theorem
and finite pigeonhole extraction give
...
Put `Psi_(s,K)=(Psi_(1;s,K),...,Psi_(7;s,K))` and
`Gap_(s,K)=F(Psi_(s,K))/pi^2`.  Retain
`GapP=B_infinity-G(q_y)=F(PhiP)/pi^2`.  Eventually `S_y>=2`.

For every exact integer `2<=s<=S_y`, every exact integer
`1<=K<=floor((2s-1)L)`, and every `c in {1,...,7}`, retain the full safe
coordinate ledger
...
Put `D=(2s-1)L` and `b_k=k!/D^k`.  For exact
`1<=k<floor D`, `b_(k+1)/b_k=(k+1)/D<=1`; hence `b_K<=1/D` for every exact
`1<=K<=floor D`.  Thus the complete `K` window is symbolic, not a finite-
fixture inference.  The common cube bound `||grad F||_1<126` supplies the
endpoint transfer and yields
...
For every fixed exact `s>=2`, set `r=s-1`.  Retain `gamma_r>0` for every
rank and the fixed-rank sharp hierarchy-only conclusions
...
Do not infer moving-rank necessity, an arbitrary-surrogate obstruction,
factorial-series convergence, a larger rank frontier, complex channels,
active `c_11`, growing clocks, `K_N`, operators, traces, zeros, RH, or Gate
conclusions.  Routes A and B are `GO` at this scope; all enlargements are
`STOP_SCOPED`.

RH-389 remains the preceding active-memory theorem edge.  For every fixed
integer `q>=1`, fixed universally safe `q`-periodic family `f`, and every
admissible terminal window `omega`, it proves
...
Its `512 -> 8` projection and predecessor charge occur only after each
fixed-clock terminal-log limit.  Do not turn TPC-137 into ordinary Cesaro
cancellation, a quantitative rate, growing-clock uniformity, or an
unbounded-clock maximum-before-limit.
...
RH-398 is complete.  Its exact fixed-lag endpoint maximum, squarefree-and-
coprime maximizer criterion, nonattained complement supremum, quantitative
`p_0>=5` gap, joint finite-clock nonattainment, source closure, release
archive, and independent final audits are closed.  Do not erase the
fixed-data order, common-support transfer, positive-density strict cylinders,
prime-`2` branch, or the conjunction in the maximizer criterion; and do not
promote it to a growing, causal, Cesaro, monotonicity, operator, zero, RH, or
Gate statement.

RH-397 is complete as RH-398's direct release/provenance predecessor only.
Its separation-`h` two-symbol-overlap relation, exact
...
RH-396 remains complete as RH-398's sole load-bearing analytic endpoint
source and as the direct collision-aware density and finite projection
predecessor.  Its every-fixed-lag centered relation model, exact
...
and independent final audits are closed.  Do not erase its analytic role for
RH-398 or the distinct overlap-geometry boundary crossed by RH-397.
...
locks, release archive, and independent final audits are closed.  Do not
erase its finite-`h=1` role when using RH-396's new lag-dependent proof.
...
projection, every-fixed-`(q,h)` charge, and capacity landscape are closed.
Do not repackage its all-lag spectrum or erase the fixed-data boundary that
RH-393 separately extends only in the odd-support compiler.

RH-391 remains complete as the preceding moving-rank theorem.  Its exact
...
and two independent final audits are closed.  Do not repackage its pair
profile or change its endpoint quantifiers.
...
charge, three remote logical locks, and release archive are closed.  Do not
promote it to ordinary Cesaro cancellation, a growing clock, or an
unbounded maximum-before-limit.
...
No RH-399 is assigned.  A fresh post-RH-398 breadth audit must compare at
least:

1. a causal/end-window graph-coupled capacity, currently `STOP_SCOPED`
   because the centered RH-395 through RH-398 tables read `mu(n+h)` and do
...
3. growing `m`, `h`, period, shifts, coefficients, or masks, currently
   ...
   RH-396 through RH-398 fix all data before the limit and take every fixed-clock
...
Run that audit before assigning RH-399.
